...
void
CG_mvMainviewSwap ( cg_window_t *av )
{
...
CG_mvUpdateClientInfo ( int pID )
{
	if ( pID >= 0 && pID < MAX_MVCLIENTS && (cg.mvClientList & (1 << pID)) )
	{
		...
		if ( ci->weaponState != ci->weaponState_last )
		{
			...
			ci->grenadeTimeStart =
				(
					ci->weaponState == WSTATE_FIRE &&
					(weap == WP_GRENADE_LAUNCHER || weap == WP_GRENADE_PINEAPPLE)
				) ? 4000 +
				cg.time : 0;
		}
...
//
void
CG_mvTransitionPlayerState ( playerState_t *ps )
{
...
//
void
CG_mvWindowOverlay ( int pID, float b_x, float b_y, float b_w, float b_h, float s, int wState, qboolean fSelected )
{
	int				x;
	rectDef_t		rect;
	...
	CG_DrawStringExt( b_x + 1, b_y + b_h - (fh + 2), va( "%s^7%d", CG_TranslateString( p_class), ci->health),
	...
	if ( ci->hintTime >= 0 )
	{
		...
		rect.y -= ( fh + 1 );
		x = CG_DrawStrlen( p_class );
		CG_DrawStringExt( b_x + (b_w - (x * (fw - 1))) / 2, b_y + b_h - (fh + 2), p_class, colorWhite, qfalse, qtrue,
						  fw - 1, fh - 1, 0 );
	}
...
{
	S_COLOR_RED,
	S_COLOR_MDRED,									// Soldier
	S_COLOR_WHITE,
	S_COLOR_MDGREY,									// Medic
	S_COLOR_BLUE,
	S_COLOR_MDBLUE,									// Engineer
	S_COLOR_GREEN,
	S_COLOR_MDGREEN,								// Lt.
	S_COLOR_YELLOW,
	S_COLOR_MDYELLOW								// CovertOps
};


// Update a particular client's info
void
CG_mvOverlayClientUpdate ( int pID, int index )
{
	cg_window_t *w;
	...
	if ( w != NULL )
	{
		strcpy( cg.mvOverlay[index].info, va( "%s%s%2d", strClassHighlights[cg.mvOverlay[index].classID * 2],
...
qboolean
CG_mvMergedClientLocate ( int pID )
{
	int i;
	for ( i = 0; i < cg.mvTotalClients; i++ )
	{
		...
		for ( i = 0; i < cg.mvTotalClients; i++ )
		{
			...
			if ( o->fActive )
			{
				...
				if ( !(cg.mvCurrentActive->mvInfo & MV_SELECTED) || cg.mvCurrentActive == cg.mvCurrentMainview )
				{
					...
					CG_DrawStringExt( x - w - 6, y + 1, cgs.clientinfo[pID].name, colorYellow, qtrue, qtrue,
									  MVINFO_TEXTSIZE - 1, MVINFO_TEXTSIZE - 1, 0 );
				}
			}
...
//////////////////////////////////////
void
CG_mvZoomSniper ( float x, float y, float w, float h )
{
	float	ws = w / 640;
	float	hs = h / 480;
...
/* */
void
CG_mvZoomBinoc ( float x, float y, float w, float h )
{
	float	ws = w / 640;
	float	hs = h / 480;
