...
///////////////////////
void
CG_Text_SetActiveFont ( int font )
{
	activeFont = font;
}
...
/* */
int
CG_Text_Width_Ext ( const char *text, float scale, int limit, fontInfo_t *font )
{
	int			count, len;
	glyphInfo_t *glyph;
	const char	*s = text;
...
/* */
int
CG_Text_Height_Ext ( const char *text, float scale, int limit, fontInfo_t *font )
{
	int			len, count;
	float		max;
	glyphInfo_t *glyph;
	...
	if ( text )
	{
		...
		while ( s && *s && count < len )
		{
			...
			else
			{
				...
				CG_Text_PaintChar_Ext( x + (glyph->pitch * scalex), y - yadj, glyph->imageWidth, glyph->imageHeight,
...
int
CG_DrawFieldWidth ( int x, int y, int width, int value, int charWidth, int charHeight )
{
	char	num[16], *ptr;
	int		l;
	int		frame;
...
/* */
void
CG_UpdateEnvmap ( void )
{
	static int	envmapid = 0;
	float		x, y, w, h;
	if ( cg_envmapupdate.integer == 0 )
	{
		return;
	}

	if ( cg.time - cg.envmapTime < cg_envmapupdate.integer )
	{
		return;
	}
	...
	x = 320;
	y = 240;
	w = h = (float) cgs.glconfig.vidHeight;
	CG_AdjustFrom640( &x, &y, &w, &h );
	if ( !envmapid )
	{
		envmapid = trap_R_GetTextureId( "textures/effects/envmap_slate.tga" );
	}

	x -= 64 + cg_gun_x.value;
	y -= 64 + cg_gun_y.value;
	trap_R_RenderToTexture( envmapid, x, y, 512, 512 );
	envmapid = trap_R_GetTextureId( "textures/effects/envmap_slate_90.tga" );
	trap_R_RenderToTexture( envmapid, x, y, 512, 512 );

	/*
envmapid = trap_R_GetTextureId( "textures/effects/envmap_ice2.tga" );
trap_R_RenderToTexture( envmapid, x, y, 128, 128 );

envmapid = trap_R_GetTextureId( "textures/effects/envmap_radar.tga" );
trap_R_RenderToTexture( envmapid, x, y, 128, 128 );
*/
	envmapid = 0;
	cg.envmapTime = cg.time;
}
...
CG_DrawBlur ( void )
{
	static int	blurtexid = 0;
	float		x, y, w, h;
	int			contents;
	contents = CG_PointContents( cg.refdef.vieworg, -1 );
	if ( contents & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA) || cg.predictedPlayerState.eFlags & EF_DEAD )
	{
	...
	h = 480;
	CG_AdjustFrom640( &x, &y, &w, &h );
	blurtexid = trap_R_GetTextureId( "gfx/misc/blur.tga" );
	x -= 256;
	y -= 256;
	trap_R_RenderToTexture( blurtexid, x, y, 512, 512 );
	if ( cg.predictedPlayerState.eFlags & EF_DEAD )
	{
		CG_DrawPic( 0, 0, w, h * (-1), cgs.media.blur );
	}
	else
	{
		CG_DrawPic( 0, 0, w, h * (-1), cgs.media.waterblur );
	}
}
...
CG_DrawScope ( void )
{
	...
	if ( !texid )
	{
		texid = trap_R_GetTextureId( "models/weapons2/fg42/glass.tga" );
	}

	x -= 64 + cg_gun_x.value;
	y -= 64 + cg_gun_y.value;
	trap_R_RenderToTexture( texid, x, y, 128, 128 );
}
...
*/
void
CG_DrawKeyModel ( int keynum, float x, float y, float w, float h, int fadetime )
{
	qhandle_t	cm;
	float		len;
...
===========================================================================================
*/
#define UPPERRIGHT_X	( cg.showGameView ? 400 : 634 ) //634
...
CG_DrawTimer ( float y )
{
	...
	rt =
		(
			...
			(cgs.clientinfo[cg.clientNum].team != TEAM_SPECTATOR || cg.snap->ps.pm_flags & PMF_FOLLOW) &&
...
int
CG_BotIsSelected ( int clientNum )
{
	int i;
...
/*
=================
Draw Clock      | Jaquboss Thanks  Demoman for help 
=================
*/
static float
CG_DrawClock ( float y )
{
	vec4_t	color = { 0.625f, 0.625f, 0.6f, 1.0f };
...
*/
static float
CG_DrawScorelimit ( float y )
{
	vec4_t	color = { 0.625f, 0.625f, 0.6f, 1.0f };
	vec4_t	scorelimitBackground = { 0.16f, 0.2f, 0.17f, 0.8f };
	vec4_t	scorelimitBorder = { 0.5f, 0.5f, 0.5f, 0.5f };
	int		axisScore, alliedScore, scorelimit;
	char	*s;
	int		w;
	axisScore = cg.teamScores[0];
	alliedScore = cg.teamScores[1];
	scorelimit = cgs.scorelimit;
	s = va( "^1%i:^4%i:^W%i", axisScore, alliedScore, scorelimit );
	w = CG_Text_Width_Ext( s, 0.19f, 0, &cgs.media.limboFont1 );
	CG_FillRect( UPPERRIGHT_X - w - 2, y, w + 5, 12 + 2, scorelimitBackground );
	CG_DrawRect_FixedBorder( UPPERRIGHT_X - w - 2, y, w + 5, 12 + 2, 1, scorelimitBorder );
	CG_Text_Paint_Ext( UPPERRIGHT_X - w, y + 11, 0.19f, 0.19f, color, s, 0, 0, 0, &cgs.media.limboFont1 );
	return ( y + 12 + 4 );
}
...
CG_DrawUpperRight ( void )
{
	...
	if ( cg_drawScore.integer && cgs.gamestate == GS_PLAYING )
	{
		if ( cgs.gametype == GT_WOLF_DM || cgs.gametype == GT_WOLF_TDM || cgs.gametype == GT_WOLF_CTF )
		{
			y = CG_DrawScorelimit( y );
		}
	}
...
CG_DrawTeamInfo ( void )
{
	...
	if ( cgs.teamLastChatPos != cgs.teamChatPos )
	{
		...
		for ( i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i-- )
		{
			...
			trap_R_SetColor( hcolor );
			CG_DrawPic( CHATLOC_X, CHATLOC_Y - (cgs.teamChatPos - i) * lineHeight, chatWidth, lineHeight,
			...
			hcolor[2] = 1;
			trap_R_SetColor( hcolor );
			CG_Text_Paint_Ext( CHATLOC_TEXT_X, CHATLOC_Y - (cgs.teamChatPos - i - 1) * lineHeight - 1, 0.2f, 0.2f,
							   hcolor, cgs.teamChatMsgs[i % chatHeight], 0, 0, 0, &cgs.media.limboFont2 );
		}
	}
}
...
CG_DrawNotify ( void )
{
	...
	if ( cgs.notifyLastPos != cgs.notifyPos )
	{
		...
		for ( i = cgs.notifyPos - 1; i >= cgs.notifyLastPos; i-- )
		{
			...
			hcolor[3] = alphapercent;
			trap_R_SetColor( hcolor );
			CG_DrawStringExt( NOTIFYLOC_X + TINYCHAR_WIDTH, yLoc - (cgs.notifyPos - i) * TINYCHAR_HEIGHT,
							  cgs.notifyMsgs[i % chatHeight], hcolor, qfalse, qfalse, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
							  maxCharsBeforeOverlay );
		}
	}
}
...
CG_DrawLagometer ( void )
{
	...
	|| ( cg_nopredictPing.integer <= cg.snap->ping && cg_nopredictPing.integer > 0 )
#ifdef ALLOW_GSYNC
...
*/
void
CG_PriorityCenterPrint ( const char *str, int y, int charWidth, int priority )
{
	char		*s;
	int			i, len;					// NERVE - SMF
	qboolean	neednewline = qfalse;	// NERVE - SMF
...
*/
static void
CG_DrawWeapReticle ( void )
{
	vec4_t		color = { 0, 0, 0, 1 };
	vec4_t		red = { 1, 0, 0, 1 };
	qboolean	fg, garand, k43;
	float		deltax, deltay;										// this is needed
	deltax = 0 /*cg.predictedPlayerState.viewangles[PITCH] - cg.refdefViewAngles[PITCH]*/ ;
	deltay = 0 /*cg.predictedPlayerState.viewangles[YAW] - cg.refdefViewAngles[YAW]*/ ;
	...
	if ( fg )
	{
		...
		// sides
		CG_FillRect( 0 + deltax, 0 + deltay, 80, 480, color );
		CG_FillRect( 560 + deltax, 0 + deltay, 80, 480, color );

		// center
		if ( cgs.media.reticleShaderSimple )
		{
			CG_DrawPic( 80 + deltax, 0 + deltay, 480, 480, cgs.media.reticleShaderSimple );
		}

		// hairs
		CG_FillRect( 84 + deltax, 239 + deltay, 150, 3, color );	// left
		CG_FillRect( 234 + deltax, 240 + deltay, 173, 1, color );	// horiz center
		CG_FillRect( 407 + deltax, 239 + deltay, 150, 3, color );	// right
		CG_FillRect( 319 + deltax, 2 + deltay, 3, 151, color );		// top center top
		CG_FillRect( 320 + deltax, 153 + deltay, 1, 114, color );	// top center bot
		CG_FillRect( 320 + deltax, 241 + deltay, 1, 87, color );	// bot center top
		CG_FillRect( 319 + deltax, 327 + deltay, 3, 151, color );	// bot center bot
	}
	else if ( garand )
	{
		...
		{
			CG_DrawPic( 80 + deltax, 0 + deltay, 480, 480, cgs.media.reticleShaderSnooper );
			CG_FillRect( 318 + deltax, 238 + deltay, 5, 5, red );	// center dot
		}
		else
		{
			CG_DrawPic( 80 + deltax, 0 + deltay, 480, 480, cgs.media.reticleShaderSimple );
		...
		// hairs
		CG_FillRect( 84 + deltax, 239 + deltay, 177, 2, color );	// left
		CG_FillRect( 320 + deltax, 242 + deltay, 1, 58, color );	// center top
		CG_FillRect( 319 + deltax, 300 + deltay, 2, 178, color );	// center bot
		CG_FillRect( 380 + deltax, 239 + deltay, 177, 2, color );	// right
	}
	else if ( k43 )
	{
...
CG_DrawMortarReticle ( void )
{
	...
	for ( val = i = 0; i < 36; i++ )
	{
		...
		if ( i % 3 == majorOffset )
		{
			...
			CG_Text_Paint_Ext( 500 - localOffset - .5f * CG_Text_Width_Ext( s, .15f, 0, &cgs.media.limboFont1), 244,
	...
	for ( val = i = 0; i < 20; i++ )
	{
		...
		if ( i % 4 == majorOffset )
		{
			...
			CG_Text_Paint_Ext( 320 - .5f * CG_Text_Width_Ext( s, .15f, 0, &cgs.media.limboFont1),
							   164 + localOffset + .5f * CG_Text_Height_Ext( s, .15f, 0, &cgs.media.limboFont1), .15f,
							   .15f, color, s, 0, 0, 0, &cgs.media.limboFont1 );
...
void	CG_FinishADSChange ( int lastweap, int newweap );		// JPW NERVE
...
/* */
static void
CG_DrawCrosshair ( vec3_t worldPoint )
{
	float		w, h;
	qhandle_t	hShader;
	float		f;
	...
	if ( cg.predictedPlayerState.eFlags & EF_AIMING && !cg.renderingThirdPerson && !cg_firstPerson.integer )
	{
		return;
	}
	...
	if ( cg_crosshair_allowed.integer == 2 )
	{																	// server disallowed any onhud crosshairs
		return;
	}
	...
	if
	(
		((cg.predictedPlayerState.leanf != 0) || (cg_crosshair_allowed.integer == 0)) &&
		!cg.renderingThirdPerson &&
		!cg_firstPerson.integer
	)
	{																	// server can disallow crosshairs, but not for thirdperson...
	...
	/// alt crosshair
	if ( cg_drawAltCrosshair.integer == 1 )
	{																	// if we have enabled alternative crosshair , type one , CoD style
		vec4_t	color;
		float	f;
		float	x, y;
		float	w, h;
		...
		/*	x = 320; // centre of scream
	y = 240; 

	if (cg.predictedPlayerState.eFlags & EF_TANK ){
		switch ( cg.predictedPlayerState.persistant[PERS_TANKUSED]) {
		case 1:
		default:
			y += 16;
			break;
		...
		if ( cg_crosshairHealth.integer )
		{																// simply , we want to use crosshairhealth info
	...
	if ( cg_drawAltCrosshair.integer == 2 )
	{																// if we have enabled alternative crosshair , type 2 , CS style
		vec4_t	color;
		float	f;
		float	x, y;
		float	w, h;
	...
	x =  cg_crosshairX_thirdperson.integer;
	y = cg_crosshairY_thirdperson.integer;
	}*/
	if ( worldPoint && VectorLength( worldPoint) )
	{
		if ( !CG_WorldCoordToScreenCoordFloat( worldPoint, &x, &y) )
...
*/
		...
		trap_R_DrawStretchPic( chX, chY, w, h, 0, 0, 1, 1,
...
CG_DrawNoShootIcon ( void )
{
	...
	if ( cg.predictedPlayerState.eFlags & EF_PRONE && (cg.snap->ps.weapon == WP_PANZERFAUST) )
	{
		...
		(
			cg.crosshairClientNoShoot // xkan, 1/6/2003 - don't shoot friend or civilian
			||
	...
	{
		x = cg_crosshairX_thirdperson.integer + 1;
		y = cg_crosshairY_thirdperson.integer + 1;
	}

	CG_AdjustFrom640( &x, &y, &w, &h );

	// FIXME precache
	trap_R_DrawStretchPic( x + 0.5 * (cg.refdef_current->width - w), y + 0.5 * (cg.refdef_current->height - h), w, h, 0,
						   0, 1, 1, cgs.media.friendShader );
}
...
*/
static float
CG_ScanForCrosshairEntity ( float *zChange, qboolean *hitClient )
{
	trace_t		trace;
...
CG_DrawCrosshairNames ( void )
{
	...
	if ( cg_entities[cg.crosshairClientNum].currentState.eType == ET_PLAYER )
	{
		if ( cgs.gametype == GT_WOLF_DM )
		{
			return;
		}
	}
	...
	if ( cg.crosshairClientNum > MAX_CLIENTS )
	{
		...
		if ( cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR )
		{
			...
			else if ( cg_entities[cg.crosshairClientNum].currentState.eType == ET_VEHICLE )
			{
				...
				playerHealth = cg_entities[cg.crosshairClientNum].currentState.dl_intensity;
				maxHealth = 100;
				if ( cg_entities[cg.crosshairClientNum].currentState.effect1Time == 0 )
				{
					s = "Churchill Tank";
				}
				else if ( cg_entities[cg.crosshairClientNum].currentState.effect1Time == 1 )
				{
					s = "Jagdpanther";
				}
				else
	...
	else if ( cgs.clientinfo[cg.crosshairClientNum].team != cgs.clientinfo[cg.snap->ps.clientNum].team )
	{
		...
		if
		(
			(cg_entities[cg.crosshairClientNum].currentState.powerups & (1 << PW_OPS_DISGUISED)) &&
		...
		)
		{
			...
			{	//if( dist > 512 ) {
				...
				playerRank = cgs.clientinfo[cg.crosshairClientNum].team != TEAM_AXIS ? rankNames_Axis[cgs.clientinfo[cg.
					crosshairClientNum].disguiseRank] : rankNames_Allies[cgs.clientinfo[cg.crosshairClientNum].
						disguiseRank];
	...
	)
	{
		...
		if ( cg_entities[cg.crosshairClientNum].currentState.eFlags & EF_TANK )
		{
			if ( cg_entities[cg.crosshairClientNum].currentState.effect1Time == 1 )
			{
				s = va( "Jagdpanther: [%s] %s %s", CG_TranslateString( playerClass), playerRank, name );
			}
			else
			{
				s = va( "Churchill tank:[%s] %s %s", CG_TranslateString( playerClass), playerRank, name );
			}
		}
		else
		{
...
CG_DrawVote ( void )
{
	...
	if
	(
		cgs.complaintEndTime > cg.time &&
		!cg.demoPlayback &&
		cg_complaintPopUp.integer > 0 &&
		cgs.complaintClient >= 0
	)
	{
		Q_strncpyz( str1, BindingFromName( "vote yes"), 32 );
		Q_strncpyz( str2, BindingFromName( "vote no"), 32 );
		s = va( CG_TranslateString( "File complaint against %s for team-killing?"),
	...
	if ( cgs.applicationEndTime > cg.time && cgs.applicationClient >= 0 )
	{
		Q_strncpyz( str1, BindingFromName( "vote yes"), 32 );
		Q_strncpyz( str2, BindingFromName( "vote no"), 32 );
		s = va( CG_TranslateString( "Accept %s's application to join your fireteam?"),
	...
	if ( cgs.invitationEndTime > cg.time && cgs.invitationClient >= 0 )
	{
		Q_strncpyz( str1, BindingFromName( "vote yes"), 32 );
		Q_strncpyz( str2, BindingFromName( "vote no"), 32 );
		s = va( CG_TranslateString( "Accept %s's invitation to join their fireteam?"),
	...
	if
	(
		(cgs.autoFireteamEndTime > cg.time && cgs.autoFireteamNum == -2) ||
...
/* */
float
CG_CalculateReinfTime_Float ( qboolean menu )
{
	team_t	team;
	int		dwDeployTime;
	if ( menu )
...
/* */
int
CG_CalculateReinfTime ( qboolean menu )
{
...
CG_DrawLimboMessage ( void )
{
	...
	// JPW NERVE
	str = ( ps->persistant[PERS_RESPAWNS_LEFT] == 0 ) ? CG_TranslateString( "No more reinforcements this round." ) : va( CG_TranslateString( "Reinforcements deploy in %d seconds."),
																															 CG_CalculateReinfTime( qfalse) );
	CG_DrawSmallStringColor( INFOTEXT_STARTX, y, str, color );
	y += 18;
...
CG_DrawFollow ( void )
{
	...
	if ( cg.snap->ps.pm_flags & PMF_LIMBO )
	{
		if ( cgs.gametype != GT_WOLF_LMS )
		{
				...
				if ( cg.snap->ps.persistant[PERS_RESPAWNS_PENALTY] >= 0 )
				{
					int deployTime =
						( cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_AXIS )
							? cg_redlimbotime.integer : cg_bluelimbotime.integer;
					deployTime *= 0.001f;
			...
			CG_DrawStringExt( INFOTEXT_STARTX, 118, deploytime, colorWhite, qtrue, qtrue, SMALLCHAR_WIDTH,
							  SMALLCHAR_HEIGHT, 80 );
		}
		...
		if ( cg.snap->ps.clientNum != cg.clientNum )
		{
			...
			CG_DrawStringExt( INFOTEXT_STARTX, 136, deploytime, colorWhite, qtrue, qtrue, SMALLCHAR_WIDTH,
							  SMALLCHAR_HEIGHT, 80 );
		}
	}
	else
	{
		CG_DrawStringExt( INFOTEXT_STARTX, 118, CG_TranslateString( "Following"), colorWhite, qtrue, qtrue,
...
CG_DrawFlashZoomTransition
  ...
  FIXME: TOD-O: use cg_fade?
==============
*/
static void
CG_DrawFlashZoomTransition ( void )
{
	vec4_t	color;
	float	frac;
	int		fadeTime;
	...
	if ( cg.snap->ps.eFlags & EF_AIMING )
	{
		return;
	}

	if ( cg.snap->ps.weapon != WP_BINOCULARS /*	&& cg.snap->ps.weapon != WP_K43 
...
CG_DrawFlashDamage ( void )
{
	...
	if ( cg.v_dmg_time > cg.time )
	{
		...
		col[3] = 0.7 * ( redFlash / 5.0 ) * ( (cg_bloodFlash.value > 1.0) ? 1.0 : (cg_bloodFlash.value < 0.0) ? 0.0 : cg_bloodFlash.value );
		CG_FillRect( -10, -10, 650, 490, col );
	}
}
...
*/
static void
CG_DrawFlashBlendBehindHUD ( void )
{
	CG_DrawFlashZoomTransition();
	CG_DrawFlashFade();
}
...
/* */
void
CG_ObjectivePrint ( const char *str, int charWidth )
{
	char		*s;
	int			i, len;					// NERVE - SMF
	qboolean	neednewline = qfalse;	// NERVE - SMF
...
*/
	...
	while ( 1 )
	{
		...
		x = 320 - w / 2;								// JPW NERVE
		CG_DrawStringExt( x, y, linebuffer, color, qfalse, qtrue, cg.oidPrintCharWidth, (int) (cg.oidPrintCharWidth * 1.5),
						  0 );
		y += cg.oidPrintCharWidth * 1.5;
		while ( *start && (*start != '\n') )
		{
...
*/
void
CG_Fade ( int r, int g, int b, int a, int time, int duration )
{
...
*/
void
CG_DrawCompassIcon ( float x, float y, float w, float h, vec3_t origin, vec3_t dest, qhandle_t shader )
{
...
CG_DrawNewCompass ( void )
{
	...
	for ( i = 0; i < MAX_CLIENTS; i++ )
	{
		...
		if
		(
			cg.predictedPlayerState.clientNum == i ||
		...
		CG_DrawCompassIcon( basex, basey, basew, baseh, cg.predictedPlayerState.origin, cent->lerpOrigin,
	...
	if ( cg.predictedPlayerState.stats[STAT_PLAYER_CLASS] == PC_MEDIC )
	{
		for ( i = 0; i < snap->numEntities; i++ )
		{
			...
			if ( (ent->eFlags & EF_DEAD) && ent->number == ent->clientNum )
			{
				if
				(
					!cgs.clientinfo[ent->clientNum].infoValid ||
...
/* */
static int
CG_PlayerAmmoValue ( int *ammo, int *clips, int *akimboammo )
{
	centity_t		*cent;
	playerState_t	*ps;
	int				weap;
...
CG_DrawPlayerStatusHead ( void )
{
	...
	else if ( cg.time > cg.nextIdleTime )
	{
		...
		cg.lastIdleTimeEnd = cg.time +
			character->hudheadanimations[cg.idleAnim].numFrames *
	...
	if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_FLAMETHROWER) )
	{
		if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_AXIS )
		{
			flamer = qtrue;
	...
	CG_DrawPlayerHead( &headRect, character, headcharacter, 180, 0, cg.snap->ps.eFlags & EF_HEADSHOT ? qfalse : qtrue,
					   anim, painshader, cgs.clientinfo[cg.snap->ps.clientNum].rank, flamer,
					   cgs.clientinfo[cg.snap->ps.clientNum].team, cgs.clientinfo[cg.snap->ps.clientNum].headModel,
					   cgs.clientinfo[cg.snap->ps.clientNum].headSkin );
...
/* */
static void
CG_DrawPlayerHealthBar ( rectDef_t *rect )
{
...
/* */
static void
CG_DrawStaminaBar ( rectDef_t *rect )
{
	vec4_t	bgcolour = { 1.f, 1.f, 1.f, 0.3f };
...
/* */
static void
CG_DrawRank ( rectDef_t *rect )
{
	clientInfo_t	*player;
	trap_R_SetColor( NULL );
	player = &cgs.clientinfo[cg.snap->ps.clientNum];
	if ( player->rank )
	{
		CG_DrawPic( rect->x, rect->y, rect->w, rect->h, rankicons[player->rank][0].shader );
	}
}
...
/* */
static void
CG_DrawFrags ( rectDef_t *rect )
{
	const char	*str;
	float		w;
	trap_R_SetColor( NULL );

	//	CG_ParseScore(cg.snap->ps.persistant[PERS_TEAM]);
	CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.pmImages[PM_DEATH] );

	//	str = va( "%i", cg.scores[cg.snap->ps.clientNum].score );
	str = va( "%i", cg.snap->ps.persistant[PERS_SCORE] );
	w = CG_Text_Width_Ext( str, 0.25f, 0, &cgs.media.limboFont1 );
	CG_Text_Paint_Ext( rect->x - w - 1.5f, rect->y + (rect->h / 2), 0.25f, 0.25f, colorWhite, str, 0, 0,
...
// jaquboss start
static void
CG_DrawPlayerStance ( rectDef_t *rect )
{
	vec4_t	color = { 0.75f, 0.75f, 0.75f, 1.0f };
	int		contents;

	if ( cg_drawstance.integer == 1 )
	{
		playerInfo_t	pi;
		clientInfo_t	*ci = &cgs.clientinfo[cg.snap->ps.clientNum];
		memset( &pi, 0, sizeof(pi) );
		pi.classNum = ci->cls;
		pi.legs = cg.predictedPlayerEntity.pe.legs;
		pi.headAnim = cg.predictedPlayerEntity.pe.head;
		pi.torso = cg.predictedPlayerEntity.pe.torso;
		pi.teamNum = ci->team;
		pi.numPendingAnimations = 0;
		VectorCopy( cg.snap->ps.viewangles, pi.viewAngles );
		VectorCopy( vec3_origin, pi.moveAngles );
		VectorCopy( vec3_origin, pi.headOrigin );
		pi.y = 480 - 250;
		pi.z = -68;
		CG_DrawPlayer_Limbo( rect->x, rect->y, rect->w, rect->h, &pi, cg.time, ci, qfalse, 25 );
		return;
	}
	contents = CG_PointContents( cg.refdef.vieworg, -1 );


	if ( cg_drawstance.integer == 3 )
	{
		CG_ColorForHealth( color );
	}
	else if ( cg_drawstance.integer == 4 )
	{
		CG_ColorForStamina( color );
	}

	trap_R_SetColor( color );
//@FIXME all under this sucks, please redo
	...
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_AXIS )
	{
		...
		{
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.swim );
		}
		else if ( cg.snap->ps.eFlags & EF_PRONE || cg.snap->ps.eFlags & EF_PRONE_MOVING )
		{
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.prone );
		}
		else if ( cg.snap->ps.eFlags & EF_CROUCHING )
		{
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.crouch );
		}
		else if ( cg.pmext.sprinting )
		{
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.sprint );
		}
		else
		{
			CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.stand );
		}
	}
	else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_ALLIES )
	{
			...
			if ( contents & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA)  )
			{
				CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.swim2 );
			}
			else if ( cg.snap->ps.eFlags & EF_PRONE || cg.snap->ps.eFlags & EF_PRONE_MOVING )
			{
				CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.prone2 );
			}
			else if ( cg.snap->ps.eFlags & EF_CROUCHING )
			{
				CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.crouch2 );
			}
			else if ( cg.pmext.sprinting )
			{
				CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.sprint2 );
			}
			else
			{
				CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.stand2 );
			}
	}
// clear color change
trap_R_SetColor( NULL );
...
CG_DrawWeapRecharge ( rectDef_t *rect )
{
	...
	{
		chargeTime = 4500.0f;
		barFrac = 1.0f - ( (float) (cg.snap->ps.weaponTime) / chargeTime );
	}
	else
	{	// normal
	...
	trap_R_SetColor( NULL );
	CG_DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.hudSprintBar );
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_AXIS && cgs.axis_strike == 0 )
	{
		trap_R_SetColor( red );
	}

	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_ALLIES && cgs.allied_strike == 0 )
	{
		trap_R_SetColor( red );
	}

	CG_DrawPic( rect->x + (rect->w * 0.25f) - 1, rect->y + rect->h + 4, (rect->w * 0.5f) + 2, rect->w + 2,
				cgs.media.hudPowerIcon );
}


/* */
static void
CG_DrawPlayerStatus ( void )
{
	...
	if ( !(cg.snap->ps.eFlags & EF_TANK) )
	{
		...
		{
			int ws =
				( cg.mvTotalClients > 0 ) ? cgs.clientinfo[cg.snap->ps.clientNum].weaponState : BG_simpleWeaponState( cg.snap->ps.weaponstate );
		...
		if ( value3 >= 0 )
		{
			...
			CG_Text_Paint_Ext( 640 - 22 - CG_Text_Width_Ext( buffer, .25f, 0, &cgs.media.limboFont1) + altx,
		...
		(
			...
			cg.snap->ps.weapon != WP_M7 &&
			cg.snap->ps.weapon != WP_GPG40 &&
			cg.snap->ps.weapon != WP_MORTAR_SET &&
			cg.snap->ps.weapon != WP_MORTAR &&
			cg.snap->ps.weapon != WP_FLAMETHROWER &&
			cg.snap->ps.weapon != WP_GRENADE_PINEAPPLE &&
			cg.snap->ps.weapon != WP_GRENADE_LAUNCHER &&
			cg.snap->ps.weaponstate != WEAPON_RELOADING
		)
		{
			rectDef_t	reloadHintRect = { 640 - 64, 480 - 30, 20, 20 };
			if ( cg_drawAltHud.integer != 1 )
			{
				reloadHintRect.x = 640 - 72;
			}

			CG_DrawKeyHint( &reloadHintRect, "+reload" );
		}
	}											// tank
	...
	else
	{
		...
		// == jaquboss start
		if ( cg_drawstance.integer )
		{
			if ( cg_drawstance.integer == 1 )	// 3d
			{
				if ( cg_drawAltHud.integer )
				{
					rect.x = 40 + 25;
				}
				else
			...
			else if ( cg_drawAltHud.integer != 1 )
			{									// 2d  , althud
				rect.x = 640 - 45;
			...
			CG_DrawPlayerStance( &rect );
		}

		// == jaquboss end
	}											// tank

	// TODO score display for CTF
	if ( (cgs.gametype == GT_WOLF_DM || cgs.gametype == GT_WOLF_TDM) && cg_drawFrags.integer > 0 )
	{
		rect.x = 640 - 45;
		if ( cg_drawAltHud.integer == 1 )
		{
...
/* */
static void
CG_DrawSkillBar ( float x, float y, float w, float h, int skill )
{
	int		i;
...
/* */
skillType_t
CG_ClassSkillForPosition ( clientInfo_t *ci, int pos )
{
	switch ( pos )
	{
		case 0:
...
CG_DrawPlayerStats ( void )
{
	...
	CG_Text_Paint_Ext( SKILLS_X - 80 - w, 480-96, 0.25f, 0.25f, colorWhite, str, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
	CG_Text_Paint_Ext( SKILLS_X - 80 + 2, 480-96, 0.2f, 0.2f, colorWhite, "HP", 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1 );
		}*/
	...
	if ( cg_drawAltHud.integer != 1 )
	{
		for ( i = 0; i < 3; i++ )
		{
			...
			CG_DrawSkillBar( i * SKILL_BAR_X_SCALE + SKILL_BAR_X, 480 - (5 * SKILL_BAR_Y_SCALE) + SKILL_BAR_Y,
	...
	if ( cg_drawAltHud.integer != 1 )
	{
		CG_Text_Paint_Ext( SKILLS_X + 28 - w, 480 - 4, 0.25f, 0.25f, clr, str, 0, 0, ITEM_TEXTSTYLE_SHADOWED,
						   &cgs.media.limboFont1 );
		CG_Text_Paint_Ext( SKILLS_X + 28 + 2, 480 - 4, 0.2f, 0.2f, clr, "XP", 0, 0, ITEM_TEXTSTYLE_SHADOWED,
						   &cgs.media.limboFont1 );
	}
	else
	{
		CG_Text_Paint_Ext( SKILLS_X - 40 - w, 480 - 4, 0.25f, 0.25f, clr, str, 0, 0, ITEM_TEXTSTYLE_SHADOWED,
						   &cgs.media.limboFont1 );
		CG_Text_Paint_Ext( SKILLS_X - 40 + 2, 480 - 4, 0.2f, 0.2f, clr, "XP", 0, 0, ITEM_TEXTSTYLE_SHADOWED,
						   &cgs.media.limboFont1 );
	}
...
/* */
void
CG_StatsDebugAddText ( const char *text )
{
...
CG_DrawStatsDebug ( void )
{
	...
	do
	{
		...
		CG_Text_Paint_Ext( 640.f - 3 - statsDebugTextWidth[i], y + h - 2, .15f, .15f, colour, statsDebugStrings[i], 0,
						   0, ITEM_TEXTSTYLE_NORMAL, &cgs.media.limboFont2 );
		y -= h;
		i--;
		if ( i < 0 )
		{
...
CG_DrawDemoRecording ( void )
{
	...
	{
		Com_sprintf( demostatus, sizeof(demostatus), " demo %s: %ik ", cl_demofilename.string,
	...
	{
		Com_sprintf( wavestatus, sizeof(demostatus), " audio %s: %ik ", cl_wavefilename.string,
	...
	CG_Text_Paint_Ext( 5, cg_recording_statusline.integer, 0.2f, 0.2f, colorWhite, status, 0, 0, 0,
...
// cg_atmospheric.c - UQ1 Added storm weather type!
extern qboolean render_storm_2D;
extern void CG_AddStorm_Outdoors ( void );
...
CG_Draw2D ( void )
{
	...
	if ( !cg.cameraMode )
	{
		...
		if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR )
		{
			if ( cg_drawAltHud.integer != 1 && cg_nohud.integer == 0 )
			{
				CG_DrawSpectator();
			}
		...
		else
		{
			...
			CG_DrawTeamInfo();
			if ( cg_drawStatus.integer && cg_nohud.integer == 0 )
			{
				Menu_PaintAll();
				CG_DrawTimedMenus();
			}
	...
	if ( !CG_DrawScoreboard() )
	{
		...
		{
			rectDef_t	rect;
			if ( cg.snap->ps.stats[STAT_HEALTH] > 0 && cg_drawAltHud.integer != 1 )
			{
				CG_DrawPlayerStatusHead();
				CG_DrawPlayerStatus();
				CG_DrawPlayerStats();
			}
			else if ( cg.snap->ps.stats[STAT_HEALTH] > 0 && cg_drawAltHud.integer == 1 )
			{
				CG_DrawPlayerStatus();
				CG_DrawPlayerStats();
			}
	...
	{						// For OM gametype flag captures...
		...
		color[2] = 1;
		CG_Text_Paint_Ext( x, y, 0.24, 0.24, color, va( "Capturing..."), 0, 0, ITEM_TEXTSTYLE_SHADOWED,
						   &cgs.media.tahoma16 );
	}
	...
	{						// For OM gametype flag captures...
		...
		CG_Text_Paint_Ext( x, y, 0.24, 0.24, color, va( "^3Captured!"), 0, 0, ITEM_TEXTSTYLE_SHADOWED,
						   &cgs.media.tahoma16 );
	}
	...
	{						// Draw the ticket numbers...
		...
		{					// Flash the axis and allied ticket numbers...
			...
			if ( last_flash )
			{
				CG_Text_Paint_Ext( x, y, 0.15, 0.15, color,
								   va( "^5[^1Axis^5: ^1%i^5] [^4Allies^5: ^1%i^5]", cgs.redtickets, cgs.bluetickets), 0,
								   0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma16 );
			}
			else
			{
				CG_Text_Paint_Ext( x, y, 0.15, 0.15, color,
								   va( "^5[^1Axis^5: ^7%i^5] [^4Allies^5: ^7%i^5]", cgs.redtickets, cgs.bluetickets), 0,
								   0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma16 );
			}
		}
		else if ( flashaxis )
		{					// Flash the axis ticket number...
			...
			{
				CG_Text_Paint_Ext( x, y, 0.15, 0.15, color,
								   va( "^5[^1Axis^5: ^1%i^5] [^4Allies^5: ^7%i^5]", cgs.redtickets, cgs.bluetickets), 0,
								   0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma16 );
			}
			else
			{
		...
		{					// Flash the allied ticket number...
			...
			{
				CG_Text_Paint_Ext( x, y, 0.15, 0.15, color,
								   va( "^5[^1Axis^5: ^7%i^5] [^4Allies^5: ^1%i^5]", cgs.redtickets, cgs.bluetickets), 0,
								   0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma16 );
			}
			else
			{
		...
		{					// Nothing special happening...
			CG_Text_Paint_Ext( x, y, 0.15, 0.15, color,
							   va( "^5[^1Axis^5: ^7%i^5] [^4Allies^5: ^7%i^5]", cgs.redtickets, cgs.bluetickets), 0, 0,
							   ITEM_TEXTSTYLE_SHADOWED, &cgs.media.tahoma16 );
		}
	}
#endif //__SUPREMACY__
...
CG_DrawActive ( stereoFrame_t stereoView )
{
	...
	if ( !(cg.limboEndCinematicTime > cg.time && cg.showGameView && cg_bigLimbo.integer) )
	{
	...
	if ( !cg.showGameView )
	{
		...
		if ( cg_renderToTexture.integer > 0 )
		{
			CG_DrawScope();						// TODO real scope as real envmap
			if ( !(cg_renderToTexture.integer & 2) )
			{
				CG_UpdateEnvmap();
			}
