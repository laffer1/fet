...
void
CG_windowReset ( cg_window_t *w, int fx, int startupLength )
{
...
// Reserve a window
cg_window_t *
CG_windowAlloc ( int fx, int startupLength )
{
	int					i;
	cg_window_t			*w;
...
/* */
void
CG_demoAviFPSDraw ( void )
{
	qboolean	fKeyDown = cgs.fKeyPressed[K_F1] |
		cgs.fKeyPressed[K_F2] |
		cgs.fKeyPressed[K_F3] |
		cgs.fKeyPressed[K_F4] |
		cgs.fKeyPressed[K_F5];
...
CG_windowDraw ( void )
{
	...
	for ( i = 0; i < cg.winHandler.numActiveWindows; i++ )
	{
		...
		bg =
			(
				(w->effects & WFX_FLASH) &&
				(milli % w->flashPeriod) > w->flashMidpoint
			) ? &w->colorBackground2 : &w->colorBackground;
		h = w->h;
		x = w->x;
		y = w->y;
		...
		for ( j = w->lineCount - 1; j >= 0; j-- )
		{
			if ( w->effects & WFX_TRUETYPE )
			{
				...
				CG_Text_Paint_Ext( x, y + h, w->fontScaleX, w->fontScaleY, textColor, (char *) w->lineText[j], 0.0f, 0,
			...
			if ( !(w->effects & WFX_TRUETYPE) )
			{
				CG_DrawStringExt2( x, y + h, (char *) w->lineText[j], textColor, qfalse, qtrue, w->fontWidth,
								   w->fontHeight, 0 );
			}
		}
	}
...
void
CG_windowNormalizeOnText ( cg_window_t *w )
{
	int i, tmp;
	if ( w == NULL )
	{
...
/* */
qboolean
CG_addString ( cg_window_t *w, char *buf )
{
	int i;
...
/* */
void
CG_removeStrings ( cg_window_t *w )
{
	int i, j;
	for ( i = 0; i < w->lineCount; i++ )
	{
					...
					)
					{
						...
						if ( fSelect )
						{
							...
							else
							{
								...
								cg.mvCurrentMainview->mvInfo = ( cg.mvCurrentMainview->mvInfo &~MV_PID ) | ( cg.mvOverlay[cg.mvTeamList[i][pos]].pID & MV_PID );
								fUpdateOverlay = qtrue;
							}
						}
					}
