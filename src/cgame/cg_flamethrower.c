...
*/
void
CG_FlameLerpVec ( const vec3_t oldV, const vec3_t newV, float backLerp, vec3_t outV )
{
...
*/
void
CG_FlameAdjustSpeed ( flameChunk_t *f, float change )
{
...
CG_FireFlameChunks ( centity_t *cent, vec3_t origin, vec3_t angles, float speedScale, qboolean firing )
{
	...
	if ( cent->currentState.number != cg.snap->ps.clientNum || cg_3rdPerson.integer )
	{
...
*/
flameChunk_t *
CG_SpawnFlameChunk ( flameChunk_t *headFlameChunk )
{
	flameChunk_t	*f;
	if ( !freeFlameChunks )
	{
...
*/
void
CG_FreeFlameChunk ( flameChunk_t *f )
{
...
*/
void
CG_MergeFlameChunks ( flameChunk_t *f1, flameChunk_t *f2 )
{
...
*/
void
CG_FlameCalcOrg ( flameChunk_t *f, int time, vec3_t outOrg )
{
...
*/
void
CG_MoveFlameChunk ( flameChunk_t *f )
{
	...
	while ( f->velSpeed > 1 && f->baseOrgTime != cg.time )
	{
		...
		if ( trace.fraction == 1.0 )
		{
			...
			if
			(
				(f->ownerCent != cg.snap->ps.clientNum) &&
...
/* */
void
CG_FlameDamage ( int owner, vec3_t org, float radius )
{
	return;
}
...
/* */
void
CG_AddFlameSpriteToScene ( flameChunk_t *f, float lifeFrac, float alpha )
{
...
*/
void
CG_AddFlameToScene ( flameChunk_t *fHead )
{
	...
	if
	(
		(cg_entities[fHead->ownerCent].currentState.eFlags & EF_FIRING) &&
			...
			if ( !skip )
			{
				...
				if ( f->blueLife > lived * (f->ignitionOnly ? 3.0 : 3.0) )
				{
					...
					blueTrailHead = CG_AddTrailJunc( blueTrailHead, NULL,	// rain - zinx's trail fix
													shader, cg.time, STYPE_STRETCH, f->org, 1, alpha, alpha, f->size * (
														 f->
														 ignitionOnly		/*&& (cg.snap->ps.clientNum != f->ownerCent || cg_thirdPerson.integer)*/
														 ? 2.0 : 1.0), FLAME_MAX_SIZE, TJFL_NOCULL | TJFL_FIXDISTORT, c,
													 c, 1.0, 5.0 );
				}
				...
				if ( !f->ignitionOnly )
				{
					...
					else
					{
						alpha = FLAME_BLUE_MAX_ALPHA *
							(
								1.0 -
									(
										(lived / 1.3 - (f->blueLife - FLAME_BLUE_FADEOUT_TIME( f->blueLife))) /
					...
					if ( !fskip )
					{
						...
						fuelTrailHead = CG_AddTrailJunc( fuelTrailHead, NULL,	// rain - zinx's trail fix
														 ...
														 alpha, alpha, (f->size / 2 < f->sizeMax / 4 ? f->size / 2 : f->sizeMax / 4),
...
#define FLAME_SPRITE_START_BLUE_SCALE	0.2
		if ( !f->ignitionOnly && ((float) (FLAME_SPRITE_START_BLUE_SCALE * f->blueLife) < (float) lived) )
		{
			...
			while ( fNext && !droppedTrail )
			{
				if
				(
					(Distance( f->org, fNext->org) < ((0.1 + 0.9 * f->lifeFrac) * f->size * 0.35)) &&
	...
	if ( fHead->ignitionOnly )
	{
		...
		trap_R_AddLightToScene2( lightOrg, 80, alpha, 0.2, 0.21, 0.5, 0, 0 );
	}
	else if ( isClientFlame || (fHead->ownerCent == cg.snap->ps.clientNum) )
	{
		...
		trap_R_AddLightToScene2( lightOrg, 320, alpha, 1.000000, 0.603922, 0.207843, 0, 0 );
	}
}
...
(
	...
	char		*dstBlend,
	char		*extras,
	qboolean	compressedVersionAvailable,
	qboolean	nomipmap
)
{
	fileHandle_t	f;
...
*/
void
CG_InitFlameChunks ( void )
{
	int		i;
	char	filename[MAX_QPATH];
	CG_ClearFlameChunks();
#ifdef GEN_FLAME_SHADER
	CG_GenerateShaders( "scripts/flamethrower.shader", "flamethrowerFire", FLAME_SPRITE_DIR, NUM_FLAME_SPRITES,
						FLAME_BLEND_SRC, FLAME_BLEND_DST, "", qtrue, qtrue );
	CG_GenerateShaders( "scripts/blacksmokeanim.shader", "blacksmokeanim", "explode1", 23, "GL_ZERO",
						"GL_ONE_MINUS_SRC_ALPHA", "\t\talphaGen const 0.2\n", qfalse, qfalse );
	CG_GenerateShaders( "scripts/viewflames.shader", "viewFlashFire", "clnfire", 16, "GL_ONE", "GL_ONE",
						"\t\talphaGen vertex\n\t\trgbGen vertex\n", qtrue, qtrue );
	CG_GenerateShaders( "scripts/twiltb.shader", "twiltb", "twiltb", 42, "GL_SRC_ALPHA", "GL_ONE_MINUS_SRC_COLOR", "",
						qtrue, qfalse );
	CG_GenerateShaders( "scripts/twiltb2.shader", "twiltb2", "twiltb2", 45, "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "",
						qtrue, qfalse );
	...
	CG_GenerateShaders( "scripts/expblue.shader",
						...
						qfalse, qfalse );
*/
	CG_GenerateShaders( "scripts/firest.shader", "firest", "firest", 36, "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "", qtrue,
						qfalse );
	CG_GenerateShaders( "scripts/explode1.shader", "explode1", "explode1", 23, "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "",
						qtrue, qfalse );
	CG_GenerateShaders( "scripts/funnel.shader", "funnel", "funnel", 21, "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "", qfalse,
						qfalse );
#endif
...
*/
void
CG_UpdateFlamethrowerSounds ( void )
{
...
#define MIN_BLOW_VOLUME 30
	...
	while ( f )
	{
		...
		if ( centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time )
		{
			...
			{
				trap_S_AddLoopingSound( f->org, vec3_origin, cgs.media.flameBlowSound,
										(int) (255.0 * centFlameStatus[f->ownerCent].blowVolume), 0 );			// JPW NERVE
			}
			else
			{
			...
			{
				trap_S_AddLoopingSound( f->org, vec3_origin, cgs.media.flameStreamSound,
										(int) (255.0 * centFlameStatus[f->ownerCent].streamVolume), 0 );		// JPW NERVE
			}
