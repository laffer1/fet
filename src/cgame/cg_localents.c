...
CG_BloodTrail ( localEntity_t *le )
{
	...
	for ( ; t <= t2; t += step )
	{
		...
		le->headJuncIndex = CG_AddTrailJunc( le->headJuncIndex, le, // rain - zinx's trail fix
											 cgs.media.bloodTrailShader, t, STYPE_STRETCH, newOrigin, 180,
											 1.0,					// start alpha
											0.0,					// end alpha
...
CG_FragmentBounceMark ( localEntity_t *le, trace_t *trace )
{
	...
	if ( le->leMarkType == LEMT_BLOOD )
	{
		...
		if ( !(lastBloodMark > cg.time || lastBloodMark > cg.time - 100) )
		{
			...
			CG_ImpactMark( cgs.media.bloodDotShaders[rand() % 5], markOrigin, projection, radius, random() * 360.0f,
...
CG_FragmentBounceSound ( localEntity_t *le, trace_t *trace )
{
	...
	switch ( le->leBounceSoundType )
	{
		...
		case LEBS_BRASS:
			...
			{
				trap_S_StartSoundVControl( trace->endpos, -1, CHAN_AUTO, cgs.media.sfx_brassSound[BRASSSOUND_METAL][rnd],
										   164 );	// test now , was 64
			}
			else if ( trace->surfaceFlags & SURF_WOOD )
			{
				trap_S_StartSoundVControl( trace->endpos, -1, CHAN_AUTO, cgs.media.sfx_brassSound[BRASSSOUND_WOOD][rnd], 128 );
			}
			else if ( trace->surfaceFlags & (SURF_GRAVEL | SURF_SNOW | SURF_CARPET | SURF_GRASS) )
			{
				trap_S_StartSoundVControl( trace->endpos, -1, CHAN_AUTO, cgs.media.sfx_brassSound[BRASSSOUND_SOFT][rnd], 78 );
			}
			else
			{
				trap_S_StartSoundVControl( trace->endpos, -1, CHAN_AUTO, cgs.media.sfx_brassSound[BRASSSOUND_STONE][rnd],
										   164 );
			}
			break;
		case LEBS_ROCK:
...
*/
void
CG_AddEmitter ( localEntity_t *le )
{
	vec3_t	dir;
...
CG_AddFragment ( localEntity_t *le )
{
	...
	if ( le->onFireStart && (le->onFireStart < cg.time && le->onFireEnd > cg.time) )
	{
		...
		trap_S_AddLoopingSound( le->refEntity.origin, vec3_origin, cgs.media.flameCrackSound, (int) (20.0 * flameAlpha),
								0 );
	}
	...
	if ( le->leFlags & LEF_SMOKING )
	{
		...
		//	TOD-O: FIXME: this is not quite right, because it'll become fps dependant - in a bad way.
	...
	if ( hasFlame )
	{
		...
		else
		{
			...
			trap_S_AddLoopingSound( newOrigin, vec3_origin, cgs.media.flameBlowSound, (int) (0.3 * 255.0 * flameAlpha),
									0 );
		}
	}
...
*/
void
CG_AddMovingTracer ( localEntity_t *le )
{
	vec3_t	start, end, dir, velocity;
	BG_EvaluateTrajectory( &le->pos, cg.time, start, qfalse, -1 );
	VectorNormalize2( le->pos.trDelta, dir );
	VectorMA( start, cg_tracerLength.value, dir, end );
	CG_DrawTracer( start, end );

	/*	BG_EvaluateTrajectoryDelta( &le->pos, cg.time, velocity, qfalse, -1 );
	trap_S_AddLoopingSound( end, velocity, cgs.media.whizzsound, 255, 0 );*/
}
...
*/
void
CG_AddSparkElements ( localEntity_t *le )
{
	vec3_t	newOrigin;
	trace_t trace;
	float	time;
...
*/
		...
		le->headJuncIndex = CG_AddSparkJunc( le->headJuncIndex, le, // rain - zinx's trail fix
											 le->refEntity.customShader, le->refEntity.origin, 200,
...
*/
void
CG_AddFuseSparkElements ( localEntity_t *le )
{
	...
	while ( time < cg.time )
	{
		...
		le->headJuncIndex = CG_AddTrailJunc( le->headJuncIndex, le, cgs.media.sparkParticleShader, time, STYPE_STRETCH,
											 le->refEntity.origin,
											 (int) (lifeFrac * (float) (le->endTime - le->startTime) / 2.0),
											 1.0 /*(1.0 - lifeFrac)*/, 0.0, FUSE_SPARK_WIDTH * (1.0 - lifeFrac),
											 FUSE_SPARK_WIDTH * (1.0 - lifeFrac), TJFL_SPARKHEADFLARE, whiteColor,
											 whiteColor, 0, 0 );
...
================
*/
extern qhandle_t rainSplash[8];
extern qhandle_t rain_splash;
void
CG_AddRainSplashLE ( localEntity_t *le )
{
	vec3_t	newOrigin;
	trace_t trace;
	float	time;
	...
	for ( numbounces = 0; numbounces < 5; numbounces++ )
	{
		...
		// add a trail
		le->headJuncIndex = CG_AddSparkJunc( le->headJuncIndex, le, // rain - zinx's trail fix
											 rainSplash[Q_TrueRand(0,7)], le->refEntity.origin, 200,
											 1.0 - lifeFrac,		// start alpha
		...
		if ( trace.fraction < 1.0 )
		{
			...
			// TOD-O: spawn a blood decal here?
			// the intersection is a fraction of the frametime
			le->pos.trTime = (int) time;
		}
...
*/
void
CG_AddBloodElements ( localEntity_t *le )
{
	vec3_t	newOrigin;
	trace_t trace;
	float	time;
	...
	for ( numbounces = 0; numbounces < 5; numbounces++ )
	{
		...
		// add a trail
		le->headJuncIndex = CG_AddSparkJunc( le->headJuncIndex, le, // rain - zinx's trail fix
											 cgs.media.bloodTrailShader, le->refEntity.origin, 200,
											 1.0 - lifeFrac,		// start alpha
											1.0 - lifeFrac,			// end alpha
											3.0, 5.0 );
		if ( trace.fraction < 1.0 )
...
*/
void
CG_AddDebrisElements ( localEntity_t *le )
{
	vec3_t	newOrigin;
	trace_t trace;
	float	lifeFrac;
	...
	for ( t = le->lastTrailTime + step; t < cg.time; t += step )
	{
		...
		{
			le->headJuncIndex = CG_AddSparkJunc( le->headJuncIndex, le,												// rain - zinx's trail fix
												 cgs.media.fireTrailShader, le->refEntity.origin,
		...
		{
			le->headJuncIndex = CG_AddSparkJunc( le->headJuncIndex, le,												// rain - zinx's trail fix
												 cgs.media.sparkParticleShader, le->refEntity.origin,
		...
		if ( le->effectFlags & 1 )
		{
			le->headJuncIndex2 = CG_AddSmokeJunc( le->headJuncIndex2, le,											// rain - zinx's trail fix
												  cgs.media.smokeTrailShader, le->refEntity.origin,
...
*/
void
CG_AddShrapnel ( localEntity_t *le )
{
	vec3_t	newOrigin;
	trace_t trace;
	if ( le->pos.trType == TR_STATIONARY )
...
/* */
void
CG_AddConstRGB ( localEntity_t *le )
{
	refEntity_t *re;
	re = &le->refEntity;
...
/* */
void
CG_DrawPlumIcon ( vec3_t position, int shader )
{
	int			rf = 0;
	refEntity_t ent;
	memset( &ent, 0, sizeof(ent) );
	VectorCopy( position, ent.origin );
	ent.origin[2] += 16;	// Above the node number...
...
CG_AddScorePlum ( localEntity_t *le )
{
	...
	if ( re->torsoFrame == 1 )
	{		// A goal node... Mark it red...
		re->shaderRGBA[0] = 0xff;
		re->shaderRGBA[1] = 0x00;
		re->shaderRGBA[2] = 0x00;
	}
	else if ( re->torsoFrame == 2 )
	{		// A jump node... Mark it yellow...
		re->shaderRGBA[0] = 0xff;
		re->shaderRGBA[1] = 0xff;
		re->shaderRGBA[2] = 0x00;
	}
	else if ( re->torsoFrame == 3 )
	{		// A duck node... Mark it ...
	...
	else
	{
		...
		{
			re->shaderRGBA[0] = 0xff;
			re->shaderRGBA[1] = 0x66;
			re->shaderRGBA[2] = 0x66;
		}
		else if ( score < 2000 )
		{
			re->shaderRGBA[0] = 0xff;
			re->shaderRGBA[1] = 0x99;
			re->shaderRGBA[2] = 0x99;
		}
		else if ( score < 3000 )
		{
			re->shaderRGBA[0] = 0xff;
	...
	for ( i = 0; i < numdigits; i++ )
	{
		if ( re->skinNum > 0 )
		{	// Draw big!
			VectorMA( origin, (float) (((float) numdigits / 2) - i) * NUMBER_SIZE * 2, vec, re->origin );
		}
		else
		{
	...
	if ( le->displayIcon > 0 )
	{		// Add an icon above it!
		vec3_t	position;
		VectorCopy( origin, position );
		CG_DrawPlumIcon( position, le->displayIcon );
	}
}
...
*/
void
CG_AddGoalNode ( localEntity_t *le )
{
	refEntity_t *re;
	vec3_t		origin, delta, dir, vec, up = { 0, 0, 1 };
	float		c, len;
	...
	VectorMA( origin, (float) NUMBER_SIZE / 2, vec, re->origin );
	re->customShader = trap_R_RegisterShader( "goalnode" );
	trap_R_AddRefEntityToScene( re );
}
