...
CG_TransitionEntity ( centity_t *cent )
{
	...
	if ( CG_EntOnFire( cent) )
	{
		...
		BG_EvaluateTrajectory( &cent->currentState.pos, cg.snap->serverTime, oldPos, qfalse,
...
CG_SetInitialSnapshot ( snapshot_t *snap )
{
	...
	for ( i = 0; i < cg.snap->numEntities; i++ )
	{
		...
		cent->interpolate = qfalse;
		cent->currentValid = qtrue;
		memcpy( &cg_entities[state->number].ps, &snap->ps, sizeof(playerState_t) );
		CG_ResetEntity( cent );
...
CG_TransitionSnapshot ( void )
{
	...
	for ( i = 0; i < cg.snap->numEntities; i++ )
	{
		...
		//		memcpy(&cent->ps, &snap->ps, sizeof(playerState_t));
	...
	memcpy( &cg_entities[cg.snap->ps.clientNum].ps, &cg.snap->ps, sizeof(playerState_t) );
	for ( i = 0; i < cg.snap->numEntities; i++ )
	{
...
CG_SetNextSnap ( snapshot_t *snap )
{
	...
	for ( num = 0; num < snap->numEntities; num++ )
	{
		...
		//cent->nextState = *es;
		memcpy( &cg_entities[es->number].ps, &snap->ps, sizeof(playerState_t) );
