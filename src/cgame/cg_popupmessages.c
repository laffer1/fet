...
{
	{ "Binoculars", "Improved Physical Fitness", "Improved Health", "Trap Awareness" },

	// battle sense
	{ "Improved use of Explosive Ammunition", "Improved Dexterity", "Improved Construction and Destruction",
			"a Flak Jacket" },

	// explosives & construction
	{ "Medic Ammo", "Improved Resources", "Full Revive", "Adrenalin Self" },

	// first aid
	{ "Improved Resources", "Improved Signals", "Improved Air and Ground Support", "Enemy Recognition" },

	// signals
	{ "Improved use of Light Weapon Ammunition", "Faster Reload", "Improved Light Weapon Handling",
			"Dual-Wield Pistols" },

	// light weapons
	{ "Improved Projectile Resources", "Heavy Weapon Proficiency", "Improved Dexterity", "Improved Weapon Handling" },

	// heavy weapons
	{ "Improved use of Scoped Weapon Ammunition", "Improved use of Sabotage and Misdirection", "Breath Control",
			"Assassin" }							// scoped weapons & military intelligence
};
const char		*cg_skillRewards2[SK_NUM_SKILLS][NUM_SKILL_LEVELS - 1] =
{
	{ "Binoculars", "Improved Physical Fitness", "Improved Health", "Trap Awareness" },
	...
	// heavy weapons
	{ "Improved use of Scoped Weapon Ammunition", "Improved use of Sabotage and Misdirection", "Breath Control",
			"Thermal scopes for Garand and K43" }	// scoped weapons & military intelligence
};
...
CG_InitPMGraphics ( void )
{
	...
	cgs.media.pmImages[PM_HEADSHOT] = trap_R_RegisterShaderNoMip( "gfx/hud/headshot" );
	cgs.media.pmImages[PM_TK] = trap_R_RegisterShaderNoMip( "gfx/hud/pm_death" );
	cgs.media.pmImages[PM_SK] = trap_R_RegisterShaderNoMip( "gfx/hud/pm_death" );
...
/* */
int
CG_TimeForPopup ( popupMessageType_t type )
{
	switch ( type )
	{
		default:
...
/* */
int
CG_TimeForBigPopup ( popupMessageBigType_t type )
{
	switch ( type )
	{
		default:
...
/* */
void
CG_AddToListFront ( pmListItem_t **list, pmListItem_t *item )
{
	item->next = *list;
	*list = item;
}
...
/* */
void
CG_AddPMItem ( popupMessageType_t type, const char *message, qhandle_t shader )
{
	pmListItem_t	*listItem;
	char			*end;
	if ( !message || !*message )
...
/* */
void
CG_PMItemBigSound ( pmListItemBig_t *item )
{
	if ( !cg.snap )
	{
		return;
...
/* */
void
CG_AddPMItemBig ( popupMessageBigType_t type, const char *message, qhandle_t shader )
{
...
CG_DrawPMItems ( void )
{
	...
	if ( cg_pmWaitingList->type == PM_SK )
	{
		colourText[0] = 1;
		colourText[1] = 1;
		colourText[2] = 0;
	}
	else if ( cg_pmWaitingList->type == PM_TK )
	{
		colourText[0] = 1;
		colourText[1] = 0;
		colourText[2] = 0;
	...
	CG_Text_Paint_Ext( 4 + size + 2, y + 12, 0.2f, 0.2f, colourText, cg_pmWaitingList->message, 0, 0, 0,
	...
	for ( i = 0; i < 4 && listItem; i++, listItem = listItem->next )
	{
		...
		if ( listItem->type == PM_SK )
		{
			colourText[0] = 1;
			colourText[1] = 1;
			colourText[2] = 0;
		}
		else if ( listItem->type == PM_TK )
		{
			colourText[0] = 1;
			colourText[1] = 0;
			colourText[2] = 0;
		...
		trap_R_SetColor( NULL );
		CG_Text_Paint_Ext( 4 + size + 2, y + 12, 0.2f, 0.2f, colourText, listItem->message, 0, 0, 0,
						   &cgs.media.limboFont2 );
	}
}
...
/* */
const char *
CG_GetPMItemText ( centity_t *cent )
{
	switch ( cent->currentState.effect1Time )
	{
		...
		case PM_MINES:
			...
			return
				(
					va( "Spotted by %s^7 at %s", cgs.clientinfo[cent->currentState.effect3Time].name,
		...
		case PM_OBJECTIVE:
			switch ( cent->currentState.density )
			{
				case 0:
					return
						(
							va( "%s have stolen %s!", cent->currentState.effect2Time == TEAM_ALLIES ? "Allies" : "Axis",
								CG_ConfigString( CS_STRINGS + cent->currentState.effect3Time))
						);
				case 1:
					return
						(
							va( "%s have returned %s!", cent->currentState.effect2Time == TEAM_ALLIES ? "Allies" : "Axis",
								CG_ConfigString( CS_STRINGS + cent->currentState.effect3Time))
						);
			}
			break;
...
/* */
void
CG_PlayPMItemSound ( centity_t *cent )
{
	switch ( cent->currentState.effect1Time )
	{
		case PM_DYNAMITE:
...
/* */
qhandle_t
CG_GetPMItemIcon ( centity_t *cent )
{
	switch ( cent->currentState.effect1Time )
	{
		case PM_CONSTRUCTION:
...
/* */
void
CG_DrawKeyHint ( rectDef_t *rect, const char *binding )
{
	int		k1, k2;
	char	buffer[256];
	char	k[2] = { 0, 0 };
	...
	CG_Text_Paint_Ext( rect->x + ((rect->w - w) * 0.5f), rect->y + 14, 0.2f, 0.2f, colorWhite, k, 0, 0, 0,
