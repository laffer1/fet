...
****/
int QDECL
CG_SortFireTeam ( const void *a, const void *b )
{
	clientInfo_t	*ca, *cb;
	int				cna, cnb;
	cna = *(int *) a;
...
fireteamData_t *
CG_IsOnSameFireteam ( int clientNum, int clientNum2 )
{
...
fireteamData_t *
CG_IsFireTeamLeader ( int clientNum )
{
	fireteamData_t	*f;
...
clientInfo_t *
CG_ClientInfoForPosition ( int pos, int max )
{
	int i, cnt = 0;
...
fireteamData_t *
CG_FireTeamForPosition ( int pos, int max )
{
	int i, cnt = 0;
...
clientInfo_t *
CG_FireTeamPlayerForPosition ( int pos, int max )
{
	int				i, cnt = 0;
...
clientInfo_t *
CG_SortedFireTeamPlayerForPosition ( int pos, int max )
{
	int				i, cnt = 0;
	fireteamData_t	*f = CG_IsOnFireteam( cg.clientNum );
	if ( !f )
...
/* */
void
CG_DrawFireTeamOverlay ( rectDef_t *rect )
{
	int				x = rect->x;
	...
	for ( i = 0; i < 6; i++ )
	{
		...
		x += 4;
		CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f, tclr, BG_ClassLetterForNumber( ci->cls), 0, 0,
		...
		CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f, tclr,
						   ...
						   ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2 );
		x += 22;
		CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f, tclr, ci->name, 0, 17, ITEM_TEXTSTYLE_SHADOWED,
		...
		{
			CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f, colorYellow, va( "%i", ci->health < 0 ? 0 : ci->health),
							   0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2 );
		}
		else
		{
			CG_Text_Paint_Ext( x, y + FT_BAR_HEIGHT, .2f, .2f, colorRed, va( "%i", ci->health < 0 ? 0 : ci->health), 0,
...
/* */
qboolean
CG_FireteamGetBoxNeedsButtons ( void )
{
...
CG_FireteamGetBoxText ( void )
{
	...
	if ( cgs.propositionEndTime > cg.time )
	{
		...
		return
			(
				va( "Accept %s's proposition to invite %s to join your fireteam?",
...
/* */
qboolean
CG_FireteamHasClass ( int classnum, qboolean selectedonly )
{
	fireteamData_t	*ft;
	int				i;
...
/* */
const char *
CG_BuildSelectedFirteamString ( void )
{
	char			buffer[256];
	clientInfo_t	*ci;
	int				cnt = 0;
