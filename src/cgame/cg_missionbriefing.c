...
/* */
const char *
CG_DescriptionForCampaign ( void )
{
...
/* */
qboolean
CG_FindCampaignInFile ( char *filename, char *campaignShortName, cg_campaignInfo_t *info )
{
	int			handle;
	pc_token_t	token;
...
/* */
qboolean
CG_FindArenaInfo ( char *filename, char *mapname, arenaInfo_t *info )
{
	int			handle;
	pc_token_t	token;
	const char	*dummy;
	...
	while ( trap_PC_ReadToken( handle, &token) )
	{
		if ( *token.string == '}' )
		{
			...
			(
				!Q_stricmp( token.string, "objectives") ||
		...
		else if ( !Q_stricmp( token.string, "map") )
		{
			...
			(
				!Q_stricmp( token.string, "Timelimit") ||
...
#ifdef __SUPREMACY__
	if ( cgs.gametype == GT_SUPREMACY_CAMPAIGN )
	{
