...
*/
void
CG_CheckPlayerstateEvents_wolf ( playerState_t *ps, playerState_t *ops )
{
	int			i;
	int			event;
	centity_t	*cent;
...
CG_CheckPlayerstateEvents ( playerState_t *ps, playerState_t *ops )
{
	...
	for ( i = ps->eventSequence - MAX_EVENTS; i < ps->eventSequence; i++ )
	{
		...
		(
			i >= ops->
				eventSequence			// or the server told us to play another event instead of a predicted event we already issued
				// or something the server told us changed our prediction causing a different event
					||
			(
				i > ops->eventSequence - MAX_EVENTS && ps->events[i & (MAX_EVENTS - 1)] != ops->events[i &
					(MAX_EVENTS - 1)]
			)
		)
		{
...
CG_CheckLocalSounds ( playerState_t *ps, playerState_t *ops )
{
	...
	/*	if ( ps->stats[STAT_HEALTH] < ops->stats[STAT_HEALTH] - 1 ) { // dont do that
		if ( ps->stats[STAT_HEALTH] > 0 ) {
			CG_PainEvent( &cg.predictedPlayerEntity, ps->stats[STAT_HEALTH], qfalse , ps->persistant[PERS_TEAM] );
			cg.painTime = cg.time;
		}
	}*/
	if ( cg_hitsound_allowed.integer > 0 && cg_hitsound.integer > 0 )
	{
		if ( ps->persistant[PERS_HEADSHOTS] > ops->persistant[PERS_HEADSHOTS] )
		{
			trap_S_StartLocalSound( cgs.media.headshot, CHAN_AUTO );

			//	CG_Printf( "Got'%i' headshots!.\n", ps->persistant[PERS_HEADSHOTS] );
		}
		else if ( ps->persistant[PERS_HITS] > ops->persistant[PERS_HITS] )
		{
			trap_S_StartLocalSound( cgs.media.hitsound, CHAN_AUTO );
		...
		{
			trap_S_StartLocalSound( cgs.media.hitteammate, CHAN_AUTO );

			//	CG_Printf( "^1 TEAMATE!.\n" );
		}
	}

	if ( cg.pmext.sprintTime < 32000 )
	{
		if ( ps->stats[STAT_HEALTH] > 0 )
		{
			CG_ExertSound( cg.pmext.sprintTime / 5, cg.time );
		}

		//CG_Printf( "Sprinting , schnell , schnell!! %i \n" , cg.pmext.sprintTime );
	}

	if ( ps->eFlags & EF_DEAD )
	{
		if ( cg.time - soundTime < trap_S_GetSoundLength( cgs.media.deathloop) )
		{
			cg.deathloop = qfalse;
		}
		else if ( !cg.deathloop )
		{
			trap_S_StartLocalSound( cgs.media.deathloop, CHAN_VOICE_BG );
			soundTime = cg.time;
			cg.deathloop = qtrue;
		}
	}
	else
	{
		...
		cg.deathloop = qfalse;
		soundTime = cg.time - trap_S_GetSoundLength( cgs.media.deathloop );
	}
	...
	if ( cgs.timelimit > 0 && cgs.gamestate == GS_PLAYING )
	{
		...
		(
			...
			(msec > (cgs.timelimit) * 60 * 1000 - 30000) &&
...
CG_TransitionPlayerState ( playerState_t *ps, playerState_t *ops )
{
	...
	if ( ps->damageEvent != ops->damageEvent && ps->damageCount && !(ps->eFlags & EF_TANK) )
	{
	...
	if ( !(ps->eFlags & EF_PRONE) && ops->eFlags & EF_PRONE )
	{
		...
		if ( cg.weaponSelect == WP_30CAL_SET )
		{
			CG_FinishWeaponChange( cg.weaponSelect, ps->nextWeapon );
		}
	}
