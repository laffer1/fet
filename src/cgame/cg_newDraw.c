...
int
CG_TrimLeftPixels ( char *instr, float scale, float w, int size )
{
	char	buffer[1024];
	char	*p, *s;
	int		tw;
...
/* */
void
CG_FitTextToWidth_Ext ( char *instr, float scale, float w, int size, fontInfo_t *font )
{
	char	buffer[1024];
	char	*s, *p, *c, *ls;
	int		l;
...
/* */
void
CG_FitTextToWidth2 ( char *instr, float scale, float w, int size )
{
	char	buffer[1024];
	char	*s, *p, *c, *ls;
	int		l;
...
/* */
void
CG_FitTextToWidth_SingleLine ( char *instr, float scale, float w, int size )
{
	char	*s, *p;
	char	buffer[1024];
	Q_strncpyz( buffer, instr, 1024 );
...
static const char	*weapname[] =
{
	...
	"Pliers",
	"Smoke Marker",
	"Karabiner43 with GPG40",
	"M1 Garand with M7",
	"Scoped M1 Garand",
	"Landmine",
	"Satchel",
	"Satchel Remote Controll",
	"Smoke Grenade",
	"Portable MachineGewehr42",
	"Scoped Karabiner43",
...
*/
void
CG_DrawPlayerWeaponIcon ( rectDef_t *rect, qboolean drawHighlighted, int align, vec4_t *refcolor )
{
	int			size;
	...
	if
	(
		cg.predictedPlayerEntity.currentState.eFlags & EF_MG42_ACTIVE ||
		cg.predictedPlayerEntity.currentState.eFlags & EF_BROWNING_ACTIVE ||
	...
	if ( cg.predictedPlayerEntity.currentState.eFlags & EF_AAGUN_ACTIVE )
	{
		icon = cgs.media.aagunIcon;
		size = 3;
	}
	else if
		(
			(
				cg.predictedPlayerEntity.currentState.eFlags & EF_MOUNTEDTANK &&
				cg_entities[cg_entities[cg_entities[cg.snap->ps.clientNum].tagParent].tankparent].currentState.
					density & 8
			) ||
			(cg.predictedPlayerEntity.currentState.eFlags & EF_BROWNING_ACTIVE)
		)
...
*/
void
CG_DrawCursorhint ( rectDef_t *rect )
{
	float		*color;
	...
	{																// this prevents to see this for your weapon when dead
		return;
	}
	...
	switch ( cg.cursorHintIcon )
	{
		...
		case HINT_MG42:
			...
			break;
		case HINT_BROWNING:
			icon = cgs.media.browningHintShader;
			break;
		case HINT_AAGUN:
			icon = cgs.media.aagunHintShader;
			break;
		case HINT_BREAKABLE:
		...
		case HINT_CLIPBOARD:
			...
			break;
		case HINT_WEAPON:
			icon = cg_weapons[cg.cursorHintValue].weaponIcon[1];	// hack
	...
	if ( cg.predictedPlayerState.weapon == WP_MOBILE_MG42 || cg.predictedPlayerState.weapon == WP_30CAL )
	{
		trace_t trace;
		trace_t trace2;
		...
		{
			CG_DrawStringExt( rect->x - 25, rect->y + 25, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
			size = weapIconDrawSize( cg.predictedPlayerState.weapon );
			rect->h *= 0.9375;
			rect->w *= 1.875;
			CG_DrawPic( rect->x - halfscale, rect->y - halfscale, rect->w + scale, rect->h + scale,
						cg_weapons[cg.predictedPlayerState.weapon].weaponIcon[1] );
			cg.cursorHintIcon = 0;									// dont draw any other icon/ HINT
		}
	}
	...
	// Jaquboss , extended hints , a lá CoD
	if ( cg.cursorHintIcon == HINT_WEAPON )
	{
		weapon_t	secondary;
		int			size;
		char		*s, *key;
		const char	*weapname1;
		const char	*weapname2;
		if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_AKIMBO_SILENCEDCOLT) )
		{
			secondary = WP_AKIMBO_SILENCEDCOLT;
		}
		else if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_AKIMBO_SILENCEDLUGER) )
		{
			secondary = WP_AKIMBO_SILENCEDLUGER;
		}
		else if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_SILENCER) )
		{
			secondary = WP_SILENCER;
		}
		else if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_SILENCED_COLT) )
		{
			secondary = WP_SILENCED_COLT;
		}
		else if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_COLT) )
		{
			secondary = WP_COLT;
		}
		else if ( COM_BitCheck( cg.predictedPlayerState.weapons, WP_LUGER) )
		{
			secondary = WP_LUGER;
		}
		else
		...
		//weapname1 = weapname[cg.predictedPlayerEntity.currentState.density];
		weapname2 = weapname[cg.cursorHintValue];
		if ( BG_IsSecondaryWeapon( cg.cursorHintValue) )
		{
			weapname1 = weapname[secondary];
		}
		else
		{
			weapname1 = weapname[cg.predictedPlayerEntity.currentState.density];
		...
		size = weapIconDrawSize( cg.cursorHintValue );
		rect->h *= 1.5;
		if ( size == 2 )
		{
			rect->w *= 2.25;										// wide weapon icons
		}
		else
		{
			rect->w *= 1.5;
		...
		key = BindingFromName( "+activate" );
		if ( BG_IsSecondaryWeapon( cg.cursorHintValue) && secondary == WP_NONE )
		{
			s = va( "Press '%s' to pickup %s", key, weapname2 );
		}
		else if ( BG_IsPrimaryWeapon( cg.cursorHintValue) && cg.predictedPlayerEntity.currentState.density == 0 )
		{
			s = va( "Press '%s' to pickup %s", key, weapname2 );
		}
		else
		{
			s = va( "Press '%s' to swap %s for %s", key, weapname1, weapname2 );
		}

		CG_DrawStringExt( rect->x - 100, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}
	...
	if ( cg.cursorHintIcon == HINT_AMMO )
	{
		char	*s, *key;
		key = BindingFromName( "+activate" );
		s = va( "Press '%s' to pickup ammo for your weapons", key );
		CG_DrawStringExt( rect->x - 75, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}

	if ( cg.cursorHintIcon == HINT_HEALTH )
	{
		char	*s, *key;
		key = BindingFromName( "+activate" );
		s = va( "Press '%s' to pickup health pack", key );
		CG_DrawStringExt( rect->x - 50, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}

	if ( cg.cursorHintIcon == HINT_UNIFORM )
	{
		char	*s, *key;
		key = BindingFromName( "+activate" );
		s = va( "Hold '%s' to pickup enemy uniform", key );
		CG_DrawStringExt( rect->x - 50, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}

	if ( cg.cursorHintIcon == HINT_MG42 )
	{
		char	*s, *key;
		key = BindingFromName( "+activate" );
		s = va( "Press '%s' to use this MG42", key );
		CG_DrawStringExt( rect->x - 50, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}

	if ( cg.cursorHintIcon == HINT_BROWNING )
	{
		char	*s, *key;
		key = BindingFromName( "+activate" );
		s = va( "Press '%s' to use this .30 cal MG", key );
		CG_DrawStringExt( rect->x - 75, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}

	if ( cg.cursorHintIcon == HINT_DISARM && cg.snap->ps.stats[STAT_PLAYER_CLASS] == PC_ENGINEER )
	{
		char	*s;
		s = va( "Use pliers to arm/disarm this object" );
		CG_DrawStringExt( rect->x - 85, rect->y + 70, s, colorWhite, qtrue, qtrue, 6, 12, 80 );
	}
	...
	if ( cg.cursorHintValue && cg.cursorHintIcon != HINT_WEAPON )
	{
		if ( yellowbar )
		{
		...
		CG_FilledBar( rect->x, rect->y + rect->h + 4, rect->w, 8, color, NULL, NULL, (float) cg.cursorHintValue / 255.0f,
					  0 );
	}
}
...
/* */
float
CG_GetValue ( int ownerDraw, int type )
{
	switch ( ownerDraw )
	{
		default:
...
*/
void
CG_DrawWeapStability ( rectDef_t *rect )
{
	...
	CG_FilledBar( rect->x, rect->y, rect->w, rect->h, goodColor, badColor, NULL,
				  (float) cg.snap->ps.aimSpreadScale / 255.0f, 2 | 4 | 256 );	// flags (BAR_CENTER|BAR_VERT|BAR_LERP_COLOR)
}
...
*/
void
CG_DrawWeapHeat ( rectDef_t *rect, int align )
{
	...
	CG_FilledBar( rect->x, rect->y, rect->w, rect->h, color, color2, NULL, (float) cg.snap->ps.curWeapHeat / 255.0f,
				  flags );
}
...
{
	rectDef_t	rect;
	if ( cg_drawStatus.integer == 0 || cg_nohud.integer == 0 )
	{
		return;
	}
...
*/
void
CG_EventHandling ( int type, qboolean fForced )
{
	...
	switch ( type )
	{
		...
		default:
			...
			if ( cgs.eventHandling == CGAME_EVENT_GAMEVIEW )
			{
				...
				if ( cg_limboMusic.integer > 0 )
				{
					trap_S_FadeBackgroundTrack( 0.0f, 500, 0 ); // ????? , LIMBOSTOPMUSIC
				}
