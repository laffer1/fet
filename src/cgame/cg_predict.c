...
)
{
	...
	for ( i = 0; i < cg_numSolidEntities; i++ )
	{
		...
		if ( ent->solid == SOLID_BMODEL )
		{
			...
			BG_EvaluateTrajectory( &cent->currentState.apos, cg.physicsTime, angles, qtrue,
								   cent->currentState.effect2Time );
			BG_EvaluateTrajectory( &cent->currentState.pos, cg.physicsTime, origin, qfalse,
								   cent->currentState.effect2Time );
		}
		else
		{
...
CG_TouchItem ( centity_t *cent )
{
	...
	if
	(
		!BG_CanItemBeGrabbed( &cent->currentState, &cg.predictedPlayerState,
							  cgs.clientinfo[cg.snap->ps.clientNum].skill, cgs.clientinfo[cg.snap->ps.clientNum].team)
	)
	{
...
void	CG_AddDirtBulletParticles
		(
			vec3_t		origin,
			vec3_t		dir,
			int			speed,
...
/* */
qboolean
CG_PredictionOk ( playerState_t *ps1, playerState_t *ps2 )
{
	vec3_t	vec;
	int		i;
	...
	if
	(
		ps1->groundEntityNum != ENTITYNUM_WORLD ||
...
#define RESET_PREDICTION	cg.lastPredictedCommand = 0; \
...
/* */
entityState_t *
CG_GetEntityState ( int entityNum )
{
	if ( entityNum < 0 || entityNum >= MAX_GENTITIES || cg_entities[entityNum].currentValid == qfalse )
	{
		return ( NULL );
	}

	return ( &cg_entities[entityNum].currentState );
}
...
CG_PredictPlayerState ( void )
{
	...
	)
	{
		...
		(
			...
			cg_pmove.ps->eFlags & EF_BROWNING_ACTIVE ||
	...
	cg_pmove.character = CG_CharacterForClientinfo( &cgs.clientinfo[cg.snap->ps.clientNum],
	...
	if ( /*(cg_pmove.ps->eFlags & EF_TANK ||*/ cg_pmove.ps->eFlags & EF_PRONE /*)*/ && cg_noCapsule.integer )
	{
	...
	cg_pmove.getentitystate = CG_GetEntityState;
	...
	if ( (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR) || (cg.snap->ps.pm_flags & PMF_LIMBO) )
	{											// JPW NERVE limbo
	...
	for ( cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++ )
	{
		...
		if ( cg.predictedPlayerState.commandTime == oldPlayerState.commandTime )
		{
			...
			else if ( !cg.showGameView || cg_bigLimbo.integer == 0 )
			{
				vec3_t	adjusted;
	...
	else
	{
		...
		// move
		cg.predictedPlayerState.origin[2] += sin( M_PI * 8 * 13 + cg.cameraShakePhase ) *
			x *
			6.0f *
			cg.cameraShakeScale;
		cg.predictedPlayerState.origin[1] += sin( M_PI * 17 * x + cg.cameraShakePhase ) *
			x *
			6.0f *
			cg.cameraShakeScale;
		cg.predictedPlayerState.origin[0] += cos( M_PI * 7 * x + cg.cameraShakePhase ) * x * 6.0f * cg.cameraShakeScale;
	}
}
