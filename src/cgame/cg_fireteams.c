...
#include "cg_local.h"
panel_button_text_t fireteamTitleFont = { 0.19f, 0.19f, { 0.6f, 0.6f, 0.6f, 1.f }, 0, 0, &cgs.media.limboFont1_lo, };
...
const char			*ftMenuRootStringsMsg[] = { "", "", "", "", "", "", "FTAttack", "FTFallBack", NULL };
const char			*ftMenuRootStringsAlphachars[] = { "S", "M", "E", "F", "C", "G", "A", "B", NULL };
const char			*ftMenuSoliderStrings[] = { "Cover Me", "Covering Fire", "Mortar", NULL };
const char			*ftMenuSoliderStringsAlphachars[] = { "C", "F", "M", NULL };
const char			*ftMenuSoliderStringsMsg[] = { "FTCoverMe", "FTCoveringFire", "FTMortar", NULL };
const char			*ftMenuMedicStrings[] = { "Heal Squad", "Heal Me", "Revive Team Mate", "Revive Me", NULL };
const char			*ftMenuMedicStringsAlphachars[] = { "H", "M", "R", "E", NULL };
const char			*ftMenuMedicStringsMsg[] = { "FTHealSquad", "FTHealMe", "FTReviveTeamMate", "FTReviveMe", NULL };
...
const char			*ftMenuEngineerStringsAlphachars[] = { "D", "R", "C", "A", "L", "M", NULL };
...
const char			*ftMenuFieldOpsStrings[] = { "Call Air-Strike", "Call Artillery", "Resupply Squad", "Resupply Me",
		NULL };
...
const char			*ftMenuCovertOpsStringsAlphachars[] = { "E", "D", "I", "U", "S", NULL };
...
/* */
void
CG_Fireteams_MenuTitleText_Draw ( panel_button_t *button )
{
...
const char	*ftOffMenuList[] = { "Apply", "Create", NULL, };
const char	*ftOffMenuListAlphachars[] = { "A", "C", NULL, };
const char	*ftOnMenuList[] = { "Propose", "Leave", NULL, };
const char	*ftOnMenuListAlphachars[] = { "P", "L", NULL, };
const char	*ftLeaderMenuList[] = { "Disband", "Leave", "Invite", "Kick", "Warn", NULL, };
const char	*ftLeaderMenuListAlphachars[] = { "D", "L", "I", "K", "W", NULL, };


/* */
int
CG_CountFireteamsByTeam ( team_t t )
{
	int cnt = 0;
	int i;
...
/* */
void
CG_DrawFireteamsByTeam ( panel_button_t *button, team_t t )
{
...
/* */
int
CG_PlayerSFFromPos ( int pos, int *pageofs )
{
	int x, i;
...
/* */
int
CG_PlayerNFFromPos ( int pos, int *pageofs )
{
	int x, i;
	if ( !CG_IsOnFireteam( cg.clientNum) )
	{
...
/* */
void
CG_DrawPlayerSF ( panel_button_t *button, int *pageofs )
{
	float		y = button->rect.y;
	const char	*str;
	int			i, x;
...
/* */
void
CG_DrawPlayerNF ( panel_button_t *button, int *pageofs )
{
	float		y = button->rect.y;
	const char	*str;
	int			i, x;
...
/* */
void
CG_Fireteams_MenuText_Draw ( panel_button_t *button )
{
	float	y = button->rect.y;
	int		i;
	switch ( cgs.ftMenuMode )
			...
			else
			{
				...
				else
				{
					...
					for ( i = 0; strings[i]; i++ )
					{
						...
						CG_Text_Paint_Ext( button->rect.x, y, button->font->scalex, button->font->scaley,
										   button->font->colour, str, 0, 0, button->font->style, button->font->font );
						y += button->rect.h;
					}
				}
			}
...
/* */
void
CG_Fireteams_KeyHandling ( int key, qboolean down )
{
	if ( down )
	{
...
/* */
qboolean
CG_FireteamCheckExecKey ( int key, qboolean doaction )
{
	if ( key == K_ESCAPE )
	{
		return ( qtrue );
						...
						for ( i = 0; ftMenuRootStrings[i]; i++ )
						{
							if ( key == tolower( *ftMenuRootStringsAlphachars[i]) )
							{
								...
								if ( doaction )
								{
									...
									{
										trap_SendClientCommand( va( "vsay_buddy -1 %s %s", CG_BuildSelectedFirteamString(),
