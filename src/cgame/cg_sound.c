...
*/
int
CG_SoundScriptPrecache ( const char *name )
{
...
*/
int
CG_SoundPickOldestRandomSound ( soundScript_t *sound, vec3_t org, int entnum )
{
	...
	if ( oldestSound )
	{
		...
		else
		{
			return
				(
					trap_S_StartStreamingSound( oldestSound->sounds[pos].filename,
...
/* */
void
CG_AddBufferedSoundScript ( soundScript_t *sound )
{
...
/* */
void
CG_UpdateBufferedSoundScripts ( void )
{
	int i;
...
*/
int
CG_SoundPlaySoundScript ( const char *name, vec3_t org, int entnum, qboolean buffer )
{
	long			hash;
	char			*s;
	soundScript_t	*sound;
...
*/
void
CG_SoundPlayIndexedScript ( int index, vec3_t org, int entnum )
{
	soundScript_t	*sound;
	if ( !index )
	{
...
*/
static void
CG_SoundParseSounds ( char *filename, char *buffer )
{
	char				*token, **text;
	int					s;
	long				hash;
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( token, "sound") )
		{
			...
			token = COM_ParseExt( text, qtrue );
			Q_strncpyz( scriptSound->sounds[scriptSound->numsounds].filename, token,
						sizeof(scriptSound->sounds[0].filename) );
...
static const char	*s_lt_string[] = { "no", "on", "off" };
static const char	*s_bt_string[] = { "no", "global", "nopvs" };


/* */
qboolean
CG_SaveSpeakersToScript ( void )
{
	int				i;
	...
	for ( i = 0; i < BG_NumScriptSpeakers(); i++ )
	{
		...
		Com_sprintf( originStr, sizeof(originStr), "\t\torigin %.2f %.2f %.2f\n", speaker->origin[0],
					 speaker->origin[1], speaker->origin[2] );
		...
		s = va( "\n\tspeakerDef {\n%s%s%s%s%s%s%s%s%s\t}\n", filenameStr, originStr, targetnameStr, loopedStr,
...
/* */
void
CG_AddLineToScene ( vec3_t start, vec3_t end, vec4_t colour )
{
	refEntity_t re;
...
/* */
void
CG_SetViewanglesForSpeakerEditor ( void )
{
	vec3_t	vec;
...
/* */
void
CG_SpeakerInfo_Text ( panel_button_t *button )
{
	char	*s, *ptr, *strptr;
	float	y, wMax, w, h;
	vec4_t	colour;
	...
	Com_sprintf( originStr, sizeof(originStr), "Speaker at %.2f %.2f %.2f\n", editSpeaker->origin[0],
				 editSpeaker->origin[1], editSpeaker->origin[2] );
	...
	s = va( "%s%s%s%s%s%s%s%s%s", originStr, filenameStr, targetnameStr, loopedStr, broadcastStr, waitStr, randomStr,
			volumeStr, rangeStr );
	y = button->rect.y + 8;
		...
		if ( *ptr == '\n' )
		{
			*ptr = '\0';
			CG_Text_Paint_Ext( button->rect.x, y, button->font->scalex, button->font->scaley, button->font->colour,
							   strptr, 0, 0, button->font->style, button->font->font );
			y += 8;
			strptr = ptr + 1;
		}
	}
...
static panel_button_t	*speakerInfoButtons[] = { &speakerInfo, NULL };


/* */
void
CG_SpeakerEditor_RenderEdit ( panel_button_t *button )
{
	vec4_t	colour;
...
/* */
void
CG_SpeakerEditor_RenderButton ( panel_button_t *button )
{
	vec4_t	colour;
	float	oldX;
	if ( button == BG_PanelButtons_GetFocusButton() )
...
/* */
char *
CG_GetStrFromStrArray ( const char *in, const int index )
{
	char	*ptr, *s;
	int		i;
	s = ptr = (char *) in;
...
/* */
void
CG_SpeakerEditor_RenderDropdown ( panel_button_t *button )
{
	vec4_t		colour;
	float		textboxW;
	rectDef_t	rect;
	...
	CG_Text_Paint_Ext( rect.x + (rect.w - CG_Text_Width_Ext( "V", button->font->scalex, 0, button->font->font)) / 2.f,
					   button->rect.y + 9.f, button->font->scalex, button->font->scaley, colour, "V", 0, 0, 0,
					   button->font->font );
	...
	CG_Text_Paint_Ext( button->rect.x + (textboxW - CG_Text_Width_Ext( s, button->font->scalex, 0, button->font->font)) / 2.f,
					   button->rect.y + 9.f, button->font->scalex, button->font->scaley, button->font->colour, s, 0, 0,
	...
	if ( button == BG_PanelButtons_GetFocusButton() )
	{
		...
		for ( i = 0; i < button->data[0]; i++ )
		{
			...
			CG_Text_Paint_Ext( rect.x + (textboxW - CG_Text_Width_Ext( s, button->font->scalex, 0, button->font->font)) / 2.f,
							   rect.y + 9.f, button->font->scalex, button->font->scaley, button->font->colour, s, 0, 0,
							   button->font->style, button->font->font );
		}
...
/* */
void
CG_SpeakerEditor_Back ( panel_button_t *button )
{
	vec4_t	colour;
	VectorCopy( colorMdBlue, colour );
	colour[3] = .5f;
...
/* */
void
CG_SpeakerEditor_LocInfo ( panel_button_t *button )
{
...
/* */
qboolean
CG_SpeakerEditor_NoiseEdit_KeyDown ( panel_button_t *button, int key )
{
	if ( button == BG_PanelButtons_GetFocusButton() )
	{
		if ( key == K_TAB )
...
/* */
void
CG_SpeakerEditor_NoiseEditFinish ( panel_button_t *button )
{
...
/* */
void
CG_SpeakerEditor_TargetnameEditFinish ( panel_button_t *button )
{
...
/* */
qboolean
CG_SpeakerEditor_Dropdown_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
...
/* */
qboolean
CG_SpeakerEditor_Looped_KeyUp ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		if ( button == BG_PanelButtons_GetFocusButton() )
...
/* */
qboolean
CG_SpeakerEditor_Broadcast_KeyUp ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		if ( button == BG_PanelButtons_GetFocusButton() )
...
/* */
void
CG_SpeakerEditor_WaitEditFinish ( panel_button_t *button )
{
	if ( *button->text )
	{
...
/* */
void
CG_SpeakerEditor_RandomEditFinish ( panel_button_t *button )
{
	if ( *button->text )
	{
...
/* */
void
CG_SpeakerEditor_VolumeEditFinish ( panel_button_t *button )
{
	if ( *button->text )
	{
...
/* */
void
CG_SpeakerEditor_RangeEditFinish ( panel_button_t *button )
{
	if ( *button->text )
	{
...
/* */
qboolean
CG_SpeakerEditor_Ok_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		BG_PanelButtons_SetFocusButton( button );
...
/* */
qboolean
CG_SpeakerEditor_Ok_KeyUp ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		if ( button == BG_PanelButtons_GetFocusButton() )
...
/* */
qboolean
CG_SpeakerEditor_Cancel_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		BG_PanelButtons_SetFocusButton( button );
...
/* */
qboolean
CG_SpeakerEditor_Cancel_KeyUp ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		if ( button == BG_PanelButtons_GetFocusButton() )
...
/* */
qboolean
CG_SpeakerEditor_Delete_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		BG_PanelButtons_SetFocusButton( button );
...
/* */
qboolean
CG_SpeakerEditor_Delete_KeyUp ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		if ( button == BG_PanelButtons_GetFocusButton() )
...
CG_SpeakerEditorDraw ( void )
{
	...
	if ( !editSpeakerActive )
	{
		...
		CG_Text_Paint_Ext( 8, y, .2f, .2f, colour, va(
							   "Current amount of speakers in map: %i (inpvs: %i max in map: %i)", BG_NumScriptSpeakers(),
		...
		Q_strupr( binding[0] );
		Q_strupr( binding[1] );
		CG_Text_Paint_Ext( 8, y + 10, .2f, .2f, colour, va( "Create new speaker: %s%s",
						   bindingkey[0] != -1 ? binding[0] : "???", bindingkey[1] != -1 ? va( " or %s", binding[1]) : ""
		...
		Q_strupr( binding[0] );
		Q_strupr( binding[1] );
		CG_Text_Paint_Ext( 8, y + 20, .2f, .2f, colour, va( "Modify target speaker: %s%s",
						   bindingkey[0] != -1 ? binding[0] : "???", bindingkey[1] != -1 ? va( " or %s", binding[1]) : ""
						   ), 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2 );
		if ( undoSpeakerIndex != -2 )
		{
			...
			Q_strupr( binding[0] );
			Q_strupr( binding[1] );
			CG_Text_Paint_Ext( 8, y + 30, .2f, .2f, colour, va( "Undo %s speaker: %s%s",
							   undoSpeakerIndex == -1 ? "remove" : "modify", bindingkey[0] != -1 ? binding[0] : "???",
							   bindingkey[1] != -1 ? va( " or %s", binding[1]) : ""), 0, 0, ITEM_TEXTSTYLE_SHADOWED,
							   &cgs.media.limboFont2 );
		}
		...
		trap_R_DrawStretchPic( x + 0.5 * (cg.refdef_current->width - w), y + 0.5 * (cg.refdef_current->height - h), w,
							   h, 0, 0, 1, 1, cgs.media.crosshairShader[cg_drawCrosshair.integer % NUM_CROSSHAIRS] );
		if ( cg.crosshairShaderAlt[cg_drawCrosshair.integer % NUM_CROSSHAIRS] )
		{
			trap_R_DrawStretchPic( x + 0.5 * (cg.refdef_current->width - w), y + 0.5 * (cg.refdef_current->height - h),
...
/* */
void
CG_SpeakerEditor_KeyHandling ( int key, qboolean down )
{
		...
		switch ( key )
		{
			case K_MOUSE1:
				...
				else if ( editSpeakerHandle.activeAxis == -1 )
				{
					...
					for ( i = 0; i < 3; i++ )
					{
						dir[i] = cg.refdef_current->viewaxis[0][i] *
							1.f +
							cg.refdef_current->viewaxis[1][i] *
							r +
							cg.refdef_current->viewaxis[2][i] *
							u;
					}
...
/* */
void
CG_SpeakerEditorMouseMove_Handling ( int x, int y )
{
	if ( !cg.editingSpeakers )
	{
		return;
...
/* */
void
CG_DeActivateEditSoundMode ( void )
{
...
// Normal Use
void
CG_ToggleActiveOnScriptSpeaker ( int index )
{
...
/* */
void
CG_UnsetActiveOnScriptSpeaker ( int index )
{
	bg_speaker_t	*speaker = BG_GetScriptSpeaker( index );
	if ( speaker )
	{
...
/* */
void
CG_SetActiveOnScriptSpeaker ( int index )
{
	bg_speaker_t	*speaker = BG_GetScriptSpeaker( index );
	if ( speaker )
	{
...
/* */
static void
CG_PlayScriptSpeaker ( bg_speaker_t *speaker, qboolean global )
{
	switch ( speaker->loop )
	{
