...
/* */
void
CG_TransformToCommandMapCoord ( float *coord_x, float *coord_y )
{
...
/* */
static qboolean
CG_ScissorEntIsCulled ( mapEntityData_t *mEnt, mapScissor_t *scissor )
{
	if ( !scissor->circular )
	{
		if
		(
			mEnt->automapTransformed[0] < scissor->tl[0] ||
...
/* */
static qboolean
CG_ScissorPointIsCulled ( vec2_t vec, mapScissor_t *scissor )
{
	if ( !scissor->circular )
	{
...
*/
void
CG_TransformAutomapEntity ( void )
{
	int i;
...
/* */
void
CG_AdjustAutomapZoom ( int zoomIn )
{
	if ( zoomIn )
	{
		automapZoom *= 1.2;
...
// END		xkan, 9/19/2002
void
CG_ParseMapEntity ( int *mapEntityCount, int *offset, team_t team )
{
...
*/
void
CG_ParseMapEntityInfo ( int axis_number, int allied_number )
{
	int i, offset;
	mapEntityCount = 0;
...
/* */
static void
CG_DrawGrid ( float x, float y, float w, float h, mapScissor_t *scissor )
{
	vec2_t	step;
	vec2_t	dim_x, dim_y;
	vec4_t	line;
			...
			(
				(
					((cg.mapcoordsMaxs[0] - cg.mapcoordsMins[0]) / gridStep[0]) -
						((int) ((cg.mapcoordsMaxs[0] - cg.mapcoordsMins[0]) / gridStep[0]))
				) * gridStep[0]
			);
		gridStartCoord[1] = .5f *
			(
				(
					((cg.mapcoordsMins[1] - cg.mapcoordsMaxs[1]) / gridStep[1]) -
						((int) ((cg.mapcoordsMins[1] - cg.mapcoordsMaxs[1]) / gridStep[1]))
				) * gridStep[1]
			);
		gridInitDone = qtrue;
	}
		...
		for ( coord_char[0] = ('A' - 1); grid_x < dim_x[1]; grid_x += step[0], coord_char[0]++ )
		{
			if ( coord_char[0] >= 'A' )
			{
				...
				CG_Text_Paint_Ext( (x + grid_x) - (.5f * step[0]) - (.5f * text_width),
								   y + dim_y[0] + textOrigin[1] + 1.5f * text_height, 0.2f, 0.2f, colorBlack,
								   coord_char, 0, 0, 0, &cgs.media.limboFont2 );
			}
		...
		for ( coord_int = -1; grid_y < dim_y[1]; grid_y += step[1], coord_int++ )
		{
			if ( coord_int >= 0 )
			{
				...
				CG_Text_Paint_Ext( x + dim_x[0] + textOrigin[0] + .5f * text_width,
								   (y + grid_y) - (.5f * step[1]) + (.5f * text_height), 0.2f, 0.2f, colorBlack,
								   coord_char, 0, 0, 0, &cgs.media.limboFont2 );
			}
...
)
{
	...
	switch ( mEnt->type )
	{
		...
		case ME_PLAYER_DM:
			...
			if ( mEnt->type == ME_PLAYER_DM && cent->currentState.clientNum != cg.snap->ps.clientNum )
			{
				return;
			}
			...
			if ( cent - cg_entities == cg.clientNum )
			{
				if ( !scissor )
				{
					mEnt->transformed[0] =
						(
							(cg.predictedPlayerEntity.lerpOrigin[0] - cg.mapcoordsMins[0]) *
							cg.mapcoordsScale[0]
						) *
						w;
					mEnt->transformed[1] =
						(
							(cg.predictedPlayerEntity.lerpOrigin[1] - cg.mapcoordsMins[1]) *
							cg.mapcoordsScale[1]
						) *
						h;
				}
			...
			else if ( ci->team == snap->ps.persistant[PERS_TEAM] && cent->currentValid )
			{
				...
				{
					mEnt->automapTransformed[0] = ( (cent->lerpOrigin[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
						w *
						scissor->zoomFactor;
					mEnt->automapTransformed[1] = ( (cent->lerpOrigin[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
						h *
						scissor->zoomFactor;
				}
			...
			else
			{
				...
				CG_DrawRotatedPic( icon_pos[0] - 1, icon_pos[1] - 1, icon_extends[0] + 2, icon_extends[1] + 2,
								   classInfo->arrow, (0.5 - (mEnt->yaw - 180.f) / 360.f) );
				trap_R_SetColor( NULL );
			}
		...
		case ME_COMMANDMAP_MARKER:
				...
				for ( j = 0; j < cg.snap->numEntities; j++ )
				{
					if ( cg.snap->entities[j].eType == ET_OID_TRIGGER && cg.snap->entities[j].teamNum == mEnt->data )
					{
						...
						else
						{
							mEnt->automapTransformed[0] =
								(
									(cent->lerpOrigin[0] - cg.mapcoordsMins[0]) *
									cg.mapcoordsScale[0]
								) *
								w *
								scissor->zoomFactor;
							mEnt->automapTransformed[1] =
								(
									(cent->lerpOrigin[1] - cg.mapcoordsMins[1]) *
									cg.mapcoordsScale[1]
								) *
								h *
								scissor->zoomFactor;
			...
			else if ( mEnt->type == ME_COMMANDMAP_MARKER )
			{
				...
				{
					mEnt->automapTransformed[0] = ( (oidInfo->origin[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
						w *
						scissor->zoomFactor;
					mEnt->automapTransformed[1] = ( (oidInfo->origin[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
						h *
						scissor->zoomFactor;
				}
			}
...
CG_DrawAutoMap ( void )
{
	...
	automapTransformed[0] = ( (cg.predictedPlayerEntity.lerpOrigin[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
		w *
		mapScissor.zoomFactor;
	automapTransformed[1] = ( (cg.predictedPlayerEntity.lerpOrigin[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
		h *
		mapScissor.zoomFactor;
...
int
CG_DrawSpawnPointInfo ( int px, int py, int pw, int ph, qboolean draw, mapScissor_t *scissor, int expand )
{
	int		i;
	char	buffer[64];
	vec2_t	point;
...
#define SPAWN_SIZEUPTIME	1000.f
	for ( i = 1; i < cg.spawnCount; i++ )
	{
		...
		if ( scissor )
		{
			point[0] = ( (cg.spawnCoordsUntransformed[i][0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
				pw *
				scissor->zoomFactor;
			point[1] = ( (cg.spawnCoordsUntransformed[i][1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
				ph *
				scissor->zoomFactor;
		}
		else
		...
		if ( changetime )
		{
			...
			(
				...
				(
					!expanded &&
					BG_RectContainsPoint( point[0] - FLAGSIZE_NORMAL * 0.5f, point[1] - FLAGSIZE_NORMAL * 0.5f,
										  FLAGSIZE_NORMAL, FLAGSIZE_NORMAL, cgDC.cursorx, cgDC.cursory)
				)
			)
		{
			if ( draw )
		...
		else
		{
			if ( draw )
			{
				...
				if ( !scissor )
				{
					...
					CG_Text_Paint_Ext( point[0] + FLAGSIZE_NORMAL * 0.25f, point[1], 0.2f, 0.2f, colorWhite, buffer, 0,
...
/* */
void
CG_DrawMortarMarker ( int px, int py, int pw, int ph, qboolean draw, mapScissor_t *scissor, int expand )
{
	if ( cg.lastFiredWeapon == WP_MORTAR_SET && cg.mortarImpactTime >= 0 )
	{
		...
		else
		{
			...
			if ( scissor )
			{
				point[0] = ( (cg.mortarImpactPos[0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
					pw *
					scissor->zoomFactor;
				point[1] = ( (cg.mortarImpactPos[1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
					ph *
					scissor->zoomFactor;
			}
			else
	...
	if ( COM_BitCheck( cg.snap->ps.weapons, WP_MORTAR_SET) )
	{
		...
		for ( i = 0; i < MAX_CLIENTS; i++ )
		{
			...
			if ( fadeTime < 5000 )
			{
				...
				if ( scissor )
				{
					point[0] = ( (cg.artilleryRequestPos[i][0] - cg.mapcoordsMins[0]) * cg.mapcoordsScale[0] ) *
						pw *
						scissor->zoomFactor;
					point[1] = ( (cg.artilleryRequestPos[i][1] - cg.mapcoordsMins[1]) * cg.mapcoordsScale[1] ) *
						ph *
						scissor->zoomFactor;
				}
				else
...
mapEntityData_t *
CG_ScanForCommandCentreEntity ( void )
{
...
/* */
qboolean
CG_CommandCentreLayersClick ( void )
{
	int x, y;
	int i;
	if ( !cgs.ccLayers )
...
/* */
qboolean
CG_CommandCentreSpawnPointClick ( void )
{
	int		i;
	vec2_t	point;
	if ( cgs.ccFilter & CC_FILTER_SPAWNS )
...
/* */
void
CG_CommandMap_SetHighlightText ( const char *text, float x, float y )
{
...
/* */
void
CG_CommandMap_DrawHighlightText ( void )
{
