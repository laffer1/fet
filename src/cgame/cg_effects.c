...
*/
void
CG_BubbleTrail ( vec3_t start, vec3_t end, float size, float spacing )
{
	vec3_t	move;
	vec3_t	vec;
	float	len;
...
*/
void
CG_AddBloodTrails ( vec3_t origin, vec3_t dir, int speed, int duration, int count, float randScale )
{
	localEntity_t	*le;
	refEntity_t		*re;
	vec3_t			velocity;
...
*/
void
CG_LoseHat ( centity_t *cent, vec3_t dir )
{
	clientInfo_t	*ci;
	int				clientNum;
	...
	{									// NPCs don't loose their hat for now...
		return;
	}
	...
	if ( clientNum < 0 || clientNum >= MAX_CLIENTS )
	{
		CG_Error( "CG_LoseHat: Bad clientNum on player entity" );
	}
#endif //__NPC__	
...
*/
int
CG_GetOriginForTag ( centity_t *cent, refEntity_t *parent, char *tagName, int startIndex, vec3_t org, vec3_t axis[3] )
{
	int				i;
...
/* */
void
CG_GibPlayer ( centity_t *cent, vec3_t playerOrigin, vec3_t gdir )
{
	...
	if ( cg_blood.integer )
	{
		...
		if ( (clientNum < 0 || clientNum >= MAX_CLIENTS) )
		{
			CG_Error( "CG_GibPlayer: Bad clientNum on player entity" );
		}
#endif //__NPC__
		ci = &cgs.clientinfo[clientNum];
		character = CG_CharacterForClientinfo( ci, cent );
		...
		for
		(
			gibIndex = 0, count = 0, foundtag = qtrue;
		...
		)
		{
			...
			(
				tagIndex = 0;
				(tagIndex = CG_GetOriginForTag( cent, re, gibTags[gibIndex], tagIndex, origin, axis)) >= 0;
				count++, tagIndex++
			)
			{
				foundtag = qtrue;
...
#define GIB_BLOOD_DOTS	3
		for ( i = 0, count = 0; i < GIB_BLOOD_DOTS * 2; i++ )
		{
			...
			if ( trace.fraction < 1.0 )
			{
				...
				trap_R_ProjectDecal( cgs.media.bloodDotShaders[rand() % 5], 1, (vec3_t *) trace.endpos, projection,
...
*/
void
CG_SparklerSparks ( vec3_t origin, int count )
{
...
/* */
float
lt_random ( int thisrandseed, int t )
{
...
/* */
float
lt_crandom ( int thisrandseed, int t )
{
	return
		(
			(
				2.0 *
					(
						(float) randtable[(thisrandseed + t + (cg.time / LT_MS) * (cg.time / LT_MS)) % NUMRANDTABLE] /
							(float) LT_RANDMAX
					)
			) - 1.0
		);
...
*/
void
CG_ProjectedSpotLight ( vec3_t start, vec3_t dir )
{
	vec3_t	end;
	trace_t tr;
	float	alpha, radius;
...
#define CORESCALE	0.6f
	...
	if ( !(flags & SL_NOIMPACT) )
	{
		if ( hitDist )
		{
			...
			projection[3] = radius * 2.0f;
			CG_ImpactMark( cgs.media.spotLightShader, tr.endpos, projection, radius, colorNorm[0], colorNorm[1],
						   colorNorm[2], 1.0f, 1.0f, -1 );
		}
	}
	...
	if ( !(flags & SL_NOFLARE) )
	{
		...
		if ( lightInEyes )
		{
			...
			trap_R_AddCoronaToScene( start, colorNorm[0], colorNorm[1], colorNorm[2], coronasize,
		...
		else
		{
			...
			trap_R_AddCoronaToScene( start, colorNorm[0], colorNorm[1], colorNorm[2], 0, cent->currentState.number,
									 qfalse );
		}
	}
}

// UQ1: RTCW Single Player Version... Because the ET one just doesn't work!!!
...
//
	...
	if(!(flags & SL_NOIMPACT)) {
		if(hitDist) {
			...
			CG_ImpactMarkOld( cgs.media.spotLightShader, impactPos, proj, 0, colorNorm[0], colorNorm[1], colorNorm[2], 0.3f, qfalse, radius, qtrue, -1 );
...
//		trap_R_AddLightToScene(dlightLoc, radius*2, colorNorm[0], colorNorm[1], colorNorm[2], 0);	// ,REF_JUNIOR_DLIGHT);
		...
		trap_R_AddLightToScene(dlightLoc, radius, radius*2, 0.3, 0.3, 0.3, /*shader*/0, /*flags*/0);	// ,REF_JUNIOR_DLIGHT);
/*
void	trap_R_AddLightToScene(	vec3_t org,	float radius,float intensity,float r,float g,float b,qhandle_t hShader,	int flags)
// RTCW
void	trap_R_AddLightToScene( vec3_t org, float intensity, float r, float g, float b, int overdraw )
*/
...
static smokesprite_t *
DeAllocSmokeSprite ( smokesprite_t *dealloc )
{
...
/* */
static qboolean
CG_SmokeSpritePhysics ( smokesprite_t *smokesprite, const float dist )
{
	trace_t tr;
	vec3_t	oldpos;
...
/* */
qboolean
CG_SpawnSmokeSprite ( centity_t *cent, float dist )
{
...
/* */
void
CG_RenderSmokeGrenadeSmoke ( centity_t *cent, const weaponInfo_t *weapon )
{
	...
	if ( cent->currentState.effect1Time > 16 )
	{
		...
		(
			...
			(
				cent->currentState.groundEntityNum != ENTITYNUM_WORLD && !VectorCompare( cent->lastLerpOrigin,
																						 cent->lerpOrigin)
			)
		)
		{
		...
		if ( smokesprite->dist > (radius * .5f * .8f) )
		{
			color[3] =
				(
					...
					smokesprite->colour[3] *
					((smokesprite->dist - (radius * .5f * .8f)) / ((radius * .5f) - (radius * .5f * .8f)))
				) *
				0xff;
		}
		else
...
// For AIMod Bot Node Editing (node display)...
...
/* */
void
CG_GoalNode ( int client, vec3_t org, int score )
{
	localEntity_t	*le;
	refEntity_t		*re;
	vec3_t			angles;
...
*/
void
CG_ScorePlum ( int client, vec3_t org, int score, int type, qboolean big, int displayIcon )
{
	localEntity_t	*le;
	refEntity_t		*re;
	vec3_t			angles;
	...
	le->startTime = cg.time;
	le->endTime = cg.time + 300;	//4000;
	le->lifeRate = 1.0 / ( le->endTime - le->startTime );
	if ( displayIcon > 0 )
	{
		le->displayIcon = displayIcon;
	}
...
/* */
void
CG_TestLine ( vec3_t start, vec3_t end, int time, unsigned int color, int radius )
{
	localEntity_t	*le;
	refEntity_t		*re;
	le = CG_AllocLocalEntity();
