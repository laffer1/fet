...
/* */
int
WM_DrawObjectives ( int x, int y, int width, float fade )
{
	const char	*s, *str;
	int			tempy, rows;
	...
	else
	{
		...
		if ( cgs.gametype != GT_WOLF_LMS )
		{
			...
			if ( msec )
			{
				...
				CG_Text_Paint_Ext( 640 - 20 - CG_Text_Width_Ext( s, 0.25f, 0, &cgs.media.limboFont1), y + 13, 0.25f,
								   0.25f, tclr, s, 0, 0, 0, &cgs.media.limboFont1 );
			}
		}
		...
		else if ( cgs.gametype == GT_WOLF_CAMPAIGN || cgs.gametype == GT_SUPREMACY_CAMPAIGN )
		{
#else //__SUPREMACY__
		}
...
/* */
static void
WM_DrawClientScore ( int x, int y, score_t *score, float *color, float fade )
{
	int				maxchars, offset;
	int				i, j;
	float			tempx;
	...
	if ( score->client == cg.snap->ps.clientNum )
	{
		...
		CG_FillRect( x - 5, y, (INFO_PLAYER_WIDTH + INFO_CLASS_WIDTH + INFO_SCORE_WIDTH + INFO_LATENCY_WIDTH + 5),
		...
		if ( ci->team == TEAM_SPECTATOR ) {
			...
			if( cg_gameType.integer == GT_WOLF_LMS || cg_gameType.integer == GT_WOLF_TDM ) {
	...
	)
	{
		if ( ci->powerups & ((1 << PW_REDFLAG) | (1 << PW_BLUEFLAG)) )
		{
			...
			offset += 12;								// CHRUKER: b072 - Need to match tempx or else the other text gets offset
			tempx += 12;
			maxchars -= 2;
		}
	...
	if ( maxchars > 0 )
	{
		CG_DrawStringExt( tempx + (BG_drawStrlen( ci->name) * SMALLCHAR_WIDTH + SMALLCHAR_WIDTH), y, buf, hcolor,
						  qfalse, qfalse, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, maxchars );
	}
	...
	if ( ci->team == TEAM_SPECTATOR )
	{
		...
		if ( score->ping <= -1 )
		{
			s = CG_TranslateString( "^3CONNECTING" );	// by shadow commando
...
/* */
static void
WM_DrawClientScore_Small ( int x, int y, score_t *score, float *color, float fade )
{
	int				maxchars, offset;
	float			tempx;
	vec4_t			hcolor;
	...
	int				i, j;
	char			buf[64];
	if ( y + /*SMALLCHAR_HEIGHT*/ MINICHAR_HEIGHT >= 470 )
	{
		return;
	}
	...
	if ( maxchars > 0 )
	{
		CG_DrawStringExt( tempx + (BG_drawStrlen( ci->name) * MINICHAR_WIDTH + MINICHAR_WIDTH), y, buf, hcolor, qfalse,
						  qfalse, MINICHAR_WIDTH, MINICHAR_HEIGHT, maxchars );
	}
	...
	else if ( cg.snap->ps.persistant[PERS_TEAM] == ci->team || cg_gameType.integer == GT_WOLF_DM )
	{
		CG_DrawStringExt( tempx, y, CG_TranslateString( BG_ShortClassnameForNumber( score->playerClass)), hcolor,
						  qfalse, qfalse, MINICHAR_WIDTH, MINICHAR_HEIGHT, 0 );
	...
	if ( cg_gameType.integer != GT_WOLF_LMS )
	{
		if ( score->respawnsLeft >= 0 )
		{
			CG_DrawStringExt( tempx, y, va( "%2i", score->respawnsLeft), hcolor, qfalse, qfalse, MINICHAR_WIDTH,
							  MINICHAR_HEIGHT, 0 );
		}
		else
		{
...
/* */
static int
WM_DrawInfoLine ( int x, int y, float fade )
{
...
static int
WM_TeamScoreboard ( int x, int y, team_t team, float fade, int maxrows, int absmaxrows )
{
	vec4_t		hcolor;
	float		tempx, tempy;
	int			height, width;
	...
	//draw header
	if ( cg_gameType.integer == GT_WOLF_DM )
	{
	...
	else
	{
		if ( team == TEAM_AXIS )
		{
			CG_Text_Paint_Ext( x, y + 13, 0.25f, 0.25f, tclr, va( "%s [%d] (%d %s)", CG_TranslateString( "AXIS"),
							   cg.teamScores[0], cg.teamPlayers[team], CG_TranslateString( "PLAYERS")), 0, 0, 0,
							   &cgs.media.limboFont1 );
		}
		else if ( team == TEAM_ALLIES )
		{
			CG_Text_Paint_Ext( x, y + 13, 0.25f, 0.25f, tclr, va( "%s [%d] (%d %s)", CG_TranslateString( "ALLIES"),
							   cg.teamScores[1], cg.teamPlayers[team], CG_TranslateString( "PLAYERS")), 0, 0, 0,
							   &cgs.media.limboFont1 );
		}
	}
...
*/
	...
	for ( i = 0; i < cg.numScores && count < maxrows; i++ )
	{
		...
		if ( /*cg.teamPlayers[team] > maxrows*/ use_mini_chars )
		{
...
// -NERVE - SMF
static int
WM_Scoreboard ( int x, int y, float fade, int maxrows, int absmaxrows )
{
	vec4_t		hcolor;
	float		tempx, tempy;
	int			height, width;
	...
	if ( cgs.gametype != GT_WOLF_LMS )
	{
		CG_DrawPicST( tempx + 2, y, INFO_LIVES_WIDTH - 4, 16, 0.f, 0.f, 0.5f, 1.f, cgs.media.hudAlliedHelmet );
		tempx += INFO_LIVES_WIDTH;
	}

	y += SMALLCHAR_HEIGHT;
	players = cg.numScores;
...
CG_DrawScoreboard ( void )
{
	...
	{
		if ( cg.numScores > 25 || (cg.snap->ps.pm_type == PM_INTERMISSION && cg.numScores > 9) )
		{
			if ( cg.snap->ps.pm_type == PM_INTERMISSION )
			{
		...
		else
		{
			...
			{
				x = 220;
				WM_Scoreboard( x, y, fade, 9, 12 );
			}
			else
			{
				x = 220;
				WM_Scoreboard( x, y, fade, cg.numScores, 25 );
			}
		}
	}
	else if ( cgs.gametype == GT_WOLF_STOPWATCH && (cg.snap->ps.pm_type == PM_INTERMISSION) )
	{
