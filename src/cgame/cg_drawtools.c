...
// TOD-O: these flags will be shared, but it was easier to work on stuff if I wasn't changing header files a lot
...
)
{
	...
	if ( (flags & BAR_BG) )
	{
		...
		else if ( flags & BAR_BGSPACING_X0Y5 )
		...
		{		// default spacing of 2 units on each side
			x += indent;
			y += indent;
			w -= ( 2 * indent );
			h -= ( 2 * indent );
	...
	{
		if ( flags & BAR_LEFT )
		{		// TOD-O: remember to swap colors on the ends here
			y += ( h * (1 - frac) );
		}
...
*/
void
CG_HorizontalPercentBar ( float x, float y, float width, float height, float percent )
{
...
/* */
void
CG_DrawSides_NoScale ( float x, float y, float w, float h, float size )
{
	CG_AdjustFrom640( &x, &y, &w, &h );
	trap_R_DrawStretchPic( x, y, size, h, 0, 0, 0, 0, cgs.media.whiteShader );
	trap_R_DrawStretchPic( x + w - size, y, size, h, 0, 0, 0, 0, cgs.media.whiteShader );
...
void
CG_DrawTopBottom_NoScale ( float x, float y, float w, float h, float size )
{
	CG_AdjustFrom640( &x, &y, &w, &h );
	trap_R_DrawStretchPic( x, y, w, size, 0, 0, 0, 0, cgs.media.whiteShader );
	trap_R_DrawStretchPic( x, y + h - size, w, size, 0, 0, 0, 0, cgs.media.whiteShader );
...
/* */
void
CG_DrawRect_FixedBorder ( float x, float y, float width, float height, int border, const float *color )
{
	trap_R_SetColor( color );
...
*/
void
CG_DrawPicST ( float x, float y, float width, float height, float s0, float t0, float s1, float t1, qhandle_t hShader )
{
	CG_AdjustFrom640( &x, &y, &width, &height );
...
*/
void
CG_DrawRotatedPic ( float x, float y, float width, float height, qhandle_t hShader, float angle )
{
	CG_AdjustFrom640( &x, &y, &width, &height );
...
*/
void
CG_DrawChar2 ( int x, int y, int width, int height, int ch )
{
	int		row, col;
	float	frow, fcol;
	float	size;
	...
	fcol = col * 0.0625;
	size = 0.0625;
	trap_R_DrawStretchPic( ax, ay, aw, ah, fcol, frow, fcol + size, frow + size, cgs.media.menucharsetShader );
}
...
// JOSEPH 4-25-00
void
CG_DrawBigString2 ( int x, int y, const char *s, float alpha )
{
	float	color[4];
	color[0] = color[1] = color[2] = 1.0;
	color[3] = alpha;
...
/* */
void
CG_DrawBigStringColor2 ( int x, int y, const char *s, vec4_t color )
{
	CG_DrawStringExt3( x, y, s, color, qfalse, qtrue, BIGCHAR_WIDTH, BIGCHAR_HEIGHT, 0 );
}
...
*/
void
CG_GetColorForHealth ( int health, vec4_t hcolor )
{
...
*/
void
CG_ColorForStamina ( vec4_t scolor )
{
	float	stamina;

	// calculate the total points of damage that can
	// be sustained at the current health / armor level
	stamina = cg.pmext.sprintTime / 350;

	// set the color based on stamina
	scolor[0] = 1.0;
	scolor[3] = 1.0;
	if ( stamina >= 100 )
	{
...
/* */
char *
CG_TranslateString ( const char *string )
{
