...
panel_button_text_t			missiondescriptionTxt = { 0.2f, 0.2f, { 0.0f, 0.0f, 0.0f, 1.f }, 0, 0, &
		bg_loadscreenfont2, };
...
panel_button_text_t			campaignpheaderTxt = { 0.2f, 0.2f, { 1.0f, 1.0f, 1.0f, 0.6f }, 0, 0, &bg_loadscreenfont2, };
panel_button_text_t			campaignpTxt = { 0.30f, 0.30f, { 1.0f, 1.0f, 1.0f, 0.6f }, 0, 0, &bg_loadscreenfont2, };
...
panel_button_t				loadScreenMap =
{
	...
	// shouldn't this be square?? // Gordon: no, the map is actually WIDER that tall, which makes it even worse...
	{ 0, 0, 0, 0, 0, 0, 0, 0 },
	NULL,							/* font		*/
	NULL,							/* keyDown	*/
	NULL,							/* keyUp	*/
...
panel_button_t				loadScreenMeterBack2 =
{
	...
	NULL, { 440 + 26, 480 - 30 + 1, 200 - 56, 20 },
	{ 1, 255, 0, 0, 255, 0, 0, 0 },
	NULL,							/* font		*/
	NULL,							/* keyDown	*/
	NULL,							/* keyUp	*/
...
*/
const char *
CG_LoadPanel_GameTypeName ( gametype_t gt )
{
	switch ( gt )
	{
...
/* */
void
CG_DrawConnectScreen ( qboolean interactive, qboolean forcerefresh )
{
	...
	if ( *buffer )
	{
		...
		y = 322;
		CG_Text_Paint_Centred_Ext( 540, y, 0.17f, 0.17f,	/*0.22f, 0.22f,*/
								  clr3, GAME_VERSION /*"^8FalckonET ^10.4.3 ^4(ET 2.60 Only!)"*/, 0, 0, 0,
								   &bg_loadscreenfont1 );	// use larger font again when ET 2.60 will be common
		y = 340;
		str = Info_ValueForKey( buffer, "sv_hostname" );
		CG_Text_Paint_Centred_Ext( 540, y, 0.2f, 0.2f, colorWhite, str && *str ? str : "ETHost", 0, 26, 0,
								   &bg_loadscreenfont2 );
		y += 14;
...
/* */
void
CG_LoadPanel_RenderLoadingBar ( panel_button_t *button )
{
	...
	CG_DrawPicST( button->rect.x, button->rect.y, button->rect.w * frac, button->rect.h, 0, 0, frac, 1,
				  button->hShaderNormal );
}


/* */
void
CG_LoadPanel_RenderCampaignTypeText ( panel_button_t *button )
{
...
*/
	CG_Text_Paint_Ext( button->rect.x, button->rect.y, button->font->scalex, button->font->scaley, button->font->colour,
					   va( "%s:", CG_LoadPanel_GameTypeName( cgs.gametype)), 0, 0, button->font->style,
					   button->font->font );
}
...
/* */
void
CG_LoadPanel_RenderCampaignNameText ( panel_button_t *button )
{
	const char	*cs;
	float		w;
	...
	if ( cgs.gametype == GT_WOLF_CAMPAIGN || cgs.gametype == GT_SUPREMACY_CAMPAIGN )
		...
		else
		{
			...
			CG_Text_Paint_Ext( button->rect.x + (button->rect.w - w) * 0.5f, button->rect.y, button->font->scalex,
							   button->font->scaley, button->font->colour, cgs.arenaData.longname, 0, 0, 0,
							   button->font->font );
		}
}
...
/* */
void
CG_LoadingItem ( panel_button_t *button )
{
	DC->drawTextExt( button->rect.x, button->rect.y, button->font->scalex, button->font->scaley, button->font->colour,
					 cgs.loadingItem, 0, 0, 0, button->font->font );
}


/* */
void
CG_LoadPanel_RenderMissionDescriptionText ( panel_button_t *button )
{
	const char	*cs;
	char		*s, *p;
	char		buffer[1024];
	...
	if ( cgs.gametype == GT_WOLF_CAMPAIGN || cgs.gametype == GT_SUPREMACY_CAMPAIGN )
		...
		else if ( cgs.gametype == GT_WOLF_LMS || cgs.gametype == GT_WOLF_DM || cgs.gametype == GT_WOLF_TDM )
		{
	...
	while ( *p )
	{
		...
		{
			*p++ = '\0';
			DC->drawTextExt( button->rect.x + 4, y, button->font->scalex, button->font->scaley, button->font->colour, s,
...
/* */
void
CG_LoadPanel_KeyHandling ( int key, qboolean down )
{
...
/* */
qboolean
CG_LoadPanel_ContinueButtonKeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
...
/* */
void
CG_LoadPanel_RenderCampaignPins ( panel_button_t *button )
{
	int			i;
	qhandle_t	shader;
	...
	{	// FIXME , just display map image instead this
		float	px, py;
		if ( !cgs.arenaInfoLoaded )
		{
			return;
