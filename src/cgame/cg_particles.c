...
static float		shaderAnimSTRatio[
	MAX_SHADER_ANIMS] = { 1, // NERVE - SMF - changed from 1.405 to 1
		1, 1, 1, 1, 1, 1, };
...
void
CG_ParticleDirtBulletDebris ( vec3_t org, vec3_t vel, int duration )
{
	int			r = rand() % 3;
	cparticle_t *p;
	if ( !free_particles )
...
void
CG_Particle_OilParticle ( qhandle_t pshader, vec3_t origin, vec3_t dir, int ptime, int snum )	// snum is parent ent number?
{
	cparticle_t *p;
	int			time;
	int			time2;
...
/* */
void
CG_ParticleBloodCloudZombie ( centity_t *cent, vec3_t origin, vec3_t dir )
{
	float		length;
	float		dist;
	float		crittersize;
