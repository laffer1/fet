...
{	// Returns the music volume as a float...
	...
	volume = atof( vol );
	{
		const char	*volChar = va( "%f", volume * 100 );
		// Kamikazee: is this only used for debuging purposes?
		trap_Cvar_Set( "s_musicvolumePercent", volChar );
	}
...
vmCvar_t		cg_bobroll;
vmCvar_t		cg_hunkCheck;
vmCvar_t		cg_nazi;
vmCvar_t		cg_breathVolume;		// jaquboss
vmCvar_t		cg_limboMusic;			// jaquboss
vmCvar_t		cg_goatsound;
vmCvar_t		cg_bobyaw;
vmCvar_t		cg_gun_pullback;
vmCvar_t		cg_swingSpeed;
vmCvar_t		cg_shadows;
vmCvar_t		cg_gibs;
vmCvar_t		cg_drawFrags;
vmCvar_t		cg_draw2D;
vmCvar_t		cg_compassTolerance;
vmCvar_t		cg_drawFPS;
...
vmCvar_t		cg_crosshairX_thirdperson;
vmCvar_t		cg_crosshairY_thirdperson;
vmCvar_t		cg_crosshairHealth;
vmCvar_t		cg_teamChatsOnly;
vmCvar_t		cg_noVoiceChats;		// NERVE - SMF
vmCvar_t		cg_noVoiceText;			// NERVE - SMF
vmCvar_t		cg_drawStatus;
vmCvar_t		cg_drawAltHud;
...
vmCvar_t		cg_nopredictPing;
vmCvar_t		cg_nopredict;
vmCvar_t		cg_noPlayerAnims;
vmCvar_t		cg_showmiss;
vmCvar_t		cg_footsteps;
vmCvar_t		cg_markTime;
vmCvar_t		cg_markdistance;
vmCvar_t		cg_brassTime;
vmCvar_t		cg_envmapupdate;
vmCvar_t		cg_envmap_fov;
vmCvar_t		cg_blur;
vmCvar_t		cg_letterbox;			//----(SA)	added
vmCvar_t		cg_drawGun;
vmCvar_t		cg_draw3DlimboWeapons;	//-- Jaquboss
vmCvar_t		cg_bigLimbo;
vmCvar_t		cg_n1a;
vmCvar_t		cg_n1x;
vmCvar_t		cg_n2a;
vmCvar_t		cg_n2x;
vmCvar_t		cg_myHeadSkin;
vmCvar_t		cg_myHeadModel;
vmCvar_t		cg_drawclock;
vmCvar_t		cg_drawstance;
vmCvar_t		cg_cursorHints;			//----(SA)	added
vmCvar_t		cg_gun_frame;
vmCvar_t		cg_gun_realisticposition;
vmCvar_t		cg_gun_x;

vmCvar_t		CG_FlashBlend; // unique1's quakelike dlights

//vmCvar_t	cg_playerLitMode;
vmCvar_t		cg_dynamicLight;
vmCvar_t		cg_forceLowLod;
vmCvar_t		cg_gun_y;
vmCvar_t		cg_gun_z;
...
vmCvar_t		cg_AutoReloads;
vmCvar_t		cg_3rdPerson_allowed;	// jaquboss
vmCvar_t		cg_realmedic;

//vmCvar_t	cg_debugBullets;
//vmCvar_t		cg_3rdPerson_follow;	// Kamikazee: use cg_3rdPerson
vmCvar_t		cg_3rdPerson_maxrange;	//
vmCvar_t		cg_3rdPerson_maxangle;	//

//vmCvar_t	cg_thirdPersonRange;//
//vmCvar_t	cg_thirdPersonAngle;//
vmCvar_t		cg_3rdPersonFocus;		//
vmCvar_t		cg_3rdPersonRange;		//
vmCvar_t		cg_3rdPersonAngle;		//
vmCvar_t		cg_stereoSeparation;
vmCvar_t		cg_renderToTexture;

//vmCvar_t	cg_atmosphere_height;
//vmCvar_t	cg_snow;
//vmCvar_t	cg_rain;
//vmCvar_t	cg_debugtags;
vmCvar_t		cg_firstPerson;
vmCvar_t		cg_hitsound_allowed;
...
//vmCvar_t	cg_staminaonlyjumps;
vmCvar_t		cg_noCapsule;
vmCvar_t		developer;
...
//vmCvar_t	cg_nobipodmove;
vmCvar_t		cg_crosshair_allowed;
...
vmCvar_t		cg_camo_axis;
vmCvar_t		cg_camo_allies;
#ifdef __MUSIC_ENGINE__
...
cvarTable_t cvarTable[] =
{
	...
	{ &cg_goatsound, "cg_goatsound", "0", CVAR_ARCHIVE },
	{ &cg_renderToTexture, "cg_renderToTexture", "0", CVAR_ARCHIVE },
	{ &cg_drawstance, "cg_drawstance", "3", CVAR_ARCHIVE },
	{ &cg_bigLimbo, "cg_bigLimbo", "0", CVAR_ARCHIVE },
	{ &cg_draw3DlimboWeapons, "cg_draw3DlimboWeapons", "0", CVAR_ARCHIVE },
	{ &cg_drawGun, "cg_drawGun", "1", CVAR_CHEAT },
	{ &cg_n1a, "cg_n1a", "0", CVAR_ROM },
	{ &cg_n2x, "cg_n2x", "0", CVAR_ROM },
	{ &cg_n1x, "cg_n1x", "0", CVAR_ROM },
	{ &cg_n2a, "cg_n2a", "0", CVAR_ROM },
	{ &cg_myHeadSkin, "cg_myHeadSkin", "0", CVAR_ARCHIVE | CVAR_ROM },

	// store head we want to use for ourselfs, for keeping it while disconnecting..
	{ &cg_myHeadModel, "cg_myHeadModel", "0", CVAR_ARCHIVE | CVAR_ROM },
	...
	{ &cg_zoomDefaultSniper, "cg_zoomDefaultSniper", "20", CVAR_ARCHIVE },

	// JPW NERVE changed per atvi req
	...
	{ &cg_crosshairX_thirdperson, "cg_crosshairX_thirdperson", "0", CVAR_ARCHIVE },
	{ &cg_crosshairY_thirdperson, "cg_crosshairY_thirdperson", "0", CVAR_ARCHIVE },
	...
	{ &cg_gun_realisticposition, "cg_gun_realisticposition", "1", CVAR_ARCHIVE },
	{ &cg_firstPerson, "cg_trueFirstPerson", "0", 0 },

	//	{ &cg_playerLitMode, "cg_playerLitMode", "1", CVAR_ARCHIVE },
	{ &cg_dynamicLight, "cg_dynamicLight", "1", CVAR_ARCHIVE }, // jaquboss still slow and ugly, works almost flawless for static flamethrower and maybe for muzzle flashes, better to not add it to light trap, was a test anyway
	{ &cg_forceLowLod, "cg_forceLowLod", "0", CVAR_ARCHIVE },
	{ &cg_limboMusic, "cg_limboMusic", "0", CVAR_ARCHIVE },
	{ &cg_breathVolume, "cg_breathVolume", "40", CVAR_ARCHIVE },
	{ &cg_gun_x, "cg_gunX", "0", CVAR_CHEAT },
	{ &cg_gun_y, "cg_gunY", "0", CVAR_CHEAT },
	{ &cg_gun_z, "cg_gunZ", "0", CVAR_CHEAT },
	{ &cg_centertime, "cg_centertime", "5", CVAR_CHEAT },

	// DHM - Nerve :: changed from 3 to 5
	{ &cg_runpitch, "cg_runpitch", "0.005", CVAR_ARCHIVE | CVAR_CHEAT },
	{ &cg_runroll, "cg_runroll", "0.005", CVAR_ARCHIVE | CVAR_CHEAT },
	{ &cg_bobup, "cg_bobup", "0.005", CVAR_ARCHIVE | CVAR_CHEAT },
	{ &cg_bobpitch, "cg_bobpitch", "0.005", CVAR_ARCHIVE | CVAR_CHEAT },
	{ &cg_bobroll, "cg_bobroll", "0.005", CVAR_ARCHIVE | CVAR_CHEAT },
	{ &cg_bobyaw, "cg_bobyaw", "0.005", CVAR_ARCHIVE | CVAR_CHEAT },
	{ &cg_gun_pullback, "cg_gun_pullback", "1", CVAR_ARCHIVE },
	{ &cg_draw2dironsights, "", "0", CVAR_ARCHIVE, CVAR_CHEAT },
	{ &cg_hunkCheck, "cg_hunkCheck", "1", CVAR_ARCHIVE },
	...
	{ &cg_swingSpeed, "cg_swingSpeed", "0.1", CVAR_CHEAT },
	...
	{ &cg_nopredictPing, "cg_nopredictPing", "0", CVAR_ARCHIVE }, // jaquboss - similar to bellow, you may not want to predict something which wont happen, right?
	{ &cg_nopredict, "cg_nopredict", "0", /*CVAR_CHEAT*/ }, // jaquboss - enabled useful for laggers to do not do highping prediction
	...
	{ &cg_3rdPersonRange, "cg_3rdPersonRange", "80", },

	// JPW NERVE per atvi req
	{ &cg_3rdPersonAngle, "cg_3rdPersonAngle", "0", },
	{ &cg_3rdPersonFocus, "cg_3rdPersonFocus", "800", },
	{ &cg_3rdPerson, "cg_3rdPerson", "1", },	// Kamikazee: Used when following a player as a spectator

	{ &CG_FlashBlend, "CG_FlashBlend", "0", CVAR_ARCHIVE }, // @FIXME default 1 or 0 ?
	...
	{ &cg_corona_flash, "cg_corona_flash", "1", CVAR_ARCHIVE },
	{ &cg_coronas, "cg_coronas", "1", CVAR_ARCHIVE },
	{ &cg_predictItems, "cg_predictItems", "1", CVAR_ARCHIVE },
	{ &cg_deferPlayers, "cg_deferPlayers", "1", CVAR_ARCHIVE },
	{ &cg_drawTeamOverlay, "cg_drawTeamOverlay", "2", CVAR_ARCHIVE },
	{ &cg_stats, "cg_stats", "0", 0 },
	{ &cg_blinktime, "cg_blinktime", "0", CVAR_ARCHIVE },

	// used as debug cvar
	//	{ &cg_debugtags, "cg_debugtags", "0", CVAR_CHEAT }, // FIXME cheat me!
	...
	{ &cg_buildScript, "com_buildScript", "0", 0 },
...
#endif // ALLOW_GSYNC
	{ &cg_envmapupdate, "cg_envmapupdate", "2500", CVAR_ARCHIVE },
	{ &cg_shadowPlanes, "cg_shadowPlanes", "0", CVAR_ARCHIVE },
	{ &cg_blur, "cg_blur", "0", CVAR_ARCHIVE },
	{ &cg_envmap_fov, "cg_envmap_fov", "90", CVAR_ARCHIVE },

	// Rafael - particle switch
	//{ &cg_3rdPerson_follow, "cg_3rdPerson_follow", "0", CVAR_ARCHIVE },	//Kamikazee: use cg_3rdPerson
	{ &cg_hitsound, "cg_hitsound", "0", CVAR_ARCHIVE },
	{ &cg_wolfparticles, "cg_wolfparticles", "1", CVAR_ARCHIVE },
	{ &cg_gameType, "g_gametype", "0", 0 },

	// communicated by systeminfo
	{ &cg_norender, "cg_norender", "0", 0 },

	// only used during single player, to suppress rendering until the server is ready
	{ &cg_bluelimbotime, "", "30000", 0 },

	// communicated by systeminfo
	{ &cg_redlimbotime, "", "30000", 0 },

	// communicated by systeminfo
	{ &cg_movespeed, "g_movespeed", "76", 0 },

	// actual movespeed of player
	...
	// Jaquboss , Server info cvars...
	{ &cg_crosshair_allowed, "", "", 0 },
	{ &cg_snoopers, "", "", 0 },
	{ &cg_3rdPerson_allowed, "", "", 0 },
	{ &cg_realmedic, "", "", 0 },
	{ &cg_hitsound_allowed, "", "", 0 },
	{ &cg_3rdPerson_maxangle, "", "", 0 },
	{ &cg_3rdPerson_maxrange, "", "", 0 },
	{ &cg_nohud, "", "", 0 },
	{ &cg_noCapsule, "", "", 0 },

	//	{ &cg_staminaonlyjumps, "", "", 0 },
	{ &cg_camo_axis, "", "", 0 },
	{ &cg_camo_allies, "", "", 0 },
	{ &cg_nazi, "", "", 0 },
	{ &cg_AutoReloads, "", "", 0 },
#ifdef __PORTAL_SCOPE__
	{ &cg_portalScope, "cg_portalScope", "1", CVAR_ARCHIVE },
#endif
	//	{ &cg_rain, "", "", 0 },
	//	{ &cg_snow, "", "", 0 },
	//	{ &cg__height, "", "", 0 },
	...
	{ &cg_drawAltCrosshair, "cg_drawAltCrosshair", "1", CVAR_ARCHIVE },
	...
	{ &cg_drawScore, "cg_drawScore", "1", CVAR_ARCHIVE },
#ifdef SAVEGAME_SUPPORT
	...
	{ &s_inetRadio, "s_inetRadio", "0", CVAR_ARCHIVE },	// jaquboss, to turn of the radio stuff
#endif //__MUSIC_ENGINE__
#ifdef __PARTICLE_SYSTEM__
	{ &cg_debugParticles, "cg_debugParticles", "0", CVAR_ARCHIVE },
...
CG_UpdateCvars ( void )
{
	...
	/*	if ( cg_bobyaw.value != 0.01 || cg_bobroll.value != 0.005 || cg_bobpitch.value != 0.01 || cg_bobup.value != 0.01 || cg_runpitch.value != 0.01 || cg_runroll.value != 0.01) {
	trap_Cvar_Set( "cg_runpitch", "0.01" );
	trap_Cvar_Set( "cg_runroll", "0.01" );
	trap_Cvar_Set( "cg_bobup", "0.01" );
	trap_Cvar_Set( "cg_bobpitch", "0.01" );
	trap_Cvar_Set( "cg_bobroll", "0.005" );
	trap_Cvar_Set( "cg_bobyaw", "0.01" );
	}*/
	{
		char	buf[32];
		trap_Cvar_VariableStringBuffer( "com_hunkMegs", buf, sizeof(buf) );
		trap_Cvar_Update( &cg_hunkCheck );
		if ( atoi( buf) < 96 && cg_hunkCheck.integer )
		{
			trap_UI_Popup( UIMENU_INGAME_HUNKS );
		}
	}

	if ( !developer.integer )
	{
		trap_Cvar_Set( "r_AmbientScale", "0.2" );
		if ( cg_forceLowLod.integer > 0 )
		{
			trap_Cvar_Set( "r_LodScale", "750" );	// to fix strange problem ..
		}

		trap_Cvar_Set( "r_DirectedScale", "2" );
	}
	...
	for ( i = 0, cv = cvarTable; i < cvarTableSize; i++, cv++ )
	{
		if ( cv->vmCvar )
		{
			...
			if ( cv->modificationCount != cv->vmCvar->modificationCount )
			{
				...
				if
				(
					cv->vmCvar == &cg_autoAction ||
					cv->vmCvar == &cg_autoReload ||
					cv->vmCvar == &int_cl_timenudge ||
					cv->vmCvar == &int_cl_maxpackets ||
					cv->vmCvar == &cg_autoactivate ||
					cv->vmCvar == &cg_predictItems
				)
				{
					fSetFlags = qtrue;
				}
				...
				{
					BG_setCrosshair( cg_crosshairColor.string, cg.xhairColor, cg_crosshairAlpha.value,
									 "cg_crosshairColor" );
				}
...
CG_RestoreProfile ( void )
{
	...
	Com_sprintf( cfgPath, sizeof(cfgPath), "profiles/%s/%s", profile, CONFIG_NAME );
	Com_sprintf( bakPath, sizeof(bakPath), "profiles/%s/%s.etpub", profile, CONFIG_NAME );
...
CG_setClientFlags ( void )
{
	...
	trap_Cvar_Set( "cg_uinfo", va( "%d %d %d",	// Client Flags
				  (((cg_autoReload.integer > 0) ? CGF_AUTORELOAD : 0) |
					   ((cg_autoAction.integer & AA_STATSDUMP) ? CGF_STATSDUMP : 0) |
					   ((cg_autoactivate.integer > 0) ? CGF_AUTOACTIVATE : 0) |
					   ((cg_predictItems.integer > 0) ? CGF_PREDICTITEMS : 0)	// Add more in here, as needed
	   ),										// Timenudge
					  int_cl_timenudge.integer, // MaxPackets
...
void
CG_nameCleanFilename ( const char *pszIn, char *pszOut, unsigned int dwOutSize )
{
...
CG_generateFilename ( void )
{
	...
	return
		(
			va( "%d-%02d-%02d-%02d%02d%02d-%s%s", 1900 + ct.tm_year, ct.tm_mon + 1, ct.tm_mday, ct.tm_hour, ct.tm_min,
				ct.tm_sec, Info_ValueForKey( pszServerInfo, "mapname"), (cg.mvTotalClients < 1) ? "" : "-MVD")
		);
}
...
/* */
void
CG_printConsoleString ( char *str )
{
...
CG_LoadObjectiveData ( void )
{
	...
	(
		...
		cg_gameType.integer == GT_WOLF_DM ||
		cg_gameType.integer == GT_WOLF_TDM ||
		cg_gameType.integer == GT_WOLF_CTF
	)
	{	// FIXME own
		handle = trap_PC_LoadSource( va( "maps/%s_lms.objdata", Q_strlwr( cgs.rawmapname)) );
	}	/* else if( cg_gameType.integer == GT_WOLF_TDM || cg_gameType.integer == GT_WOLF_DM) {
		handle = trap_PC_LoadSource( va( "maps/%s_tdm.objdata", Q_strlwr(cgs.rawmapname) ) );
	}*/
	else
	{
...
CG_RegisterSounds ( void )
{
	...
	cgs.media.sparkSounds[0] = trap_S_RegisterSound ("sound/world/saarc2.wav", qfalse);
	cgs.media.sparkSounds[1] = trap_S_RegisterSound ("sound/world/arc2.wav", qfalse);
	cgs.media.siren_sound = trap_S_RegisterSound("sound/world/siren.wav", qfalse);
#ifdef __NPC__
	trap_S_RegisterSound( "sound/weapons/airstrike/airstrike_plane_early.wav", qfalse );
	trap_S_RegisterSound( "sound/weapons/airstrike/airstrike_plane_late.wav", qfalse );
#endif //__NPC__
	cgs.media.exert[0] = trap_S_RegisterSound( "sound/misc/exert0.wav", qfalse );
	cgs.media.exert[1] = trap_S_RegisterSound( "sound/misc/exert1.wav", qfalse );
	cgs.media.exert[2] = trap_S_RegisterSound( "sound/misc/exert2.wav", qfalse );
	cgs.media.exert[3] = trap_S_RegisterSound( "sound/misc/exert3.wav", qfalse );
	cgs.media.hitsound = trap_S_RegisterSound( "sound/misc/hitsound.wav", qfalse );
	cgs.media.hitteammate = trap_S_RegisterSound( "sound/misc/hitteammate.wav", qfalse );
	cgs.media.headshot = trap_S_RegisterSound( "sound/misc/headshot.wav", qfalse );
	cgs.media.sfx_axispain1 = trap_S_RegisterSound( "painsounds/axis/pain1.wav", qfalse );
	cgs.media.sfx_axispain2 = trap_S_RegisterSound( "painsounds/axis/pain2.wav", qfalse );
	cgs.media.sfx_axispain3 = trap_S_RegisterSound( "painsounds/axis/pain3.wav", qfalse );
	cgs.media.sfx_axispain4 = trap_S_RegisterSound( "painsounds/axis/pain4.wav", qfalse );
	cgs.media.sfx_alliespain1 = trap_S_RegisterSound( "painsounds/allies/pain1.wav", qfalse );
	cgs.media.sfx_alliespain2 = trap_S_RegisterSound( "painsounds/allies/pain2.wav", qfalse );
	cgs.media.sfx_alliespain3 = trap_S_RegisterSound( "painsounds/allies/pain3.wav", qfalse );
	cgs.media.sfx_alliespain4 = trap_S_RegisterSound( "painsounds/allies/pain4.wav", qfalse );
	...
	cgs.media.weaponbounce = trap_S_RegisterSound( "sound/weapons/bounce.wav", qfalse );
	cgs.media.garandping = trap_S_RegisterSound( "sound/weapons/garand/ping.wav", qfalse );
	...
	for ( i = 0; i < 2; i++ )
	{
		cgs.media.grenadebounce[FOOTSTEP_NORMAL][i] = cgs.media.grenadebounce[FOOTSTEP_GRAVEL][i] = cgs.media.
			grenadebounce[FOOTSTEP_SPLASH][i] = trap_S_RegisterSound( va( "sound/weapons/grenade/bounce_hard%i.wav", i + 1),
																	  qfalse );
		cgs.media.grenadebounce[FOOTSTEP_METAL][i] = cgs.media.grenadebounce[FOOTSTEP_ROOF][i] = trap_S_RegisterSound( va( "sound/weapons/grenade/bounce_metal%i.wav", i + 1),
																														   qfalse );
		cgs.media.grenadebounce[FOOTSTEP_WOOD][i] = trap_S_RegisterSound( va( "sound/weapons/grenade/bounce_wood%i.wav", i + 1),
																			  qfalse );
		cgs.media.grenadebounce[FOOTSTEP_GRASS][i] = cgs.media.grenadebounce[FOOTSTEP_SNOW][i] = cgs.media.
			grenadebounce[FOOTSTEP_CARPET][i] = trap_S_RegisterSound( va( "sound/weapons/grenade/bounce_soft%i.wav", i + 1),
																	  qfalse );
	}
	...
	cgs.media.countFight_axis = trap_S_RegisterSound( "sound/chat/axis/32a.wav", qfalse );
	cgs.media.countFight_allies = trap_S_RegisterSound( "sound/chat/allies/32a.wav", qfalse );
	...
	// OSP
	cgs.media.tankMoveSound = trap_S_RegisterSound( "sound/vehicles/tank/tank_move.wav", qfalse );
	cgs.media.tankIdleSound = trap_S_RegisterSound( "sound/vehicles/tank/tank_idle.wav", qfalse );
	cgs.media.tankStartSound = trap_S_RegisterSound( "sound/vehicles/tank/tank_revup.wav", qfalse );
	cgs.media.tankStopSound = trap_S_RegisterSound( "sound/vehicles/tank/tank_revdown.wav", qfalse );
	cgs.media.tankTurretSound = trap_S_RegisterSound( "sound/vehicles/tank/turret_spin.wav", qfalse );
	cgs.media.tankTurretEndSound = trap_S_RegisterSound( "sound/vehicles/tank/turret_end.wav", qfalse );
	cgs.media.tankFireSound = trap_S_RegisterSound( "sound/vehicles/tank/tank_fire.wav", qfalse );
	...
	cgs.media.boneBounceSound = trap_S_RegisterSound( "sound/world/boardbreak.wav", qfalse );	// TOD-O: need a real sound for this
	cgs.media.sfx_flakimpact = trap_S_RegisterSound( "sound/weapons/aagun/aagun_impact.wav", qfalse );
	...
	cgs.media.pushPlayer = trap_S_RegisterSound( "sound/misc/pushplayer.wav", qfalse );
	...
	for ( i = 0; i < 3; i++ )
	{
		cgs.media.sndMedicCall_allies[i] = trap_S_RegisterSound( va( "sound/chat/allies/medic%i.wav", i + 1), qfalse );
		cgs.media.sndMedicCall_axis[i] = trap_S_RegisterSound( va( "sound/chat/axis/medic%i.wav", i + 1), qfalse );
	}

	cgs.media.flagsound = trap_S_RegisterSound( "sound/chat/ctf/youhavetheflag.wav", qfalse );
	cgs.media.flagsound_axis = trap_S_RegisterSound( "sound/chat/ctf/axis/taken.wav", qfalse );
	cgs.media.flagsound_allies = trap_S_RegisterSound( "sound/chat/ctf/allies/taken.wav", qfalse );
...
//===================================================================================
extern void CG_LoadAllCharacters ( void );
...
CG_RegisterGraphics ( void )
{
	...
	cgs.loadingItem = cgs.mapname;
	trap_R_LoadWorldMap( cgs.mapname );
	CG_LoadingString( "entities" );
	cgs.loadingItem = "entities";
	numSplinePaths = 0;
	numPathCorners = 0;
	...
	cgs.loadingItem = "game media";
	CG_LoadingString( " - textures" );
	cgs.loadingItem = "textures";
...
" );
	...
	// done
	cgs.media.tankShader[0] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/bits_forward" );

	//	cgs.media.tankShader[1] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/bits_backward" );
	cgs.media.tankShader[2] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/bits_static" );
	cgs.media.tankShader[3] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/jag_tracks_left_s" );
	cgs.media.tankShader[1] = trap_R_RegisterShaderNoMip( "models/mapobjects/tanks_sd/jag_tracks_alt_forward" );
	...
	cgs.media.reticleShaderSnooper = trap_R_RegisterShader( "gfx/misc/snooper" );
	cgs.media.binocShaderSimple = trap_R_RegisterShader( "gfx/misc/binocsimple" );
	cgs.media.blur = trap_R_RegisterShader( "gfx/misc/blur" );
	cgs.media.paineffect = trap_R_RegisterShader( "gfx/misc/paineffect" );
	cgs.media.waterblur = trap_R_RegisterShader( "gfx/misc/waterblur" );
	...
	cgs.media.scopeShader = trap_R_RegisterShader( "gfx/misc/portalScope" );
	cgs.media.scopeShaderTrans = trap_R_RegisterShader( "gfx/misc/portalscope_trans" );
	...
	cgs.media.browningHintShader = trap_R_RegisterShader( "gfx/2d/browningHint" );
	cgs.media.aagunHintShader = trap_R_RegisterShader( "gfx/2d/aagunHint" );
	cgs.media.aagunTagmodel = trap_R_RegisterModel( "models/mapobjects/weapons/flak_tag.md3" );
	...
	cgs.media.satchelchargeHintShader = trap_R_RegisterShaderNoMip( "gfx/2d/satchelchargeHint" ), cgs.media.landmineHintShader = trap_R_RegisterShaderNoMip( "gfx/2d/landmineHint" );
...
/*	cgs.media.waypointAttackShader = trap_R_RegisterShaderNoMip( "sprites/waypoint_attack" );
	...
	CG_LoadingString( " - Head Skins" );
	cgs.loadingItem = "Head Skins";
	CG_RegisterHeadSkins();
	CG_LoadingString( " - Head Models" );
	cgs.loadingItem = "Head Models";
	CG_RegisterHeadModels();
	CG_LoadingString( " - models" );
	cgs.loadingItem = "models";
	...
	cgs.media.panzerfaustsmokeParticleShader = trap_R_RegisterShader( "panzerfaustsmokeParticle" );
...
//	cgs.media.dlightShader = trap_R_RegisterShader( "dymamicLight" );									// for generic dlights projections
...
#define CC_FILTER_CMDPOST		(1 << 3) // TOD-O
#define CC_FILTER_HACABINETS	(1 << 4) // TOD-O
#define CC_FILTER_CONSTRUCTIONS	(1 << 5)
#define CC_FILTER_DESTRUCTIONS	(1 << 6)
#define CC_FILTER_WAYPOINTS		(1 << 7)
#define CC_FILTER_OBJECTIVES	(1 << 8) // TOD-O
*/
	...
	cgs.media.tankModel[0] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/churchhill_notracks.md3" );
	cgs.media.tankModel[1] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/jagdpanther_notracks.md3" );
	cgs.media.tankTracksModel[0] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/churchhill_tracks.md3" );
	cgs.media.tankTracksModel[1] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/jagdpanther_tracks.md3" );
	cgs.media.tankTurretModel[0] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/churchhill_turret2.md3" );
	cgs.media.tankBarrelModel[0] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/churchhill_barrel.md3" );
	cgs.media.tankBarrelModel[1] = trap_R_RegisterModel( "models/mapobjects/tanks_sd/jagdpanther_temperate_turret.md3" );
	cgs.media.tankFlashModel = trap_R_RegisterModel( "models/weapons2/machinegun/mg42_flash.md3" );
	...
	for ( i = 0; i < 12; i++ ){
		if ( i == 0 )
		cgs.media.mortarload[0] = trap_R_RegisterModel( "models/common/mortar.md3" );
		else 
		cgs.media.mortarload[i] = trap_R_RegisterModel( va("models/common/mortar%i.md3" , i+1) );

	}


	cgs.media.bulletload3 = trap_R_RegisterModel( "models/common/bullets2.md3" );
	cgs.media.bulletload2 = trap_R_RegisterModel( "models/common/bullets1.md3" );
	cgs.media.bulletload = trap_R_RegisterModel( "models/common/bullets.md3" );
	cgs.media.BacksModel2 = trap_R_RegisterModel( "models/common/backmodel2.md3" );
	cgs.media.BacksModel = trap_R_RegisterModel( "models/common/backmodel.md3" );
	cgs.media.gmask = trap_R_RegisterModel( "models/players/temperate/common/mask.md3" );
	cgs.media.flamethrowerBackModel = trap_R_RegisterModel( "models/weapons2/flamethrower/backmodel.md3" );
	...
	cgs.media.radiomodel = trap_R_RegisterModel( "models/multiplayer/satchel/radio.md3" );				// NERVE - SMF
	cgs.media.blueflag = trap_R_RegisterModel( "models/multiplayer/flag/alliedtakenflag.md3" );			// NERVE - SMF
	cgs.media.redflag = trap_R_RegisterModel( "models/multiplayer/flag/axistakenflag.md3" );			// NERVE - SMF
	...
	// Wolf model
	cgs.media.wolfkickModel = trap_R_RegisterModel( "models/weapons2/foot/v_wolfoot_10f.md3" );
	...
	// TOD-O: FIXME:  REMOVE REGISTRATION OF EACH MODEL FOR EVERY LEVEL LOAD
	...
	cgs.media.deathloop = trap_S_RegisterSound( "sound/misc/deathloop.wav", qfalse );
	cgs.media.browningloop = trap_S_RegisterSound( "sound/weapons/30cal/30cal_fire_loop.wav", qfalse );
	cgs.media.mg42loop = trap_S_RegisterSound( "sound/weapons/mg42/mg42_fire_loop.wav", qfalse );
	cgs.media.mg42calm = trap_S_RegisterSound( "sound/weapons/mg42/mg42_cooldown.wav", qfalse );
	cgs.media.browningcalm = trap_S_RegisterSound( "sound/weapons/30cal/30cal_cooldown.wav", qfalse );

	//	cgs.media.whizzsound = trap_S_RegisterSound( "sound/weapons/whizzsound.wav", qfalse );
	cgs.media.chatsound = trap_S_RegisterSound( "sound/misc/chatsound.wav", qfalse );
	cgs.media.hflakWeaponSnd = trap_S_RegisterSound( "sound/weapons/aagun/aagun_fire.wav", qfalse );
	...
	cgs.media.flameMark = trap_R_RegisterShaderNoMip( "flameMark" );
	cgs.media.gunflash = trap_R_RegisterShaderNoMip( "gunflash" );
	cgs.media.shadowShader = trap_R_RegisterShaderNoMip( "shadowShader" );
	...
	CG_LoadAllCharacters();
	CG_LoadingString( " - particles" );
	cgs.loadingItem = "particles";
	CG_ClearParticles();
	InitSmokeSprites();
	CG_LoadingString( " - classes" );
	cgs.loadingItem = "classes";
	CG_RegisterPlayerClasses();
	CG_LoadingString( " - PM Graphics" );
	cgs.loadingItem = "PM Graphics";
	CG_InitPMGraphics();
	CG_LoadingString( " - game media" );
	cgs.loadingItem = "game media";

	// Plane propeller...
	cgs.media.hPropeller = trap_R_RegisterModel( "models/mapobjects/vehicles/m109_prop.mdc" );
	...
	// UQ1: Added... For Single Player/Coop...
	//cgs.media.hSearchLight = trap_R_RegisterModel( "models/mapobjects/light/searchlight1_l.mdc" );
	// spotlight
	// shaders
	cgs.media.spotLightShader = trap_R_RegisterShader( "spotLight" );
	cgs.media.spotLightBeamShader = trap_R_RegisterShader( "lightBeam" );
...
#ifdef __NPC__
	// LCVP Attachments...
	//	cgs.media.hLCVPSpawns =	trap_R_RegisterModel( "models/pop/vehicles/pop_lcvp_spawns.md3" );
	//	cgs.media.hLCVPRamp =	trap_R_RegisterModel( "models/pop/vehicles/pop_lcvp_ramp.md3" );
	//	cgs.media.hLCVPMG =	trap_R_RegisterModel( "models/multiplayer/browning/thirdperson.md3" );
	//	cgs.media.hLCVSoldier =	trap_R_RegisterModel( /*"models/players/temperate/allied/soldier/body.mdm"*/"models/mapobjects/vehicles/m109_prop.mdc" );
	//cgs.media.hLCVPRamp =	trap_R_RegisterModel( "models/pop/vehicles/pop_lcvp_ramp.md3" );
	//cgs.media.hLCVPRamp =	trap_R_RegisterModel( "models/pop/vehicles/pop_lcvp_ramp.md3" );
	// Destroyer...
	//	cgs.media.hDestroyerGun =	trap_R_RegisterModel( "models/Saberpeak_md3/sp_shipCannon.md3" );
	//	cgs.media.hDestroyerRadar =	trap_R_RegisterModel( "models/Saberpeak_md3/SP_radar.md3" );
	//	cgs.media.hDestroyerFlag =	trap_R_RegisterModel( "models/Saberpeak_md3/SP_flag.md3" );
#endif //__NPC__
	...
	// FIXME: temp models
	cgs.media.hMountedFPMG42_2 = trap_R_RegisterModel( "models/multiplayer/mg42/v_mg42.md3" );
	cgs.media.hMountedFPMG42 = trap_R_RegisterModel( "models/multiplayer/mg42/v_mg42_tank.md3" );
	...
	cgs.media.tahoma16 = cgs.media.limboFont1;
	...
	cgs.media.backstab = trap_R_RegisterShaderNoMip( "gfx/hud/backstab" );
...
#ifdef __SUPREMACY__
	cgs.media.hudFlagAxisIcon = trap_R_RegisterShaderNoMip( "icons/miscellaneous/flag_axis" );
	cgs.media.hudFlagAlliesIcon = trap_R_RegisterShaderNoMip( "icons/miscellaneous/flag_allied" );
	cgs.media.hudFlagNeutralIcon = trap_R_RegisterShaderNoMip( "icons/miscellaneous/flag_neutral" );
#endif //__SUPREMACY__

	// jaquboss start
	cgs.media.prone = trap_R_RegisterShaderNoMip( "gfx/hud/axis_prone" );
	cgs.media.sprint = trap_R_RegisterShaderNoMip( "gfx/hud/axis_sprint" );
	cgs.media.crouch = trap_R_RegisterShaderNoMip( "gfx/hud/axis_crouch" );
	cgs.media.stand = trap_R_RegisterShaderNoMip( "gfx/hud/axis_stand" );
	cgs.media.swim = trap_R_RegisterShaderNoMip( "gfx/hud/axis_swim" );
	cgs.media.prone2 = trap_R_RegisterShaderNoMip( "gfx/hud/allied_prone" );
	cgs.media.sprint2 = trap_R_RegisterShaderNoMip( "gfx/hud/allied_sprint" );
	cgs.media.crouch2 = trap_R_RegisterShaderNoMip( "gfx/hud/allied_crouch" );
	cgs.media.stand2 = trap_R_RegisterShaderNoMip( "gfx/hud/allied_stand" );
	cgs.media.swim2 = trap_R_RegisterShaderNoMip( "gfx/hud/allied_swim" );
	// jaquboss end
	...
	cgs.media.hudDamagedStates[4] = trap_R_RegisterSkin( "models/players/hud/damagedskins/blood05.skin" );
	cgs.media.browningIcon = trap_R_RegisterShaderNoMip( "icons/iconw_browning_1_select" );
	cgs.media.aagunIcon = trap_R_RegisterShaderNoMip( "icons/iconw_aagun_1_select" );
	...
	cgs.media.disconnectIcon = trap_R_RegisterShaderNoMip( "gfx/2d/lag" );
	for ( i = 0; i < 6; i++ )
	{
...
#ifdef __PARTICLE_SYSTEM__
	CG_LoadingString( " - particle systems" );
	cgs.loadingItem = "particle systems";
	...
	for ( i = 1; i < MAX_GAME_PARTICLE_SYSTEMS; i++ )
	{
		...
		//CG_Printf("CGAME: Registering particle system %i (%s).\n", i, psName);
	}
#endif //__PARTICLE_SYSTEM__
	CG_LoadingString( " - game media done" );
	cgs.loadingItem = "game media done";
}
...
/* */
int
CG_ConfigStringCopy ( int index, char *buff, int buffsize )
{
...
CG_QueueMusic ( void )
{
	...
	// TOD-O: \/		the values stored in here will be made accessable so
...
/* */
static const char *
CG_FeederItemText ( float feederID, int index, int column, qhandle_t *handle, int *numhandles )
{
	int				scoreIndex = 0;
...
#define DEBUG_INITPROFILE_EXEC( f )															 \
	if ( developer.integer )																 \
	{																						 \
		CG_Printf( "^5%s passed in %i msec\n", f, elapsed = trap_Milliseconds() - dbgTime ); \
		dbgTime += elapsed;																	 \
	}
#endif // _DEBUG
...
/* */
void
CG_Init ( int serverMessageNum, int serverCommandSequence, int clientNum, qboolean demoPlayback )
{
	const char	*s;
	int			i;
#ifdef _DEBUG
...
#ifdef __SUPREMACY__
	if ( cgs.gametype == GT_WOLF_CAMPAIGN || cgs.gametype == GT_SUPREMACY_CAMPAIGN )
#else //__SUPREMACY__
