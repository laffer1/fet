...
//==========================================================================
static void
CG_BotDebugLine ( vec3_t start, vec3_t end, vec3_t color )
{
	localEntity_t	*le;
	refEntity_t		*re;
...
/* */
static void
CG_BotDebugRadius ( vec3_t pos, vec3_t info, vec3_t color )
{
	localEntity_t	*le;
	refEntity_t		*re;
...
CG_Obituary ( entityState_t *ent )
{
	...
	qhandle_t		backstabShader = cgs.media.backstab;
	qhandle_t		deathShader = cgs.media.pmImages[PM_DEATH];
	qhandle_t		headshotShader = cgs.media.pmImages[PM_HEADSHOT];
	qboolean		SK = qfalse;				//( self kill )
	qboolean		TK = qfalse;				//( team kill )
	target = ent->otherEntityNum;
	attacker = ent->otherEntityNum2;
#ifdef __NPC__
	if ( cg_entities[target].currentState.eType == ET_NPC )
	{											// FIXME: Will fill this in later for NPCs.. Unqiue1.
...
#endif
	mod = ent->eventParm;
	if ( ent->effect3Time == 1 )
	{
		isHeadshot = qtrue;
	}

	if ( ent->effect3Time == 2 )
	{
		backstab = qtrue;
	}
	...
	if ( target < 0 || target >= MAX_CLIENTS )
	{
		...
		/*		if (attacker != ENTITYNUM_WORLD)
		{
			...
			if (cg_entities[target].currentState.teamNum == TEAM_AXIS)
				targetTeamName = "An Axis";
			else
				targetTeamName = "An Allied";

			if ( attacker < 0 || attacker >= MAX_CLIENTS )
			{
				char attackerTeamName;

				if (cg_entities[attacker].currentState.teamNum == TEAM_AXIS)
					attackerTeamName = "an Axis";
				else
					attackerTeamName = "an Allied";

				CG_AddPMItemObit( PM_DEATH, va("%s %s",attackerTeamName, "NPC" ),va( "%s %s", targetTeamName, "NPC"), 
						cg_entities[attacker].currentState.teamNum, cg_entities[target].currentState.teamNum, deathShader );
			}
			else
				CG_AddPMItemObit( PM_DEATH, va("%s", ca->name ), va( "%s ^7%s", targetTeamName, "NPC"), 
						ca->team , cg_entities[target].currentState.teamNum, deathShader );
		}
		else
		{
			char attackerTeamName;
			...
			CG_AddPMItemObit( PM_DEATH, va("%s %s", attackerTeamName, "NPC"), va( "%s", "NPC"),
					cg_entities[attacker].currentState.teamNum, cg_entities[attacker].currentState.teamNum, deathShader );
		}
*/
		return;
	}
	...
	if ( attacker == target )
	{
		switch ( mod )
		{
			...
			case MOD_FLAMETHROWER:				// rain
				SK = qtrue;
				message = "burned himself";		// Jaquboss , 'played with fire sucks
				break;
			case MOD_EXPLOSIVE:
				SK = qtrue;
	...
	if ( message )
	{
		...
		if ( SK == qtrue )
		{
			CG_AddPMItem( PM_SK, va( "%s %s.", targetName, message), deathShader );
		}
		else
		{
	...
	{
		char	*s;
		if ( ci->team == ca->team && cgs.gametype != GT_WOLF_DM )
		{
	...
	if ( ca )
	{
		switch ( mod )
		{
			case MOD_KNIFE:
				if ( backstab == qfalse )
				{
					...
					if
					(
						(attacker == cg.snap->ps.clientNum || target == cg.snap->ps.clientNum) &&
						cg_goatsound.integer >= 2
					)
					{
				...
				else
				{
					...
					// OSP - goat luvin
					if ( cg_goatsound.integer == 1 || cg_goatsound.integer == 3 )
					{
						trap_S_StartSound( cg.snap->ps.origin, cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.goatAxis );
					}
				}
			...
			case MOD_BROWNING2:
				message = "was perforated by";
				message2 = "'s crew-served browning 30cal";
				break;
			case MOD_MG42:
				message = "was perforated by";
			...
			case MOD_30CAL:
				...
				message2 = "'s Mobile browning 30cal";
				break;
		...
		if ( ci->team == ca->team )
		{
			if ( mod == MOD_SWAP_PLACES )
			{
				message = " was kicked to limbo by";
				message2 = "";
			}
			else
			{
		...
		if ( message )
		{
			...
			if ( message2 )
			{
				...
				// cleared
				if ( TK == qtrue && cgs.gametype != GT_WOLF_DM )
				{
					CG_AddPMItem( PM_TK, va( "^1TK:%s ^1%s ^1%s^1%s", targetName, message, attackerName, message2),
								  deathShader );
				}
				else if ( isHeadshot && ci->team != ca->team && mod != MOD_KNIFE )
				{
					CG_AddPMItem( PM_HEADSHOT, va( "%s %s %s%s", targetName, message, attackerName, message2),
								  headshotShader );
				}
				else if ( backstab == qtrue )
				{
					CG_AddPMItem( PM_DEATH, va( "%s %s %s%s", targetName, message, attackerName, message2),
								  backstabShader );
				}
				else
				{
...
/* */
void
CG_PainEvent ( centity_t *cent, int health, qboolean crouching, team_t team )
{		// Jaquboss , added team
	...
	if ( team == TEAM_AXIS )
	{	// Jaquboss , compatibile with W:ET
		if ( health < 25 )
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_axispain4 );
		}
		else if ( health < 50 )
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_axispain3 );
		}
		else if ( health < 75 )
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_axispain2 );
		}
		else
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_axispain1 );
		}
	}
	else if ( team == TEAM_ALLIES )
	{
		if ( health < 25 )
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_alliespain4 );
		}
		else if ( health < 50 )
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_alliespain3 );
		}
		else if ( health < 75 )
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_alliespain2 );
		}
		else
		{
			trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sfx_alliespain1 );
		}
	}
...
*/
void
CG_ExertSound ( int sprintTime, int Time )
{
	int rnd = rand() % 3;
	int contents;
	contents = CG_PointContents( cg.refdef.vieworg, -1 );
	if ( contents & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA) )
	{	// dont do this underwater..
		return;
	}

	if ( cg.time - cg.exertTime < sprintTime )
	{
		return;
	}

	if ( cg.time - cg.exertTime < trap_S_GetSoundLength( cgs.media.exert[rnd]) )
	{	// do not spam
		return;
	}

	trap_S_StartLocalSoundVControll( cgs.media.exert[rnd], CHAN_LOCAL, cg_breathVolume.integer );
	cg.exertTime = Time;
}
...
static fxSound_t	fxSounds[POSSIBLE_PIECES] =
{
	...
	// glass
	{ 3, { -1, -1, -1 }, { "sound/world/glassbreak1.wav", "sound/world/glassbreak2.wav",
...
*/
void
CG_Explode ( centity_t *cent, vec3_t origin, vec3_t dir, qhandle_t shader )
{
...
*/
void
CG_Rubble ( centity_t *cent, vec3_t origin, vec3_t dir, qhandle_t shader )
{
	qhandle_t	inheritmodel = 0;
...
*/
void
CG_Explodef ( vec3_t origin, vec3_t dir, int mass, int type, qhandle_t sound, int forceLowGrav, qhandle_t shader )
{
	int				i;
	localEntity_t	*le;
	refEntity_t		*re;
...
*/
void
CG_Effect ( centity_t *cent, vec3_t origin, vec3_t dir )
{
	localEntity_t	*le;
	refEntity_t		*re;
...
*/
void
CG_Shard ( centity_t *cent, vec3_t origin, vec3_t dir )
{
	localEntity_t	*le;
	refEntity_t		*re;
	int				type;
...
/* */
void
CG_ShardJunk ( centity_t *cent, vec3_t origin, vec3_t dir )
{
	localEntity_t	*le;
	refEntity_t		*re;
	int				type;
...
// Gordon: debris test
void
CG_Debris ( centity_t *cent, vec3_t origin, vec3_t dir )
{
	localEntity_t	*le;
	refEntity_t		*re;
	int				type;
...
// ===================
void
CG_MortarImpact ( centity_t *cent, vec3_t origin, int sfx, qboolean dist )
{
	if ( sfx >= 0 )
	{
...
/* */
void
CG_MortarMiss ( centity_t *cent, vec3_t origin )
{
	if ( cent->currentState.clientNum == cg.snap->ps.clientNum && cg.mortarImpactTime != -2 )
	{
		VectorCopy( origin, cg.mortarImpactPos );
...
static void
CG_StartFootStepSound ( bg_playerclass_t *classInfo, entityState_t *es, sfxHandle_t sfx )
{
...
CG_EntityEvent ( centity_t *cent, vec3_t position )
{
	...
	switch ( event )
	{
		...
		case EV_FOOTSTEP:
			...
			if ( es->eventParm != FOOTSTEP_TOTAL )
			{
				...
				{
					CG_StartFootStepSound( classInfo, es,
										   cgs.media.footsteps[character->animModelInfo->footsteps][footstepcnt] );
				}
			}
			break;
		case EV_FOOTSPLASH:
	...
	case EV_EXERT3:
		...
		break;*/
		case EV_GARAND:
			trap_S_StartSound( NULL, es->number, CHAN_WEAPON, cgs.media.garandping );
			break;
		case EV_STEP_4:
		case EV_STEP_8:
		case EV_STEP_12:
			...
			DEBUGNAME( "EV_GLOBAL_ITEM_PICKUP" );
			{
				...
				if ( *item->pickup_sound )
				{
					...
					trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_AUTO,
									   trap_S_RegisterSound( item->pickup_sound, qfalse) );				// FIXME: precache
				}
		...
		case EV_CHANGE_WEAPON_2:
			...
			break;
			break;
		case EV_TANK_MG_FIRE:
			CG_FireWeapon2( cent );
			break;
		case EV_TANK_FIRE:
			CG_FireWeapon( cent );
			break;
		case EV_TANK_EXPLODE:
			ByteToDir( es->eventParm, dir );
			CG_MissileHitWall( WP_DYNAMITE, 0, position, dir, 0 );										//@TODO finish to make it better
			break;
		...
		case EV_FIRE_WEAPON_BROWNING:
			...
			DEBUGNAME( "EV_FIRE_WEAPON_BROWNING" );
			CG_FireWeapon( cent );
			break;
		...
		case EV_ITEM_RESPAWN:
			break;
		case EV_WEAPON_BOUNCE:
			if ( cgs.media.weaponbounce )
			{
				int volume;
				switch ( es->eventParm )
				{
				...
				trap_S_StartSoundVControl( NULL, es->number, CHAN_ITEM, cgs.media.weaponbounce, volume );
			}
			break;
		case EV_GRENADE_BOUNCE:
			...
			break;
		case EV_RAILTRAIL:
			CG_RailTrail( color /*&cgs.clientinfo[ es->otherEntityNum2 ]*/, es->origin2, es->pos.trBase, es->dmgFlags );	//----(SA)	added 'type' field
			break;
		...
		case EV_MG42BULLET_HIT_WALL:
			...
			ByteToDir( es->eventParm, dir );
			CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qfalse, ENTITYNUM_WORLD, es->otherEntityNum2,
					   es->origin2[0], es->effect1Time, qfalse );
			break;
		case EV_MG42BULLET_HIT_FLESH:
			DEBUGNAME( "EV_MG42BULLET_HIT_FLESH" );
			CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qtrue, es->eventParm, es->otherEntityNum2, 0,
					   es->effect1Time, qfalse );
			break;
		case EV_AAGUN_HIT_WALL:
			DEBUGNAME( "EV_AAGUN_HIT_WALL" );
			ByteToDir( es->eventParm, dir );
			CG_Smokey( 0, position, dir );
			CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qfalse, ENTITYNUM_WORLD, es->otherEntityNum2,
					   es->origin2[0], 0, qfalse );
			break;
		...
		case EV_BULLET_HIT_FLESH:
			...
			CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qtrue, es->eventParm, es->otherEntityNum2, 0, 0, qfalse );
			break;
		case EV_WOLFKICK_HIT_WALL:
			DEBUGNAME( "EV_WOLFKICK_HIT_WALL" );
			ByteToDir( es->eventParm, dir );
			CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qfalse, ENTITYNUM_WORLD, es->otherEntityNum2,
					   es->origin2[0], 0, qtrue );
			trap_S_StartSound( NULL, es->number, CHAN_AUTO, cgs.media.fkickwall );
			break;
		case EV_WOLFKICK_HIT_FLESH:
			DEBUGNAME( "EV_WOLFKICK_HIT_FLESH" );
			CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, qtrue, es->eventParm, es->otherEntityNum2, 0, 0, qtrue );
		...
		case EV_DESTROY_PARTICLES:
			CG_DestroyAllEntityParticles( &cg_entities[es->number] );
			DEBUGNAME( "EV_DESTROY_PARTICLES" );
			break;
	#endif \
		\
		//__PARTICLE_SYSTEM__
		...
		case EV_PAIN:
			...
			CG_PainEvent( cent, es->eventParm, qfalse , cent->currentState.teamNum );

		}*/
			CG_PainEvent( cent, es->eventParm, qfalse, cgs.clientinfo[cent->currentState.clientNum].team );
			break;
		case EV_CROUCH_PAIN:
			...
			/*		if ( cent->currentState.number != cg.snap->ps.clientNum ) {
			CG_PainEvent( cent, es->eventParm, qtrue , cent->currentState.teamNum);
		}*/
			CG_PainEvent( cent, es->eventParm, qtrue, cgs.clientinfo[cent->currentState.clientNum].team );
			break;
		case EV_DEATH1:
		case EV_DEATH2:
		case EV_DEATH3:
		...
		case EV_FLAMETHROWER_EFFECT:
			...
			//	CG_FireFlameChunks( cent, cent->currentState.origin, cent->lerpAngles, 1.0, 2 );
			break;
		case EV_DUST:
		...
		/*	case EV_PUSH_SOUND:
		trap_S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, cgs.media.pushPlayer );
		break;*/
		...
		case EV_ARTYMESSAGE:
			...
			break;
		case EV_FLAG:
			trap_S_StartLocalSound( cgs.media.flagsound, CHAN_VOICE );
			break;
		case EV_FLAG_ALLIES:
			trap_S_StartLocalSound( cgs.media.flagsound_allies, CHAN_VOICE );
			break;
		case EV_FLAG_AXIS:
			trap_S_StartLocalSound( cgs.media.flagsound_axis, CHAN_VOICE );
			break;
		case EV_MEDIC_CALL:
			switch ( cgs.clientinfo[cent->currentState.number].team )
			{
				...
				CG_Printf( "%i\n", rnd );*/
					trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sndMedicCall_axis[rand() % 3] );
					break;
				case TEAM_ALLIES:
				...
				CG_Printf( "%i\n", rnd );*/
					trap_S_StartSound( NULL, cent->currentState.number, CHAN_VOICE, cgs.media.sndMedicCall_allies[rand() % 3] );
					break;
		...
		case EV_SCOREPLUM:
			...
			if ( cent->currentState.weapon == 4 )
			{		// A Land Vehicle Waypoint.. Show a tank icon above it!
				if ( cent->currentState.density == 1 )
				{	// Big!
					CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon,
								  qtrue, cgs.media.ccTankIcon );
				}
				else
				{
					CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon,
								  qfalse, cgs.media.ccTankIcon );
				}
			}
			else if ( cent->currentState.density == 1 )
			{		// Big!
				CG_ScorePlum( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, es->weapon,
							  qtrue, 0 );
			}
		...
		case EV_TESTLINE:
			...
			break;
		case EV_GOALNODE:
			DEBUGNAME( "EV_GOALNODE" );
			CG_GoalNode( cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time );
			break;
	#endif
		case EV_NONE:
			DEBUGNAME( "EV_NONE" );
...
*/
			CG_Printf( "^1ERROR^5: Unknown event: ^7%i^5.\n", event );
			break;
	}
	{
		int rval;
...
CG_CheckEvents ( centity_t *cent )
{
	...
	BG_EvaluateTrajectory( &cent->currentState.pos, cg.snap->serverTime, cent->lerpOrigin, qfalse,
