...
/* */
qboolean
CG_SpawnString ( const char *key, const char *defaultString, char **out )
{
	int i;
	if ( !cg.spawning )
	{
...
/* */
qboolean
CG_SpawnFloat ( const char *key, const char *defaultString, float *out )
{
	char		*s;
	qboolean	present;
...
/* */
qboolean
CG_SpawnInt ( const char *key, const char *defaultString, int *out )
{
	char		*s;
	qboolean	present;
	present = CG_SpawnString( key, defaultString, &s );
...
/* */
qboolean
CG_SpawnVector ( const char *key, const char *defaultString, float *out )
{
	char		*s;
	qboolean	present;
	present = CG_SpawnString( key, defaultString, &s );
...
/* */
qboolean
CG_SpawnVector2D ( const char *key, const char *defaultString, float *out )
{
	char		*s;
	qboolean	present;
	present = CG_SpawnString( key, defaultString, &s );
...
/* */
void
SP_info_train_spline_main ( void )
{
	char			*targetname;
	char			*target;
	char			*control;
...
SP_misc_gamemodel ( void )
{
	...
	if
	(
		CG_SpawnString( "targetname", "", &model) ||
...
/* */
void
SP_trigger_objective_info ( void )
{
	char	*temp;
...
*/
void
CG_ParseEntityFromSpawnVars ( void )
{
	int		i;
	char	*classname;
...
*/
char *
CG_AddSpawnVarToken ( const char *string )
{
	int		l;
	char	*dest;
	l = strlen( string );
...
*/
qboolean
CG_ParseSpawnVarsEx ( int handle )
{
	pc_token_t	token;
	char		keyname[MAX_TOKEN_CHARS];
	cg.numSpawnVars = 0;
	...
	if ( token.string[0] != '{' )
	{
		CG_Error( "CG_ParseSpawnVars: found %s when expecting {", token.string );
	}
	...
	while ( 1 )
	{
		...
		cg.spawnVars[cg.numSpawnVars][0] = CG_AddSpawnVarToken( keyname );
		cg.spawnVars[cg.numSpawnVars][1] = CG_AddSpawnVarToken( token.string );
		cg.numSpawnVars++;
	}
...
SP_worldspawn ( void )
{
	...
	if
	(
		CG_SpawnVector2D( "mapcoordsmins", "-128 128", cg.mapcoordsMins) && // top left
	...
	CG_EffectParse( s );
	cg.fiveMinuteSound_g[0] = cg.fiveMinuteSound_a[0] = cg.twoMinuteSound_g[0] = cg.twoMinuteSound_a[0] = cg.thirtySecondSound_g[0] = cg.thirtySecondSound_a[0] = '\0';
...
*/
void
CG_ParseEntitiesFromString ( void )
{
	...
	{
		handle = trap_PC_LoadSource( va( "maps/%s_supremacy.ovrents", cgs.rawmapname) );
	}
	else
#endif //__SUPREMACY__
		if ( cgs.gametype == GT_WOLF_LMS )
		{
			handle = trap_PC_LoadSource( va( "maps/%s_lms.ovrents", cgs.rawmapname) );
		}
		else if ( cgs.gametype == GT_SINGLE_PLAYER || cgs.gametype == GT_COOP )
		{
			handle = trap_PC_LoadSource( va( "maps/%s_coop.ovrents", cgs.rawmapname) );
		}
		else
		{
			handle = trap_PC_LoadSource( va( "maps/%s.ovrents", cgs.rawmapname) );
		}

	if ( handle )
	{
		if ( !CG_ParseSpawnVarsEx( handle) )
		{
			CG_Error( "SpawnEntities: no entities" );
		}
	...
	{									// Pass all the new .ovrents file's entities one at a time...
		while ( CG_ParseSpawnVarsEx( handle) )
		{
			CG_ParseEntityFromSpawnVars();
		}
	...
	if ( cgs.gametype == GT_SUPREMACY || cgs.gametype == GT_SUPREMACY_CAMPAIGN )
	{
		handle = trap_PC_LoadSource( va( "maps/%s_supremacy.entities", cgs.rawmapname) );
	}
#endif //__SUPREMACY__
	if ( cgs.gametype == GT_WOLF_LMS )
	{
		handle = trap_PC_LoadSource( va( "maps/%s_lms.entities", cgs.rawmapname) );
	}
	else if ( cgs.gametype == GT_SINGLE_PLAYER || cgs.gametype == GT_COOP )
	{
		handle = trap_PC_LoadSource( va( "maps/%s_coop.entities", cgs.rawmapname) );
	}
	else
	{
		handle = trap_PC_LoadSource( va( "maps/%s.entities", cgs.rawmapname) );
	}

	if ( handle )
	{
		if ( CG_ParseSpawnVarsEx( handle) == qfalse )
		{
			CG_Error( "SpawnEntities: no entities" );
		}
