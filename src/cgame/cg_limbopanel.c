...
void	CG_LimboPanel_RenderCounter_BigLimbo ( panel_button_t *button );
void	CG_LimboPanel_Border_Draw_Big ( panel_button_t *button );


/* */
void
CG_PanelButtonsRender_BigLimbo ( panel_button_t *button )
{
	if ( cg_bigLimbo.integer )
	{
...
/* */
void
BG_PanelButtonsRender_Img_Big ( panel_button_t *button )
{
	if ( cg_bigLimbo.integer )
	{
...
team_t				teamOrder[3] = { TEAM_ALLIES, TEAM_AXIS, TEAM_SPECTATOR, };
...
panel_button_text_t classBarFont = { 0.22f, 0.24f, { 0.f, 0.f, 0.f, 0.8f }, 0, 0, &cgs.media.limboFont2, };
panel_button_text_t titleLimboFont = { 0.24f, 0.28f, { 1.f, 1.f, 1.f, 0.6f }, 0, 0, &cgs.media.limboFont1, };
panel_button_text_t titleLimboFontBig = { 0.3f, 0.3f, { 1.f, 1.f, 1.f, 0.6f }, 0, 0, &cgs.media.limboFont1, };
...
panel_button_text_t spawnLimboFont = { 0.18f, 0.22f, { 1.f, 1.f, 1.f, 0.6f }, 0, 0, &cgs.media.limboFont1, };
panel_button_text_t weaponButtonFont = { 0.33f, 0.33f, { 0.f, 0.f, 0.f, 0.6f }, 0, 0, &cgs.media.limboFont1, };
panel_button_text_t weaponPanelNameFont = { 0.20f, 0.24f, { 1.0f, 1.0f, 1.0f, 0.4f }, 0, 0, &cgs.media.limboFont1, };
...
panel_button_text_t objectivePanelTxt = { 0.2f, 0.2f, { 0.0f, 0.0f, 0.0f, 0.5f }, 0, 0, &cgs.media.limboFont2, };
...
#define TEAM_COUNTER( number )																							\
	...
	{																													\
		...
		{ TEAM_COUNTER_X + TEAM_COUNTER_GAP +																			\
				(																										\
					number *																							\
					(TEAM_COUNTER_GAP + TEAM_COUNTER_WIDTH) +															\
					(TEAM_COUNTER_BUTTON_DIFF / 2.f)																	\
				) -																										\
						17 +																							\
						TEAM_COUNTER_SPACING, 188 +																		\
					TEAM_COUNTER_SPACING, TEAM_COUNTER_WIDTH -															\
					TEAM_COUNTER_BUTTON_DIFF +																			\
					20 -																								\
					2 *																									\
					TEAM_COUNTER_SPACING, 44 - 2 * TEAM_COUNTER_SPACING },												\
		{ number, 0, 0, 0, 0, 0, 0, 0 },																				\
		NULL,								/* font		*/																	\
...
#define CLASS_COUNTER( number )																					 \
	...
	{																											 \
		...
		{ CLASS_COUNTER_X +																						 \
				CLASS_COUNTER_GAP +																				 \
				( number * (CLASS_COUNTER_GAP + CLASS_COUNTER_WIDTH) ), 302, CLASS_COUNTER_WIDTH, 14 },			 \
	...
	{																											 \
		NULL,																									 \
		NULL,																									 \
		{ CLASS_COUNTER_X + CLASS_COUNTER_GAP + ( number * (CLASS_COUNTER_GAP + CLASS_COUNTER_WIDTH) ) +		 \
				( CLASS_COUNTER_BUTTON_DIFF / 2.f ), 266, CLASS_COUNTER_WIDTH - CLASS_COUNTER_BUTTON_DIFF, 34 }, \
		{ 0, number, 0, 0, 0, 0, 0, 0 },																		 \
		NULL,								/* font		*/															 \
...
panel_button_t	filterTitleText =
{
	...
	NULL,									/* keyDown	*/
	NULL,									/* keyUp	*/
	CG_PanelButtonsRender_BigLimbo,
	NULL,
};
...
panel_button_t	playerLimboHead =
{
	...
	{ 0, 0, 0, 0, 0, 0, 0, 0 },
	NULL,									/* font		*/
	CG_LimboPanel_Hat_KeyDown,				/* keyDown	*/
	NULL,									/* keyUp	*/
...
panel_button_t	mapTimeCounter =
{
	...
	NULL,									/* keyDown	*/
	NULL,									/* keyUp	*/
	CG_LimboPanel_RenderCounter_BigLimbo,
	NULL,
};
...
panel_button_t	enlargeButton =
{
	NULL,
	NULL, { 452, 5, 60, 12 },
	{ 0, 0, 0, 0, 0, 0, 0, 0 },
	NULL,									/* font		*/
	CG_LimboPanel_Enlarge_KeyDown,			/* keyDown	*/
	NULL,									/* keyUp	*/
...
{
	NULL,
	NULL, { CC_2D_X, CC_2D_Y, CC_2D_W, CC_2D_H },
	{ 0, 0, 0, 0, 0, 0, 0, 0 },
	NULL,									/* font		*/
	NULL,									/* keyDown	*/
	NULL,									/* keyUp	*/
...
panel_button_t	*limboPanelButtons[] =
{
	...
	&objectivePanelTitle,
	&objectivePanelText,
	&objectivePanelButtonUp,
	&objectivePanelButtonDown,
	&limboTitleText,
	&enlargeButton,
	&enlargeText,
	&skillsText,
	...
	&weaponLight2Text,
	&weaponPanel,
	&weaponStatsShotsText,
	&weaponStatsHitsText,
	&weaponStatsAccText,
	&weaponStatsShotsCounter,
	&weaponStatsHitsCounter,
	&weaponStatsAccCounter,
...
/* */
qboolean
CG_LimboPanel_BriefingButton_KeyDown ( panel_button_t *button, int key )
{
...
/* */
void
CG_LimboPanel_BriefingButton_Draw ( panel_button_t *button )
{
	if ( cg_gameType.integer == GT_WOLF_LMS )
	{
		return;
	...
	if ( cg.limboEndCinematicTime > cg.time )
	{
		CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h, BG_CursorInRect( &button->rect
					) ? cgs.media.limboBriefingButtonStopOn : cgs.media.limboBriefingButtonStopOff );
	}
	else
	{
...
/* */
void
CG_LimboPanel_NameEditFinish ( panel_button_t *button )
{
	char	buffer[256];
...
/* */
qboolean
CG_LimboPanel_CancelButton_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_CANCEL;
...
/* */
qboolean
CG_LimboPanel_PlusButton_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_SELECT;
...
/* */
qboolean
CG_LimboPanel_MinusButton_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_SELECT;
...
/* */
void
CG_LimboPanel_SendSetupMsg ( qboolean forceteam )
{
	...
	// send head setup
	trap_SendClientCommand( va( "head %i %i", cg_myHeadModel.integer, cg_myHeadSkin.integer) );
	...
	}
	{
		...
		CG_PriorityCenterPrint( va( "You will spawn as an %s %s with a %s.", str,
								BG_ClassnameForNumber( CG_LimboPanel_GetClass()), wt ? wt->desc : "^1UNKNOWN WEAPON"),
...
/* */
qboolean
CG_LimboPanel_OkButton_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_SELECT;
...
/* */
qboolean
CG_LimboPanel_TeamButton_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_SELECT;
...
/* */
void
CG_LimboPanel_RenderTeamButton ( panel_button_t *button )
{
	...
	CG_DrawPic( button->rect.x + 1, button->rect.y + 1, button->rect.w, button->rect.h,
...
/* */
qboolean
CG_LimboPanel_ClassButton_KeyDown ( panel_button_t *button, int key )
{
...
/* */
void
CG_LimboPanel_ClassBar_Draw ( panel_button_t *button )
{
	...
	CG_Text_Paint_Ext( button->rect.x + (button->rect.w - w) * 0.5f, button->rect.y, button->font->scalex,
					   button->font->scaley, button->font->colour, buffer, 0, 0, button->font->style, button->font->font );
}


/* */
void
CG_LimboPanel_RenderClassButton ( panel_button_t *button )
{
	...
	{
		CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h,
					cgs.media.limboClassButtons2[button->data[1]] );
	}
	else
	{
		trap_R_SetColor( clr2 );
...
/* */
int
CG_LimboPanel_GetMaxObjectives ( void )
{
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
		return ( 0 );
...
/* */
qboolean
CG_LimboPanel_ObjectiveText_KeyDown ( panel_button_t *button, int key )
{
	int max;
	if ( key == K_MOUSE1 )
	{
...
/* */
void
CG_LimboPanel_RenderObjectiveText ( panel_button_t *button )
{
	const char	*cs;
	char		*info, *s, *p;
	float		y;
	...
	while ( *p )
	{
		...
		{
			*p++ = '\0';
			CG_Text_Paint_Ext( button->rect.x + 4, y, button->font->scalex, button->font->scaley, button->font->colour,
							   s, 0, 0, 0, &cgs.media.limboFont2 );
			y += 8;
			s = p;
		}
		else
	...
	if ( cg_gameType.integer != GT_WOLF_LMS && CG_LimboPanel_GetTeam() != TEAM_SPECTATOR )
	{
		...
		CG_Text_Paint_Ext( x, button->rect.y + button->rect.h - 2, 0.2f, 0.2f, colorBlack, ofTxt, 0, 0, 0,
						   &cgs.media.limboFont2 );
	}

	if ( status == 1 )
	{
		CG_DrawPic( button->rect.x + 87, button->rect.y + 8, button->rect.w - 174, button->rect.h - 8,
					cgs.media.ccStamps[0] );
	}
	else if ( status == 2 )
	{
		CG_DrawPic( button->rect.x + 87, button->rect.y + 8, button->rect.w - 174, button->rect.h - 8,
					cgs.media.ccStamps[1] );
	}
}
...
/* */
void
CG_LimboPanel_RenderObjectiveBack ( panel_button_t *button )
{
...
/* */
void
CG_LimboPanel_RenderCommandMap ( panel_button_t *button )
{
	if ( cg_bigLimbo.integer == 0 )
	{
		return;
...
/* */
qboolean
CG_LimboPanel_RenderLight_GetValue ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 0:
...
/* */
void
CG_LimboPanel_RenderLight ( panel_button_t *button )
{
...
)
{
	...
	float			x, y, w, h;
	refdef_t		refdef;
	refEntity_t		head, hat, mrank, gmask;
	clientInfo_t	*ci;
	if ( !character )
	{
		return;
	...
	if ( team == TEAM_AXIS )
	{
		head.customSkin = headSkinsAxis[skin].skin;
		head.hModel = headModelsAxis[model].model;
	}
	else
	{
		head.customSkin = headSkinsAllies[skin].skin;
		head.hModel = headModelsAllies[model].model;
	}
	...
	if ( !cg.showGameView )
	{
		head.renderfx |= /*RF_LIGHTING_ORIGIN |*/ RF_MINLIGHT | RF_NOSHADOW;
	}

	CG_HudHeadAnimation( headcharacter, &cg.predictedPlayerEntity.pe.hudhead, &head.oldframe, &head.frame,
						 &head.backlerp, animation );
	if ( drawHat )
	{
	...
	if ( spectator )										// with gmask really
	{
		memset( &gmask, 0, sizeof(gmask) );
		gmask.hModel = cgs.media.gmask;
		gmask.renderfx = RF_NOSHADOW | RF_FORCENOLOD;
		CG_PositionEntityOnTag( &gmask, &head, "tag_mouth", 0, NULL );
	}
	...
	if ( spectator )
	{
		trap_R_AddRefEntityToScene( &gmask );
	}
...
/* */
void
CG_DrawLimboGun ( rectDef_t *rect, float yaw, float pitch, weaponInfo_t *weapon )
{
	float		len;
	vec3_t		origin;
	vec3_t		mins, maxs, angles;
	...
	refEntity_t gun;
	qhandle_t	gunmodel;
	gunmodel = weapon->weaponModel[W_PU_MODEL].model;
	if ( !gunmodel )
	{
		gunmodel = weapon->weaponModel[W_TP_MODEL].model;
	}
	...
	// offset the origin y and z to center the head
	trap_R_ModelBounds( gunmodel, mins, maxs );
	origin[2] = -0.7 * ( mins[2] + maxs[2] );
	origin[1] = 0.5 * ( mins[1] + maxs[1] );
	...
	gun.hModel = gunmodel;
	gun.renderfx = RF_NOSHADOW | RF_FORCENOLOD; // no stencil shadows
	if ( !cg.showGameView )
	{
		gun.renderfx |= RF_MINLIGHT | RF_NOSHADOW;
	}
...
/* */
void
CG_LimboPanel_RenderHead ( panel_button_t *button )
{
	vec4_t		clrBack = { 0.05f, 0.05f, 0.05f, 1.f };
	qboolean	flamer = qfalse;

	//	clientInfo_t* ci = &cgs.clientinfo[cg.clientNum];
	if ( CG_LimboPanel_GetSelectedWeaponForSlot( 1) == WP_FLAMETHROWER && CG_LimboPanel_GetTeam() == TEAM_AXIS )
	{
		flamer = qtrue;
	}
	...
	if ( CG_LimboPanel_GetTeam() != TEAM_SPECTATOR )
	{
		...
		CG_DrawPlayerHead( &button->rect, CG_LimboPanel_GetCharacter(), CG_LimboPanel_GetCharacter(), 180, 0, qtrue,
						   HD_IDLE1, 0, cgs.clientinfo[cg.clientNum].rank,	/*qfalse*/
						  flamer, CG_LimboPanel_GetTeam(), cg_myHeadModel.integer, cg_myHeadSkin.integer );
	}
	else
	{
...
/* */
qboolean
CG_LimboPanel_Enlarge_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_FILTER;
		if ( cg_bigLimbo.integer > 0 )
		{
			trap_Cvar_Set( "cg_bigLimbo", "0" );
		}
		else
		{
			trap_Cvar_Set( "cg_bigLimbo", "1" );
		}
...
/* */
qboolean
CG_LimboPanel_Filter_KeyDown ( panel_button_t *button, int key )
{
	if ( key == K_MOUSE1 )
	{
		SOUND_FILTER;
...
/* */
void
CG_LimboPanel_Filter_Draw ( panel_button_t *button )
{
	if ( cg_bigLimbo.integer == 0 )
	{
		return;
...
/* */
void
CG_LimboPanel_RenderSkillIcon ( panel_button_t *button )
{
	qhandle_t	shader;
	if ( cg_gameType.integer == GT_WOLF_LMS /*|| CG_LimboPanel_GetTeam() == TEAM_SPECTATOR*/ )
	{
...
/* */
qboolean
CG_LimboPanel_WeaponLights_KeyDown ( panel_button_t *button, int key )
{
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
		return ( qfalse );
...
/* */
void
CG_LimboPanel_WeaponLights ( panel_button_t *button )
{
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
	...
	{
		CG_DrawPic( button->rect.x, button->rect.y, button->rect.w, button->rect.h,
					button->data[0] == cgs.ccSelectedWeaponNumber ? cgs.media.limboWeaponNumber_on : cgs.media.
						limboWeaponNumber_off );
	}
}
...
/* */
qboolean
CG_LimboPanel_Hat_KeyDown ( panel_button_t *button, int key )
{
	int number;
	int team = CG_LimboPanel_GetTeam();
	int max = 0;
	button->data[7] = 0;
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
	...
	if ( key == K_MOUSE1 )
	{
		...
		if ( cg_myHeadSkin.integer + 1 <= max )
		{
			number = cg_myHeadSkin.integer + 1;
			trap_Cvar_Set( "cg_myHeadSkin", va( "%i", number) );
		}
		else
		{
			trap_Cvar_Set( "cg_myHeadSkin", "0" );
		}
	}
	...
	if ( key == K_MOUSE2 )
	{
		...
		{
			number = cg_myHeadSkin.integer + 1;
			trap_Cvar_Set( "cg_myHeadModel", va( "%i", number) );
		}
		else
		{
			trap_Cvar_Set( "cg_myHeadModel", "0" );
		}
	}
...
/* */
qboolean
CG_LimboPanel_WeaponPanel_KeyDown ( panel_button_t *button, int key )
{
	button->data[7] = 0;
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
...
/* */
qboolean
CG_LimboPanel_WeaponPanel_KeyUp ( panel_button_t *button, int key )
{
	int			cnt, i;
	rectDef_t	rect;
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
...
/* */
void
CG_LimboPanel_WeaponPanel_DrawWeapon
(
	rectDef_t	*rect,
	weapon_t	weap,
	qboolean	highlight,
...
)
{
	...
	if ( wt->desc )
	{
		if ( highlight && BG_CursorInRect( rect) )
		{
			...
			clr[3] *= 1.5;
			CG_Text_Paint_Ext( rect->x + 4, rect->y + 12, weaponPanelNameFont.scalex, weaponPanelNameFont.scaley, clr,
							   wt->desc, 0, 0, weaponPanelNameFont.style, weaponPanelNameFont.font );
		}
		else
		{
			CG_Text_Paint_Ext( rect->x + 4, rect->y + 12, weaponPanelNameFont.scalex, weaponPanelNameFont.scaley,
							   weaponPanelNameFont.colour, wt->desc, 0, 0, weaponPanelNameFont.style,
							   weaponPanelNameFont.font );
		}
	}
	{
		...
		w *= rect->w;
		h *= rect->h * 0.75f;
		if ( cg_draw3DlimboWeapons.integer > 0 )
		{
			CG_DrawLimboGun( rect, 90, 0, weapon );
		}
		else
		{
...
/* */
void
CG_DrawBorder ( float x, float y, float w, float h, qboolean fill, qboolean drawMouseOver )
{
...
/* */
void
CG_LimboPanel_Border_Draw ( panel_button_t *button )
{
...
/* */
void
CG_LimboPanel_Border_Draw_Big ( panel_button_t *button )
{
	if ( cg_bigLimbo.integer )
	{
		CG_DrawBorder( button->rect.x, button->rect.y, button->rect.w, button->rect.h, qtrue, qtrue );
...
CG_LimboPanel_WeaponPanel ( panel_button_t *button )
{
	...
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
		...
		trap_R_SetColor( NULL );
		CG_Text_Paint_Ext( button->rect.x + 4, button->rect.y + 12, weaponPanelNameFont.scalex,
						   weaponPanelNameFont.scaley, weaponPanelNameFont.colour, "SPECTATOR", 0, 0,
						   weaponPanelNameFont.style, weaponPanelNameFont.font );
		return;
	}
	...
	if ( BG_PanelButtons_GetFocusButton() == button && cnt > 1 )
	{
		...
		rectDef_t	rect;
		memcpy( &rect, &button->rect, sizeof(rect) );
		CG_LimboPanel_WeaponPanel_DrawWeapon( &rect, weap, qtrue,
		...
		CG_DrawBorder( button->rect.x, button->rect.y - ((cnt - 1) * button->rect.h), button->rect.w,
					   button->rect.h * cnt, qfalse, qfalse );
	}
	else
	{
		vec4_t	clr = { 0.f, 0.f, 0.f, 0.4f };
...
/* */
void
CG_LimboPanel_RenderCounterNumber
(
	float		x,
	float		y,
	float		w,
...
/* */
int
CG_LimboPanel_RenderCounter_ValueForButton ( panel_button_t *button )
{
	int i, count = 0;
	switch ( button->data[0] )
	{
...
/* */
int
CG_LimboPanel_RenderCounter_RollTimeForButton ( panel_button_t *button )
{
	float	diff;
	switch ( button->data[0] )
	{
...
/* */
int
CG_LimboPanel_RenderCounter_MaxChangeForButton ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 2: // xp
...
/* */
int
CG_LimboPanel_RenderCounter_NumRollers ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 0: // class counts
...
/* */
qboolean
CG_LimboPanel_RenderCounter_CountsDown ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 4: // skill
...
/* */
qboolean
CG_LimboPanel_RenderCounter_CountsUp ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 4: // skill
...
/* */
qboolean
CG_LimboPanel_RenderCounter_StartSet ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 3: // respawn time
...
/* */
void
CG_LimboPanel_RenderMedal ( panel_button_t *button )
{
	...
	{
		CG_DrawPic( button->rect.x - 2, button->rect.y, button->rect.w + 4, button->rect.h,
					cgs.media.medals[button->data[0]] );
	}
}
...
/* */
qboolean
CG_LimboPanel_RenderCounter_IsReversed ( panel_button_t *button )
{
	switch ( button->data[0] )
	{
		case 4: // skill
...
/* */
void
CG_LimboPanel_RenderCounter_GetShaders
(
...
/* */
void
CG_LimboPanelRenderText_NoLMS ( panel_button_t *button )
{
	if ( cg_gameType.integer == GT_WOLF_LMS )
	{
		return;
...
/* */
void
CG_LimboPanelRenderText_SkillsText ( panel_button_t *button )
{
	if ( cg_gameType.integer == GT_WOLF_LMS /*|| CG_LimboPanel_GetTeam() == TEAM_SPECTATOR*/ )
	{
		return;
...
void
CG_LimboPanel_RenderCounter ( panel_button_t *button )
{
	float		x, w;
	...
	if ( CG_LimboPanel_RenderCounter_IsReversed( button) )
	{
		for ( i = 0; i < num; i++ )
		{
			CG_LimboPanel_RenderCounterNumber( x, button->rect.y, w, button->rect.h, count[i], shaderBack, shaderRoll,
											   numimages );
			x += w + button->data[6];
		}
	}
...
/* */
void
CG_LimboPanel_RenderCounter_BigLimbo ( panel_button_t *button )
{
	if ( cg_bigLimbo.integer )
	{
...
/* */
void
CG_LimboPanel_Setup ( qboolean music )
{
	panel_button_t		*button;
	...
	if ( cg_limboMusic.integer > 0 && music )
	{
		if ( cg.showGameView )
		{
			trap_S_FadeBackgroundTrack( 0.0f, 500, 0 ); // stop old track
			trap_S_StartBackgroundTrack( "sound/music/limbo_drums.wav", "", 0 );	// test it !
		}
		else
		{
	...
	cg.deathloop = qfalse;
	trap_S_StartSoundEx( NULL, cg.predictedPlayerState.clientNum, CHAN_VOICE_BG, 0, SND_CUTOFF_ALL );
...
CG_LimboPanel_Draw ( void )
{
	...
	if ( cg.limboEndCinematicTime > cg.time && cg_bigLimbo.integer )
	{
		...
		if ( cg.time - cgs.ccLastObjectiveRequestTime > 1000 )
		{
			...
			else
			{
				...
				)
				{
					...
					(
						!(
							cgs.ccRequestedObjective == CG_LimboPanel_GetMaxObjectives() &&
							cgs.ccCurrentCamObjective == -1 &&
							cgs.ccPortalEnt == -1
						)
					)
					{
...
/* */
void
CG_LimboPanel_KeyHandling ( int key, qboolean down )
{
	int b1, b2;
...
/* */
team_t
CG_LimboPanel_GetRealTeam ( void )
{
	return
		(
			cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR ? CG_LimboPanel_GetTeam()
...
/* */
bg_character_t *
CG_LimboPanel_GetCharacter ( void )
{
...
/* */
bg_playerclass_t *
CG_LimboPanel_GetPlayerClass ( void )
{
...
/* */
int
CG_LimboPanel_WeaponCount ( void )
{
...
/* */
int
CG_LimboPanel_WeaponCount_ForSlot ( int number )
{
	if ( number == 1 )
	{
		bg_playerclass_t	*classInfo = CG_LimboPanel_GetPlayerClass();
...
/* */
int
CG_LimboPanel_GetWeaponNumberForPos ( int pos )
{
	int i, cnt = 0;
...
/* */
weapon_t
CG_LimboPanel_GetWeaponForNumber ( int number, int slot, qboolean ignoreDisabled )
{
	bg_playerclass_t	*classInfo;
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
	...
	else
	{
		if ( cgs.clientinfo[cg.clientNum].skill[SK_HEAVY_WEAPONS] >= 4 && CG_LimboPanel_GetClass() == PC_SOLDIER )
		{
			if ( cgs.clientinfo[cg.clientNum].skill[SK_LIGHT_WEAPONS] >= 4 )
			{
				if ( number == 3 )
				{
					return ( CG_LimboPanel_GetTeam() == TEAM_AXIS ? WP_G43 : WP_M1GARAND );
				}
			...
			else
			{
				...
				if ( number == 2 )
				{
					return ( CG_LimboPanel_GetTeam() == TEAM_AXIS ? WP_MP40 : WP_M1GARAND );
				}
			}
		}
...
/* */
weapon_t
CG_LimboPanel_GetSelectedWeaponForSlot ( int index )
{
...
/* */
void
CG_LimboPanel_SetSelectedWeaponNumForSlot ( int index, int number )
{
	if ( index == 0 )
	{
...
/* */
weapon_t
CG_LimboPanel_GetSelectedWeapon ( void )
{
...
/* */
int
CG_LimboPanel_GetSelectedWeaponNum ( void )
{
...
/* */
void
CG_LimboPanel_RequestWeaponStats ( void )
{
...
/* */
void
CG_LimboPanel_RequestObjective ( void )
{
	int max = CG_LimboPanel_GetMaxObjectives();
...
/* */
void
CG_LimboPanel_SetSelectedWeaponNum ( int number )
{
...
/* */
extWeaponStats_t
CG_LimboPanel_GetSelectedWeaponStat ( void )
{
...
/* */
int
CG_LimboPanel_TeamCount ( weapon_t weap )
{
	int i, cnt;
	if ( weap == -1 )
...
/* */
qboolean
CG_IsHeavyWeapon ( weapon_t weap )
{
	int i;
...
/* */
qboolean
CG_LimboPanel_WeaponIsDisabled ( int index )
{
	bg_playerclass_t	*classinfo;
	int					count, wcount;
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
...
/* */
qboolean
CG_LimboPanel_RealWeaponIsDisabled ( weapon_t weap )
{
	int count, wcount;
	if ( CG_LimboPanel_GetTeam() == TEAM_SPECTATOR )
	{
