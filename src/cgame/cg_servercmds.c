...
CG_ForceCvar ( void )
{
	...
	if ( len < 1 )
	{
		...
		if ( len <= 0 )
		{
			CG_Printf( "forcecvar: could not read \"%s\" for backup\n", cfgPath );
			return;
		}
	...
	trap_Cvar_Register( NULL, cvarName, cvarValue, CVAR_ROM );
	trap_Cvar_Set( cvarName, cvarValue );
	CG_Printf( "forcecvar: server set cvar \"%s\" equal to \"%s\"\n", cvarName, cvarValue );
	cg.forcecvar = qtrue;
}
...
CG_ParseScore3 ( void )
{
	cg.teamScores[1] = atoi( CG_Argv( 1) );
}
...
CG_ParseServerinfo ( void )
{
	...
	//	cg_staminaonlyjumps.integer = atoi ( Info_ValueForKey(  info, "g_staminaonlyjumps" ) );
	if ( !cgs.localServer )
	{
	...
	cg_hitsound_allowed.integer = atoi( Info_ValueForKey( info, "g_hitsound") );
	cg_crosshair_allowed.integer = atoi( Info_ValueForKey( info, "g_crosshair") );
	cg_3rdPerson_allowed.integer = atoi( Info_ValueForKey( info, "g_thirdPerson") );
	cg_3rdPerson_maxrange.integer = atoi( Info_ValueForKey( info, "g_thirdPersonRange") );
	cg_3rdPerson_maxangle.integer = atoi( Info_ValueForKey( info, "g_thirdPersonAngle") );
	cg_snoopers.integer = atoi( Info_ValueForKey( info, "g_snoopers") );
	cg_nohud.integer = atoi( Info_ValueForKey( info, "g_nohud") );
	cg_realmedic.integer = atoi( Info_ValueForKey( info, "g_realmedic") );
	cg_noCapsule.integer = atoi( Info_ValueForKey( info, "g_noCaps") );

	/*	cg_numHeadModelsAllies.integer = atoi( Info_ValueForKey(info,"g_n2a"));
	cg_numHeadSkinsAllies.integer = atoi( Info_ValueForKey(info,"g_n1a"));
	cg_numHeadModelsAxis.integer = atoi( Info_ValueForKey(info,"g_n2x"));
	cg_numHeadSkinsAxis.integer = atoi( Info_ValueForKey(info,"g_n1x"));
*/
	camo = va( Info_ValueForKey( info, "g_camo_axis") );
	camo2 = va( Info_ValueForKey( info, "g_camo_allies") );
	Q_strncpyz( cg_camo_axis.string, camo, sizeof(cg_camo_axis.string) );
	Q_strncpyz( cg_camo_allies.string, camo2, sizeof(cg_camo_allies.string) );
	cg_nazi.integer = atoi( Info_ValueForKey( info, "g_nazi") );
	cg_AutoReloads.integer = atoi( Info_ValueForKey( info, "g_AutoReloads") );
...
*/
oidInfo_t *
CG_OIDInfoForEntityNum ( int num )
{
	int i;
...
CG_ParseWolfinfo ( void )
{
	...
	cgs.axis_strike = atoi( Info_ValueForKey( info, "g_axis_strike") );
	cgs.allied_strike = atoi( Info_ValueForKey( info, "g_allied_strike") );

	//	cg_atmosphere_speed.value = atoi ( Info_ValueForKey(  info, "g_atspeed" ) );
	//	cg_atmosphere_height.integer = atoi ( Info_ValueForKey(  info, "g_atheight" ) );
	//	cg_snow.integer = atoi ( Info_ValueForKey(  info, "g_snow" ) );
	//	cg_rain.integer = atoi ( Info_ValueForKey(  info, "g_rain" ) );
	...
	if ( old_gs != GS_PLAYING && cgs.gamestate == GS_PLAYING )
	{
		...
		{
			trap_S_StartLocalSound( cgs.media.countFight_axis, CHAN_ANNOUNCER );
		}
		else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_ALLIES )
		{
			trap_S_StartLocalSound( cgs.media.countFight_allies, CHAN_ANNOUNCER );
		}
		else
		{
...
void
CG_ParseServerVersionInfo ( const char *pszVersionInfo )
{
...
void
CG_ParseReinforcementTimes ( const char *pszReinfSeedString )
{
	...
	while ( o && *o )
	{
		...
		if ( n && *n )
		{
			...
			if ( o )
			{
				...
				timeOffset[o - t] = 0;
				o++;
				trap_R_RemapShader( cgs.gameShaderNames[atoi( originalShader)], cgs.gameShaderNames[atoi( newShader)],
									timeOffset );
			}
		}
		else
...
CG_ConfigStringModified ( void )
{
	...
	else if ( num >= CS_SKINS && num < CS_SKINS + MAX_CS_SKINS )
	{
		...
		if( !BG_FindCharacter( va("%s/%s", cg_camo_allies.string, str) ) ) {
			if( !BG_FindCharacter( va("%s/%s", cg_camo_axis.string, str) ) ) {
				if( !BG_FindCharacter( str ) ) {
					...
					if( !CG_RegisterCharacter( va("%s/%s", cg_camo_allies.string, str), cgs.gameCharacters[ num - CS_CHARACTERS ] ) ) {
						if( !CG_RegisterCharacter( va("%s/%s", cg_camo_axis.string, str), cgs.gameCharacters[ num - CS_CHARACTERS ] ) ) {
...
*/
static void
CG_AddToTeamChat ( const char *str, int clientnum )
{
	int		len;
	char	*p, *ls;
	int		lastcolor;
...
CG_ParseVoiceChats ( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats )
{
	...
	{
		trap_Print( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len,
					MAX_VOICEFILESIZE) );
	...
	while ( 1 )
	{
		...
		Com_sprintf( voiceChats[voiceChatList->numVoiceChats].id, sizeof(voiceChats[voiceChatList->numVoiceChats].id),
					 "%s", token );
		token = COM_ParseExt( p, qtrue );
		if ( Q_stricmp( token, "{") )
		{
...
*/
int
CG_GetVoiceChat ( voiceChatList_t *voiceChatList, const char *id, sfxHandle_t *snd, qhandle_t *sprite, char **chat )
{
	int i, rnd;
...
*/
void
CG_VoiceChatLocal ( int mode, qboolean voiceOnly, int clientNum, int color, const char *cmd, vec3_t origin )
{
	char				*chat;
	...
	if ( CG_GetVoiceChat( voiceChatList, cmd, &snd, &sprite, &chat) )
	{
		...
		if ( mode == SAY_TEAM || !cg_teamChatsOnly.integer )
		{
			...
			if ( mode == SAY_TEAM )
			{
				Com_sprintf( vchat.message, sizeof(vchat.message), "(%s)%c%c(%s): %c%c%s", ci->name, Q_COLOR_ESCAPE,
							 COLOR_YELLOW, loc, Q_COLOR_ESCAPE, color, CG_TranslateString( chat) );
			}
			...
			{
				Com_sprintf( vchat.message, sizeof(vchat.message), "%s%c%c: %c%c%s", ci->name, Q_COLOR_ESCAPE,
							 COLOR_YELLOW, Q_COLOR_ESCAPE, color, CG_TranslateString( chat) );
			}
...
CG_VoiceChat ( int mode )
{
	...
	if ( cg_noTaunt.integer != 0 )
	{
		if
		(
			!strcmp( cmd, VOICECHAT_KILLINSULT) ||
			!strcmp( cmd, VOICECHAT_TAUNT) ||
...
*/
const char *
CG_LocalizeServerCommand ( const char *buf )
{
...
/* */
void
CG_topshotsParse_cmd ( qboolean doBest )
{
	int				iArg = 1;
	...
	while ( iWeap )
	{
		...
		iArg++;
		if ( ts->cWeapons < WS_MAX * 2 && (kills > 0 || hits > 0 || atts > 0) )
		{
			BG_cleanName( cgs.clientinfo[cnum].name, name, 17, qfalse );
			Q_strncpyz( ts->strWS[ts->cWeapons++],
						va( "%-12s %5.1f %4d/%-4d %5d  %s", aWeaponInfo[iWeap - 1].pszName, acc, hits, atts, kills, name),
						sizeof(ts->strWS[0]) );
		}
...
// Cached stats
void
CG_parseWeaponStatsGS_cmd ( void )
{
	clientInfo_t	*ci;
	...
	if ( weaponMask != 0 )
	{
			...
			if ( weaponMask & (1 << i) )
			{
				...
				if ( nShots > 0 || nHits > 0 )
				{
					Q_strcat( strName, sizeof(strName), va( "%5.1f %4d/%-4d ",
							  ((nShots == 0) ? 0.0 : (float) (nHits * 100.0 / (float) nShots)), nHits, nShots) );
				}
				else
				{
				...
				Q_strncpyz( gs->strWS[gs->cWeapons++], va( "%s%5d %6d%s", strName, nKills, nDeaths,
		...
		if ( gs->fHasStats )
		{
			...
			Q_strncpyz( gs->strExtra[0], va( "Damage Given: %-6d  Team Damage: %d", dmg_given, team_dmg),
			...
			if ( cgs.gametype == GT_WOLF_CAMPAIGN || cgs.gametype == GT_SUPREMACY_CAMPAIGN )
...
#endif //__SUPREMACY__
				{
					...
					Q_strncpyz( gs->strSkillz[gs->cSkills++],
								va( "%-15s %3d %-15s %6d", skillNames[i], ci->skill[i], str, ci->medals[i]),
...
void
CG_parseWeaponStats_cmd ( void (txt_dump) (char *) )
{
	clientInfo_t	*ci;
...
/* */
void
CG_parseBestShotsStats_cmd ( qboolean doTop, void (txt_dump) (char *) )
{
	int			iArg = 1;
	qboolean	fFull = ( txt_dump != CG_printWindow );
	int			iWeap = atoi( CG_Argv( iArg++) );
...
/* */
void
CG_parseTopShotsStats_cmd ( qboolean doTop, void (txt_dump) (char *) )
{
	int i, iArg = 1;
	...
	for ( i = 0; i < cClients; i++ )
	{
		...
		float		acc = ( atts > 0 ) ? (float) ( hits * 100 ) / (float) atts : 0.0;
		const char	*color =
			( ((doTop) ? acc : ((float) wBestAcc) + 0.999) >= ((doTop) ? wBestAcc : acc) ) ? "^3" : "^7";
		char		name[32];
		BG_cleanName( cgs.clientinfo[cnum].name, name, 30, qfalse );
...
CG_dumpStats ( void )
{
	...
	{
		fDoScores = qtrue;
		cgs.dumpStatsFileName = va( "stats/%d.%02d.%02d/%02d%02d%02d.txt", 1900 + ct.tm_year, ct.tm_mon + 1, ct.tm_mday,
									ct.tm_hour, ct.tm_min, ct.tm_sec );
	}
...
CG_ServerCommand ( void )
{
	...
	if ( !Q_stricmp( cmd, "cp") )
	{
		...
		{
			CG_CenterPrint( CG_LocalizeServerCommand( CG_Argv( 1)), SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.20),
	...
	if ( !Q_stricmp( cmd, "chat") )
	{
		...
		trap_S_StartLocalSound( cgs.media.chatsound, CHAN_AUTO );
		CG_Printf( "%s\n", text );
		return;
	}
	...
	if ( !strcmp( cmd, "rstn") )
	{
		char	*station = va( "%s", CG_Argv( 1) );
		trap_Cvar_Set( "s_radioStation", va( "%s", station) );
	}

	if ( !strcmp( cmd, "snd") )
	{												// UQ addeed: For ingame announcements...
		char	*sound = va( "%s", CG_Argv( 2) );
		int		teamNum = atoi( CG_Argv( 1) );
		if ( (cg.snap->ps.pm_flags & PMF_FOLLOW) )
		{											// Viewing (following) a bot/player...
			if ( teamNum == cgs.clientinfo[cg.snap->ps.clientNum].team )
			{										// Only sounds for our team.. TEAM_SPECTATOR is used for global sounds...
				trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_VOICE, trap_S_RegisterSound( sound, qfalse) );
			}
		}
		else if ( cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_SPECTATOR && teamNum == TEAM_ALLIES )
		{											// Spectators hear Allied sounds as well...
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_VOICE, trap_S_RegisterSound( sound, qfalse) );
		}
		else if ( cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_FREE && teamNum == TEAM_ALLIES )
		{											// Spectators hear Allied sounds as well...
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_VOICE, trap_S_RegisterSound( sound, qfalse) );
		}
		else if ( teamNum == cgs.clientinfo[cg.snap->ps.clientNum].team )
		{											// Only sounds for our team..
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_VOICE, trap_S_RegisterSound( sound, qfalse) );
		}
		else if ( teamNum == TEAM_FREE )
		{											// TEAM_FREE is used for global sounds...
			trap_S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_VOICE, trap_S_RegisterSound( sound, qfalse) );
		}
	...
	if ( !Q_stricmp( cmd, "spawnserver") )
	{
		...
		CG_PriorityCenterPrint( va( "%s", CG_TranslateString( "^3Server Restarting")),
								SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH, 999999 );
