...
*/
void
CG_SetInitialCamera ( const char *name, qboolean startBlack )
{
...
*/
void
CG_StartCamera ( const char *name, qboolean startBlack )
{
	char	lname[MAX_QPATH];

#ifdef _DEBUG
	CG_Printf("CG_StartCamera\n");
#endif
...
/* */
void
CG_QuickFireteamMessage_f ( void )
{
	if ( cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR )
	{
		return;
...
/*
===============
CG_ToggleThirdperson

===============
*/
static void
CG_ToggleThirdperson_f ( void )
{
	trap_Cvar_Set( "cg_3rdPerson", (cg_3rdPerson.integer > 0) ? "0" : "1" );
}


/* */
static void
CG_SetWeaponCrosshair_f ( void )
{
	char	crosshair[64];
...
/* */
static void
CG_ToggleAutomap_f ( void )
{
	cgs.autoMapOff = !cgs.autoMapOff;
}

// OSP
const char	*aMonths[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };


/* */
void
CG_currentTime_f ( void )
{
	qtime_t ct;
	trap_RealTime( &ct );
...
static consoleCommand_t commands[] =
{
	...
	{ "thirdperson", CG_ToggleThirdperson_f },
...
CG_InitConsoleCommands ( void )
{
	...
	trap_RemoveCommand( "-lookdown" );
	trap_AddCommand( "thirdperson" );
	trap_AddCommand( "dropweapon" );
	trap_AddCommand( "dropobj" );

	//	trap_AddCommand ("holster");
	//	trap_AddCommand ("forcecvar");

	// Kamikazee: Unique1's AIMod commands
	trap_AddCommand( "autowaypoint" );
	trap_AddCommand( "aw" );
