...
/* */
float
CG_ShadowPlane ( centity_t *cent )
{
	vec3_t	end;
	trace_t trace;
	if ( cg_shadowPlanes.integer == 0 )
	{
		return ( qfalse );
	}
...
*/
void
CG_PositionRotatedEntityOnTag ( refEntity_t *entity, const refEntity_t *parent, const char *tagName )
{
	int				i;
	orientation_t	lerped;
	vec3_t			tempAxis[3];
...
*/
void
CG_AddLightstyle ( centity_t *cent )
{
	float	lightval;
	int		cl;
	int		r, g, b;
	...
	if ( cent->currentState.angles[0] || cent->currentState.angles[1] || cent->currentState.angles[2] )
	{
		...
		trap_R_AddLightToScene( normal, 256, lightval, (float) r / 255.0f, (float) r / 255.0f, (float) r / 255.0f, 0,
								REF_DIRECTED_DLIGHT );
	}
	...
	{
		trap_R_AddLightToScene( cent->lerpOrigin, 256, lightval, (float) r / 255.0f, (float) g / 255.0f,
								(float) b / 255.0f, 0, 0 );
	}
}
...
CG_EntityEffects ( centity_t *cent )
{
...
#endif //__NPC__
	&& cent->currentState.eType != ET_PLAYER && cent->currentState.eType != ET_CORPSE )
	{
		int cl;
		int i, r, g, b;
		...
		else
		{
			...
			b = ( cl >> 16 ) & 255;
			i = ( (cl >> 24) & 255 ) * 4;
			trap_R_AddLightToScene( cent->lerpOrigin, i, 1.0, (float) r / 255.0f, (float) g / 255.0f, (float) b / 255.0f,
									0, 0 );
		}
	}
	...
	else if ( cent->currentState.eFlags & EF_SMOKING )
	{
		...
		if ( cent->lastTrailTime < cg.time )
		{
			...
			//			dir[2] = 10 + rnd * 30;
			// jpw
			CG_SmokePuff( cent->lerpOrigin, dir, 15 + (random() * 10), 0.3 + rnd, 0.3 + rnd, 0.3 + rnd, 0.4,
						  1500 + (rand() % 500), cg.time, cg.time + 500, 0, cgs.media.smokePuffShader );
		}
	}

	// dhm - end
	// JPW NERVE same thing but for smoking barrels instead of nasty server-side effect from single player
	else if ( cent->currentState.eFlags & EF_SMOKINGBLACK ) // EF_SMOKINGBLACK
	{
		float	rnd = random();
		if ( cent->lastTrailTime < cg.time )
		{
...
CG_General ( centity_t *cent )
{
	...
	if
	(
		cent->currentState.dl_intensity == cg.snap->ps.clientNum &&
		cent->currentState.eType == ET_MG42_BARREL &&
	...
	if ( cent->currentState.eFlags & EF_TAGCONNECT && cg_shadowPlanes.integer )
	{
		if ( cg_entities[cent->tagParent].currentState.eType == ET_MOVER )
		{
			ent.shadowPlane = CG_ShadowPlane( cent );
			ent.customShader = cgs.media.shadowShader;
			trap_R_AddRefEntityToScene( &ent );
		}
	}
...
void	CG_tank_slope ( centity_t *cent, vec3_t slopeangles, int yaw );
...
*/
	...
	CG_tank_slope( cent, slope, cent->lerpAngles[1] );
	angles[0] = slope[0];
	angles[2] = slope[2];
...
/* */
qboolean
CG_PlayerSeesItem ( playerState_t *ps, entityState_t *item, int atTime, int itemType )
{
...
CG_Item ( centity_t *cent )
{
	...
	if ( !(cent->usehighlightOrigin) )
	{
		...
		{	// adjust midpoint by offset and orientation
			cent->highlightOrigin[i] += offset[0] *
				ent.axis[0][i] +
				offset[1] *
				ent.axis[1][i] +
				offset[2] *
...
*/
static void
CG_DrawMineMarkerFlag ( centity_t *cent, refEntity_t *ent, const weaponInfo_t *weapon )
{
	entityState_t	*s1;
	s1 = &cent->currentState;
...
CG_Missile ( centity_t *cent )
{
	...
	if
	(
		cent->currentState.eType == ET_FP_PARTS ||
	...
	if ( weapon->missileDlight )
	{
		...
		trap_R_AddLightToScene( cent->lerpOrigin, weapon->missileDlight, 1.0, weapon->missileDlightColor[0],
								weapon->missileDlightColor[1], weapon->missileDlightColor[2], 0, 0 );
	}
	...
	{
		team_t	missileTeam = cent->currentState.weapon == WP_LANDMINE ? cent->currentState.teamNum %
			4 : cent->currentState.teamNum;
		ent.hModel = weapon->missileModel;
			...
			(
				(
					cgs.clientinfo[cg.snap->ps.clientNum].team !=
						(!cent->currentState.otherEntityNum2 ? TEAM_ALLIES : TEAM_AXIS)
				)
			)
			{
...
static animation_t	multi_flagpoleAnims[] =
{
	...
	// (no flags, idle)
	{ 0, "", 0, 15, 0, 1000 / 15, 1000 / 15 },

	// (axis flag rising)
	{ 0, "", 490, 15, 0, 1000 / 15, 1000 / 15 },
	...
	{ 0, "", 235, 15, 0, 1000 / 15, 1000 / 15 },

	// (axis switching to american)
	{ 0, "", 470, 15, 0, 1000 / 15, 1000 / 15 },

	// (american switching to axis)
	{ 0, "", 510, 15, 0, 1000 / 15, 1000 / 15 },

	// (axis flag falling)
	{ 0, "", 530, 15, 0, 1000 / 15, 1000 / 15 },

	// (american flag falling)
	{ 0, "", 0, 190, 0, 1000 / 15, 1000 / 15 }	// (CTF idle)
};
...
*/
static void
CG_TrapSetAnim ( centity_t *cent, lerpFrame_t *lf, int newAnim )
{
...
*/
static void
CG_SpotlightEfx ( centity_t *cent )
{
	...
	CG_SpotlightRTCW( cent, color, cent->currentState.origin, normalized_direction, 999, 2048, 10, fov, 0 );
}
...
*/
static void
CG_Constructible ( centity_t *cent )
{
	refEntity_t		ent;
	entityState_t	*s1;
	s1 = &cent->currentState;
...
CG_Mover ( centity_t *cent )
{
	...
	if (cent->currentState.density == 7 || cent->currentState.density == 8) 
	{
		...
		propeller.hModel = cgs.media.hPropeller;
		...
		if (cent->currentState.density == 8)
		{
			...
			trap_R_AddLightToScene( flash.origin, 100 + (rand()&31), 200 + (rand()&31),1.0, 0.6, 0.23, 0, 0 );
...
/*		else if (cent->currentState.frame == 0)
		{
			if (!siren_looping)
			{
				int i;
				...
				for (i = 0; i < MAX_GENTITIES; i++)
				{
					...
					if (cent->currentState.eType == ET_SPEAKER)
					{
						cent->currentState.loopSound = cgs.media.siren_sound;
					}
				}
...
/* */
void
CG_Mover_PostProcess ( centity_t *cent )
{
	...
	/*if (cent->currentState.eType == ET_VEHICLE)
	{
		...
		CG_AttachBitsToTank( cent, &mg42base, &mg42upper, &mg42gun, &player, &flash,
						 angles, "tag_player", cent->currentState.density & 8 ? qtrue : qfalse );
		...
		CG_Printf("Tank bits rendered (ent %i).\n", cent->currentState.number);
		return;
	}
	...
	(
		...
		cg_entities[cg.snap->ps.clientNum].tagParent == cent->currentState.effect3Time
	)
	{
	...
	if ( i != MAX_CLIENTS )
	{
		...
		{
			angles = cg.predictedPlayerState.viewangles;	//cg_entities[i].lerpAngles;// cg.predictedPlayerState.viewangles;
		}
	}
	else
	{
	...
	CG_AttachBitsToTank( &cg_entities[cent->currentState.effect3Time], &mg42base, &mg42upper, &mg42gun, &player, &flash,
						 angles, "tag_player", cent->currentState.density & 8 ? qtrue : qfalse );
...
CG_Prop ( centity_t *cent )
{
	...
	else
	{
		...
		if ( cent->currentState.density )
		{
			...
			ent.backlerp = 1 - cg.frameInterpolation;
			ent.renderfx = RF_DEPTHHACK | RF_FIRST_PERSON | RF_NOSHADOW;
...
{
	{ { "tag_ammo01", "tag_ammo02", "tag_ammo03", "tag_ammo04", "tag_ammo05", "tag_ammo06",
			...
			"tag_obj1",
			"tag_obj1",*/
		}, { "models/multiplayer/supplies/ammobox_wm.md3", "models/multiplayer/supplies/ammobox_wm.md3",
				"models/multiplayer/supplies/ammobox_wm.md3", "models/multiplayer/supplies/ammobox_wm.md3",
					"models/multiplayer/supplies/ammobox_wm.md3",
						"models/multiplayer/supplies/ammobox_wm.md3", }, { 0, 0, 0, 0, 0, 0 },
						"models/mapobjects/supplystands/stand_ammo.md3",

		//		"models/mapobjects/blitz_sd/blitzbody.md3",
		0, },
	{ { "tag_Medikit_01", "tag_Medikit_02", "tag_Medikit_03", "tag_Medikit_04", "tag_Medikit_05", "tag_Medikit_06", }, {
				"models/multiplayer/supplies/healthbox_wm.md3", "models/multiplayer/supplies/healthbox_wm.md3",
					"models/multiplayer/supplies/healthbox_wm.md3", "models/multiplayer/supplies/healthbox_wm.md3",
						"models/multiplayer/supplies/healthbox_wm.md3",
							"models/multiplayer/supplies/healthbox_wm.md3", }, { 0, 0, 0, 0, 0, 0 },
							"models/mapobjects/supplystands/stand_health.md3", 0, },
};
...
*/
void
CG_Cabinet ( centity_t *cent, cabinetType_t type )
{
	refEntity_t cabinet;
	refEntity_t mini_me;
	int			i, cnt;
...
*/
void
CG_AdjustPositionForMover ( const vec3_t in, int moverNum, int fromTime, int toTime, vec3_t out, vec3_t outDeltaAngles )
{
	centity_t	*cent;
...
CG_CalcEntityLerpPositions ( centity_t *cent )
{
	...
	{										// UQ1: Need to interpolate NPCs as well!
		CG_InterpolateEntityPosition( cent );
		return;
	}
	...
	// player state
	if ( cent != &cg.predictedPlayerEntity && (!cg.showGameView || cg_bigLimbo.integer == 0) )
	{
...
// ET Ver..
extern void CG_Explodef(vec3_t origin, vec3_t dir, int mass, int type, qhandle_t sound, int forceLowGrav, qhandle_t shader);
static void CG_Efx( centity_t *cent )
{
	if (cent->currentState.eType == ET_SPOTLIGHT_EF) {
...
//				trap_S_StartSound (NULL, cent->currentState.number, CHAN_AUTO, cgs.media.sparkSounds[0] );
			...
			if(!cg_paused.integer) {	// don't add while paused
				if(!(rand()%3))
					CG_ParticleImpactSmokePuffExtendedRTCW(cgs.media.smokeParticleShader, cent->currentState.origin, tv(0,0,1), 8, 1000, 8, 20, 20, 0.25f);
			}
		}
		...
		CG_SpotlightRTCW( cent, color, cent->currentState.origin, normalized_direction, 999, 4096, 10, fov, 0);
...
*/
#ifdef __SUPREMACY__
extern void CG_Flag ( centity_t *cent );	// UQ1: cg_om_flagsys.c
#endif //__SUPREMACY__

/* */
static void
CG_ProcessEntity ( centity_t *cent )
{
	switch ( cent->currentState.eType )
	{
		...
		case ET_CORPSE:
		case ET_PLAYER:
			if ( cg.showGameView && cg.filtercams && cg_bigLimbo.integer )
			{
				break;
			}
		...
		case ET_EF_SPOTLIGHT:
			...
			break;
		case ET_SPOTLIGHT_EF: // UQ1: Added for SP..
			CG_Efx( cent );
			break;
		case ET_EXPLOSIVE:
			CG_Explosive( cent );
...
/* */
qboolean
CG_AddLinkedEntity ( centity_t *cent, qboolean ignoreframe, int atTime )
{
	entityState_t	*s1;
...
*/
qboolean
CG_AddEntityToTag ( centity_t *cent )
{
	centity_t		*centParent;
	entityState_t	*sParent;
	refEntity_t		ent;
	...
	if ( cent->currentState.eType != ET_PLAYER )
	{
		...
		{	// face our angles exactly
			BG_EvaluateTrajectory( &cent->currentState.apos, cg.time, cent->lerpAngles, qtrue,
...
*/
qboolean
CG_AddCEntity_Filter ( centity_t *cent )
{
	if ( cent->processedFrame == cg.clientFrame 
		&& !cg.subscene
		&& cg.mvTotalClients < 2 )
...
CG_AddPacketEntities ( void )
{
	...
	for ( num = 0; num < cg.snap->numEntities; num++ )
	{
		...
		else if ( cg_entities[cg.snap->entities[num].number].currentState.eType == ET_VEHICLE )
		{
			CG_Mover_PostProcess( &cg_entities[cg.snap->entities[num].number] );
		}
#ifdef __NPC__
		else if ( cg_entities[cg.snap->entities[num].number].currentState.eType == ET_NPC )
		{
			CG_NPC( &cg_entities[cg.snap->entities[num].number] );
		}
#endif //__NPC__
	}
...
/* */
void
CGRefEntityToTag ( refEntity_t *ent, tag_t *tag )
{
	int i;
...
/* */
void
CGTagToRefEntity ( refEntity_t *ent, tag_t *tag )
{
	int i;
