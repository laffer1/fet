...
/* */
void
trap_Cvar_LatchedVariableStringBuffer ( const char *var_name, char *buffer, int bufsize )
{
...
/* */
int
trap_FS_Delete ( const char *filename )
{
...
)
{
	return
		(
			syscall( CG_CM_MARKFRAGMENTS, numPoints, points, projection, maxPoints, pointBuffer, maxFragments,
					 fragmentBuffer)
		);
}
...
/* */
void
trap_S_StartSoundVControl ( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int volume )
{
...
//----(SA)	added
void
trap_S_StartSoundEx ( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int flags )
{
...
//----(SA)	end
void
trap_S_StartSoundExVControl ( vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx, int flags, int volume )
{
...
/* */
void
trap_S_StartLocalSoundVControll ( sfxHandle_t sfx, int channelNum, int volume )
{
...
/* */
void
trap_S_ClearLoopingSounds ( void )
{
...
/* */
void
trap_S_ClearSounds ( qboolean killmusic )
{
...
/* */
void
trap_S_StopStreamingSound ( int entityNum )
{
...
int
trap_S_GetVoiceAmplitude ( int entityNum )
{
...
int
trap_S_GetSoundLength ( sfxHandle_t sfx )
{
...
int
trap_S_GetCurrentSoundTime ( void )
{
...
/* */
void
trap_S_StartBackgroundTrack ( const char *intro, const char *loop, int fadeupTime )
{
#ifdef __MUSIC_ENGINE__
	CG_SoundEngineStartMusicTrack(intro, qtrue );
#else
...
/* */
void
trap_S_FadeBackgroundTrack ( float targetvol, int time, int num )
...
/* */
void
trap_S_FadeAllSound ( float targetvol, int time, qboolean stopsounds )
{
...
//----(SA)	added
qboolean
trap_R_GetSkinModel ( qhandle_t skinid, const char *type, char *name )
{
...
/* */
qhandle_t
trap_R_GetShaderFromModel ( qhandle_t modelid, int surfnum, int withlightmap )
{
...
/* */
void
trap_R_AddPolyBufferToScene ( polyBuffer_t *pPolyBuffer )
{
...
//%	}
extern void CG_AddFlashLighting ( vec3_t origin, vec4_t rgb );
	...
	)
{
	...
	if ( CG_FlashBlend.integer && !(flags & REF_JUNIOR_DLIGHT)  && !(flags & REF_DIRECTED_DLIGHT) ) {  // quake style cvar name :)
		vec4_t		rgb;
		trace_t		tr;
		qboolean	vis = qfalse;

		CG_Trace( &tr, cg.refdef.vieworg, NULL, NULL, org, cg.clientNum, MASK_ALL&~CONTENTS_BODY ); // jaquboss, ignore players
		if ( VectorDistance( tr.endpos, org) < 32   )
		{	// Close enough!
			rgb[0] = r;
			rgb[1] = g;
			rgb[2] = b;
			rgb[3] = intensity;
			CG_AddFlashLighting( org, rgb );

			if ( tr.fraction == 1 )  // jaquboss, check added for visibility
				vis = qtrue;

			trap_R_AddCoronaToScene( org, r, g, b, radius * 0.04, hShader, vis );
		}
	}

	syscall( CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT( radius ), PASSFLOAT( intensity ), PASSFLOAT( r),
	...
	)
{
	...
	syscall( CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT( radius), PASSFLOAT( intensity), PASSFLOAT( r), PASSFLOAT( g),
			 PASSFLOAT( b), hShader, flags );
}


//----(SA)	
void
trap_R_AddCoronaToScene ( const vec3_t org, float r, float g, float b, float scale, int id, qboolean visible )
{
...
//----(SA)
void
trap_R_SetFog ( int fogvar, int var1, int var2, float r, float g, float b, float density )
{
...
//----(SA)	
void
trap_R_SetGlobalFog ( qboolean restore, int duration, float r, float g, float b, float depthForOpaque )
{
	syscall( CG_R_SETGLOBALFOG, restore, duration, PASSFLOAT( r), PASSFLOAT( g), PASSFLOAT( b),
			 PASSFLOAT( depthForOpaque) );
}
...
{
	syscall( CG_R_DRAWROTATEDPIC, PASSFLOAT( x), PASSFLOAT( y), PASSFLOAT( w), PASSFLOAT( h), PASSFLOAT( s1),
			 PASSFLOAT( t1), PASSFLOAT( s2), PASSFLOAT( t2), hShader, PASSFLOAT( angle) );
}
...
{
	syscall( CG_R_DRAWSTRETCHPIC_GRADIENT, PASSFLOAT( x), PASSFLOAT( y), PASSFLOAT( w), PASSFLOAT( h), PASSFLOAT( s1),
			 PASSFLOAT( t1), PASSFLOAT( s2), PASSFLOAT( t2), hShader, gradientColor, gradientType );
}


/* */
void
trap_R_Add2dPolys ( polyVert_t *verts, int numverts, qhandle_t hShader )
{
...
/* */
int
trap_R_LerpTag ( orientation_t *tag, const refEntity_t *refent, const char *tagName, int startIndex )
{
...
/* */
void
trap_SetUserCmdValue ( int stateValue, int flags, float sensitivityScale, int mpIdentClient )
{
...
/* */
void
trap_SetClientLerpOrigin ( float x, float y, float z )
{
...
/* */
qboolean
trap_loadCamera ( int camNum, const char *name )
{
...
/* */
qboolean
trap_getCameraInfo ( int camNum, int time, vec3_t *origin, vec3_t *angles, float *fov )
{
...
void
trap_Key_KeysForBinding ( const char *binding, int *key1, int *key2 )
{
...
/* */
int
trap_PC_UnReadToken ( int handle )
{
...
/* */
void
trap_UI_ClosePopup ( const char *arg0 )
{
...
void
trap_TranslateString ( const char *string, char *buf )
{
...
{
	sfxHandle_t					snd;
	DEBUG_REGISTERPROFILE_INIT	CG_DrawInformation ( qtrue );
	...
	DEBUG_REGISTERPROFILE_EXEC( "trap_S_RegisterSound", sample ) trap_PumpEventLoop();
	return ( snd );
}
...
trap_R_RegisterModel ( const char *name )
{
	...
	DEBUG_REGISTERPROFILE_EXEC( "trap_R_RegisterModel", name ) trap_PumpEventLoop();
	return ( handle );
}
...
trap_R_RegisterSkin ( const char *name )
{
	...
	DEBUG_REGISTERPROFILE_EXEC( "trap_R_RegisterSkin", name ) trap_PumpEventLoop();
	return ( handle );
}
...
trap_R_RegisterShader ( const char *name )
{
	...
	DEBUG_REGISTERPROFILE_EXEC( "trap_R_RegisterShader", name ) trap_PumpEventLoop();
	return ( handle );
}
...
trap_R_RegisterFont ( const char *fontName, int pointSize, fontInfo_t *font )
{
	DEBUG_REGISTERPROFILE_INIT	CG_DrawInformation ( qtrue );
	syscall( CG_R_REGISTERFONT, fontName, pointSize, font );
	DEBUG_REGISTERPROFILE_EXEC( "trap_R_RegisterFont", fontName ) trap_PumpEventLoop();
}
...
trap_CM_LoadMap ( const char *mapname )
{
	DEBUG_REGISTERPROFILE_INIT	CG_DrawInformation ( qtrue );
	syscall( CG_CM_LOADMAP, mapname );
	DEBUG_REGISTERPROFILE_EXEC( "trap_CM_LoadMap", mapname ) trap_PumpEventLoop();
}
...
trap_R_LoadWorldMap ( const char *mapname )
{
	DEBUG_REGISTERPROFILE_INIT	CG_DrawInformation ( qtrue );
	syscall( CG_R_LOADWORLDMAP, mapname );
	DEBUG_REGISTERPROFILE_EXEC( "trap_R_LoadWorldMap", mapname ) trap_PumpEventLoop();
}
...
/* */
void
trap_GetHunkData ( int *hunkused, int *hunkexpected )
{
...
void
trap_SendMessage ( char *buf, int buflen )
{
...
qboolean
trap_R_LoadDynamicShader ( const char *shadername, const char *shadertext )
{
...
void
trap_R_RenderToTexture ( int textureid, int x, int y, int w, int h )
{
...
/* */
int
trap_R_GetTextureId ( const char *name )
{
