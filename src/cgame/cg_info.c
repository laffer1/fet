...
*/
void
CG_DrawInformation ( qboolean forcerefresh )
{
...
void
CG_DemoClick ( int key, qboolean down )
{
	int milli = trap_Milliseconds();
	...
	switch ( key )
	{
		...
		case K_ENTER:
			if ( !down )
			{
				trap_Cvar_Set( "cg_3rdperson", ((cg_3rdPerson.integer == 0) ? "1" : "0") );
			}
		...
		case K_RIGHTARROW:
			if ( milli > cgs.thirdpersonUpdate )
			{
				float	angle = cg_3rdPersonAngle.value - DEMO_ANGLEDELTA;
				cgs.thirdpersonUpdate = milli + DEMO_THIRDPERSONUPDATE;
				if ( angle < 0 )
				{
					angle += 360.0f;
				...
				trap_Cvar_Set( "cg_3rdPersonAngle", va( "%f", angle) );
			}
...
CG_GameStatsDraw ( void )
{
	...
	else
	{
		...
		1 +													// Stats + extra
		tSpacing * ( (gs->cWeapons > 0) ? gs->cWeapons : 1 ) + tSpacing * ( (gs->fHasStats) ? 3 : 0 ) +
			(
				(cgs.gametype == GT_WOLF_LMS) ? 0 :
					(
						...
						4 +
						2 *
						tSpacing +							// Skill columns
						1 +									// Skillz
						tSpacing *
						((gs->cSkills > 0) ? gs->cSkills : 1)
					)
			) + 2;
...
CG_TopShotsDraw ( void )
{
	...
	else
	{
		...
		{
			y += tSpacing;
			CG_Text_Paint_Ext( x, y, tScale, tScale, tColor, "No qualifying weapon info available.", 0.0f, 0, tStyle,
							   tFont );
		}
		else
		{
...
/* */
char *
CG_getBindKeyName ( const char *cmd, char *buf, int len )
{
	int j;
	for ( j = 0; j < 256; j++ )
	{
