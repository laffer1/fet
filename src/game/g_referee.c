...
//
void
G_refAddTank_cmd ( gentity_t *ent )
{
	if ( g_refTanks.integer )
	{
...
/* */
void
G_refAddPanzer_cmd ( gentity_t *ent )
{
	if ( g_refTanks.integer )
	{
		gentity_t	*self = G_Spawn();
...
//
qboolean
G_refCommandCheck ( gentity_t *ent, char *cmd )
{
	...
	else if ( !Q_stricmp( cmd, "addtank") )
	{
		G_refAddTank_cmd( ent );
	}
	else if ( !Q_stricmp( cmd, "addpanzer") )
	{
		G_refAddPanzer_cmd( ent );
	}
	else
	{
		return ( qfalse );
...
*/
		...
		CP( "print \"^5lock remove	remove^7 <pid> ^5unpause unmute ^7<pid>\n\"" );
...
void
G_ref_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
void
G_refAllReady_cmd ( gentity_t *ent )
{
	int			i;
	gclient_t	*cl;
...
void
G_refLockTeams_cmd ( gentity_t *ent, qboolean fLock )
{
	char	*status;
...
void
G_refPause_cmd ( gentity_t *ent, qboolean fPause )
{
	char	*status[2] = { "^5UN", "^1" };
...
void
G_refPlayerPut_cmd ( gentity_t *ent, int team_id )
{
	int			pid;
	char		arg[MAX_TOKEN_CHARS];
	gentity_t	*player;
...
void
G_refSpeclockTeams_cmd ( gentity_t *ent, qboolean fLock )
{
	char	*status;
...
/* */
void
G_refWarning_cmd ( gentity_t *ent )
{
	char	cmd[MAX_TOKEN_CHARS];
	...
	if ( kicknum != MAX_CLIENTS )
	{
		if
		(
			level.clients[kicknum].sess.referee == RL_NONE ||
...
// (Un)Mutes a player
void
G_refMute_cmd ( gentity_t *ent, qboolean mute )
{
	int			pid;
	char		arg[MAX_TOKEN_CHARS];
	gentity_t	*player;
...
//
int
G_refClientnumForName ( gentity_t *ent, const char *name )
{
...
/* */
void
G_refPrintf ( gentity_t *ent, const char *fmt, ... )
{
	va_list argptr;
	char	text[1024];
	va_start( argptr, fmt );
