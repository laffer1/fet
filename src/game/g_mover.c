...
char *hintStrings[HINT_NUM_HINTS] =
{
	...
	// END Mad Doc - TDF
	"HINT_AAGUN",
	"HINT_BROWNINGHINT_REALREVIVE", // HINT_BAD_USER
};
...
*/
gentity_t *
G_TestEntityPosition ( gentity_t *ent, qboolean tankPush )
{
	trace_t tr;
	int		mask;
	if ( ent->clipmask )
	...
	if ( ent->client )
	{
		...
		{												// for bodies clipping
			vec3_t	deadmaxs;
			VectorCopy( ent->r.maxs, deadmaxs );
			deadmaxs[2] = ent->client->ps.crouchMaxZ;
			trap_TraceCapsule( &tr, ent->client->ps.origin, ent->r.mins, deadmaxs, ent->client->ps.origin,
							   ent->s.number, mask );
		}
		else
		{
			trap_TraceCapsule( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, ent->client->ps.origin,
							   ent->s.number, mask );
		}

		if ( !tr.startsolid && ent->client->ps.eFlags & EF_PRONE || tankPush )
		{
	...
	{
		trap_TraceCapsule( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ent->s.number,
						   mask );

		// Will need to do this some other way... NPCs dont actually use NPC_client structure... // CHECKME: They do now... Unique1

		/*		if( !tr.startsolid && ent->NPC_client->ps.eFlags & EF_PRONE )
		{
			vec3_t org, flatforward, point;
			...
			//org[2] = ent->client->ps.origin[2] + 12;	// 12 units to play with
			org[2] = ent->r.currentOrigin[2] + 24.f;	// 12 units to play with
...
*/
void
G_TestEntityDropToFloor ( gentity_t *ent, float maxdrop )
{
	trace_t tr;
	int		mask;
	vec3_t	endpos;
...
*/
void
G_TestEntityMoveTowardsPos ( gentity_t *ent, vec3_t pos )
{
	trace_t tr;
	int		mask;
	if ( ent->clipmask )
...
#define JITTER_MAX	( check->r.maxs[0] / 2.0 )
	...
	if ( (pusher->localEFlags & LEF_MOVER_STOP) && check->s.groundEntityNum != pusher->s.number )
	{

		//pusher->localEFlags |= LEF_MOVER_BLOCKED;
		return ( qfalse );
	}
	...
	block = G_TestEntityPosition( check, (qboolean) pusher->s.eFlags & EF_TANK );
	if ( !block )
	{
	...
	if ( pusher->client && pusher->client->ps.eFlags & EF_TANK )
	{	// do not try to do anything else if we cant be pushed back .. will improve crushing
		return ( qfalse );
	}
...
G_MoverPush ( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle )
{
	...
	*obstacle = NULL;
	...
	for ( e = 0; e < listedEntities; e++ )
	{
		...
		if ( pusher->client && pusher->client->ps.eFlags & EF_TANK && pusher == check )
		{						// do not push ourselfs
			continue;
		}
		...
		if ( check->s.groundEntityNum != pusher->s.number )
		{
			...
			if ( G_TestEntityPosition( check, (qboolean) pusher->s.eFlags & EF_TANK) != pusher )
			{
				continue;
			}
		}
...
G_MoverTeam ( gentity_t *ent )
{
	...
	for ( part = ent; part; part = part->teamchain )
	{
		...
		//if (part->localEFlags == LEF_MOVER_STOP)
		//	part->localEFlags &= ~LEF_MOVER_BLOCKED;
		if ( part->localEFlags == LEF_MOVER_STOP )
		{
			part->localEFlags &= ~LEF_MOVER_BLOCKED;
		}
...
*/
void
MatchTeamReverseAngleOnSlaves ( gentity_t *teamLeader, int moverState, int time )
{
	gentity_t	*slave;
	for ( slave = teamLeader; slave; slave = slave->teamchain )
	{
...
*/
void
ReturnToPos1Rotate ( gentity_t *ent )
{
...
*/
qboolean
IsBinaryMoverBlocked ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	vec3_t		dir, angles;
	vec3_t		pos;
	vec3_t		vec;
...
*/
void
Reached_TrinaryMover ( gentity_t *ent )
{
...
*/
void
Use_TrinaryMover ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	int			total;
	int			partial;
...
Use_BinaryMover ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	if ( level.time <= 4000 )
	{											// hack.  don't play door sounds if in the first /four/ seconds of game (FIXME: TOD-O: THIS IS STILL A HACK)
		nosound = qtrue;
	}
...
/* */
void
Blocked_DoorRotate ( gentity_t *ent, gentity_t *other )
{
	gentity_t	*slave;
	int			time;
...
*/
qboolean
findNonAIBrushTargeter ( gentity_t *ent )
{
...
*/
void
finishSpawningKeyedMover ( gentity_t *ent )
{
	gentity_t	*slave;
...
#ifdef __BOT__
	if ( ent->allowteams & 4 /*TEAM_AXIS*/ )
	{
		BOT_AddAxisDoor( ent->s.pos.trBase );
	}

	if ( ent->allowteams & 8 /*TEAM_ALLIES*/ )
	{
		BOT_AddAlliedDoor( ent->s.pos.trBase );
	}
#endif //__BOT__
...
*/
void
Door_reverse_sounds ( gentity_t *ent )
{
	int stemp;
	stemp = ent->sound1to2;
...
*/
void
DoorSetSounds ( gentity_t *ent, int doortype, qboolean isRotating )
{
...
*/
void
G_TryDoor ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	qboolean	walking = qfalse;
	qboolean	kicked = qfalse;
	...
	if ( (ent->s.apos.trType == TR_STATIONARY && ent->s.pos.trType == TR_STATIONARY) )
	{
		if ( ent->active == qfalse )
		{
			...
			(
				...
				(G_IsSinglePlayerGame() && ent->key == KEY_LOCKED_PICKABLE)
			)													// door force locked
			{
				G_AddEvent( ent, EV_GENERAL_SOUND, ent->soundPos3 );
				return;
...
*/
void
SP_path_corner_2 ( gentity_t *self )
{
	if ( !self->targetname )
	{
...
*/
void
SP_info_train_spline_main ( gentity_t *self )
{
	int				i;
	char			*end;
	splinePath_t	*spline;
...
*/
void
info_limbo_camera_setup ( gentity_t *self )
{
	limbo_cam_t *caminfo;
	gentity_t	*target;
	vec3_t		vec;
...
/* */
void
SP_info_limbo_camera ( gentity_t *self )
{
	...
	if ( g_gametype.integer != GT_WOLF_TDM && g_gametype.integer != GT_WOLF_DM )
	{
...
SP_func_train ( gentity_t *self )
{
	...
	{
		self->damage = 0;
		self->localEFlags |= LEF_MOVER_STOP;
	}
	else
	{
		if ( !self->damage )
...
*/
void
Think_BeginMoving_rotating ( gentity_t *ent )
{
	ent->s.pos.trTime = level.time;
	ent->s.pos.trType = TR_LINEAR_STOP;
}
...
*/
void
Reached_Train_rotating ( gentity_t *ent )
{
	gentity_t	*next;
	float		speed;
	vec3_t		move;
...
*/
void
Think_SetupTrainTargets_rotating ( gentity_t *ent )
{
	gentity_t	*path, *next, *start;
	ent->nextTrain = G_FindByTargetname( NULL, ent->target );
	if ( !ent->nextTrain )
...
*/
void
SP_func_train_rotating ( gentity_t *self )
{
	VectorClear( self->s.angles );
	if ( self->spawnflags & TRAIN_BLOCK_STOPS )
	{
...
*/
void
Use_Static ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->r.linked )
	{
		trap_UnlinkEntity( ent );
...
/* */
void
Static_Pain ( gentity_t *ent, gentity_t *attacker, int damage, vec3_t point )
{
	vec3_t	temp;
	if ( ent->spawnflags & 4 )
	{
...
*/
void
Use_Func_Rotate ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->spawnflags & 4 )
	{
...
//
void
SP_func_door_rotating ( gentity_t *ent )
{
	int key, doortype;
	G_SpawnInt( "type", "0", &doortype );
	if ( doortype )
...
*/
void
target_effect ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
	gentity_t	*tent;
...
*/
void
SP_target_effect ( gentity_t *ent )
{
	int		mass;
	char	*type;
...
*/
void
ThrowDebris ( gentity_t *self, char *modelname, float speed, vec3_t origin )
{
...
*/
void
BecomeExplosion ( gentity_t *self )
{
	self->die = NULL;
	self->pain = NULL;
	self->touch = NULL;
...
*/
void
func_explosive_explode ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
	vec3_t		origin;
	vec3_t		size;
	vec3_t		dir = { 0, 0, 1 };
...
*/
void
func_explosive_touch ( gentity_t *self, gentity_t *other, trace_t *trace )
{
...
*/
void
func_explosive_use ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
...
*/
void
func_explosive_alert ( gentity_t *self )
{
...
*/
void
func_explosive_spawn ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
...
*/
void
target_explosion_use ( gentity_t *self, gentity_t *other, gentity_t *attacker )
{
	vec3_t		dir = { 0, 0, 1 };
	gentity_t	*tent = NULL;
...
/* */
void
SP_target_explosion ( gentity_t *ent )
{
	char	*type;
	char	*s;
...
*/
void
SP_func_explosive ( gentity_t *ent )
{
	...
	if ( ent->spawnflags & EXPLOSIVE_START_INVIS && (!ent->targetname || Q_stricmp( ent->targetname, "backdoor")) )
	{
	...
	{						// tank objective
		ent->takedamage = qtrue;
		ent->constructibleStats.destructxpbonus = 4;
		ent->constructibleStats.health = 100;
		ent->health = 100;
		ent->constructibleStats.weaponclass = 3;
	}
...
// TOD-O: stick this somewhere ;) "sonic from australia is the bestest ninja in the world"
...
*/
void
use_invisible_user ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
/* */
void
SP_func_invisible_user ( gentity_t *ent )
{
	int		i;
	char	*sound;
	char	*cursorhint;
...
*/
void
G_Activate ( gentity_t *ent, gentity_t *activator )
{
...
*/
void
InitConstructible ( gentity_t *ent )
{
	ent->s.angles2[0] = 0;
...
*/
void
func_constructible_use ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
	self->s.angles2[0] = 0;
	self->grenadeFired = 0;
	self->s.modelindex = 0;
	...
	// Arnout: TOD-O - make this explode or so?
...
*/
void
func_constructible_spawn ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
	trap_LinkEntity( self );
...
*/
void
func_constructible_explode ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
	if ( self->desstages )
	{
...
/* */
void
func_constructible_underconstructionthink ( gentity_t *ent )
{
...
/* */
void
func_constructiblespawn ( gentity_t *ent )
{
...
/* */
void
SP_func_constructible ( gentity_t *ent )
{
	int /*health, wait, */ i;
	...
	{
		ent->constructibleStats = g_constructible_classes[i];
		G_SpawnFloat( "constructible_chargebarreq", va( "%i", ent->constructibleStats.chargebarreq),
		...
		G_SpawnInt( "constructible_weaponclass", va( "%i", ent->constructibleStats.weaponclass),
					&ent->constructibleStats.weaponclass );
		G_SpawnInt( "constructible_duration", va( "%i", ent->constructibleStats.duration),
					&ent->constructibleStats.duration );
	}
	else
	{
...
*/
void
func_brushmodel_delete ( gentity_t *ent )
{
	G_FreeEntity( ent );
}
...
/* */
void
SP_func_brushmodel ( gentity_t *ent )
{
	if ( !ent->model )
	{
