...
*/
int
G_FindConfigstringIndex ( const char *name, int start, int max, qboolean create )
{
	int		i;
	...
	if ( i == max )
	{
		if ( start == CS_MODELS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_MODELS" );
		}
		else if ( start == CS_SOUNDS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_SOUNDS" );
		}
		else if ( start == CS_SKINS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_SKINS" );
		}
		else if ( start == CS_SHADERS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_SHADERS" );
		}
		else if ( start == CS_CHARACTERS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_CHARACTERS" );
		}
		else if ( start == CS_STRINGS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_STRINGS" );
		}

#ifdef __PARTICLE_SYSTEM__
		else if ( start == CS_PARTICLE_SYSTEMS )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_PARTICLE_SYSTEMS" );
		}
#endif //__PARTICLE_SYSTEM__
		else if ( start == CS_SPLINES )
		{
			G_Error( "G_FindConfigstringIndex: overflow - CS_SPLINES" );
		}
		else
		{
			G_Error( "G_FindConfigstringIndex: overflow - %i", start );
		}
	}
...
/* */
int
G_CharacterIndex ( const char *name )
{
...
/* */
int
G_StringIndex ( const char *string )
{
...
*/
gentity_t *
G_FindByTargetname ( gentity_t *from, const char *match )
{
	gentity_t	*max = &g_entities[level.num_entities];
...
gentity_t *
G_FindByTargetnameFast ( gentity_t *from, const char *match, int hash )
{
	gentity_t	*max = &g_entities[level.num_entities];
	if ( !from )
	{
...
/* */
qboolean
G_AllowTeamsAllowed ( gentity_t *ent, gentity_t *activator )
{
...
*/
void
G_UseEntity ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
/* */
gentity_t *
G_PopupMessage ( popupMessageType_t type )
{
	gentity_t	*e;
	e = G_Spawn();
...
*/
void
G_Sound ( gentity_t *ent, int soundIndex )
{
	gentity_t	*te;
...
*/
void
G_AnimScriptSound ( int soundIndex, vec3_t org, int client )
{
	gentity_t	*e;
	e = &g_entities[client];
...
*/
void
G_SetAngle ( gentity_t *ent, vec3_t angle )
{
...
*/
qboolean
infront ( gentity_t *self, gentity_t *other )
{
	vec3_t	vec;
	float	dot;
	vec3_t	forward;
...
*/
void
G_ProcessTagConnect ( gentity_t *ent, qboolean clearAngles )
{
	if ( !ent->tagName )
	{
...
*/
void
G_SetEntState ( gentity_t *ent, entState_t state )
{
...
/* */
static qboolean
G_LoadCampaignsFromFile ( const char *filename )
{
	int			handle;
	pc_token_t	token;
	const char	*s;
...
#ifdef __SUPREMACY__
			if ( strstr( token.string, "supremacy") )
			{
				g_campaigns[level.campaignCount].typeBits |= ( 1 << GT_SUPREMACY );
			}
#endif //__SUPREMACY__
		}
		else if ( !Q_stricmp( token.string, "maps") )
		{
			...
			while ( *ptr )
			{
				...
				*mapnamePtr = '\0';
				...
				{
					Q_strncpyz( g_campaigns[level.campaignCount].mapnames[g_campaigns[level.campaignCount].mapCount],
								mapname, MAX_QPATH );
...
/* */
qboolean
G_MapIsValidCampaignStartMap ( void )
{
	int i;
...
#ifdef __SUPREMACY__
	if ( g_gametype.integer == GT_SUPREMACY_CAMPAIGN )
	{
		numdirs = trap_FS_GetFileList( "scripts", ".supremacyCampaign", dirlist, 1024 );
	}
	else
#endif //__SUPREMACY__
...
/* */
void
G_PrintClientSpammyCenterPrint ( int entityNum, char *text )
{
...
/* */
team_t
G_GetTeamFromEntity ( gentity_t *ent )
{
	switch ( ent->s.eType )
	{
		case ET_PLAYER:
