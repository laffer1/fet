...
#ifdef __SUPREMACY__
extern vec4_t	popBG;
extern vec4_t	popBorder;
extern vec4_t	popHover;
extern vec4_t	popText;
extern vec4_t	popLime;
extern vec4_t	popCyan;
extern vec4_t	popRed;
extern vec4_t	popBlue;
extern vec4_t	popOrange;
extern vec4_t	popDefaultGrey;
extern vec4_t	popLightGrey;
extern vec4_t	popDarkGrey;
extern vec4_t	popAlmostWhite;
extern vec4_t	popAlmostBlack;
#endif //__SUPREMACY__
...
#define Vector4Average( v, b, s, o )																				 \
		(																											 \
			(o)[0] = ((v)[0] * (1 - (s))) + ((b)[0] * (s)), (o)[1] = ((v)[1] * (1 - (s))) + ((b)[1] * (s)), (o)[2] = \
				(																									 \
					(v)[2] *																						 \
					(1 - (s))																						 \
				) +																									 \
			...
			x[0] == 1.0 ? PLANE_X :																				 \
				(																								 \
					x[1] == 1.0 ? PLANE_Y :																		 \
						(																						 \
							x[2] == 1.0 ? PLANE_Z :																 \
								(x[0] == 0.f && x[1] == 0.f && x[2] == 0.f ? PLANE_NON_PLANAR : PLANE_NON_AXIAL) \
						)																						 \
				)																								 \
		)
...
#define MAX_SOUNDS	128					// so they cannot be blindly increased
...
typedef enum
{
	...
	ET_WOLF_OBJECTIVE,
	ET_VEHICLE,
	ET_DEFENCEPOINT_HINT,
#ifdef __SUPREMACY__
	ET_FLAG,
	ET_ASSOCIATED_SPAWNAREA,
	ET_UNASSOCIATED_SPAWNAREA,
#endif //__SUPREMACY__
#ifdef __PARTICLE_SYSTEM__
	ET_PARTICLE_SYSTEM,
#endif //__PARTICLE_SYSTEM__
	...
	ET_NPC,
#endif //__NPC__
	// UQ1: -- RTCW Single Player --
	ET_SPOTLIGHT_EF,
	// UQ1: -- End RTCW Single Player --

	ET_EVENTS,							// any of the EV_* events can be added freestanding

	// by setting eType to ET_EVENTS + eventNum
	// this avoids having to set eFlags and eventNum
	ET_TEMPBODY,						// Temp body for bullets
	ET_TEMPGROIN,						// Temp body for bullets
} entityType_t;
typedef struct entityState_s
{
	...
	int				constantLight;		// r + (g<<8) + (b<<16) + (intensity<<24) , leanf*1000 for players
	...
	int				density;			// for particle effects , or number of primary weapon ( for players)
	...
	int				nextWeapon;
	int				teamNum;
	int				effect1Time;		// Jaquboss: Number of ammo for primary weapon if used by player
	int				effect2Time;		// Jaquboss: if 1 player died on headshot..
	int				effect3Time;
	aistateEnum_t	aiState;			// xkan, 1/10/2003
...
#endif // __Q_SHARED_H
#ifdef __NPC__
#ifndef NPC_CLASSES_INITIALIZED
#define NPC_CLASSES_INITIALIZED 1

// The basic NPC classtypes.
enum
{
	CLASS_HUMANOID,					// A standard player-style npc...
	CLASS_BOAT,						// Water vehicles...
	CLASS_LCVP,						// Water vehicles... This one is for spawning on...
	CLASS_DESTROYER,				// Water vehicles (Destroyer with turret)...
...
enum
{
	HUMANOID_TYPE_DEFAULT,			// A standard player-style npc...
	HUMANOID_TYPE_FODDER,			// A standard fodder npc...
	HUMANOID_TYPE_FODDER_ATTACKER,	// A fodder npc attacker that may attack...
	HUMANOID_TYPE_FODDER_DEFENDER,	// A fodder npc defender that may attack...
	HUMANOID_TYPE_MAX				// Simply marks the end of the class types... (Pointless???)
};
#endif // NPC_CLASSES_INITIALIZED
#endif // __NPC__
#ifndef TAG_TYPES_INITIALIZED
#define TAG_TYPES_INITIALIZED	1

// Smarter Unique1 TAG Connecting...
enum
{
	TAG_HOOK						= 995,
	TAG_HOOK_DRIVER					= 996,
	TAG_HOOK_GUNNER1				= 997,
	TAG_HOOK_GUNNER2				= 998,
	TAG_UNHOOK						= 999,
};
