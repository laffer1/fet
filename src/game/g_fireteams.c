...
/* */
team_t
G_GetFireteamTeam ( fireteamData_t *ft )
{
	if ( !ft->inuse )
	{
		return ( -1 );
...
/* */
int
G_CountTeamFireteams ( team_t team )
{
	int i, cnt = 0;
	for ( i = 0; i < MAX_FIRETEAMS; i++ )
	{
...
/* */
void
G_UpdateFireteamConfigString ( fireteamData_t *ft )
{
	char	buffer[128];
	int		i;
	int		clnts[2] = { 0, 0 };
...
/* */
qboolean
G_IsOnFireteam ( int entityNum, fireteamData_t **teamNum )
{
	int i, j;
...
/* */
qboolean
G_IsFireteamLeader ( int entityNum, fireteamData_t **teamNum )
{
	int i;
	if ( (entityNum < 0 || entityNum >= MAX_CLIENTS) || !g_entities[entityNum].client )
	{
...
/* */
int
G_FindFreeFireteamIdent ( team_t team )
{
...
void
G_RegisterFireteam ( /*const char* name,*/ int entityNum )
{
	fireteamData_t	*ft;
	gentity_t		*leader;
	int				count, ident;
	...
	{
		G_ClientPrintAndReturn( entityNum, "No fireteams in deathmatch" );
	}
...
void
G_AddClientToFireteam ( int entityNum, int leaderNum )
{
	fireteamData_t	*ft;
	int				i;
	if ( (entityNum < 0 || entityNum >= MAX_CLIENTS) || !g_entities[entityNum].client )
...
void
G_RemoveClientFromFireteams ( int entityNum, qboolean update, qboolean print )
{
	fireteamData_t	*ft;
	int				i, j;
	if ( (entityNum < 0 || entityNum >= MAX_CLIENTS) || !g_entities[entityNum].client )
...
void
G_InviteToFireTeam ( int entityNum, int otherEntityNum )
{
	fireteamData_t	*ft;
	if ( (entityNum < 0 || entityNum >= MAX_CLIENTS) || !g_entities[entityNum].client )
	{
...
/* */
void
G_DestroyFireteam ( int entityNum )
{
	fireteamData_t	*ft;
	if ( (entityNum < 0 || entityNum >= MAX_CLIENTS) || !g_entities[entityNum].client )
	{
...
/* */
void
G_WarnFireTeamPlayer ( int entityNum, int otherEntityNum )
{
...
/* */
void
G_KickFireTeamPlayer ( int entityNum, int otherEntityNum )
{
	fireteamData_t	*ft, *ft2;;
	if ( entityNum == otherEntityNum )
	{
...
void
G_ApplyToFireTeam ( int entityNum, int fireteamNum )
{
	gentity_t		*leader;
	fireteamData_t	*ft;
	if ( (entityNum < 0 || entityNum >= MAX_CLIENTS) || !g_entities[entityNum].client )
...
/* */
void
G_ProposeFireTeamPlayer ( int entityNum, int otherEntityNum )
{
	fireteamData_t	*ft;
	gentity_t		*leader;
	if ( entityNum == otherEntityNum )
...
/* */
int
G_FireteamNumberForString ( const char *name, team_t team )
{
	int i, fireteam = 0;
	for ( i = 0; i < MAX_FIRETEAMS; i++ )
	{
...
/* */
fireteamData_t *
G_FindFreePublicFireteam ( team_t team )
{
	int i, j;
	for ( i = 0; i < MAX_FIRETEAMS; i++ )
	{
...
// Command handler
void
Cmd_FireTeam_MP_f ( gentity_t *ent )
{
	char	command[32];
	int		i;
	if ( trap_Argc() < 2 )
