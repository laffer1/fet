...
*/
void
G_MissileImpact ( gentity_t *ent, trace_t *trace, int impactDamage )
{
	gentity_t	*other;
	gentity_t	*temp;
	...
	if ( other->classname && Q_stricmp( other->classname, "func_explosive") == 0 )
	{
		...
		if ( other->health && impactDamage >= other->health )
		{
			...
			if ( other->takedamage )
			{
				...
				G_Damage( other, ent, &g_entities[ent->r.ownerNum], velocity, ent->s.origin, impactDamage, 0,
						  ent->methodOfDeath );
			}
	...
	if ( other->takedamage || other->dmgparent )
	{
		if ( ent->damage )
		{
			...
			G_Damage( other->dmgparent ? other->dmgparent : other, ent, &g_entities[ent->r.ownerNum], velocity,
	...
	if ( ent->splashDamage )
	{
		G_RadiusDamage( trace->endpos, ent, ent->parent, ent->splashDamage, ent->splashRadius, other,
...
G_ExplodeMissile ( gentity_t *ent )
{
	...
	if ( ent->splashDamage )
	{
		...
		{
			etpro_RadiusDamage( origin, ent, ent->parent, ent->splashDamage, ent->splashRadius, ent,
		...
		{
			G_RadiusDamage( origin, ent, ent->parent, ent->splashDamage, ent->splashRadius, ent,
	...
	if ( etype == ET_MISSILE || etype == ET_BOMB )
	{
		...
		{												// do some scoring
			...
			for ( i = 0; i < num; i++ )
			{
				...
				if
				(
					((hit->spawnflags & AXIS_OBJECTIVE) && (ent->s.teamNum == TEAM_ALLIES)) ||
				...
				)
				{
					...
					{
						G_AddKillSkillPointsForDestruction( ent->parent, MOD_DYNAMITE,
		...
		(
			...
			ent->s.weapon == WP_MAPMORTAR ||
			ent->s.weapon == WP_ARTY ||
			ent->s.weapon == WP_SMOKE_MARKER ||
			ent->s.weapon == WP_LANDMINE ||
			ent->s.weapon == WP_SATCHEL					//|| ent->s.weapon == WP_TRIPMINE /*|| ent->s.weapon == WP_SMOKE_BOMB*/
		)
		{
...
*/
void
G_MissileDie ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
	if ( inflictor == self )
	{
		return;
...
*/
void
Landmine_Check_Ground ( gentity_t *self )
{
	vec3_t	mins, maxs;
	vec3_t	start, end;
	trace_t tr;
...
G_RunMissile ( gentity_t *ent )
{
	...
	)
	{
		if ( ent->count )
		{
			if
			(
				ent->r.currentOrigin[0] < level.mapcoordsMins[0] ||
	...
	if ( tr.fraction != 1 )
	{
		...
		else								// never explode or bounce on sky
...
*/
void
G_PredictBounceMissile ( gentity_t *ent, trajectory_t *pos, trace_t *trace, int time )
{
...
*/
int
G_PredictMissile ( gentity_t *ent, int duration, vec3_t endPos, qboolean allowBounce )
{
	vec3_t			origin;
	trace_t			tr;
	int				time;
...
#define FLAME_MAX_SPEED			6400.0								// jaquboss
...
/* */
void
G_BurnTarget ( gentity_t *self, gentity_t *body, qboolean directhit )
{
	int		i;
	float	radius, dist;
	vec3_t	point, v;
	...
	if ( body->client )
	{
		...
		//			return;
		if ( !(g_friendlyFire.integer) && OnSameTeam( body, self->parent) && g_gametype.integer != GT_WOLF_DM )
		{
			return;
		}
	}
...
#ifdef __NPC__
		if ( body->s.eType == ET_NPC )
		{
			point[2] += body->NPC_client->ps.viewheight;
		}
		else
#endif //__NPC__
			if ( body->client )
...
G_RunFlamechunk ( gentity_t *ent )
{
	...
	if ( level.time - ent->timestamp > 50 )
	{
		...
		if ( speed > FLAME_MAX_SPEED )
		{
			speed = FLAME_MAX_SPEED;
		}
...
*/
gentity_t *
fire_flamechunk ( gentity_t *self, vec3_t start, vec3_t dir )
{
	gentity_t	*bolt;
...
void
G_FadeItems ( gentity_t *ent, int modType )
{
	gentity_t	*e;
	int			i;
...
/* */
int
G_CountTeamLandmines ( team_t team )
{
	gentity_t	*e;
	int			i;
	int			cnt = 0;
...
/* */
qboolean
G_SweepForLandmines ( vec3_t origin, float radius, int team )
{
	gentity_t	*e;
	int			i;
	vec3_t		dist;
...
*/
qboolean
G_HasDroppedItem ( gentity_t *ent, int modType )
{
	gentity_t	*e;
	int			i;
	e = &g_entities[MAX_CLIENTS];
...
*/
qboolean
G_ExplodeSatchels ( gentity_t *ent )
{
	gentity_t	*e;
	vec3_t		dist;
	int			i;
...
/* */
void
LandMineTrigger ( gentity_t *self )
{
...
/* */
void
LandMinePostTrigger ( gentity_t *self )
{
...
/* */
qboolean
sEntWillTriggerMine ( gentity_t *ent, gentity_t *mine )
{
...
void
G_LandmineThink ( gentity_t *self )
{
	int			entityList[MAX_GENTITIES];
	int			i, cnt;
...
/* */
void
LandminePostThink ( gentity_t *self )
{
	int			entityList[MAX_GENTITIES];
	int			i, cnt;
	vec3_t		range = { LANDMINE_TRIGGER_DIST, LANDMINE_TRIGGER_DIST, LANDMINE_TRIGGER_DIST };
...
*/
void
G_LandminePrime ( gentity_t *self )
{
	self->nextthink = level.time + FRAMETIME;
...
/* */
qboolean
G_LandmineSnapshotCallback ( int entityNum, int clientNum )
{
	...
	if
	(
		clEnt->client->sess.playerType == PC_COVERTOPS &&
...
*/
gentity_t *
fire_grenade ( gentity_t *self, vec3_t start, vec3_t dir, int grenadeWPID /*, qboolean underhand*/ )
{
	gentity_t	*bolt;
	...
	switch ( grenadeWPID )
	{
		...
		case WP_GRENADE_LAUNCHER:
			...
			//Ryan - Shootable Axis Grenades : Start
			if ( g_shootablegrenades.value >= 1 )
			{
				bolt->health = 5;
		...
		case WP_GRENADE_PINEAPPLE:
			...
			//Ryan - Shootable Allies Grenades : Start
			if ( g_shootablegrenades.value >= 1 )
			{
				bolt->health = 5;
				bolt->takedamage = qtrue;
	...
	if ( grenadeWPID != WP_DYNAMITE )
	{														// jaquboss ,fix for big boom :D , TODO G_GetWeaponRadius ??
...
*/
gentity_t *
fire_rocket ( gentity_t *self, vec3_t start, vec3_t dir, qboolean gravited )
{
	gentity_t	*bolt;
	VectorNormalize( dir );
	bolt = G_Spawn();
	bolt->classname = "rocket";
	bolt->nextthink = level.time + ( g_panzerfaustlifetime.integer * 1000 );	// push it out a little
	bolt->think = G_ExplodeMissile;

	//Ryan - Shootable Rockets : Start
	if ( g_shootablerockets.value >= 1 )
	{
		bolt->health = 5;
		bolt->takedamage = qtrue;
		bolt->die = G_MissileDie;
		bolt->r.contents = CONTENTS_BODY;
		VectorSet( bolt->r.mins, -12, -4, 0 );
		VectorCopy( bolt->r.mins, bolt->r.absmin );
		VectorSet( bolt->r.maxs, 12, 4, 7 );
		VectorCopy( bolt->r.maxs, bolt->r.absmax );
	}

	//End - Shootable Rockets
	bolt->accuracy = 4;
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_BROADCAST;
	...
	bolt->splashRadius = 150;													// jaquboss , this need tuning  now bit bigger
	...
	VectorScale( dir, g_panzerfaustspeed.integer, bolt->s.pos.trDelta );
...
*/
gentity_t *
fire_tankshell ( gentity_t *self, vec3_t start, vec3_t dir )
{
	gentity_t	*bolt;
	...
	bolt->s.eType = ET_MISSILE;
	bolt->r.svFlags = SVF_BROADCAST;
	bolt->s.weapon = WP_MORTAR_SET; //self->s.weapon;
	bolt->r.ownerNum = self->s.number;
	bolt->parent = self;
	bolt->damage = 250;
	bolt->splashDamage = 250;
	bolt->splashRadius = 120;
	bolt->methodOfDeath = MOD_TANKSHELL;
	bolt->splashMethodOfDeath = MOD_TANKSHELL;
	bolt->clipmask = MASK_MISSILESHOT;
	bolt->s.pos.trType = TR_LINEAR;
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;
	VectorCopy( start, bolt->s.pos.trBase );
	VectorScale( dir, 9000, bolt->s.pos.trDelta );
...
*/
gentity_t *
fire_flamebarrel ( gentity_t *self, vec3_t start, vec3_t dir )
{
	gentity_t	*bolt;
	VectorNormalize( dir );
	bolt = G_Spawn();
...
*/
void
fire_lead ( gentity_t *self, vec3_t start, vec3_t dir, int damage, int mod )
{
	trace_t		tr;
	vec3_t		end;
	gentity_t	*tent;
...
*/
qboolean
visible ( gentity_t *self, gentity_t *other )
{
...
*/
gentity_t *
fire_mortar ( gentity_t *self, vec3_t start, vec3_t dir )
{
	gentity_t	*bolt;
