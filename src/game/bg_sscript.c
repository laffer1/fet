...
/* */
void
BG_ClearScriptSpeakerPool ( void )
{
	numScriptSpeakers = 0;
}
...
/* */
int
BG_GetIndexForSpeaker ( bg_speaker_t *speaker )
{
...
/* */
qboolean
BG_SS_StoreSpeaker ( bg_speaker_t *speaker )
{
...
/* */
static qboolean
BG_SS_ParseError ( int handle, char *format, ... )
{
	int			line;
	char		filename[128];
	va_list		argptr;
...
/* */
qboolean
BG_LoadSpeakerScript ( const char *filename )
{
	pc_token_t	token;
	int			handle;
