...
G_GetEntityState (int entityNum)
{
	if ( entityNum < 0 || entityNum >= MAX_GENTITIES || g_entities[entityNum].inuse == qfalse )
	{
		return ( NULL );
	}
...
P_WorldEffects ( gentity_t *ent )
{
	...
	//
	// JPW NERVE MP way
	if ( ent->s.onFireEnd && ent->client && !(ent->client->ps.eFlags & EF_TANK) )
	{
...
/* */
void
PushBot ( gentity_t *ent, gentity_t *other )
{
	vec3_t	dir, ang, f, r;
	float	oldspeed;
...
qboolean
ReadyToCallArtillery ( gentity_t *ent )
{
	if ( ent->client->sess.skill[SK_SIGNALS] >= 2 )
	{
		if
		(
			level.time - ent->client->ps.classWeaponTime <=
...
qboolean
ReadyToConstruct ( gentity_t *ent, gentity_t *constructible, qboolean updateState )
{
	...
	else
	{
		...
		{
			weaponTime += 0.66f * constructible->constructibleStats.chargebarreq *
				(
					(float) level.engineerChargeTime[ent->client->sess.sessionTeam - 1] /
						(constructible->constructibleStats.duration / (float) FRAMETIME)
				);
		...
		{
			weaponTime += constructible->constructibleStats.chargebarreq *
				(
					(float) level.engineerChargeTime[ent->client->sess.sessionTeam - 1] /
						(constructible->constructibleStats.duration / (float) FRAMETIME)
				);
...
*/
void
CheckBotImpacts ( gentity_t *ent, gentity_t *other )
{
...
G_TouchTriggers ( gentity_t *ent )
{
	...
	{
		VectorSubtract( ent->NPC_client->ps.origin, range, mins );
		VectorAdd( ent->NPC_client->ps.origin, range, maxs );
		num = trap_EntitiesInBox( mins, maxs, touch, MAX_GENTITIES );

		// can't use ent->absmin, because that has a one unit pad
		VectorAdd( ent->NPC_client->ps.origin, ent->r.mins, mins );
		VectorAdd( ent->NPC_client->ps.origin, ent->r.maxs, maxs );
	}
	else
	{
...
SpectatorThink ( gentity_t *ent, usercmd_t *ucmd )
{
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW )
	{
		...
		pm.getentitystate = G_GetEntityState;
...
ClientInactivityTimer ( gclient_t *client )
{
	...
	if
	(
		(g_inactivity.integer == 0 && client->sess.sessionTeam != TEAM_SPECTATOR) ||
...
ClientTimerActions ( gentity_t *ent, int msec )
{
	...
	while ( client->timeResidual >= 1000 )
	{
		...
		// regenerate
		if ( client->sess.playerType == PC_MEDIC && client->ps.eFlags & EF_TANK )
		{
			if ( ent->BackupHealth < client->ps.stats[STAT_MAX_HEALTH] )
			{
				ent->BackupHealth += 3;
				if ( ent->BackupHealth > client->ps.stats[STAT_MAX_HEALTH] * 1.1 )
				{
					ent->BackupHealth = client->ps.stats[STAT_MAX_HEALTH] * 1.1;
				}
			}
			else if ( ent->BackupHealth < client->ps.stats[STAT_MAX_HEALTH] * 1.12 )
			{
				ent->BackupHealth += 2;
				if ( ent->BackupHealth > client->ps.stats[STAT_MAX_HEALTH] * 1.12 )
				{
					ent->BackupHealth = client->ps.stats[STAT_MAX_HEALTH] * 1.12;
				}
			}
		}
		else if ( client->sess.playerType == PC_MEDIC )
		{
	...
	for ( i = oldEventSequence; i < client->ps.eventSequence; i++ )
	{
		...
		switch ( event )
		{
			...
			case EV_FIRE_WEAPON_BROWNING:
				...
				if ( g_gamestate.integer == GS_PLAYING )
	#endif
					ent->client->sess.aWeaponStats[BG_WeapStatForWeapon( WP_30CAL )].atts++;
				break;
...
float
ClientHitboxMaxZ ( gentity_t *hitEnt )
{
	if ( !hitEnt )
	{
		return ( 0 );
...
extern float	tankAngle ( gentity_t *ent );
#ifdef SAVEGAME_SUPPORT


/* */
void
ClientThink_real_SaveGame ( gentity_t *ent )
{
	...
	client = ent->client;
	if ( ent->s.eFlags & EF_TANK || client->ps.eFlags & EF_TANK )
	{
		float	turretDiff;
		turretDiff = ent->client->ps.viewangles[PITCH];
		if ( turretDiff - tankAngle( ent) > 15 )
		{
			turretDiff = 15;
		}
		else if ( turretDiff - tankAngle( ent) < -15 )
		{
			turretDiff = -15;
		}
		else
		{
			turretDiff -= tankAngle( ent );
		}

		ent->client->ps.stats[STAT_CROSSHAIR_PITCH] = turretDiff;

		// mg horizontal rotation
		turretDiff = AngleNormalize180( ent->client->ps.stats[STAT_DEAD_YAW] - ent->client->ps.viewangles[YAW] );
		if ( turretDiff > 15 )
		{
			turretDiff = 15;
		}
		else if ( turretDiff < -15 )
		{
			turretDiff = -15;
		}

		ent->turretYaw = turretDiff;

		// DONE
	}

	if ( !ent->reviving && ent->revivingAnim )
	{
		BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo,	/*ANIM_ET_PICKUPGRENADE*/
							ANIM_ET_DROP, qfalse, qtrue );
		ent->revivingAnim = qfalse;
	}
	...
	if ( client->warping && g_maxWarp.integer )
	{
		...
		client->ps.commandTime = level.previousTime - ( frames * (level.time - level.previousTime) );
		client->warped = qtrue;
	}
	...
	if
	(
		(client->ps.eFlags & EF_VIEWING_CAMERA) ||
	...
	)
	{
		...
		if ( level.match_pause != PAUSE_NONE )
		{
			...
			(
				(client->ps.eFlags & EF_VIEWING_CAMERA) ||
				(
					g_gametype.integer == GT_SINGLE_PLAYER && g_reloading.integer &
...
#define MOVEMENT_SPEED	235
	...
	if ( /*(ent->client->ps.eFlags & EF_TANK ||*/ ent->client->ps.eFlags & EF_PRONE /*)*/ && g_noCaps.integer )
	{
		pm.trace = trap_Trace;
	}
	else
	...
	//	pm.ps->eFlags |= EF_CONNECTION;
	//	client->ps.eFlags |= EF_CONNECTION;
	//} else {
	//	pm.ps->eFlags &= ~EF_CONNECTION;
	//}
#ifndef __BOT__									// UQ1: What the fuck is this???
	if ( ent->r.svFlags & SVF_BOT )
	{
		client->ps.holding = 1;
	}
	...
	VectorCopy( client->ps.viewangles, client->oldAngles );
	...
	if ( client->ps.pm_type != PM_DEAD && level.timeCurrent - client->pers.lastBattleSenseBonusTime > 45000 )
	{
		if ( client->combatState != COMBATSTATE_COLD )
		{
			if
			(
				client->combatState & (1 << COMBATSTATE_KILLEDPLAYER) &&
			...
			)
			{
				...
				(
					client->combatState & (1 << COMBATSTATE_DAMAGEDEALT) &&
					client->combatState & (1 << COMBATSTATE_DAMAGERECEIVED)
				)
			{
	...
	)
	{
		...
		//	G_Printf("Halalalalala\n");
		validkick = Cmd_WolfKick_f( ent );
		if ( validkick )
		{
			ent->wolfkicktimer = level.time + 1000;
		}
	}

	if ( ent->client->ps.persistant[PERS_WOLFKICK] )
	{
		if ( ent->wolfkicktimer < level.time )
		{
			ent->client->ps.persistant[PERS_WOLFKICK] = 0;
		}
		else
		{
			ent->client->ps.persistant[PERS_WOLFKICK] = (int) ( 22 - ((ent->wolfkicktimer - level.time) / 50) );
...
*/
	...
	if ( client->ps.stats[STAT_HEALTH] <= 0 )
	{
		...
		if ( g_gametype.integer == GT_WOLF_DM ){
			...
			if ( level.timeCurrent > client->respawnTime || (client->ps.stats[STAT_HEALTH] < -350) )
		...
		if ( level.timeCurrent > client->respawnTime && !(ent->client->ps.pm_flags & PMF_LIMBO) )
		{
			...
			(
				(
					...
					level.timeCurrent -
					client->respawnTime > g_forcerespawn.integer *
					1000
				) ||
				client->ps.stats[STAT_HEALTH] <= GIB_HEALTH ||
				client->ps.stats[STAT_HEALTH] <= FORCE_LIMBO_HEALTH 
			)
			{
				limbo( ent, (client->ps.stats[STAT_HEALTH] > GIB_HEALTH) );
			}
	...
	{
		if ( g_debugbbox.integer > 0 )
		{
			gentity_t	*bboxEnt, *traceBox, *front, *back;
			vec3_t		b1, b2;
			...
			VectorCopy( ent->r.currentOrigin, b1 );
			VectorCopy( ent->r.currentOrigin, b2 );
			VectorAdd( b1, tankTraceMins, b1 );
			VectorAdd( b2, tankTraceMaxs, b2 );
			traceBox = G_TempEntity( b1, EV_RAILTRAIL );
			VectorCopy( b2, traceBox->s.origin2 );
			traceBox->s.dmgFlags = 1;
			if ( g_debugbbox.integer > 1 )
			{
				vec3_t	ofs, org;
				vec3_t	flatforward;
				float	angle;
				...
				VectorAdd( ent->r.currentOrigin, ofs, org );
				VectorCopy( org, b1 );
				VectorCopy( org, b2 );
				VectorAdd( b1, tankTraceMins, b1 );
				VectorAdd( b2, tankTraceMaxs, b2 );
				front = G_TempEntity( b1, EV_RAILTRAIL );
				VectorCopy( b2, front->s.origin2 );
				front->s.dmgFlags = 1;
				VectorScale( flatforward, 72, ofs );
				VectorAdd( ent->r.currentOrigin, ofs, org );
				VectorCopy( org, b1 );
				...
				VectorAdd( b1, tankTraceMins, b1 );
				VectorAdd( b2, tankTraceMaxs, b2 );
				back = G_TempEntity( b1, EV_RAILTRAIL );
				VectorCopy( b2, back->s.origin2 );
				back->s.dmgFlags = 1;
			}
		}
	}
	else if ( g_debugbbox.integer == 2 )
	{
		gentity_t	*bboxEnt, *head, *body, *groin, *legs;
		vec3_t		b1, b2;
		vec3_t		maxs;
		VectorCopy( ent->r.currentOrigin, b1 );
		VectorCopy( ent->r.currentOrigin, b2 );
		...
		maxs[2] = ent->r.maxs[2];				//ClientHitboxMaxZ(ent);
		...
		if ( g_realhead.integer > 1 )
		{
			...
			G_FreeEntity( groin );
			legs = G_BuildLeg( ent );
			VectorCopy( legs->r.currentOrigin, b1 );
			VectorCopy( legs->r.currentOrigin, b2 );
			VectorAdd( b1, legs->r.mins, b1 );
			VectorAdd( b2, legs->r.maxs, b2 );
			bboxEnt = G_TempEntity( b1, EV_RAILTRAIL );
			VectorCopy( b2, bboxEnt->s.origin2 );
			bboxEnt->s.dmgFlags = 1;
			G_FreeEntity( legs );
...
SpectatorClientEndFrame ( gentity_t *ent )
{
	...
	if ( (ent->client->sess.spectatorState == SPECTATOR_FOLLOW) || (ent->client->ps.pm_flags & PMF_LIMBO) )
	{
		...
		if ( g_gametype.integer != GT_WOLF_LMS )
		{
			if
			(
				(g_maxlives.integer > 0 || g_alliedmaxlives.integer > 0 || g_axismaxlives.integer > 0) &&
	...
	{
		ent->client->ps.powerups[PW_BLACKOUT] = ( G_blockoutTeam( ent, TEAM_AXIS) * TEAM_AXIS ) | ( G_blockoutTeam( ent, TEAM_ALLIES) * TEAM_ALLIES );
	}
}
...
/* */
void
WolfRevivePushEnt ( gentity_t *self, gentity_t *other )
{
	vec3_t	dir, push;
...
void
WolfReviveBbox ( gentity_t *self )
{
	int			touch[MAX_GENTITIES];
	int			num, i, touchnum = 0;
	gentity_t	*hit = NULL;	// TTimo: init
	vec3_t		mins, maxs;
	hit = G_TestEntityPosition( self, qfalse );
	if
	(
		hit &&
		(
...
ClientEndFrame ( gentity_t *ent )
{
	...
	for ( i = 0; i < PW_NUM_POWERUPS; i++ )
	{
		if
		(
			i == PW_FIRE ||								// these aren't dependant on level.time
			i == PW_ELECTRIC ||
			i == PW_BREATHER ||
			i == PW_NOFATIGUE ||
	...
	if ( g_debugbbox.integer == 1 || g_debugbbox.integer == 2 )
	{													// show bounding box
		gentity_t	*bboxEnt;
		vec3_t		b1, b2;
		VectorCopy( ent->r.currentOrigin, b1 );
