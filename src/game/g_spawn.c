...
/* */
qboolean
G_SpawnStringExt ( const char *key, const char *defaultString, char **out, const char *file, int line )
{
	int i;
	if ( !level.spawning )
	{
...
/* */
qboolean
G_SpawnFloatExt ( const char *key, const char *defaultString, float *out, const char *file, int line )
{
	char		*s;
	qboolean	present;
...
/* */
qboolean
G_SpawnIntExt ( const char *key, const char *defaultString, int *out, const char *file, int line )
{
	char		*s;
	qboolean	present;
	present = G_SpawnStringExt( key, defaultString, &s, file, line );
...
/* */
qboolean
G_SpawnVectorExt ( const char *key, const char *defaultString, float *out, const char *file, int line )
{
	char		*s;
	qboolean	present;
	present = G_SpawnStringExt( key, defaultString, &s, file, line );
...
/* */
qboolean
G_SpawnVector2DExt ( const char *key, const char *defaultString, float *out, const char *file, int line )
{
	char		*s;
	qboolean	present;
	present = G_SpawnStringExt( key, defaultString, &s, file, line );
...
field_t fields[] =
{
	...
	{ "popup", FOFS( message ), F_LSTRING },

	// (SA) mutually exclusive from 'message', but makes the ent more logical for the level designer
	{ "book", FOFS( message ), F_LSTRING },
	...
	{ "_color", FOFS( dl_color ), F_VECTOR },

	// color of the light	(the underscore is inserted by the color picker in QER)
	{ "color", FOFS( dl_color ), F_VECTOR },

	// color of the light
	{ "stylestring", FOFS( dl_stylestring ), F_LSTRING },

	// user defined stylestring "fffndlsfaaaaaa" for example
	// done
	//----(SA)	
	{ "shader", FOFS( dl_shader ), F_LSTRING },

	// shader to use for a target_effect or dlight
...
void	NPC_LCVP_Spawn ( gentity_t *ent );
void	NPC_LCVP_Waypoint ( gentity_t *ent );
void	NPC_LCVP_Waypoint_Target ( gentity_t *ent );
void	NPC_Destroyer_Spawn ( gentity_t *ent );
void	NPC_Destroyer_Waypoint ( gentity_t *ent );
void	NPC_Destroyer_Waypoint_Target ( gentity_t *ent );
void	SP_tank ( gentity_t *ent );
void	SP_panzer ( gentity_t *ent );

/* RTCW Single Player Spawners */
void	NPC_CoopSoldier ( gentity_t *ent );
void	NPC_CoopVenomSoldier ( gentity_t *ent );
void	NPC_CoopBlackGuard ( gentity_t *ent );
void	NPC_CoopEliteGuard ( gentity_t *ent );
void	NPC_CoopHelga ( gentity_t *ent );
void	NPC_CoopProtoSoldier ( gentity_t *ent );
void	NPC_CoopSuperSoldier ( gentity_t *ent );
#endif //__NPC__
...
/* */
void
SP_tankclip ( gentity_t *self )
{
	trap_SetBrushModel( self, self->model );
	level.tankclip[0] = self;
}


/* */
void
SP_panzerclip ( gentity_t *self )
{
	trap_SetBrushModel( self, self->model );
	level.tankclip[1] = self;
}
...
spawn_t spawns[] =
{
	...
	{ "misc_tank", SP_tank },
	{ "misc_panzer", SP_panzer },
	{ "npc_tank", SP_tank },
	{ "npc_churchhill", SP_tank },
	{ "npc_panzer", SP_panzer },
	{ "tankclip", SP_tankclip },
	{ "panzerclip", SP_panzerclip },
#ifdef __BOT__
	...
	{ "npc_lander_enter", NPC_LCVP_Spawn },
	{ "npc_lander_waypoint", NPC_LCVP_Waypoint },
	{ "npc_lander_waypoint_target", NPC_LCVP_Waypoint_Target },
	{ "npc_destroyer_enter", NPC_Destroyer_Spawn },
	{ "npc_destroyer_waypoint", NPC_Destroyer_Waypoint },
	{ "npc_destroyer_waypoint_target", NPC_Destroyer_Waypoint_Target },
	{ "npc_tank", SP_tank },
	{ "npc_churchill", SP_tank },
	{ "npc_panzer", SP_panzer },

	/* RTCW Single Player Spawners */
	{ "ai_zombie", NPC_CoopSoldier },
	{ "ai_soldier", NPC_CoopSoldier },
	{ "ai_venom", NPC_CoopVenomSoldier },
	{ "ai_blackguard", NPC_CoopBlackGuard },
	{ "ai_eliteguard", NPC_CoopEliteGuard },
	{ "ai_boss_helga", NPC_CoopHelga },
	{ "ai_protosoldier", NPC_CoopProtoSoldier },
	{ "ai_supersoldier", NPC_CoopSuperSoldier },

	//	{"ai_civilian",		NPC_CoopCivilian },
#endif //__NPC__
#ifdef __SUPREMACY__
...
G_CallSpawn ( gentity_t *ent )
{
	...
	for ( item = bg_itemlist + 1; item->classname; item++ )
	{
		if ( !strcmp( item->classname, ent->classname) )
		{
			...
			if (g_gametype.integer < GT_WOLF)
			{
				...
				G_SP_RTCW_script_ScriptEvent( ent, "spawn", "" );
			}
			else 
	...
	for ( s = spawns; s->name; s++ )
	{
		if ( !strcmp( s->name, ent->classname) )
		{
			...
			// RF, entity scripting
#ifdef __RTCW_SP_SCRIPTS__
			if (ent->scriptName && g_gametype.integer < GT_WOLF)
			{
				G_SP_RTCW_script_ScriptParse( ent );
				G_SP_RTCW_script_ScriptEvent( ent, "spawn", "" );
			}
...
SP_worldspawn ( void )
{
	...
	if
	(
		G_SpawnVector2D( "mapcoordsmins", "-128 128", level.mapcoordsMins) &&	// top left
...
G_SpawnEntitiesFromString ( void )
{
	...
	SP_worldspawn();
#ifdef __SUPREMACY__						// UQ added.. We need to pass all the new entity list, not just worldspawn...
	if ( handle )
