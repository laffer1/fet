...
*/
void
AddLean ( gentity_t *ent, vec3_t point )
{
#ifdef __NPC__
	if ( ent->s.eType == ET_NPC )
	{
		if ( ent->NPC_client->ps.leanf )
		{
			vec3_t	right, up;
			AngleVectors( ent->NPC_client->ps.viewangles, NULL, right, up );
			VectorMA( point, ent->NPC_client->ps.leanf, right, point );
			VectorMA( point, abs( ent->NPC_client->ps.leanf) * (-0.33), up, point );
		}
	}
	else
#endif //__NPC__
		if ( ent->client )
		{
			if ( ent->client->ps.leanf )
			{
				...
				VectorMA( point, abs( ent->client->ps.leanf) * (-0.33), up, point );
			}
		}
}
...
/* */
qboolean
G_WeaponIsExplosive ( meansOfDeath_t mod )
{
	switch ( mod )
	{
...
/* */
int
G_GetWeaponClassForMOD ( meansOfDeath_t mod )
{
	switch ( mod )
	{
		case MOD_GRENADE_LAUNCHER:
...
*/
void
Weapon_Knife ( gentity_t *ent )
{
	trace_t		tr;
	gentity_t	*traceEnt, *tent;
	int			damage, mod;
	vec3_t		pforward, eforward;
	qboolean	backstab = qfalse;
	vec3_t		end;
	mod = MOD_KNIFE;
#ifdef __NPC__
	if ( ent->s.eType == ET_NPC )
	...
	//	if((tr.entityNum >= MAX_CLIENTS /*&& g_entities[tr.entityNum].s.eType != ET_CORPSE*/) || (&g_entities[ tr.entityNum ] && g_entities[ tr.entityNum ].client && g_entities[ tr.entityNum ].client->ps.eFlags & EF_TANK )) {	// world brush or non-player entity (no blood), edit: tank players dont do fleshy, but corpses do
#ifdef __NPC__
	if
	(
		(
			(tr.entityNum >= MAX_CLIENTS && g_entities[tr.entityNum].s.eType != ET_CORPSE) ||
			(g_entities[tr.entityNum].client && g_entities[tr.entityNum].client->ps.eFlags & EF_TANK)
		) &&
		g_entities[tr.entityNum].s.eType != ET_NPC
	)
#else //!__NPC__
		if
		(
			...
			(
				...
				g_entities[tr.entityNum].client->ps.eFlags & EF_TANK
			)
		)
#endif //__NPC__
		{												// world brush or non-player entity (no blood), edit: tank players dont do fleshy, but corpses do
	...
	if ( traceEnt->client && traceEnt->client->ps.eFlags & EF_TANK )
	{
		return;
	}
...
#ifdef __NPC__
	if ( traceEnt->NPC_client )
	{
		if ( ent->s.eType == ET_NPC )
		{
			AngleVectors( ent->NPC_client->ps.viewangles, pforward, NULL, NULL );
		}
		else
		{
			AngleVectors( ent->client->ps.viewangles, pforward, NULL, NULL );
		...
		AngleVectors( traceEnt->NPC_client->ps.viewangles, eforward, NULL, NULL );
		if ( DotProduct( eforward, pforward) > 0.6f )	// from behind(-ish)
		{
			damage = 100;								// enough to drop a 'normal' (100 health) human with one jab
			mod = MOD_KNIFE;
			...
			//if (ent->s.eType == ET_NPC)
			//{
			//	if ( traceEnt->health > 0 && ent->NPC_client->sess.skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 4 ) {
			//		damage = traceEnt->health;
			//	}
			//}
			//else
	...
	if ( backstab )
	{
		G_Damage( traceEnt, ent, ent, vec3_origin, tr.endpos, (damage + rand() % 5), DAMAGE_BACKSTAB, mod );
	}
	else
	{
...
//	Cmd_WolfKick
//===================
// Kamikazee: [FIXME] KICK_DAMAGE too high
#define KICK_DAMAGE 15
#define WOLFKICKDISTANCE	96


/* */
int
Cmd_WolfKick_f ( gentity_t *ent )
{
	trace_t		tr;
	vec3_t		end;
	gentity_t	*traceEnt;
	...
	damage = (int)( KICK_DAMAGE * ( g_damage_multi.integer / 100.0) );
	if ( ent->client->ps.eFlags & EF_AIMING || ent->client->ps.eFlags & EF_TANK || ent->client->ps.eFlags & EF_PRONE )
	{
		return ( 0 );
	}

	if ( BG_PlayerMounted( ent->client->ps.eFlags) )
	{
		return ( 0 );
	}
	...
	if
	(
		ent->client->ps.weapon == WP_MORTAR_SET ||
		ent->client->ps.weapon == WP_MOBILE_MG42_SET ||
		ent->client->ps.weapon == WP_30CAL_SET
	)
	{
		return ( 0 );
	}

	if ( ent->oldkicktime && ent->oldkicktime > kicktime )
	{
		return ( 0 );
	}
	else
	{
		ent->oldkicktime = kicktime + 1000;
	}

	/*	ent->client->ps.pm_flags |= PMF_TIME_LOCKMOVE;
			ent->client->ps.pm_time = 500;*/

	// YUCK!!!
	// play the anim
	BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo, ANIM_ET_KICK, qfalse, qtrue );
	...
	traceEnt = &g_entities[tr.entityNum];
	if ( traceEnt && traceEnt->client && traceEnt->client->ps.eFlags & EF_TANK )
	{
		tank = qtrue;
	}
	...
	if ( Q_stricmp( traceEnt->classname, "func_door_rotating") == 0 )
	{
		...
		(
			(Q_stricmp( traceEnt->classname, "func_button") == 0) &&
	...
	// send bullet impact
	if ( traceEnt->takedamage && traceEnt->client && !tank )
	{
...
// DHM - Nerve
void
Weapon_RealMedic ( gentity_t *ent )
{
	trace_t		tr;
	gentity_t	*traceEnt;
	vec3_t		end;
	...
	traceEnt = &g_entities[tr.entityNum];
	if ( traceEnt && traceEnt->client && !(traceEnt->client->ps.eFlags & EF_DEAD) )
	{
		if ( traceEnt->client->ps.stats[STAT_HEALTH] < traceEnt->client->ps.stats[STAT_MAX_HEALTH] )
		{
			if
			(
				ent->client->ps.classWeaponTime +
					level.medicChargeTime[ent->client->sess.sessionTeam - 1] < level.time
			)
			{
			...
			{
				ent->client->ps.classWeaponTime += level.medicChargeTime[ent->client->sess.sessionTeam - 1] * 0.015;	// same as giving medpack
			}
			else
			{
				ent->client->ps.classWeaponTime += level.medicChargeTime[ent->client->sess.sessionTeam - 1] * 0.025;
			}
...
Weapon_Medic ( gentity_t *ent )
{
	...
	if ( ent->s.eType == ET_NPC )
	{
		offset[2] += ent->NPC_client->ps.viewheight / 2;
	}
	else
#endif //__NPC__
...
*/
void
Weapon_MagicAmmo ( gentity_t *ent )
{
	gitem_t		*item;
	gentity_t	*ent2;
	vec3_t		velocity, offset;
...
qboolean
ReviveEntity ( gentity_t *ent, gentity_t *traceEnt )
{
	vec3_t		org;
	trace_t		tr;
	...
	memcpy( powerups, traceEnt->client->ps.powerups, sizeof(int) * MAX_POWERUPS );
	ClientSpawn( traceEnt, qtrue );
	BODY_REVIVE( traceEnt ) = 0;
	...
	memcpy( traceEnt->client->ps.powerups, powerups, sizeof(int) * MAX_POWERUPS );
	if ( headshot )
	{
	...
	trap_SendServerCommand( traceEnt - g_entities, va( "cp \"You have been revived by [lof]%s[lon] [lof]%s!\n\"",
							ent->client->sess.sessionTeam == TEAM_ALLIES ? rankNames_Allies[ent->client->sess.rank] :
								rankNames_Axis[ent->client->sess.rank], ent->client->pers.netname) );
	traceEnt->props_frame_state = ent->s.number;
#ifdef __BOT__

	// log this as help so bots can reply
	ent->last_helped_client = traceEnt->client->ps.clientNum;
	ent->last_helped_time = level.time;
	if ( traceEnt->r.svFlags & SVF_BOT && !(ent->r.svFlags & SVF_BOT) )
	{
		G_AI_Voice( traceEnt, NULL, SAY_TEAM, "Thanks", qfalse, 500 + rand() % 1000, AIVOICE_TALK );
	}
#endif //__BOT__
	...
	{
		BG_AnimScriptEvent( &traceEnt->client->ps, traceEnt->client->pers.character->animModelInfo, ANIM_ET_JUMP,
							qfalse, qtrue );
	}
	else
	{
		...
		BG_AnimScriptEvent( &traceEnt->client->ps, traceEnt->client->pers.character->animModelInfo, ANIM_ET_REVIVE,
							qfalse, qtrue );
	...
	if ( tr.fraction < 1.0 )
	{
		...
		if ( traceEnt->client != NULL )
		{
			...
			(
				...
				traceEnt->client->ps.stats[STAT_HEALTH] > FORCE_LIMBO_HEALTH
			)
			{
...
*/
void
Weapon_AdrenalineSyringe ( gentity_t *ent )
{
...
int
EntsThatRadiusCanDamage ( vec3_t origin, float radius, int *damagedList )
{
	float		dist;
	gentity_t	*ent;
...
/* */
static void
MakeTemporarySolid ( gentity_t *ent )
{
...
/* */
static void
UndoTemporarySolid ( gentity_t *ent )
{
...
static void
HandleEntsThatBlockConstructible
(
	gentity_t	*constructor,
...
)
{
	...
	for ( e = 0; e < listedEntities; e++ )
	{
		...
		if ( (block = G_TestEntityPosition( check, qfalse)) == NULL )
		{
			continue;
		}
	...
	if ( handleBlockingEnts )
	{
		for ( e = 0; e < blockingEntities; e++ )
		{
			...
			{
				G_Damage( block, constructible, constructor, NULL, NULL, 9999, DAMAGE_NO_PROTECTION,
...
TryConstructing ( gentity_t *ent )
{
	...
	if ( constructible->s.eType == ET_CONSTRUCTIBLE && ent->client->touchingTOI->chain )
	{
		...
		if
		(
			otherconstructible->s.angles2[0] ||
	...
	if ( constructible->s.eType == ET_CONSTRUCTIBLE && constructible->s.teamNum == ent->client->sess.sessionTeam )
	{
		...
		if
		(
			!(constructible->spawnflags & CONSTRUCTIBLE_INVULNERABLE) &&
		...
		)
		{
			...
			else
			{
				...
				for ( i = 0, check = g_entities; i < level.num_entities; i++, check++ )
				{
					if
					(
						check->s.eType != ET_EXPLOSIVE_INDICATOR &&
...
/* */
void
AutoBuildConstruction ( gentity_t *constructible )
{
	int			i;
	gentity_t	*check;
...
/* */
qboolean
G_LandmineTriggered ( gentity_t *ent )
{
...
/* */
qboolean
G_LandmineUnarmed ( gentity_t *ent )
{
...
/* */
qboolean
G_LandmineSpotted ( gentity_t *ent )
{
...
Weapon_Engineer ( gentity_t *ent )
{
	...
	if ( ent->s.eType == ET_NPC )
	{
		...
		AngleVectors( ent->NPC_client->ps.viewangles, forward, right, up );
		VectorCopy( ent->NPC_client->ps.origin, muzzleTrace );
		muzzleTrace[2] += ent->NPC_client->ps.viewheight;
	}
	else
	{
																																								...
																																								/// tank repairing
	{
		...
		(
			traceEnt &&
			(
				(traceEnt->client && traceEnt->client->ps.eFlags & EF_TANK) || !Q_stricmp( traceEnt->classname,
																						   "churchilltank") || !Q_stricmp( traceEnt->classname, "panzertank")
			)
		)
		{
			if ( traceEnt->health < 100 )
			{
				...
				(
					ent->client->ps.classWeaponTime +
						level.engineerChargeTime[ent->client->sess.sessionTeam - 1] < level.time
				)
				{
				...
				{
					ent->client->ps.classWeaponTime += .66f * 500;
				}
				else
				{
					ent->client->ps.classWeaponTime += 500;
				}
...
notank:
	...
	if ( G_EmplacedGunIsRepairable( traceEnt, ent) )
	{
		...
		if ( ent->s.eType == ET_NPC )
		{
			if ( ent->NPC_client->ps.classWeaponTime + level.engineerChargeTime[ent->s.teamNum - 1] < level.time )
			{
				ent->NPC_client->ps.classWeaponTime = level.time - level.engineerChargeTime[ent->s.teamNum - 1];
			}

			//if( ent->NPC_client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3 ) {
			//	ent->NPC_client->ps.classWeaponTime += .66f * 150;
			//} else {
			ent->NPC_client->ps.classWeaponTime += 150;

			//}
			if ( ent->NPC_client->ps.classWeaponTime > level.time )
			{
				ent->NPC_client->ps.classWeaponTime = level.time;
				return;																						// Out of "ammo"
			}
		}
		else
...
*/
			...
			if ( ent->s.eType == ET_NPC )
			{
				...
				/*				if( traceEnt->sound3to2 != ent->s.teamNum ) {
					AddScore( ent, WOLF_REPAIR_BONUS ); // JPW NERVE props to the E for the fixin'
					G_AddSkillPoints( ent, SK_EXPLOSIVES_AND_CONSTRUCTION, 3.f );
					G_DebugAddSkillPoints( ent, SK_EXPLOSIVES_AND_CONSTRUCTION, 3.f, "repairing a MG42" );
				}*/
	...
	else
	{
		...
		if ( traceEnt->methodOfDeath == MOD_LANDMINE )
		{
			...
			if
			(
				!(tr2.surfaceFlags & SURF_LANDMINE) ||
				(
			...
			)
			{
				...
				//} else if ( ent->s.eType != ET_NPC && G_CountTeamLandmines(ent->client->sess.sessionTeam) >= MAX_TEAM_LANDMINES && G_LandmineTeam(traceEnt) == ent->client->sess.sessionTeam) {
			}
			else if
				(
					ent->s.eType != ET_NPC &&
					G_CountTeamLandmines( ent->client->sess.sessionTeam) >= MAX_TEAM_LANDMINES &&
			...
			if ( traceEnt->s.teamNum >= 4 )
			{
				...
				}
				{
					...
					for ( e = 0; e < numListedEntities; e++ )
					{
						...
						if
						(
							hit->spawnflags & CONSTRUCTIBLE_INVULNERABLE ||
				...
				for ( i = 0; i < num; i++ )
				{
					...
					if ( (hit->s.eType == ET_OID_TRIGGER) )
					{
						...
						if
						(
							((hit->spawnflags & AXIS_OBJECTIVE) && (ent->client->sess.sessionTeam == TEAM_AXIS)) ||
				...
				for ( i = 0; i < num; i++ )
				{
					...
					if ( (hit->s.eType == ET_OID_TRIGGER) )
					{
						...
						)
						{
							...
							{
								Bot_Util_AddGoal( (GameEntity *) traceEnt, ET_GOAL_DYNAMITE, (1 << ET_TEAM_ALLIES),
												  NULL, NULL );
							}
							else
							{
								Bot_Util_AddGoal( (GameEntity *) traceEnt, ET_GOAL_DYNAMITE, (1 << ET_TEAM_AXIS), NULL,
												  NULL );
							}
#endif
				...
				// Arnout - first see if the dynamite is planted near a constructable object that can be destroyed
				{
					...
					for ( e = 0; e < numListedEntities; e++ )
					{
						...
						if ( hit->parent )
						{
							...
							if
							(
								(!(hit->parent->spawnflags & OBJECTIVE_DESTROYED)) &&
								hit->s.teamNum &&
								(hit->s.teamNum == ent->client->sess.sessionTeam)
							)
							{														// ==, as it's inverse
								AddScore( traceEnt->parent, WOLF_DYNAMITE_PLANT );	// give drop score to guy who dropped it
								if ( traceEnt->parent && traceEnt->parent->client )
...
/* */
void
G_AirStrikeExplode ( gentity_t *self )
{
...
/* */
qboolean
G_AvailableAirstrikes ( gentity_t *ent )
{
	qboolean	axis = qtrue;
	qboolean	allies = qtrue;
	if ( level.axisBombCounter >= 60 * 1000 )
	{
		axis = qfalse;
		trap_Cvar_Set( "g_axis_strike", "0" );
	}
	else
	{
		trap_Cvar_Set( "g_axis_strike", "1" );
	}

	if ( level.alliedBombCounter >= 60 * 1000 )
	{
		allies = qfalse;
		trap_Cvar_Set( "g_allied_strike", "0" );
	}
	else
	{
		trap_Cvar_Set( "g_allied_strike", "1" );
	}
...
/* */
void
G_AddAirstrikeToCounters ( gentity_t *ent )
{
...
/* */
void
weapon_checkAirStrikeThink1 ( gentity_t *ent )
{
	if ( !weapon_checkAirStrike( ent) )
	{
		if ( g_safemarkers.integer > 0 )
		{
...
/* */
void
weapon_checkAirStrikeThink2 ( gentity_t *ent )
{
	if ( !weapon_checkAirStrike( ent) )
	{
		if ( g_safemarkers.integer > 0 )
...
/* */
void
weapon_callSecondPlane ( gentity_t *ent )
{
	gentity_t	*te;
...
/* */
qboolean
weapon_checkAirStrike ( gentity_t *ent )
{
	...
	if ( ent->s.teamNum == TEAM_AXIS )
	{
		...
		{
			G_SayTo( ent->parent, ent->parent, 2, COLOR_YELLOW, "HQ: ", "All available planes are already en-route.",
					 qtrue );
...
/* */
void
weapon_callAirStrike ( gentity_t *ent )
{
	int			i, j;
	vec3_t		bombaxis, lookaxis, pos, bomboffset, fallaxis, temp, dir, skypoint, check, check2;
	gentity_t	*bomb;
	trace_t		tr;
	trace_t		bodycheck;
	float		traceheight, bottomtraceheight;
	qboolean	body_check = qfalse;
	VectorCopy( ent->s.pos.trBase, bomboffset );
	bomboffset[2] += 4096.f;
	VectorCopy( ent->s.pos.trBase, check );
	check[2] -= 8.f;
	VectorCopy( ent->s.pos.trBase, check2 );
	check2[2] += 16.f;
	...
	trap_Trace( &bodycheck, check, NULL, NULL, check2, ent->s.number, CONTENTS_BODY | CONTENTS_CORPSE );
	if ( bodycheck.fraction < 1.0 )
	{
		if ( g_entities[bodycheck.entityNum].client )
		{
			if ( g_entities[bodycheck.entityNum].client->ps.eFlags & EF_PRONE )
			{
				body_check = qtrue;
			}
		}

		if ( g_entities[bodycheck.entityNum].s.eType == ET_CORPSE )
		{
			body_check = qtrue;
		}
	}
	...
	{
		gentity_t	*tent;
		tent = G_TempEntity( ent->s.pos.trBase, EV_RAILTRAIL );
		VectorCopy( tr.endpos, tent->s.origin2 );
	}
	else if ( g_debugBullets.integer == 2 )
	{
		gentity_t	*tent2;
		tent2 = G_TempEntity( check, EV_RAILTRAIL );
		VectorCopy( bodycheck.endpos, tent2->s.origin2 );
	}

	if ( (tr.fraction < 1.0) && (!(tr.surfaceFlags & SURF_NOIMPACT)) || (body_check == qtrue) )
...
void
artilleryThink_real ( gentity_t *ent )
{
...
void
artillerySpotterThink ( gentity_t *ent )
{
	gentity_t	*bomb;
	vec3_t		tmpdir;
	int			i;
...
/* */
void
G_GlobalClientEvent ( int event, int param, int client )
{
...
*/
void
Weapon_Artillery ( gentity_t *ent )
{
	trace_t		trace;
	int			i, count;
	...
	AddLean( ent, muzzlePoint );						// jaquboss
...
#ifdef __BOT__
	if ( (ent->r.svFlags & SVF_BOT) && VectorDistance( ent->r.currentOrigin, trace.endpos) < 1024 )
	{
		return;											// Bots should not arty too close to themselves...
	}
#endif //__BOT__
	...
	if ( ent->client->sess.skill[SK_SIGNALS] >= 2 )
	{
		...
		(
			level.time -
				ent->client->ps.classWeaponTime > level.lieutenantChargeTime[ent->client->sess.sessionTeam - 1]
		)
		{
			ent->client->ps.classWeaponTime = level.time - level.lieutenantChargeTime[ent->client->sess.sessionTeam - 1];
		}
...
// quickly
void
weapon_smokeBombExplode ( gentity_t *ent )
{
	int lived = 0;
...
G_GetWeaponDamage ( int weapon )
{
	switch ( weapon )
	{
		default:
			return ( 1 );
		case WP_KNIFE:
			return ( 25 * g_knifeDamage.value );
		case WP_STEN:
			return ( 40 * g_smgDamage.value );
		case WP_CARBINE:
		case WP_GARAND:
		case WP_KAR98:
		case WP_K43:
			return ( 85 * g_rifleDamage.value );
		case WP_FG42:
			return ( 50 * g_rifleDamage.value );
		case WP_LUGER:
		case WP_SILENCER:
		...
		case WP_AKIMBO_SILENCEDCOLT:
			return ( 40 * g_pistolDamage.value );
	#ifdef NWEAPS
		case WP_MP44:
			return ( 55 * g_rifleDamage.value );
		case WP_BAR:
			return ( 60 * g_rifleDamage.value );
	#endif
		case WP_THOMPSON:
			return ( 45 * g_smgDamage.value );
		case WP_MP40:
			return ( 40 * g_smgDamage.value );
		case WP_30CAL:
		case WP_30CAL_SET:
			return ( 55 * g_mgDamage.value );

		// like for static mg42;
		case WP_MOBILE_MG42:
		case WP_MOBILE_MG42_SET:
			return ( 32 * g_mgDamage.value );

		// fixme do something to balance machineguns..
		case WP_SMOKE_MARKER:
			return ( 140 * g_explosivesDamage.value );

		// just enough to kill somebody standing on it
		case WP_MAPMORTAR:
		...
		case WP_LANDMINE:
		case WP_SATCHEL:
			return ( 250 * g_explosivesDamage.value );

		//		case WP_TRIPMINE:
		//			return 300;
		case WP_PANZERFAUST:
			return ( 750 * g_explosivesDamage.value );
		case WP_MORTAR_SET:
			return ( 400 * g_explosivesDamage.value );
		case WP_DYNAMITE:
			return ( 1000 * g_explosivesDamage.value );
	}
}
...
G_GetWeaponSpread ( int weapon )
{
	switch ( weapon )
	{
		...
		case WP_COLT:
		case WP_SILENCED_COLT:
			return ( 400 * g_pistolSpread.value );
		case WP_AKIMBO_LUGER:
		case WP_AKIMBO_SILENCEDLUGER:
		case WP_AKIMBO_COLT:
		case WP_AKIMBO_SILENCEDCOLT:
			return ( 400 * g_pistolSpread.value );
		case WP_MP40:
			return ( 350 * g_smgSpread.value );
		case WP_THOMPSON:
			return ( 400 * g_smgSpread.value );
		case WP_STEN:
			return ( 300 * g_smgSpread.value );
		case WP_FG42:
			return ( 350 * g_rifleSpread.value );
		case WP_GARAND:
		case WP_K43:
			return ( 450 * g_rifleSpread.value );
		case WP_CARBINE:
		case WP_KAR98:
			return ( 350 * g_rifleSpread.value );
		case WP_MOBILE_MG42_SET:
			return ( 350 * g_mgSpread.value );
		case WP_30CAL_SET:
			return ( 250 * g_mgSpread.value );
		case WP_MOBILE_MG42:
			return ( 400 * g_mgSpread.value );
		case WP_30CAL:
			return ( 300 * g_mgSpread.value );
	#ifdef NWEAPS
		case WP_MP44:
			return ( 225 * g_rifleSpread.value );
		case WP_BAR:
			return ( 275 * g_rifleSpread.value );
	#endif
	}

	// Kamikazee: [FIXME] It does get here in some rare cases (Weapon 40, using bots)
...
#define MP44_DAMAGE G_GetWeaponDamage( WP_MP44 )					// JPW
#define BAR_SPREAD	G_GetWeaponSpread( WP_BAR )
#define BAR_DAMAGE	G_GetWeaponDamage( WP_BAR )						// JPW
#endif
...
#define BROWNING_SET_SPREAD G_GetWeaponSpread( WP_30CAL_SET )
#define BROWNING_SET_DAMAGE G_GetWeaponDamage( WP_30CAL_SET )

/*#define BROWNING_SPREAD	G_GetWeaponSpread(WP_30CAL)
#define	BROWNING_DAMAGE	G_GetWeaponDamage(WP_30CAL)*/
#define MOBILE_MG42_SET_SPREAD	G_GetWeaponSpread( WP_MOBILE_MG42_SET )
#define MOBILE_MG42_SET_DAMAGE	G_GetWeaponDamage( WP_MOBILE_MG42_SET )
...
/* */
void
RubbleFlagCheck ( gentity_t *ent, trace_t tr )
{
...
*/
void
EmitterCheck ( gentity_t *ent, gentity_t *attacker, trace_t *tr )
{
	gentity_t	*tent;
	vec3_t		origin;
...
*/
void
Bullet_Endpos ( gentity_t *ent, float spread, vec3_t *end )
{
	float		r, u;
	...
	r = crandom() * spread;
	u = crandom() * spread;
	if ( ent->client && BG_IsScopedWeapon2( ent->s.weapon) && ent->client->ps.eFlags & EF_AIMING )
	{
...
*/
void
Bullet_Fire ( gentity_t *ent, float spread, int damage, qboolean distance_falloff )
{
	vec3_t	end;
	...
	switch ( ent->s.weapon )
	{
		...
		case WP_SILENCER:
		case WP_SILENCED_COLT:
			if ( ent->client && ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 3 )
			{
				spread *= .65f;
			}
			break;
	...
	if ( ent->client && ent->client->ps.eFlags & EF_AIMING )
	{
		switch ( ent->s.weapon )
		{
			case WP_KAR98:
...
)
{
	...
	if ( distance_falloff )
	{
		...
		scale = ( dist - 2500.f ) / ( 3500.f - 2500.f );
	...
	if ( distance_falloff )
	{
		flags |= DAMAGE_DISTANCEFALLOFF;
	}

	// send bullet impact
	//	if ( traceEnt->takedamage && traceEnt->client && !(traceEnt->client->ps.eFlags & EF_TANK) ) {
#ifdef __NPC__
	if
	(
		traceEnt->takedamage &&
		(traceEnt->client || traceEnt->s.eType == ET_NPC) &&
		!(traceEnt->client && traceEnt->client->ps.eFlags & EF_TANK)
	)
#else //!__NPC__
		if ( traceEnt->takedamage && (traceEnt->client) /*&& !(traceEnt->flags & FL_DEFENSE_GUARD)*/ )
#endif //__NPC__
		{
			gentity_t	*tent = G_TempEntity( tr.endpos, EV_BULLET_HIT_FLESH );
			tent->s.eventParm = traceEnt->s.number;
	...
	// dont harm tanks and so
	//if ( (traceEnt->client && traceEnt->client->ps.eFlags & EF_TANK) || traceEnt->s.eType == ET_VEHICLE ) {
	//	//return qfalse;
	//	damage = 1; // UQ1: Do slight damage!
	//}
	if ( traceEnt->takedamage
#ifdef __NPC__
	...
	)
	{
		...
		(
			attacker &&
			attacker->client &&
			BG_IsScopedWeapon2( attacker->client->ps.weapon) &&
			attacker->client->ps.eFlags & EF_AIMING
		)
		{
			meansOfDeath_t	mod;
			mod = GetAmmoTableData( attacker->s.weapon )->mod;
...
#ifdef BONE_HITTESTS
			if ( traceEnt->client && g_mdx_hit_test.integer > 0 )
			{
				G_PlayerBulletDamage( traceEnt, attacker, attacker, start, tr.endpos, forward, damage, flags, mod );
			}
			else
			{
#endif
				G_Damage( traceEnt, attacker, attacker, forward, tr.endpos, damage, flags, mod );
#ifdef BONE_HITTESTS
			}
#endif
		}
		else
		{
			...
			(
				traceEnt->client &&
				!G_WeaponIsExplosive( GetAmmoTableData( attacker->s.weapon)->mod) &&
				g_mdx_hit_test.integer > 0
			)
			{
				G_PlayerBulletDamage( traceEnt, attacker, attacker, start, tr.endpos, forward, damage, flags,
									  GetAmmoTableData( attacker->s.weapon)->mod );
			}
			else
			{
#endif
				G_Damage( traceEnt, attacker, attacker, forward, tr.endpos, damage, flags,
						  GetAmmoTableData( attacker->s.weapon)->mod );
#ifdef BONE_HITTESTS
			}
#endif
		...
		if ( traceEnt->s.eType == ET_EXPLOSIVE  && traceEnt->health <= damage )
		{
			...
			return ( Bullet_Fire_Extended( traceEnt, attacker, tr.endpos, end, 0, damage, distance_falloff, trynum + 1) );
		}
	}
...
/* */
gentity_t *
weapon_gpg40_fire ( gentity_t *ent, int grenType )
{
	...
	if ( ent->s.eType == ET_NPC )
	{
		viewpos[2] += ent->NPC_client->ps.viewheight;
	}
	else
#endif //__NPC__
...
/* */
gentity_t *
weapon_mortar_fire ( gentity_t *ent, int grenType )
{
	gentity_t	*m;
	trace_t		tr;
...
/* */
gentity_t *
weapon_grenadelauncher_fire ( gentity_t *ent, int grenType )
{
	gentity_t	*m;
	trace_t		tr;
	vec3_t		viewpos;
	...
	{
		trap_Trace( &tr, viewpos, tv( -12.f, -12.f, 0.f), tv( 12.f, 12.f, 20.f), tosspos, ent->s.number,
					MASK_MISSILESHOT );
	}
	else if ( grenType == WP_LANDMINE )
	{
		trap_Trace( &tr, viewpos, tv( -16.f, -16.f, 0.f), tv( 16.f, 16.f, 16.f), tosspos, ent->s.number,
					MASK_MISSILESHOT );
	}
	else
	{
	...
	// jpw
	//----(SA)	adjust for movement of character.  TOD-O: Probably comment in later, but only for forward/back not strafing
...
*/
void
Weapon_Panzerfaust_Fire ( gentity_t *ent )
{
	gentity_t	*m;
	qboolean	gravited = qfalse;
	if ( g_panzerfaust_gravited.integer > 0 )
	{
		gravited = qtrue;
	}

	m = fire_rocket( ent, muzzleEffect, forward, gravited );
...
/* */
void
Weapon_FlamethrowerFire ( gentity_t *ent )
{
	gentity_t	*traceEnt;
	vec3_t		start;
	vec3_t		trace_start;
	...
	if ( ent->s.eType == ET_NPC )
	{
		start[2] += ent->NPC_client->ps.viewheight;
	}
	else
#endif //__NPC__
		start[2] += ent->client->ps.viewheight;
	...
	if ( ent->s.eType == ET_NPC )
	{
		ent->NPC_client->flametime = level.time + 2500;
	}
	else
#endif //__NPC__
...
*/
qboolean
AccuracyHit ( gentity_t *target, gentity_t *attacker )
{
	...
	if ( OnSameTeam( target, attacker) && g_gametype.integer != GT_WOLF_DM )
	{
		return ( qfalse );
	}
...
*/
void
CalcMuzzlePoint ( gentity_t *ent, int weapon, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint )
{
	VectorCopy( ent->r.currentOrigin, muzzlePoint );
	...
	if ( ent->s.eType == ET_NPC )
	{
		...
		if ( ent->NPC_client->ps.viewangles[PITCH] > 30 && (ent->NPC_client->ps.eFlags & EF_CROUCHING) )
		{
			muzzlePoint[2] += 30;
		}
		else
		{
			muzzlePoint[2] += ent->NPC_client->ps.viewheight;
		}
	}
	else
#endif //__NPC__
	...
	switch ( weapon )	// Ridah, changed this so I can predict weapons
	{
		case WP_PANZERFAUST:
			if ( ent->client && ent->client->ps.eFlags & EF_AIMING )
			{
				VectorMA( muzzlePoint, ent->client->ps.leanf / 5, right, muzzlePoint ); // add slight value of lean
			}
			else
			{
...
// Rafael - for activate
void
CalcMuzzlePointForActivate ( gentity_t *ent, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint )
{
...
// Ridah
void
CalcMuzzlePoints ( gentity_t *ent, int weapon )
{
	vec3_t	viewang;
#ifdef __NPC__
	if ( ent->s.eType == ET_NPC )
	{
		VectorCopy( ent->NPC_client->ps.viewangles, viewang );
	}
	else
#endif //__NPC__
...
#ifndef PMOVE_SWAY
		if ( ent->client && ent->client->ps.eFlags & EF_AIMING || ent->client && ent->client->ps.eFlags & EF_ZOOMING )
		{
			...
			if
			(
				(BG_IsScopedWeapon2( weapon) && !(ent->client->ps.eFlags & EF_PRONE)) ||
				(ent->client->ps.weaponTime > 50 && ent->client->ps.weaponstate == WEAPON_FIRING)
			)
			{
...
/* */
qboolean
G_PlayerCanBeSeenByOthers ( gentity_t *ent )
{
	int			i;
	gentity_t	*ent2;
	vec3_t		pos[3];
	...
	for ( i = 0, ent2 = g_entities; i < level.maxclients; i++, ent2++ )
	{
		...
		if
		(
			G_VisibleFromBinoculars( ent2, ent, pos[0]) ||
...
FireWeapon ( gentity_t *ent )
{
	...
	// Rafael mg42
	if ( ent->client && ent->client->ps.persistant[PERS_HWEAPON_USE] && ent->active )
	{
		return;
	}
	...
	if ( g_userAim.integer )
	{
		...
		if ( ent->s.eType == ET_NPC )
		{
			aimSpreadScale = ent->NPC_client->currentAimSpreadScale;
		}
		else
		{
	...
	switch ( ent->s.weapon )
	{
		...
		case WP_SMOKE_MARKER:
			...
			if ( ent->s.eType == ET_NPC )
			{
				if ( level.time - ent->NPC_client->ps.classWeaponTime > level.lieutenantChargeTime[ent->s.teamNum - 1] )
				{
					ent->NPC_client->ps.classWeaponTime = level.time - level.lieutenantChargeTime[ent->s.teamNum - 1];
				}
		...
		case WP_SATCHEL_DET:
			...
			if ( ent->s.eType == ET_NPC )
			{
				...
				{
					ent->NPC_client->ps.ammo[WP_SATCHEL_DET] = 0;
					ent->NPC_client->ps.ammoclip[WP_SATCHEL_DET] = 0;
					ent->NPC_client->ps.ammoclip[WP_SATCHEL] = 1;
		...
		break;*/
		case WP_30CAL:
			Bullet_Fire( ent, BROWNING_SPREAD * aimSpreadScale, BROWNING_DAMAGE, qfalse );	// uses info from static browning
			break;
		case WP_30CAL_SET:
			Bullet_Fire( ent, BROWNING_SET_SPREAD * aimSpreadScale, BROWNING_SET_DAMAGE, qfalse );
			break;
		case WP_MOBILE_MG42_SET:
			Bullet_Fire( ent, MOBILE_MG42_SET_SPREAD * aimSpreadScale, MOBILE_MG42_SET_DAMAGE, qfalse );
			break;
		case WP_MOBILE_MG42:
	#ifdef __NPC__
			if ( ent->s.eType == ET_NPC )
			{
				if ( ent->NPC_client->ps.pm_flags & PMF_DUCKED || ent->NPC_client->ps.eFlags & EF_PRONE )
				{
	...
	#ifdef NWEAPS
		case WP_MP44:
			Bullet_Fire( ent, MP44_SPREAD * aimSpreadScale, MP44_DAMAGE, qtrue );
			break;
		case WP_BAR:
			Bullet_Fire( ent, BAR_SPREAD * aimSpreadScale, BAR_DAMAGE, qtrue );
			break;
	#endif
		case WP_MP40:
		...
		case WP_PANZERFAUST:
			...
			if ( ent->s.eType == ET_NPC )
			{
				if ( level.time - ent->NPC_client->ps.classWeaponTime > level.soldierChargeTime[ent->s.teamNum - 1] )
				{
					ent->NPC_client->ps.classWeaponTime = level.time - level.soldierChargeTime[ent->s.teamNum - 1];
				}
			...
			else
			{
				...
				(
					level.time -
						ent->client->ps.classWeaponTime > level.soldierChargeTime[ent->client->sess.sessionTeam - 1]
				)
				{
		...
		case WP_M7:
			...
			if ( ent->s.eType == ET_NPC )
			{
				if ( level.time - ent->NPC_client->ps.classWeaponTime > level.engineerChargeTime[ent->s.teamNum - 1] )
				{
					ent->NPC_client->ps.classWeaponTime = level.time - level.engineerChargeTime[ent->s.teamNum - 1];
				}
			...
			else
			{
				...
				(
					level.time -
						ent->client->ps.classWeaponTime > level.engineerChargeTime[ent->client->sess.sessionTeam - 1]
				)
				{
					ent->client->ps.classWeaponTime = level.time - level.engineerChargeTime[ent->client->sess.sessionTeam - 1];
				}
		...
		case WP_MORTAR_SET:
			...
			if ( ent->s.eType == ET_NPC )
			{
				...
				ent->NPC_client->ps.classWeaponTime += .5f * level.soldierChargeTime[ent->s.teamNum - 1];
		...
		case WP_SMOKE_BOMB:
			...
			if ( ent->s.eType == ET_NPC )
			{
				...
				{
					if ( level.time - ent->NPC_client->ps.classWeaponTime > level.covertopsChargeTime[ent->s.teamNum - 1] )
					{
						ent->NPC_client->ps.classWeaponTime = level.time - level.covertopsChargeTime[ent->s.teamNum - 1];
					}
			...
			else
			{
				if ( ent->s.weapon == WP_SMOKE_BOMB || ent->s.weapon == WP_SATCHEL )
				{
					...
					(
						level.time -
							ent->client->ps.classWeaponTime > level.covertopsChargeTime[ent->client->sess.sessionTeam - 1]
					)
					{
	...
	if ( g_gamestate.integer == GS_PLAYING )
		...
		if ( ent->s.eType == ET_NPC )
		{
			ent->NPC_client->sess.aWeaponStats[BG_WeapStatForWeapon( ent->s.weapon )].atts += shots;
		}
		else
		{
			ent->client->sess.aWeaponStats[BG_WeapStatForWeapon( ent->s.weapon )].atts += shots;
...
//
qboolean
IsSilencedWeapon ( // The type of weapon in question.  Is it silenced?
int weaponType )
{
...
//
void
MuzzleRayTracing ( gentity_t *ent, int weapon )
{
	gentity_t	*tent;
	CalcMuzzlePoints( ent, weapon );
	tent = G_TempEntity( muzzleEffect, EV_RAILTRAIL );
	tent->s.otherEntityNum2 = ent->s.number;
	VectorMA( muzzleTrace, 8192, forward, tent->s.origin2 );
	VectorCopy( muzzleTrace, ent->s.angles2 );
}
