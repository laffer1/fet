...
/* */
int
trap_FS_Rename ( const char *from, const char *to )
{
...
/* */
void
trap_DropClient ( int clientNum, const char *reason, int length )
{
...
trap_EntitiesInBox ( const vec3_t mins, const vec3_t maxs, int *list, int maxcount )
{
	...
	// UQ1: Will just add ent 0.. Is faster this way...
	gentity_t	*gcheck = &g_entities[0];
	int			count = syscall( G_ENTITIES_IN_BOX, mins, maxs, list, maxcount );
	vec3_t		temp_mins, temp_maxs;
	VectorCopy( mins, temp_mins );
	VectorCopy( maxs, temp_maxs );
	if ( gcheck )
	{
		vec3_t	entMins, entMaxs;
		VectorCopy( gcheck->r.mins, entMins );
		VectorCopy( gcheck->r.maxs, entMaxs );
		entMins[0] += gcheck->r.currentOrigin[0];
		entMins[1] += gcheck->r.currentOrigin[1];
		entMins[2] += gcheck->r.currentOrigin[2];
		entMaxs[0] += gcheck->r.currentOrigin[0];
		entMaxs[1] += gcheck->r.currentOrigin[1];
		entMaxs[2] += gcheck->r.currentOrigin[2];
		if
		(
			entMins[0] < temp_maxs[0] &&
			entMins[1] < temp_maxs[1] &&
			entMins[2] < temp_maxs[2] &&
			entMaxs[0] > temp_mins[0] &&
			entMaxs[1] > temp_mins[1] &&
			entMaxs[2] > temp_mins[2]
		)
		{	// Add local entity...
			list[count] = 0;
			count++;
...
/* */
int
trap_BotAllocateClient ( int clientNum )
{
...
/* */
int
trap_GetSoundLength ( sfxHandle_t sfxHandle )
{
...
/* */
sfxHandle_t
trap_RegisterSound ( const char *sample, qboolean compressed )
{
...
/* */
qboolean
trap_GetTag ( int clientNum, int tagFileNumber, char *tagName, orientation_t *or )
{
...
/* */
qboolean
trap_LoadTag ( const char *filename )
{
...
void
trap_AAS_SetCurrentWorld ( int index )
{
...
/* */
void
trap_AAS_AreaCenter ( int areanum, vec3_t center )
{
...
/* */
qboolean
trap_AAS_AreaWaypoint ( int areanum, vec3_t center )
{
...
/* */
int
trap_AAS_AreaLadder ( int areanum )
{
...
(
	void /* struct aas_clientmove_s */ *move,
	int									entnum,
	vec3_t								origin,
	int									presencetype,
	int									onground,
...
)
{
	return
		(
			syscall( BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT, move, entnum, origin, presencetype, onground, velocity,
...
// Ridah, route-tables
void
trap_AAS_RT_ShowRoute ( vec3_t srcpos, int srcnum, int destnum )
{
...
)
{
	return
		(
			syscall( BOTLIB_AAS_NEARESTHIDEAREA, srcnum, origin, areanum, enemynum, enemyorigin, enemyareanum,
					 travelflags, PASSFLOAT( maxdist), distpos)
		);
}
...
/* */
int
trap_AAS_ListAreasInRange ( vec3_t srcpos, int srcarea, float range, int travelflags, float **outareas, int maxareas )
{
...
/* */
int
trap_AAS_AvoidDangerArea ( vec3_t srcpos, int srcarea, vec3_t dangerpos, int dangerarea, float range, int travelflags )
{
...
)
{
	return
		(
			syscall( BOTLIB_AAS_RETREAT, dangerSpots, dangerSpotCount, srcpos, srcarea, dangerpos, dangerarea,
					 PASSFLOAT( range), PASSFLOAT( dangerRange), travelflags)
		);
}
...
/* */
void
trap_AAS_SetAASBlockingEntity ( vec3_t absmin, vec3_t absmax, int blocking )
{
...
/* */
void
trap_AAS_RecordTeamDeathArea ( vec3_t srcpos, int srcarea, int team, int teamCount, int travelflags )
{
...
/* */
void
trap_EA_UseItem ( int client, char *it )
{
...
/* */
void
trap_EA_DropItem ( int client, char *it )
{
...
/* */
void
trap_EA_UseInv ( int client, char *inv )
{
...
/* */
void
trap_EA_DropInv ( int client, char *inv )
{
...
/* */
void
trap_EA_ResetInput ( int client, void *init )
{
...
/* */
int
trap_BotLoadCharacter ( char *charfile, int skill )
{
...
/* */
int
trap_BotFindMatch ( char *str, void /* struct bot_match_s */ *match, unsigned long int context )
{
...
/* */
void
trap_BotSetChatName ( int chatstate, char *name )
{
...
// Ridah
void
trap_BotInitAvoidReach ( int handle )
{
...
/* */
void
trap_PbStat ( int clientNum, char *category, char *values )
{
...
/* */
void
trap_SendMessage ( int clientNum, char *buf, int buflen )
{
...
/* */
messageStatus_t
trap_MessageStatus ( int clientNum )
{
