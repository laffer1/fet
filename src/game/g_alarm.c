...
*/
void
alarmbox_updateparts ( gentity_t *ent, qboolean matestoo )
{
	gentity_t	*t, *mate;
...
*/
void
alarmbox_use ( gentity_t *ent, gentity_t *other, gentity_t *foo )
{
	if ( !(ent->active) )
	{
		return;
...
*/
void
alarmbox_die ( gentity_t *ent, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
	propExplosion( ent );
	ent->s.frame = 2;
	ent->active = qfalse;
...
*/
void
alarmbox_finishspawning ( gentity_t *ent )
{
	gentity_t	*mate;
...
// UQ1: Use RTCW Single Player Alarms.. These actually work!
#include "g_local.h"
...
void alarmbox_use(gentity_t *ent, gentity_t *other, gentity_t *foo)
{
	...
	if(ent->s.frame)
	{
		...
		while ( (trent = G_Find( trent, FOFS( classname), "target_speaker")) )
		{
			if ( trent->spawnflags == 4 || trent->spawnflags == 34 )
		...
		while ( (trent = G_Find( trent, FOFS( classname), "alarm_box")) )
		{
			if (trent->s.frame != 2)
				trent->s.frame = 0;
		}
	...
	else
	{
		...
		while ( (trent = G_Find( trent, FOFS( classname), "target_speaker")) )
		{
			if ( trent->spawnflags == 4 || trent->spawnflags == 34 )
			{// Switch off the sound!
				trent->s.loopSound = ent->soundLoop;
			}
		}
...
void SP_alarm_box(gentity_t *ent)
{
	...
	{// UQ1: Add default sound!
		ent->soundLoop = G_SoundIndex( "sound/world/siren.wav" );
	}
