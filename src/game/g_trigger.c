...
/* */
void
trigger_push_use ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
...
/* */
void
heal_touch ( gentity_t *self, gentity_t *other, trace_t *trace )
{
	int			i, clientcount = 0;
	...
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
		...
		if
		(
			level.clients[j].ps.stats[STAT_MAX_HEALTH] > g_entities[j].health &&
			trap_EntityContactCapsule( g_entities[j].r.absmin, g_entities[j].r.absmax, self) &&
			G_IsAllowedHeal( &g_entities[j])
		)
		{
...
/* */
void
trigger_heal_think ( gentity_t *self )
{
...
/* */
void
trigger_heal_setup ( gentity_t *self )
{
...
*/
void
SP_misc_cabinet_health ( gentity_t *self )
{
...
*/
void
SP_trigger_heal ( gentity_t *self )
{
	char	*spawnstr;
	int		healvalue;
	InitTrigger( self );
...
/* */
void
ammo_touch ( gentity_t *self, gentity_t *other, trace_t *trace )
{
...
/* */
void
trigger_ammo_think ( gentity_t *self )
{
...
/* */
void
trigger_ammo_setup ( gentity_t *self )
{
	self->target_ent = G_FindByTargetname( NULL, self->target );
	if ( !self->target_ent )
	{
...
*/
void
SP_misc_cabinet_supply ( gentity_t *self )
{
	VectorSet( self->r.mins, -20, -20, 0 );
	VectorSet( self->r.maxs, 20, 20, 60 );
	G_SetOrigin( self, self->s.origin );
...
*/
void
SP_trigger_ammo ( gentity_t *self )
{
	char	*spawnstr;
	int		ammovalue;
	InitTrigger( self );
...
void
trigger_aidoor_stayopen ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	gentity_t	*door;
...
/* */
void
SP_trigger_aidoor ( gentity_t *ent )
{
	if ( !ent->targetname )
	{
...
/* */
void
Touch_flagonly ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	gentity_t	*tmp;
	if ( !other->client )
	{
...
/* */
void
Touch_flagonly_multiple ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	gentity_t	*tmp;
	if ( !other->client )
	{
...
*/
void
SP_trigger_flagonly ( gentity_t *ent )
{
...
*/
void
SP_trigger_flagonly_multiple ( gentity_t *ent )
{
	char	*scorestring;	// JPW NERVE
...
void
explosive_indicator_think ( gentity_t *ent )
{
	gentity_t	*parent;
...
void
constructible_indicator_think ( gentity_t *ent )
{
	gentity_t	*parent;
...
/* */
void
G_SetConfigStringValue ( int num, const char *key, const char *value )
{
...
/* */
void
Touch_ObjectiveInfo ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	if ( !other->client )
	{
		return;
...
void
Think_SetupObjectiveInfo ( gentity_t *ent )
{
	...
	if ( ent->target_ent->s.eType == ET_EXPLOSIVE )
	{
		...
		if ( (ent->spawnflags & AXIS_OBJECTIVE) || (ent->spawnflags & ALLIED_OBJECTIVE) )
		{
			...
			// Bots require an origin for goals... Copy it!
			if ( e->s.origin[0] == 0 && e->s.origin[1] == 0 && e->s.origin[2] == 0 )
			{
				VectorCopy( e->s.pos.trBase, e->s.origin );
				VectorCopy( e->s.pos.trBase, ent->s.origin );
			}
#endif //__BOT__
			trap_LinkEntity( e );
	...
	else if ( ent->target_ent->s.eType == ET_CONSTRUCTIBLE )
	{
		...
		else
		{
			...
			//VectorCopy(e->s.pos.trBase, ent->s.pos.trBase);
			//G_Printf("Constructable at %f %f %f.\n", e->s.pos.trBase[0], e->s.pos.trBase[1], e->s.pos.trBase[2]);
#endif //__BOT__
...
/* */
void
SP_trigger_objective_info ( gentity_t *ent )
{
	char	*scorestring;
	char	*customimage;
...
void
trigger_concussive_touch ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	return; // FIXME this should be NULLed out in SP_trigger_concussive_dust after everything works
	G_Printf( "hit concussive ent %d mins=%f,%f,%f maxs=%f,%f,%f\n", ent - g_entities, ent->r.mins[0], ent->r.mins[1],
			  ent->r.mins[2], ent->r.maxs[0], ent->r.maxs[1], ent->r.maxs[2] );
}
...
*/
void
SP_trigger_concussive_dust ( gentity_t *self )
{
	InitTrigger( self );
...
void AICast_Touch_Trigger( gentity_t *self, gentity_t *other, trace_t *trace ) {
	if( !other->client
#ifdef __NPC__
		|| other->NPC_client/*(other->r.svFlags & SVF_CASTAI)*/
#endif
		) {
		return;
	}
