...
*/
static void
UpdateIPBans ( ipFilterList_t *ipFilterList )
{
	byte	b[4];
	byte	m[4];
	int		i, j;
...
*/
qboolean
G_FilterPacket ( ipFilterList_t *ipFilterList, char *from )
{
	int			i;
	unsigned	in;
	byte		m[4];
...
/* */
qboolean
G_FilterMaxLivesIPPacket ( char *from )
{
...
*/
qboolean
G_FilterMaxLivesPacket ( char *from )
{
	int i;
	for ( i = 0; i < numMaxLivesFilters; i++ )
	{
...
*/
void
AddIP ( ipFilterList_t *ipFilterList, const char *str )
{
	int i;
	for ( i = 0; i < ipFilterList->numIPFilters; i++ )
	{
...
*/
gclient_t *
G_GetPlayerByNum ( int clientNum )
{
	gclient_t	*cl;
...
*/
void
Svcmd_ResetMatch_f ( qboolean fDoReset, qboolean fDoRestart )
{
	int i;
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
...
#ifdef __BOT__
void	SOD_SPAWN_SaveBots ( void );
#endif //__BOT__

/* */
void
Svcmd_Campaign_f ( void )
{
	char				str[MAX_TOKEN_CHARS];
	int					i;
...
Svcmd_Kick_f ( void )
{
	...
	if ( !cl )
	{
		...
		{
			G_Printf( "Kicking everyone out...\n" );
			for( i = 0; i < level.maxclients; i++ )
			{
				if (
					g_entities[i].inuse
					&& g_entities[i].client != 0
				...
				)
				{
					if ( timeout != -1 )
					{
						...
						if ( USE_ENGINE_BANLIST )
						{
							...
							trap_DropClient( i, "player kicked", timeout );
						}
						else
						{
							trap_DropClient( i, "player kicked", 0 );
...
*/
void
Svcmd_AddTank_f ( void )
{
	int		tanktype = 0;
	char	sx[MAX_TOKEN_CHARS];
	char	sy[MAX_TOKEN_CHARS];
	char	sz[MAX_TOKEN_CHARS];
	char	stanktype[MAX_TOKEN_CHARS];
	int		x, y, z = 0;
	vec3_t	origin;
	if ( trap_Argc() < 5 || trap_Argc() > 5 )
	{
		G_Printf( "Usage: addTank <tank type> <x> <y> <z> \n" );
		return;
	}

	trap_Argv( 1, stanktype, sizeof(stanktype) );
	tanktype = atoi( stanktype );
	trap_Argv( 2, sx, sizeof(sx) );
	x = atoi( sx );
	trap_Argv( 3, sy, sizeof(sy) );
	y = atoi( sy );
	trap_Argv( 4, sz, sizeof(sz) );
	z = atoi( sz );
	origin[0] = x;
	origin[1] = y;
	origin[2] = z;
...
ConsoleCommand ( void )
{
	...
	if ( Q_stricmp( cmd, "addbot") == 0 )
	{
		...
		{
			G_Printf( "Bots are not allowed , use bot_enable to allow them\n" );
			return ( qfalse );
		}
	}
	...
	if ( Q_stricmp( cmd, "addbot") == 0 )
	{
		if( ! bot_dynamic.integer ) {
			// Removed obsolete addbot functionality !
			...
			teamnr = TeamForName( team );
			if( teamnr != TEAM_ALLIES && teamnr != TEAM_AXIS )
			{
				G_Printf( S_COLOR_YELLOW "Invalid team. Using automic team select.\n" );
				Q_strncpyz( team, TeamName(PickTeam(-1)), sizeof( team) );
			}
			...
			if ( *name == 0 )
				Q_strncpyz( name, va( "%s", PickName( teamnr )), sizeof(name) );

			if ( SOD_SPAWN_AddBotShort( team, name, iskill, NULL) )
			{
				G_Printf( S_COLOR_GREEN "Spawned bot %s with skill level %d.\n", name, iskill );
			}
		} else {
			G_Printf( S_COLOR_RED "Addbot command disabled when bot_dynamic is on.\n" );
		}
		return qtrue;
	}
...
#endif //__BOT__
#ifdef __NPC__
	else if ( Q_stricmp( cmd, "savenodes") == 0 && npc_node_edit.integer )
	{
		NPC_NODES_SaveNodes();
		return ( qtrue );
	}
#endif //__NPC__
	if ( Q_stricmp( cmd, "addtank") == 0 )
	{
		Svcmd_AddTank_f();
		return ( qtrue );
	}
	...
	//if ( g_dedicated.integer )
	{
		...
		else if ( !Q_stricmp( cmd, "forcecvar") )
		{
			...
			Q_strncpyz( level.forceCvars[level.forceCvarCount][0], cvarName, sizeof(level.forceCvars[0][0]) );
			Q_strncpyz( level.forceCvars[level.forceCvarCount][1], cvarValue, sizeof(level.forceCvars[0][1]) );
			level.forceCvarCount++;
			trap_SendServerCommand( -1, va( "forcecvar \"%s\" \"%s\"", cvarName, cvarValue) );
			return ( qtrue );
		}
		...
		if( Q_stricmp( cmd, "cpmsay" ) == 0 )
		{
			// Thanks to Dreadnought for this one
			trap_SendServerCommand( -1, va( "cpm \"%s\n\"", ConcatArgs( 1)) );
			G_LogPrintf(va("cpmsay: %s\n", ConcatArgs(1)));
			return ( qtrue );
		}

		if( Q_stricmp( cmd, "qsay" ) == 0 )
		{
			// Kamikazee: Ok, cpmsay uses "cpm %s" now, qsay uses "chat %s"
			trap_SendServerCommand( -1, va("chat \"%s\n\"", ConcatArgs(1) ) );
			G_LogPrintf(va("qsay: %s\n", ConcatArgs(1)));
			return ( qtrue );
		}
