...
/* */
qboolean
G_smvCommands ( gentity_t *ent, char *cmd )
{
#ifdef MV_SUPPORT
...
/* */
void
G_smvAddTeam_cmd ( gentity_t *ent, int nTeam )
{
	int i, pID;
...
void
G_smvAddView ( gentity_t *ent, int pID )
{
	int			i;
	mview_t		*mv = NULL;
	gentity_t	*v;
...
qboolean
G_smvLocateEntityInMVList ( gentity_t *ent, int pID, qboolean fRemove )
{
	int i;
	if ( ent->client->pers.mvCount > 0 )
	{
...
void
G_smvRemoveEntityInMVList ( gentity_t *ent, mview_t *ref )
{
	ref->entID = -1;
	ref->fActive = qfalse;
...
unsigned int
G_smvGenerateClientList ( gentity_t *ent )
{
...
// Calculates a given client's MV player list
void
G_smvRegenerateClients ( gentity_t *ent, int clientList )
{
	int i;
...
void
G_smvUpdateClientCSList ( gentity_t *ent )
{
...
void
G_smvRemoveInvalidClients ( gentity_t *ent, int nTeam )
{
	int i, id;
...
void
G_smvAllRemoveSingleClient ( int pID )
{
	int			i;
	gentity_t	*ent;
	for ( i = 0; i < level.numConnectedClients; i++ )
...
G_smvRunCamera ( gentity_t *ent )
{
	...
	if
	(
		ent->target_ent->client->pers.connected != CON_CONNECTED ||
	...
	hintTime =
		(
			tps->serverCursorHint != HINT_BUILD &&
			(tps->serverCursorHintVal >= 255 || tps->serverCursorHintVal == 0)
