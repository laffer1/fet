...
#endif //__BOT__
#ifdef __NPC__
extern void NPC_NODES_SaveNodes ( void );
extern void NPC_FODDER_AddNode ( int clientNum, int teamNum, qboolean isCoverPoint );
	...
	for ( team = 0; team < 2; team++ )
	{
		...
		*startbuffer = '\0';
		...
		for ( ; i < numSorted; i++ )
		{
			...
			if
			(
				cl->ps.persistant[PERS_TEAM] == ent->client->ps.persistant[PERS_TEAM] ||
			...
			else
			{
				...
				Com_sprintf( entry, sizeof(entry), " %i %i %i %i %i %i %i", level.sortedClients[i], totalXP, ping,
							 (level.time - cl->pers.enterTime) / 60000, g_entities[level.sortedClients[i]].s.powerups,
...
*/
qboolean
KillOk ( gentity_t *ent )
{
	if ( g_nokill.integer == 1 )
	{
		trap_SendServerCommand( ent - g_entities, va( "print \"Selfkills are not enabled on this server.\n\"") );
		return ( qfalse );
	}
...
*/
void
SanitizeString ( char *in, char *out, qboolean fToLower )
{
	while ( *in )
	{
		if ( *in == 27 || *in == '^' )
...
*/
void
Cmd_ListBotGoals_f ( gentity_t *ent )
{
	int		i;
	team_t	t;
	if ( !CheatsOk( ent) )
...
*/
void
Cmd_Dropweapon_f ( gentity_t *ent )
{
	DropCurrentWeapon( ent );
}
...
/*
void Cmd_Holster_f( gentity_t *ent )
{
...
Cmd_Kill_f ( gentity_t *ent )
{
	...
	if ( g_nokill.integer == 4 )
	{
		if ( WolfFindEnemy( ent) )
		{
			trap_SendServerCommand( ent - g_entities, va( "print \"You cant kill yourself , enemy is nearby.\n\"") );
			return;
		}

		if ( ent->client->combatState != COMBATSTATE_COLD )
		{
			trap_SendServerCommand( ent - g_entities,
									va( "print \"In combat selfkills are not allowed on this server.\n\"") );
			return;
		}
	}

	if ( WolfFindEnemy( ent) && (g_nokill.integer == 3) )
	{
		trap_SendServerCommand( ent - g_entities, va( "print \"You cant kill yourself , enemy is nearby.\n\"") );
		return;
	}

	if ( ent->client->combatState != COMBATSTATE_COLD && g_nokill.integer == 2 )
	{
		trap_SendServerCommand( ent - g_entities, va( "print \"In combat selfkills are not allowed on this server.\n\"") );
		return;
	}
	...
	//	G_Damage( ent, ent, ent, NULL, NULL, 9999, DAMAGE_NO_PROTECTION, MOD_SUICIDE );	// just kill him with this :-D
	ent->client->ps.stats[STAT_HEALTH] = ent->health = 0;
	ent->client->ps.persistant[PERS_HWEAPON_USE] = 0;	// TTimo - if using /kill while at MG42
	player_die( ent, ent, ent, 0, MOD_SUICIDE );		// from (g_gamestate.integer == GS_PLAYING) ? 100000 : 135
}
...
/* */
void
G_TeamDataForString ( const char *teamstr, int clientNum, team_t *team, spectatorState_t *sState, int *specClient )
{
...
*/
qboolean
SetTeam ( gentity_t *ent, char *s, qboolean force, weapon_t w1, weapon_t w2, qboolean setweapons )
{
	team_t				team, oldTeam;
	gclient_t			*client;
	int					clientNum;
	...
	if ( team != TEAM_SPECTATOR )
	{
		...
		(
			...
			(team != ent->client->sess.sessionTeam && ent->client->sess.sessionTeam != TEAM_SPECTATOR) &&
			g_gamestate.integer == GS_PLAYING &&
			!force
		)
		{
			trap_SendServerCommand( clientNum,
									"cp \"You cannot switch during a match, please wait until the round ends.\n\"" );
			return ( qfalse );		// ignore the request
		}

		// Kamikazee: Check if there are auto-bots on the server. When a human
		//	player switches teams, he should not be bothered by bots.
		if
			(
				!( ent->r.svFlags & SVF_BOT )
				&& bot_dynamic.integer > 0
				&& Count_Human_Players() < bot_minplayers.integer
				// && team != oldTeam // Kamikazee: is redundant
			)
		{
			AIMOD_ReservePlace( team );
		}

		if
		(
			(
				(g_gametype.integer == GT_WOLF_LMS && g_lms_teamForceBalance.integer)
				|| g_teamForceBalance.integer
			) &&
			!force &&
			// jaquboss, allow to choose every team in DM
			g_gametype.integer != GT_WOLF_DM
		)
		{
...
/* */
int
G_NumPlayersWithWeapon ( weapon_t weap, team_t team )
{
	int i, j, cnt = 0;
...
/* */
int
G_NumPlayersOnTeam ( team_t team )
{
	int i, j, cnt = 0;
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
...
/* */
int
G_TeamCount ( gentity_t *ent, weapon_t weap )
{
	int i, j, cnt;
	if ( weap == -1 )
...
/* */
qboolean
G_IsWeaponDisabled ( gentity_t *ent, weapon_t weapon )
{
	int count, wcount;
	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR )
	{
...
/* */
void
G_SetClientWeapons ( gentity_t *ent, weapon_t w1, weapon_t w2, qboolean updateclient )
{
...
*/
void
Cmd_Team_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
*/
void
Cmd_Head_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
	char	model[4], skin[4];
	int		modelNum, skinNum;
	int		maxium;
	if ( trap_Argc() < 2 )
	{
		CP( va( "print \"model: %i skin: %i\n\"", ent->client->sess.headModel, ent->client->sess.headSkin) );
		return;
	}

	trap_Argv( 1, model, sizeof(model) );
	trap_Argv( 2, skin, sizeof(skin) );
	modelNum = atoi( model );
	skinNum = atoi( skin );
	if ( ent->client->sess.sessionTeam == TEAM_AXIS )
	{
		maxium = g_n2x.integer;
	}
	else
	{
		maxium = g_n2a.integer;
	}

	ent->client->sess.latchPlayerHeadModel = modelNum;
	if ( ent->client->sess.latchPlayerHeadModel < 0 || ent->client->sess.latchPlayerHeadModel > maxium )
	{
		ent->client->sess.latchPlayerHeadModel = maxium;
	}

	if ( ent->client->sess.sessionTeam == TEAM_AXIS )
	{
		maxium = g_n1x.integer;
	}
	else
	{
		maxium = g_n1a.integer;
	}

	ent->client->sess.latchPlayerHeadModel = skinNum;
	if ( ent->client->sess.latchPlayerHeadModel < 0 || ent->client->sess.latchPlayerHeadModel > maxium )
	{
		ent->client->sess.latchPlayerHeadModel = maxium;
	}
...
/* */
void
Cmd_ResetSetup_f ( gentity_t *ent )
{
	qboolean	changed = qfalse;
	if ( !ent || !ent->client )
	{
...
/* */
void
Cmd_SetClass_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
/* */
void
Cmd_SetWeapons_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
*/
void
Cmd_Follow_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
	int		i;
...
Cmd_FollowCycle_f ( gentity_t *ent, int dir )
{
	...
	if ( g_gametype.integer == GT_WOLF_DM && ent->client->ps.pm_flags & PMF_LIMBO )
	{

		/*	ent->client->sess.spectatorClient = ent->s.clientNum;
		ent->client->sess.spectatorState = SPECTATOR_FOLLOW;*/
		return;
	}
...
)
{
	...
	if ( (mode == SAY_TEAM || mode == SAY_TEAMNL) && !OnSameTeam( ent, other) && g_gametype.integer != GT_WOLF_DM )
	{
		return;
	}
	...
	if
	(
		match_mutespecs.integer > 0 &&
		ent->client->sess.referee == 0 &&	// OSP
		(
	...
	else
	{
		...
		trap_SendServerCommand( other - g_entities, va( "%s \"%s%c%c%s\" %i %i",
								mode == SAY_TEAM || mode == SAY_BUDDY ? "tchat" : "chat", name, Q_COLOR_ESCAPE, color,
								message, ent - g_entities, localize) );
	}
}
...
G_Say ( gentity_t *ent, gentity_t *target, int mode, const char *chatText )
{
	...
	switch ( mode )
	{
		...
		case SAY_BUDDY:
			...
			Com_sprintf( name, sizeof(name), "[lof](%s%c%c) (%s): ", ent->client->pers.netname, Q_COLOR_ESCAPE, COLOR_WHITE,
						 loc );
			color = COLOR_YELLOW;
			break;
		case SAY_TEAM:
...
G_VoiceTo ( gentity_t *ent, gentity_t *other, int mode, const char *id, qboolean voiceonly )
{
	...
	if ( mode == SAY_TEAM && !OnSameTeam( ent, other) && g_gametype.integer != GT_WOLF_DM )
	{
		return;
	}
	...
	// Send this voice macro to the bot as an event.
	{
		...
		botUserData.m_DataType = dtString;
		...
		Bot_Interface_SendEvent( iMessageId, other->s.number, ent->s.number, 0.0f, &botUserData );
	}
#endif
	if ( voiceonly == 2 )
	{
	...
	{
		CPx( other - g_entities, va( "%s %d %d %d %s %i %i %i", cmd, voiceonly, ent - g_entities, color, id,
			 (int) ent->s.pos.trBase[0], (int) ent->s.pos.trBase[1], (int) ent->s.pos.trBase[2]) );
	}
	else
	{
...
*/
qboolean
Cmd_CallVote_f ( gentity_t *ent, unsigned int dwCommand, qboolean fRefCommand )
{
	int		i;
...
/* */
qboolean
G_FindFreeComplainIP ( gclient_t *cl, ipFilter_t *ip )
{
	int i = 0;
...
Cmd_Vote_f ( gentity_t *ent )
{
	...
	if
	(
		ent->client->pers.complaintEndTime > level.time &&
		g_gamestate.integer == GS_PLAYING &&
	...
	)
	{
		...
		if ( msg[0] == 'y' || msg[1] == 'Y' || msg[1] == '1' )
		{
			...
			if ( !(ent->r.svFlags & SVF_BOT) )
				...
				if ( ent->s.eType != ET_NPC )
#endif //__NPC__
					trap_SendServerCommand( ent->client->pers.complaintClient, va(
												"cpm \"^1Warning^7: Complaint filed against you by %s^* You have Lost XP.\n\"",
											ent->client->pers.netname) );
			trap_SendServerCommand( ent - g_entities, "complaint -1" );
	...
	if ( level.voteInfo.vote_fn == G_Kick_v )
	{
		...
		if
		(
			g_entities[pid].client->sess.sessionTeam != TEAM_SPECTATOR &&
...
/* */
qboolean
G_canPickupMelee ( gentity_t *ent )
{
...
*/
void
Cmd_StartCamera_f ( gentity_t *ent )
{
...
*/
void
Cmd_StopCamera_f ( gentity_t *ent )
{
...
*/
void
Cmd_SetCameraOrigin_f ( gentity_t *ent )
{
	char	buffer[MAX_TOKEN_CHARS];
	int		i;
	vec3_t	origin;
...
*/
void
Cmd_InterruptCamera_f ( gentity_t *ent )
{
	gentity_t	*player;
...
/* */
qboolean
G_TankIsOccupied ( gentity_t *ent )
{
	if ( !ent->tankLink )
	{
		return ( qfalse );
...
/* */
qboolean
G_TankIsMountable ( gentity_t *ent, gentity_t *other )
{
...
*/
qboolean
Do_Activate2_f ( gentity_t *ent, gentity_t *traceEnt )
{
	...
	if ( ent->client->sess.playerType == PC_COVERTOPS && !ent->client->ps.powerups[PW_OPS_DISGUISED] )
	{
		if
		(
			(!ent->client->ps.powerups[PW_BLUEFLAG] && !ent->client->ps.powerups[PW_REDFLAG]) ||
			g_realisticdisguise.integer == 1
		)
		{
...
qboolean
Do_Activate_f ( gentity_t *ent, gentity_t *traceEnt )
{
	qboolean	found = qfalse;
	...
	if ( ent->client->pers.cmd.buttons & BUTTON_WALKING || ent->client->ps.eFlags & EF_AIMING )
	{																// jaquboss , ( TCE-style)
		walking = qtrue;
	}
	...
	{
		if ( (ent->NPC_client->pers.cmd.buttons & BUTTON_WALKING) || (ent->NPC_client->ps.pm_flags & PMF_DUCKED) )
		{
			walking = qtrue;
		}
	}
	...
	if ( traceEnt->classname )
	{
...
#ifdef __NPC__
			if ( ent->s.teamNum == TEAM_SPECTATOR )
			{
				return ( qfalse );
			}
#endif //__NPC__
		...
		else if ( !Q_stricmp( traceEnt->classname, "churchilltank") && !(ent->client->ps.eFlags & EF_TANK) )
		{
			ent->client->ps.eFlags |= EF_TANK;
			ent->client->ps.persistant[PERS_TANKUSED] = 1;
			...
			if ( g_colisionMaps.integer && level.tankclip[0] && level.tankclip[0]->model )
			{
				trap_SetBrushModel( ent, level.tankclip[0]->model );
			}

			//	ent->client->ps.standViewHeight = DEFAULT_VIEWHEIGHT;
			ent->client->ps.runSpeedScale = 0.5;
			ent->client->ps.friction = 0.5;
			...
			ent->BackupHealth = ent->client->ps.stats[STAT_HEALTH];
			ent->health = ent->client->ps.stats[STAT_HEALTH] = traceEnt->health;
			ent->backupWeaponTime = ent->client->ps.weaponTime;
			ent->client->ps.weaponTime = traceEnt->backupWeaponTime;
			traceEnt->active = qtrue;
			G_FreeEntity( traceEnt );
		}
		else if ( !Q_stricmp( traceEnt->classname, "panzertank") && !(ent->client->ps.eFlags & EF_TANK) )
		{
			ent->client->ps.eFlags |= EF_TANK;
			ent->client->ps.persistant[PERS_TANKUSED] = 2;
			VectorClear( ent->client->ps.velocity );
			VectorCopy( tankMins, ent->r.mins );
			VectorCopy( tankMaxs, ent->r.maxs );
			VectorCopy( ent->r.mins, ent->client->ps.mins );
			VectorCopy( ent->r.maxs, ent->client->ps.maxs );
			if ( g_colisionMaps.integer && level.tankclip[1] && level.tankclip[1]->model )
			{
				trap_SetBrushModel( ent, level.tankclip[1]->model );
			}
...
/* */
void
G_LeaveTank ( gentity_t *ent, qboolean position )
{
	gentity_t	*tank;
...
/* */
gentity_t *
tankSpawn ( gentity_t *self, vec3_t origin, vec3_t angle )
{
	gentity_t	*tank;
	tank = G_Spawn();
	...
	switch ( self->client->ps.persistant[PERS_TANKUSED] )
	{
		...
		case 1:
			...
			tank->s.modelindex2 = G_ModelIndex( "models/mapobjects/tanks_sd/churchhill_full.md3" );
			break;
		case 2:
			...
			tank->s.modelindex2 = G_ModelIndex( "models/mapobjects/tanks_sd/jagdpanther_full.md3" );
			break;
	}

	if
	(
		g_colisionMaps.integer &&
		level.tankclip[self->client->ps.persistant[PERS_TANKUSED] - 1] &&
		level.tankclip[self->client->ps.persistant[PERS_TANKUSED] - 1]->model
	)
	{
		trap_SetBrushModel( tank, level.tankclip[self->client->ps.persistant[PERS_TANKUSED] - 1]->model );
	}
	else
	{
	...
	tank->s.pos.trType = TR_STATIONARY;
	tank->s.apos.trType = TR_STATIONARY;
...
*/
	...
	G_SetAngle( tank, angle );
	tank->active = qfalse;
	tank->health = self->client->ps.stats[STAT_HEALTH];
...
/* */
void
G_LeavePlayerTank ( gentity_t *ent, qboolean position )
{
	gentity_t	*tank;
	vec3_t		axis[3];
	vec3_t		pos;
	...
	ent->client->ps.weaponTime = ent->backupWeaponTime;
	ent->client->ps.eFlags &= ~EF_TANK;
	ent->s.eFlags &= ~EF_TANK;
	ent->client->ps.persistant[PERS_TANKUSED] = 0;
	ent->TankMarker = 0;											// we are no longer spawners tank
	health = ent->BackupHealth;

	//	ent->health = ent->client->ps.stats[STAT_HEALTH] = ent->BackupHealth;
	if ( position )
	{
		AnglesToAxis( tank->s.angles, axis );
		VectorMA( ent->client->ps.origin, 128, axis[2], pos );		// always spawn on tank
		trap_Trace( &tr, pos, playerMins, playerMaxs, pos, -1, CONTENTS_SOLID );
		if ( tr.startsolid )
		{
			VectorMA( ent->client->ps.origin, 128, axis[0], pos );	// unless it isnt possible
			trap_Trace( &tr, pos, playerMins, playerMaxs, pos, -1, CONTENTS_SOLID );
			if ( tr.startsolid )
			{
		...
		/*	if ( g_colisionMaps.integer )*/
		{															// fixme, required to get rid of brush model on player
			...
			//	ent->client->ps.standViewHeight = DEFAULT_VIEWHEIGHT;
			ent->client->ps.runSpeedScale = 0.8;
			ent->client->ps.friction = 1.0;
			trap_LinkEntity( ent );
		}
...
Cmd_Activate_f ( gentity_t *ent )
{
	...
	if ( ent->s.weapon == WP_MORTAR_SET || ent->s.weapon == WP_MOBILE_MG42_SET || ent->s.weapon == WP_30CAL_SET )
	{
		return;
	}
	...
	if ( ent->active )
	{
		if ( ent->client->ps.persistant[PERS_HWEAPON_USE] )
		{
			...
			ent->client->ps.eFlags &= ~EF_BROWNING_ACTIVE;
			ent->client->ps.eFlags &= ~EF_AIMING;
	...
	else if ( ent->client->ps.eFlags & EF_TANK && ent->s.eFlags & EF_TANK )
	{
 ...
 */
qboolean
G_UniformSteal ( gentity_t *ent, gentity_t *traceEnt )
{
	qboolean	found = qfalse;
	...
	if ( BODY_VALUE( traceEnt) < 250 )
	{
		...
		//	BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo, ANIM_ET_DROP, qtrue, qtrue );
		ent->client->ps.pm_flags |= PMF_TIME_LOCKPLAYER;
		ent->client->ps.pm_time = 650;
		BODY_VALUE( traceEnt ) += 5;
		return ( qfalse );
	}

	ent->client->ps.pm_time = 501;
	traceEnt->nextthink = traceEnt->timestamp + BODY_TIME( BODY_TEAM( traceEnt) );
	BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo,	/*ANIM_ET_PICKUPGRENADE*/
						ANIM_ET_DROP, qfalse, qtrue );									// can look nice ?
 ...
 */
qboolean
G_PushPlayer ( gentity_t *ent, gentity_t *victim )
{
	vec3_t	dir, push;
	if ( !g_shove.integer )
	{
	...
	{
		G_AddEvent( victim, EV_GENERAL_SOUND, G_SoundIndex( g_shoveSound.string) );
	}
 ...
 */
qboolean
G_DragCorpse ( gentity_t *dragger, gentity_t *corpse )
{
	vec3_t	dir, pull, res;
	float	dist;
...
Cmd_Activate2_f() is only for:
	...
	dragging
*/
void	Weapon_RealMedic ( gentity_t *ent );
void	ReviveEntity ( gentity_t *ent, gentity_t *tracEnt );


/* */
void
Cmd_Activate2_f ( gentity_t *ent )
{
	trace_t		tr;
	vec3_t		end;
	gentity_t	*traceEnt;
	...
	if ( tr.entityNum >= 0 )
	{
		...
		if ( traceEnt->client )
		{
			if ( ent->client->sess.playerType == PC_MEDIC && g_realmedic.integer )
			{
				if ( BODY_REVIVE( traceEnt) < 250 )
				{
					BODY_REVIVE( traceEnt ) += 5;
					BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo, ANIM_ET_REVIVING,
										qtrue, qtrue );
					ent->client->ps.pm_flags |= PMF_TIME_LOCKPLAYER;
					ent->client->ps.pm_time = 650;
					ent->reviving = qtrue;
					ent->revivingAnim = qtrue;
					return;
				}
				else
				{
					ent->client->ps.pm_time = 501;
					BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo, ANIM_ET_DROP,
										qfalse, qtrue );
	...
	if ( tr.entityNum >= 0 )
	{
		...
		if ( traceEnt->client && !(traceEnt->client->ps.eFlags & EF_TANK) )
		{
			if
			(
				ent->client->sess.playerType == PC_MEDIC &&
				g_realmedic.integer &&
				ent->client->ps.weapon == WP_MEDKIT
			)
			{
				Weapon_RealMedic( ent );						// heal the guy
				BG_AnimScriptEvent( &ent->client->ps, ent->client->pers.character->animModelInfo, ANIM_ET_FIREWEAPON,
									qtrue, qtrue );
				return;
			}
...
*/
void
SetPlayerSpawn ( gentity_t *ent, int spawn, qboolean update )
{
...
/* */
void
Cmd_SetSpawnPoint_f ( gentity_t *ent )
{
	char	arg[MAX_TOKEN_CHARS];
	int		val, i;
	if ( trap_Argc() != 2 )
	...
	for ( i = 0; i < level.numLimboCams; i++ )
	{
		...
		if ( level.limboCams[i].spawn && x == val )
		{
			...
			trap_SendServerCommand( ent - g_entities, va( "portalcampos %i %i %i %i %i %i %i %i", val - 1,
...
*/
void
Cmd_SetSniperSpot_f ( gentity_t *clent )
{
	gentity_t		*spot;
	...
	Com_sprintf( buf, sizeof(buf),
				 "{\n\"classname\" \"%s\"\n\"origin\" \"%.3f %.3f %.3f\"\n\"angles\" \"%.2f %.2f %.2f\"\n\"aiTeam\" \"%i\"\n}\n\n",
			 spot->classname, spot->s.origin[0], spot->s.origin[1], spot->s.origin[2], spot->s.angles[0],
				 spot->s.angles[1], spot->s.angles[2], spot->aiTeam );
...
/* */
void
Cmd_WeaponStat_f ( gentity_t *ent )
{
	char				buffer[16];
...
/* */
void
Cmd_IntermissionWeaponStats_f ( gentity_t *ent )
{
	char	buffer[1024];
	int		i, clientNum;
	if ( !ent || !ent->client )
	...
	for ( i = 0; i < WS_MAX; i++ )
	{
		Q_strcat( buffer, sizeof(buffer), va( "%i %i %i ", level.clients[clientNum].sess.aWeaponStats[i].atts,
...
/* */
void
Cmd_IntermissionReady_f ( gentity_t *ent )
{
	if ( !ent || !ent->client )
	{
		return;
...
/* */
void
Cmd_IntermissionPlayerKillsDeaths_f ( gentity_t *ent )
{
	char	buffer[1024];
	int		i;
	if ( !ent || !ent->client )
...
/* */
void
Cmd_IntermissionWeaponAccuracies_f ( gentity_t *ent )
{
	char	buffer[1024];
	int		i;
	if ( !ent || !ent->client )
...
/* */
void
Cmd_SelectedObjective_f ( gentity_t *ent )
{
	int		i, val;
	char	buffer[16];
	...
	for ( i = 0; i < level.numLimboCams; i++ )
	{
		if ( !level.limboCams[i].spawn && level.limboCams[i].info == val )
		{
			...
			{
				dist = VectorDistanceSquared( level.limboCams[i].origin,
...
*/
void
Cmd_SwapPlacesWithBot_f ( gentity_t *ent, int botNum )
{
	gentity_t			*botent;
	gclient_t			cl, *client;
...
ClientCommand ( int clientNum )
{
	...
	else if ( Q_stricmp( cmd, "kill") == 0 )
	{
		...
		if ( ent->client ) {
			if ( ent->client->ps.eFlags & EF_TANK ) {
				ent->client->ps.eFlags &= ~EF_TANK;
			} else {
				ent->client->ps.eFlags |= EF_TANK;
			}
	...
	*/
		...
		/*	} else if (Q_stricmp (cmd, "addtank") == 0) {
			char		userinfo[MAX_INFO_STRING];
	...
	}  else if (Q_stricmp (cmd, "addpanzer") == 0) {
		char		userinfo[MAX_INFO_STRING];
...
*/
	...
	else if ( Q_stricmp( cmd, "checkbotreach") == 0 )
	{
		AIMod_CheckMapPaths( ent );
	}
	else if ( Q_stricmp( cmd, "gotonode") == 0 )
	{
	...
	else if ( Q_stricmp( cmd, "checknodelinks") == 0 )
	{
	...
	{						// Raise a node to force it to be a jump node.
		...
		G_Printf( "^1*** ^3AIMod: ^5Node ^7%i^5 raised to force jump.\n", node );
	}
	...
	{						// Move a node to current player position.
		...
		G_Printf( "^1*** ^3AIMod: ^5Node ^7%i^5 moved to current player position.\n", node );
	}
	...
	{						// Delete the last added node.
		...
		G_Printf( "^1*** ^3AIMod: ^5Node ^7%i^5 deleted.\n", number_of_nodes );
		number_of_nodes--;
#endif //__BOT__
#ifdef __NPC__
	}
	else if ( Q_stricmp( cmd, "savenodes") == 0 && npc_node_edit.integer > 0 )
	{
		NPC_NODES_SaveNodes();
	}
	else if ( Q_stricmp( cmd, "addnode") == 0 && npc_node_edit.integer > 0 )
	{
		int			teamNum = ent->client->sess.sessionTeam;
		char		cover[MAX_TOKEN_CHARS];
		qboolean	isCoverPoint = qfalse;
		trap_Argv( 1, cover, sizeof(cover) );
		if ( Q_stricmp( cover, "") )
		{
			isCoverPoint = qtrue;
		}

		NPC_FODDER_AddNode( ent->s.number, teamNum, isCoverPoint );
	}
	else if ( Q_stricmp( cmd, "addnpctank") == 0 )
	{

		//NPC_Tank_Spawn( ent->r.currentOrigin );
		gentity_t	*self = G_Spawn();
		G_SetOrigin( self, ent->r.currentOrigin );
		G_SetAngle( self, ent->r.currentAngles );
		VectorCopy( ent->r.currentOrigin, self->s.origin );
	...
	else if ( Q_stricmp( cmd, "addnpc") == 0 )
	{
		Debug_NPC_Spawn( ent );
#endif //__NPC__
	}
