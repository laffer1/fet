...
/* */
qboolean
CheckAboveOK_Player ( vec3_t origin )	// For player/npc/bot spawns!
...
/* */
qboolean
AdvancedWouldTelefrag ( vec3_t point )
{
	int			i, num;
...
/* */
qboolean
CheckEntitiesInSpot ( vec3_t point )
...
#ifdef __NPC__
		if ( ent->NPC_client && VectorDistance( point, ent->NPC_client->ps.origin) < 128 )
		{	// Bad point.
			return ( qtrue );
		}
#endif //__NPC__
...
/* */
qboolean
Extrapolate_Advanced_Spawnpoint ( vec3_t original_origin, vec3_t newspawn )
