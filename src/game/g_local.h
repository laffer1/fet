...
#ifdef __BOT__
#ifndef MAX_NODES
#define MAX_NODES			10000		//8000//32000//16000	// Maximum Nodes (8000)
#define MAX_PATHLIST_NODES	MAX_NODES	//1024	// MAX_NODES??
#endif //MAX_NODES
#endif //__BOT__
...
// MP : Both 10 seconds
#define BODY_TIME( t )	( g_corpsestaytime.integer * 1000 )
#define MAX_MG42_HEAT	3500.f			// balance
...
// UQ1: Single Player Scripts...
#define SCFL_WAITING_RESTORE	0x8
...
int								G_GetWeaponForMOD ( meansOfDeath_t mod );
meansOfDeath_t					G_GetMODforSnipers ( weapon_t weapon );
...
// Destroyer boat nodes structure...

//
typedef struct Destroyer_node_s						// Node Structure
{
	vec3_t		origin;								// Node Origin
	qboolean	land_point;
	qboolean	reverse;
...
Destroyer_node_t	Destroyer_nodes[2000];


typedef struct bot_input_glocal_s
{
...
//
// END - Destroyer boat nodes structure...
//
#endif //__NPC__
struct gentity_s
{
	...
	char				*target;
	char				*targetdeath;	// fire this on death exclusively //----(UQ1)	added
	char				*targetname;
...
//	int						missionLevel;			// mission we are currently trying to complete
	...
	g_script_status_t		scriptStatusCurrent;
	...
	qboolean				is_dead; // UQ1: g_tramcar.c
#ifdef __SUPREMACY__
	...
	// realmedic
	qboolean				reviving;
	qboolean				revivingAnim;

	// wolfkick
	int						wolfkicktimer;
	int						oldkicktime;
	int						backupMask;				// backup client contents
	int						localEFlags;
#ifdef __BOT__
...
//	vec3_t		bot_sp_waypoints[64];
	...
	int					pathsize;
	short int			pathlist[MAX_NODES /*MAX_PATHLIST_NODES*/ ];
	int					shortTermGoal;
	int					long_term_goal_timer;
	int					longterm_pathsize;
	short int			longterm_pathlist[MAX_NODES /*MAX_PATHLIST_NODES*/ ];
	...
	bot_input_glocal_t	*bi;	// jaquboss, to remove conflicts within source
...
typedef struct
{
	...
	int					headModel;
	int					headSkin;
	int					latchPlayerHeadModel;
	int					latchPlayerHeadSkin;
} clientSession_t;
...
*/
	...
	int					dropObjectiveTime;						// JPW NERVE last time a Objective was dropped
...
typedef struct
{
	...
	int					forceCvarCount;
	gentity_t			*tankclip[2];
#ifdef __BOT__
	sod_level_info_t	sodInfo;								// AIMod
#endif //__BOT__

	// UQ1: Single Player Stuff..
	int					numSecrets;
} level_locals_t;
typedef struct
{
...
qboolean	checkLocation ( gentity_t *targ, vec3_t dir, vec3_t point, int mod );
void		G_Damage
			(
				gentity_t	*targ,
				gentity_t	*inflictor,
...
void		DropCurrentWeapon ( gentity_t *self );
gentity_t	*G_BuildBody ( gentity_t *ent );
gentity_t	*G_BuildHead ( gentity_t *ent );
gentity_t	*G_BuildLeg ( gentity_t *ent );
gentity_t	*G_BuildGroin ( gentity_t *ent );

// damage flags
#define DAMAGE_RADIUS				0x00000001										// damage was indirect
#define DAMAGE_HALF_KNOCKBACK		0x00000002										// Gordon: do less knockback
#define DAMAGE_NO_KNOCKBACK			0x00000008										// do not affect velocity, just view angles
#define DAMAGE_NO_PROTECTION		0x00000016										// armor, shields, invulnerability, and godmode have no effect
#define DAMAGE_DISTANCEFALLOFF		0x00000020										// distance falloff
#define DAMAGE_BACKSTAB				0x00000040
#define DAMAGE_DISTANCEFALLOFFLOW	0x00000080
...
gentity_t	*fire_grenade ( gentity_t *self, vec3_t start, vec3_t aimdir, int grenadeWPID /*, qboolean underhand*/ );
gentity_t	*fire_rocket ( gentity_t *self, vec3_t start, vec3_t dir, qboolean gravited );
gentity_t	*fire_speargun ( gentity_t *self, vec3_t start, vec3_t dir );
#define Fire_Lead( ent, activator, spread, damage, muzzle, forward, right, up, mod ) \
	Fire_Lead_Ext( ent, activator, spread, damage, muzzle, forward, right, up, mod )
...
void		fire_lead ( gentity_t *self, vec3_t start, vec3_t dir, int damage, int mod );
...
// g_mover.c
//
gentity_t	*G_TestEntityPosition ( gentity_t *ent, qboolean tankPush );
...
void		browning_fire ( gentity_t *other );
...
// g_client.c
//
void		G_CheckVersion ( gentity_t *ent, int clientNum );
...
void		AddCaptureScore ( gentity_t *ent, int score );
void		CalculateRanks ( void );
qboolean	SpotWouldTelefrag ( gentity_t *spot );
qboolean	G_CheckForExistingModelInfo
			(
...
void		G_RegisterHeadSkins ( void );
void		G_RegisterHeadModels ( void );
...
// Written: 12/26/2002
//
qboolean	IsSilencedWeapon ( // The type of weapon in question.  Is it silenced?
int weaponType );
...
void		G_SayTo
			(
				...
				const char	*message,
				qboolean	localize
			);	// JPW NERVE removed static declaration so it would link
...
void		Cmd_Gender_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue );
...
void		Cmd_Head_f ( gentity_t *ent, unsigned int dwCommand, qboolean fValue );
...
qboolean									WolfFindEnemy ( gentity_t *self );
...
int											Bot_GetWeaponForClassAndTeam
											(
												int			classNum,
												int			teamNum,
...
extern vmCvar_t g_landmine_limit;			// Jaquboss
...
extern vmCvar_t g_realisticdisguise;		// Jaquboss , no disguise loosing
extern vmCvar_t g_headshot_min;				// Jaquboss
extern vmCvar_t g_headshot_ratio;			// Jaquboss , in %
extern vmCvar_t g_XPSave;					// Jaquboss

//extern	vmCvar_t	g_xpstorage_noncampaign;
extern vmCvar_t g_damage_multi;				// Jaquboss , in %
extern vmCvar_t g_wolfKick;
extern vmCvar_t g_knifeDamage;
extern vmCvar_t g_mgDamage;
extern vmCvar_t g_mgSpread;
extern vmCvar_t g_smgDamage;
extern vmCvar_t g_smgSpread;
extern vmCvar_t g_rifleDamage;
extern vmCvar_t g_rifleSpread;
extern vmCvar_t g_explosivesDamage;
extern vmCvar_t g_pistolDamage;
extern vmCvar_t g_pistolSpread;

//extern	vmCvar_t	g_xpload;	// Jaquboss
...
extern vmCvar_t g_shootablegrenades;		//Ryan - Shootable Grenades CVAR
extern vmCvar_t g_shootablerockets;			//Ryan - Shootable Rockets CVAR
extern vmCvar_t g_panzerfaustspeed;			// Jaquboss , panzerfaust velocity
extern vmCvar_t g_panzerfaustlifetime;		// jaquboss
extern vmCvar_t g_panzerfaust_gravited;		// jaquboss
extern vmCvar_t g_nohud;
extern vmCvar_t g_noxp;
extern vmCvar_t g_nokill;
...
extern vmCvar_t g_debugTDM;
extern vmCvar_t g_debugLights;
...
extern vmCvar_t g_latcheddeath;
extern vmCvar_t g_corpsestaytime;
extern vmCvar_t g_noautopickups;
...
extern vmCvar_t g_mapScriptsDir;
...
extern vmCvar_t g_axis_strike;
extern vmCvar_t g_allied_strike;
...
//extern vmCvar_t		g_nobipodmove;
extern vmCvar_t g_crosshair;
extern vmCvar_t g_snoopers;
extern vmCvar_t g_thirdperson;				// jaquboss
extern vmCvar_t g_thirdpersonRange;			// jaquboss
extern vmCvar_t g_thirdpersonAngle;			// jaquboss
extern vmCvar_t g_PopHelmets;				// jaquboss
extern vmCvar_t g_HelmetPiercingFactor;		// jaquboss
extern vmCvar_t g_helmet_popchance;			// jaquboss
extern vmCvar_t g_hitsound;
extern vmCvar_t g_revive_protection;		// jaquboss , in s
extern vmCvar_t g_spawn_protection;			// jaquboss , in s

// -NERVE - SMF
//extern vmCvar_t		g_usercamo;		// jaquboss , cammo number
...
extern vmCvar_t g_snowspeed;
extern vmCvar_t g_snowheight;

//extern vmCvar_t		g_staminaonlyjumps;
// Jaqbuoss
extern vmCvar_t g_tankSpawnDelay;
extern vmCvar_t g_refTanks;
extern vmCvar_t g_akimboPickups;
extern vmCvar_t g_n1a;
extern vmCvar_t g_n1x;
extern vmCvar_t g_n2a;
extern vmCvar_t g_n2x;
...
extern vmCvar_t bot_dynamic;	// Used to disable automatic bot-adding code, moved here from g_bot.c
extern vmCvar_t bot_minplayers;	// Used for automatic bot-adding code, minimum number of players slots to be filled

//extern vmCvar_t		bot_useai;
#ifdef BONE_HITTESTS
extern vmCvar_t g_mdx_hit_test;
#endif
...
// unlagged server options
extern vmCvar_t g_safemarkers;
extern vmCvar_t g_allonmap;
...
void			trap_BotMoveToGoal
				(
					void /* struct bot_moveresult_s */ *result,
					int									movestate,
					void /* struct bot_goal_s */ *goal,
					int									travelflags
				);
...
int				trap_BotMovementViewTarget
				(
					int							movestate,
					void /* struct bot_goal_s */ *goal,
					int							travelflags,
...
int				trap_GeneticParentsAndChildSelection
				(
					int		numranks,
					float	*ranks,
					int		*parent1,
...
#define BODY_REVIVE( ENT )		ENT->s.effect3Time
...
mapEntityData_t				*G_FindMapEntityDataSingleClient
							(
...
qboolean													G_EmplacedGunIsRepairable
															(
																gentity_t	*ent,
																gentity_t	*other
															);
qboolean													G_EmplacedGunIsMountable
															(
																gentity_t	*ent,
																gentity_t	*other
															);
...
qboolean													G_IsFireteamLeader
															(
																int				entityNum,
																fireteamData_t	**teamNum
															);
...
//------------------------------------------------------------------------------------
//#define MAX_BOTS            32              // Maximum Bots (32) // fretn
//#define MAX_NODES           1200            // Maximum Nodes (1200)
//#define MAX_NODES           1400            // Maximum Nodes (1400) // Works fine
//#define MAX_NODES           1440            // Maximum Nodes (1450)
//#define MAX_NODES           1800            // Maximum Nodes (2400) // UniqueMod.. Need more!
//#define MAX_NODES           15000            // Maximum Nodes (3000)
//#define MAX_NODES           3000            // Maximum Nodes (3000)
//#define MAX_NODES           8000            // Maximum Nodes (8000)
//#define MAX_NODES           120000            // Maximum Nodes (8000)
//#define MAX_PATHLIST_NODES MAX_NODES//1024 //MAX_NODES
...
void				AIMOD_MAPPING_WriteNodes ( void );						// fretn
#ifdef __VEHICLES__
qboolean			BOT_HaveVehicleWaypoints ( void );
#endif //__VEHICLES__
int					AIMOD_NAVIGATION_FindClosestReachableNode
					(
						gentity_t	*uniquebot,
						int			range,
						int			type,
...
extern int		SOD_SPAWN_ChooseClass ( int team );
...
extern void		SOD_AI_ScriptLoad ( void );
extern void		SOD_AI_ScriptParse ( void );
extern void		SOD_AI_Determine_LR_Goal ( gentity_t *bot );
extern vec3_t	SP_Bot_Position[MAX_CLIENTS];
extern void		SOD_MAPPING_InitMapNodes ( void );
...
int			Count_Bot_Players ( int team );
int			Count_Playerclass( int playerclass, int team );
int			Count_Human_Players ( void );
char		*PickName ( int teamNum );
int			Pick_Redundant_Bot ( int team );
...
void		AIMOD_MAPPING_SetRelocateNode ( void );
...
void		AIMOD_ReservePlace( int team );
...
void			NPC_Bomber_Spawn ( vec3_t marker_origin, int marker_entitynum, gentity_t *owner );
void			NPC_LCVP_Check ( void );
void			NPC_Destroyer_Check ( void );
#endif //__NPC__
...
typedef struct
{
	...
	vec3_t		spawnpoints[64];
	vec3_t		spawnangles[64];
	int			num_spawnpoints;
} flag_list_t;
