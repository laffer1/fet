...
CG_GenerateTracemap ( void )
{
	...
	CG_Printf( "%i of %i gridpoints calculated (%.2f%%), %i total traces\n", i * TRACEMAP_SIZE,
			   TRACEMAP_SIZE * TRACEMAP_SIZE, ((i * TRACEMAP_SIZE) / (float) (TRACEMAP_SIZE * TRACEMAP_SIZE)) * 100.f,
			   tracecount );
	trap_UpdateScreen();
...
/* */
qboolean
BG_LoadTraceMap ( char *rawmapname, vec2_t world_mins, vec2_t world_maxs )
{
	int				i, j;
	fileHandle_t	f;
...
/* */
static void
BG_ClampPointToTracemapExtends ( vec3_t point, vec2_t out )
{
...
/* */
float
BG_GetSkyHeightAtPoint ( vec3_t pos )
{
	int		i, j;
	vec2_t	point;
...
/* */
float
BG_GetSkyGroundHeightAtPoint ( vec3_t pos )
{
	int		i, j;
	vec2_t	point;
...
/* */
float
BG_GetGroundHeightAtPoint ( vec3_t pos )
{
	int		i, j;
	vec2_t	point;
...
/* */
int
BG_GetTracemapGroundFloor ( void )
{
	if ( !tracemap.loaded )
	{
		return ( MIN_WORLD_HEIGHT );
...
// them to go outside the array
void
etpro_FinalizeTracemapClamp ( int *x, int *y )
{
	if ( *x < 0 )
	{
		*x = 0;
