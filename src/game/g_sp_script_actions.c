...
extern void G_SP_RTCW_script_ScriptEvent( gentity_t *ent, char *eventStr, char *params );
...
qboolean G_SP_ScriptAction_GotoMarker( gentity_t *ent, char *params )
{
	...
	if (!params || ent->scriptStatus.scriptStackChangeTime < level.time) {			// we are waiting for it to reach destination
		if (ent->s.pos.trTime + ent->s.pos.trDuration <= level.time) {	// we made it
			...
			BG_EvaluateTrajectory_RTCW( &ent->s.apos, ent->s.apos.trTime + ent->s.apos.trDuration, ent->s.angles );
			...
			// stop moving
			BG_EvaluateTrajectory_RTCW( &ent->s.pos, level.time, ent->s.origin );
		...
		if (!token[0]) {
			G_Error( "G_SP_Scripting: gotomarker must have an targetname\n" );
		}
		...
		if (!target)
		{
			G_Error( "G_SP_Scripting: can't find entity with \"targetname\" = \"%s\"\n", token );
		}
		...
		token = COM_ParseExt( &pString, qfalse );
		if (!token[0]) {
			G_Error( "G_SP_Scripting: gotomarker must have a speed\n" );
		}
	...
	BG_EvaluateTrajectory_RTCW( &ent->s.pos, level.time, ent->r.currentOrigin );
	BG_EvaluateTrajectory_RTCW( &ent->s.apos, level.time, ent->r.currentAngles );
	SP_RTCW_script_linkentity(ent);
...
qboolean G_SP_ScriptAction_Wait( gentity_t *ent, char *params )
{
	...
	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error( "G_SP_Scripting: wait must have a duration\n" );
	}
	duration = atoi( token );
...
qboolean G_SP_ScriptAction_Trigger( gentity_t *ent, char *params )
{
	...
	if (!name[0])
	{
		G_Error( "G_SP_Scripting: trigger must have a name and an identifier\n" );
	}
	...
	{	// we are triggering an AI
		...
		// UQ1: !!! FIXME !!! - Add full trigger support for the AI!!!
//		AICast_ScriptEvent( AICast_GetCastState( trent->s.number ), "trigger", trigger );
	...
	if (trent) {
		oldId = trent->scriptStatus.scriptId;
		G_SP_RTCW_script_ScriptEvent( trent, "trigger", trigger );
	...
	G_Error( "G_SP_Scripting: trigger has unknown name: %s\n", name );
	return qfalse;	// shutup the compiler
}
#define Q_strcasecmp( a, b )	Q_stricmp( a, b )
/*
================
G_SP_ScriptAction_PlaySound
...
qboolean G_SP_ScriptAction_PlaySound( gentity_t *ent, char *params )
{
	...
	if (!params)
		G_Error("G_SP_Scripting: syntax error\n\nplaysound <soundname OR scriptname>\n");
...
qboolean G_SP_ScriptAction_MusicStart( gentity_t *ent, char *params) {
	...
	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error("G_SP_Scripting: syntax: mu_start <musicfile> <fadeuptime>");
	}
...
qboolean G_SP_ScriptAction_MusicPlay( gentity_t *ent, char *params) {
	...
	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error("G_SP_Scripting: syntax: mu_play <musicfile> [fadeup time]");
	}
	Q_strncpyz( cvarName, token, sizeof(cvarName) );
...
qboolean G_SP_ScriptAction_MusicFade( gentity_t *ent, char *params) {
	...
	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error("G_SP_Scripting: syntax: mu_fade <targetvol> <fadetime>");
	}
...
qboolean G_SP_ScriptAction_MusicQueue( gentity_t *ent, char *params) {
	...
	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error("G_SP_Scripting: syntax: mu_queue <musicfile>");
	}
	Q_strncpyz( cvarName, token, sizeof(cvarName) );
...
qboolean G_SP_ScriptAction_PlayAnim( gentity_t *ent, char *params )
{
	...
	if (token[0]) {
		...
		if (token[0] && !Q_strcasecmp(token, "rate")) {
			token = COM_ParseExt( &pString, qfalse );
			if (!token[0])
				G_Error("G_SP_Scripting: playanim has RATE parameter without an actual rate specified");
			rate = atoi(token);
		}
	...
	if (looping) {
		...
		ent->localEFlags |= LEF_MOVER_ANIMATE;
	} else {
		if (idealframe > endframe) {
			ent->s.frame = endframe;
			ent->localEFlags &= ~LEF_MOVER_ANIMATE;	// stop interpolation, since we have gone passed the endframe
		} else {
	...
	if (endtime <= level.time) {
		ent->localEFlags &= ~LEF_MOVER_ANIMATE;	// stop animating
		return qtrue;
	} else {
		return qfalse;
	}
...
qboolean G_SP_ScriptAction_AlertEntity( gentity_t *ent, char *params )
{
	...
	if (!params || !params[0])
	{
		G_Error( "G_SP_Scripting: alertentity without targetname\n" );
	}
	...
	if (!alertent)
	{
		G_Error( "G_SP_Scripting: alertentity cannot find targetname \"%s\"\n", params );
	}
	...
	if (alertent->client) {
		...
		{
			G_Error( "G_SP_Scripting: alertentity \"%s\" (classname = %s) doesn't have an \"AIScript_AlertEntity\" function\n", params, alertent->classname );
		}
		alertent->AIScript_AlertEntity (alertent);
	} else {
		if (!alertent->use)
		{
			G_Error( "G_SP_Scripting: alertentity \"%s\" (classname = %s) doesn't have a \"use\" function\n", params, alertent->classname );
		}
...
qboolean G_SP_ScriptAction_Accum( gentity_t *ent, char *params )
{
	...
	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error( "G_SP_Scripting: accum without a buffer index\n" );
	}

	bufferIndex = atoi(token);
	if (bufferIndex >= G_MAX_SCRIPT_ACCUM_BUFFERS) {
		G_Error( "G_SP_Scripting: accum buffer is outside range (0 - %i)\n", G_MAX_SCRIPT_ACCUM_BUFFERS );
	}

	token = COM_ParseExt( &pString, qfalse );
	if (!token[0]) {
		G_Error( "G_SP_Scripting: accum without a command\n" );
	}
...
/*
=================
G_SP_ScriptAction_MissionFailed
...
/*
=================
G_SP_ScriptAction_MissionSuccess
...
{
	if (!params || !params[0]) {
		G_Error( "G_SP_Scripting: print requires some text\n" );
	}

	G_Printf("(G_SP_Script) %s-> %s\n", ent->scriptName, params);
	return qtrue;
}

/*
=================
G_SP_ScriptAction_FaceAngles
...
qboolean G_SP_ScriptAction_FaceAngles( gentity_t *ent, char *params )
{
	...
	if (!params || !params[0]) {
		G_Error( "G_SP_Scripting: syntax: faceangles <pitch> <yaw> <roll> <duration/GOTOTIME>\n" );
	}
	...
	if (ent->scriptStatus.scriptStackChangeTime == level.time) {
		...
		token = COM_Parse(&pString);
		if (!token || !token[0]) {
			G_Error( "G_SP_Scripting: faceangles requires a <pitch> <yaw> <roll> <duration/GOTOTIME>\n" );
		}
...
qboolean G_SP_ScriptAction_TagConnect( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_TagConnect: syntax: attachtotag <targetname> <tagname>\n" );
	}
	...
	if (!parent) {
		...
		if (!parent) {
			G_Error( "G_SP_ScriptAction_TagConnect: unable to find entity with targetname \"%s\"", token );
		}
	}
...
qboolean G_SP_ScriptAction_Halt( gentity_t *ent, char *params )
{
	if (level.time == ent->scriptStatus.scriptStackChangeTime) {
		...
		// stop the angles
		BG_EvaluateTrajectory_RTCW( &ent->s.apos, level.time, ent->s.angles );
		VectorCopy( ent->s.angles, ent->s.apos.trBase );
		VectorCopy( ent->s.angles, ent->r.currentAngles );
		ent->s.apos.trTime = level.time;
		ent->s.apos.trDuration = 0;
...
qboolean G_SP_ScriptAction_StartCam( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_Cam: filename parameter required\n" );
	}
...
/*
=================
G_SP_ScriptAction_EntityScriptName
=================
*/
qboolean G_SP_ScriptAction_EntityScriptName( gentity_t *ent, char *params) {
	trap_Cvar_Set( "G_SP_ScriptName", params );
	return qtrue;
}
...
/*
=================
G_SP_ScriptAction_AIScriptName
=================
*/
...
/*
===================
G_SP_ScriptAction_MapDescription
...
/*
===================
G_SP_ScriptAction_AxisRespawntime
...
qboolean G_SP_ScriptAction_AxisRespawntime( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_AxisRespawntime: time parameter required\n" );
	}
...
/*
===================
G_SP_ScriptAction_AlliedRespawntime
...
qboolean G_SP_ScriptAction_AlliedRespawntime( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_AlliedRespawntime: time parameter required\n" );
	}
...
/*
===================
G_SP_ScriptAction_NumberofObjectives
...
qboolean G_SP_ScriptAction_NumberofObjectives( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_NumberofObjectives: number parameter required\n" );
	}

	num = atoi( token );
	if ( num < 1 || num > MAX_OBJECTIVES ) {
		G_Error( "G_SP_ScriptAction_NumberofObjectives: Invalid number of objectives\n" );
	}
...
/*
===================
G_SP_ScriptAction_ObjectiveAxisDesc
...
qboolean G_SP_ScriptAction_ObjectiveAxisDesc( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_ObjectiveAxisDesc: number parameter required\n" );
	}

	num = atoi( token );
	if ( num < 1 || num > MAX_OBJECTIVES ) {
		G_Error( "G_SP_ScriptAction_ObjectiveAxisDesc: Invalid objective number\n" );
	}

	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_ObjectiveAxisDesc: description parameter required\n" );
	}
...
/*
===================
G_SP_ScriptAction_ObjectiveAlliedDesc
...
//	int	num, cs_obj = CS_MULTI_OBJECTIVE1;
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_ObjectiveAlliedDesc: number parameter required\n" );
	}

	num = atoi( token );
	if ( num < 1 || num > MAX_OBJECTIVES ) {
		G_Error( "G_SP_ScriptAction_ObjectiveAlliedDesc: Invalid objective number\n" );
	}

	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_ObjectiveAlliedDesc: description parameter required\n" );
	}
...
qboolean G_SP_ScriptAction_SetWinner( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_SetWinner: number parameter required\n" );
	}

	num = atoi( token );
	if ( num < -1 || num > 1 ) {
		G_Error( "G_SP_ScriptAction_SetWinner: Invalid team number\n" );
	}
...
/*
===================
G_SP_ScriptAction_SetObjectiveStatus
...
//	int	num, status, cs_obj = CS_MULTI_OBJECTIVE1;
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_SetObjectiveStatus: number parameter required\n" );
	}

	num = atoi( token );
	if ( num < 1 || num > MAX_OBJECTIVES ) {
		G_Error( "G_SP_ScriptAction_SetObjectiveStatus: Invalid objective number\n" );
	}

	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_SetObjectiveStatus: status parameter required\n" );
	}

	status = atoi( token );
	if ( status < -1 || status > 1 ) {
		G_Error( "G_SP_ScriptAction_SetObjectiveStatus: Invalid status number\n" );
	}
...
qboolean G_SP_ScriptAction_Announce( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_Announce: statement parameter required\n" );
	}
...
/*
===================
G_SP_ScriptAction_SetRoundTimelimit
...
qboolean G_SP_ScriptAction_SetRoundTimelimit( gentity_t *ent, char *params )
{
	...
	token = COM_Parse(&pString);
	if (!token[0]) {
		G_Error( "G_SP_ScriptAction_SetRoundTimelimit: number parameter required\n" );
	}
...
/*
=================
G_SP_ScriptAction_BackupScript
...
/*
=================
G_SP_ScriptAction_RestoreScript
...
/*
==================
G_SP_ScriptAction_SetHealth
==================
*/
qboolean G_SP_ScriptAction_SetHealth( gentity_t *ent, char *params ) {
	if (!params || !params[0]) {
		G_Error( "G_SP_ScriptAction_SetHealth: sethealth requires a health value\n" );
	}
