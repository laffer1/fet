...
/* */
void
G_LogDeath ( gentity_t *ent, weapon_t weap )
{
...
/* */
void
G_LogKill ( gentity_t *ent, weapon_t weap )
{
	weap = BG_DuplicateWeapon( weap );
	if ( !ent->client )
	{
	...
	if ( ent->client->sess.playerType == PC_SOLDIER )
	{
		...
		for ( i = 0, j = ent->client->soldierKillMarker; i < NUM_SOLDIERKILL_TIMES; i++ )
		{
			if
			(
				!ent->client->soliderKillTimes[j] ||
				(ent->client->soliderKillTimes[j] < level.timeCurrent - SOLDIERKILL_MAXTIME)
			)
			{
				pass = qfalse;
				break;
...
/* */
void
G_LogTeamKill ( gentity_t *ent, weapon_t weap )
{
	weap = BG_DuplicateWeapon( weap );
	if ( !ent->client )
	{
	...
	trap_PbStat( ent - g_entities, "tk", va( "%d %d %d", ent->client->sess.sessionTeam, ent->client->sess.playerType, weap) );
}


/* */
void
G_LogRegionHit ( gentity_t *ent, hitRegion_t hr )
{
	if ( !ent->client )
	{
		return;
	...
	trap_PbStat( ent - g_entities, "hr", va( "%d %d %d", ent->client->sess.sessionTeam, ent->client->sess.playerType, hr) );
}


/* */
void
G_PrintAccuracyLog ( gentity_t *ent )
{
	int		i;
	char	buffer[2048];
	...
	for ( i = 0; i < WP_NUM_WEAPONS; i++ )
	{
		...
		Q_strcat( buffer, 2048, va( " %i %i %i", ent->client->pers.playerStats.weaponStats[i].kills,
	...
	for ( i = 0; i < 6 /*level.numOidTriggers*/ ; i++ )
	{
		...
		Q_strcat( buffer, 2048, va( " %i", ent->client->sess.sessionTeam == TEAM_AXIS ? level.objectiveStatsAxis[i] : level.
					  objectiveStatsAllies[i]) );
	}
...
/* */
void
G_SetPlayerScore ( gclient_t *client )
{
	int i;
...
/* */
void
G_SetPlayerSkill ( gclient_t *client, skillType_t skill )
{
	int i;
...
static void
G_UpgradeSkill ( gentity_t *ent, skillType_t skill )
{
	int i, cnt = 0;
	...
	{
		AddWeaponToPlayer( ent->client, WP_MEDIC_ADRENALINE,
...
/* */
void
G_LoseSkillPoints ( gentity_t *ent, skillType_t skill, float points )
{
	int		oldskill;
	...
	trap_PbStat( ent - g_entities, "loseskill", va( "%d %d %d %f", ent->client->sess.sessionTeam,
				 ent->client->sess.playerType, skill, oldskillpoints - ent->client->sess.skillpoints[skill]) );
	if ( g_gametype.integer != GT_WOLF_TDM && g_gametype.integer != GT_WOLF_DM && g_gametype.integer != GT_WOLF_CTF )
	{
...
/* */
void
G_AddSkillPoints ( gentity_t *ent, skillType_t skill, float points )
{
	int oldskill;
	if ( !ent->client )
	{
...
/* */
void
G_LoseKillSkillPoints ( gentity_t *tker, meansOfDeath_t mod, hitRegion_t hr, qboolean splash )
{
...
/* */
void
G_AddKillSkillPoints ( gentity_t *attacker, meansOfDeath_t mod, hitRegion_t hr, qboolean splash )
{
	if ( !attacker->client )
	{
		return;
	...
	switch ( mod )
	{
		...
		case MOD_FG42SCOPE:
			switch ( hr )
			{
				...
				case HR_GROIN:
					G_AddSkillPoints( attacker, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, 5.f );
					G_DebugAddSkillPoints( attacker, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS, 5.f, "groin kill" );
					break;
				case HR_ARMS:
...
)
{
	switch ( mod )
	{
		...
		case MOD_SATCHEL:
			...
			G_DebugAddSkillPoints( attacker, SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS,
								   constructibleStats->destructxpbonus, "destroying a constructible/explosive" );
			break;
		default:
			break;
	}
...
G_DebugOpenSkillLog ( void )
{
	...
	if
	(
		trap_FS_FOpenFile( va( "skills-%d-%02d-%02d-%02d%02d%02d-%s.log", 1900 + ct.tm_year, ct.tm_mon + 1, ct.tm_mday,
...
/* */
void
G_DebugAddSkillLevel ( gentity_t *ent, skillType_t skill )
{
	qtime_t ct;
	...
	trap_SendServerCommand( ent - g_entities, va(
								"sdbg \"^%c(SK: %2i XP: %6.2f) %s: You raised your skill level to %i.\"\n", COLOR_RED + skill,
							ent->client->sess.skill[skill], ent->client->sess.skillpoints[skill], skillNames[skill],
	...
	if ( g_debugSkills.integer >= 2 && skillDebugLog != -1 )
	{
		char	*s = va( "%02d:%02d:%02d : ^%c(SK: %2i XP: %6.2f) %s: %s raised in skill level to %i.\n", ct.tm_hour,
						 ct.tm_min, ct.tm_sec, COLOR_RED + skill, ent->client->sess.skill[skill],
						 ent->client->sess.skillpoints[skill], skillNames[skill], ent->client->pers.netname,
						 ent->client->sess.skill[skill] );
		trap_FS_Write( s, strlen( s), skillDebugLog );
	}
}
...
/* */
void
G_DebugAddSkillPoints ( gentity_t *ent, skillType_t skill, float points, const char *reason )
{
	qtime_t ct;
	if ( !g_debugSkills.integer )
	{
	...
	trap_SendServerCommand( ent - g_entities, va(
								"sdbg \"^%c(SK: %2i XP: %6.2f) %s: You gained %.2fXP, reason: %s.\"\n", COLOR_RED + skill,
							ent->client->sess.skill[skill], ent->client->sess.skillpoints[skill], skillNames[skill],
							points, reason) );
	trap_RealTime( &ct );
	if ( g_debugSkills.integer >= 2 && skillDebugLog != -1 )
	{
		char	*s = va( "%02d:%02d:%02d : ^%c(SK: %2i XP: %6.2f) %s: %s gained %.2fXP, reason: %s.\n", ct.tm_hour,
						 ct.tm_min, ct.tm_sec, COLOR_RED + skill, ent->client->sess.skill[skill],
						 ent->client->sess.skillpoints[skill], skillNames[skill], ent->client->pers.netname, points,
						 reason );
		trap_FS_Write( s, strlen( s), skillDebugLog );
	}
}
...
#define CHECKSTATMIN( XX, YY )																  \
	...
	Q_strcat( buffer, 1024, va( ";%s; %i ", best && best->XX >= YY ? best->pers.netname : "", \
			  best && best->XX >= YY ? best->sess.sessionTeam : -1) )
#define CHECKSTATSKILL( XX )																						\
	...
	{																												\
		...
		(																											\
			!best ||																								\
			(cl->sess.skillpoints[XX] - cl->sess.startskillpoints[XX]) >											\
				(best->sess.skillpoints[XX] - best->sess.startskillpoints[XX])										\
		)																											\
		{																											\
			best = cl;																								\
		}																											\
	...
	Q_strcat( buffer, 1024, va( ";%s; %i ", best /*&& best->sess.skillpoints[XX] >= 20*/ ? best->pers.netname : "", \
			  best && best->sess.skillpoints[XX] >= 20 ? best->sess.sessionTeam : -1) )
