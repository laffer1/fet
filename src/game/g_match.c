...
void
G_printFull ( char *str, gentity_t *ent )
{
	if ( ent != NULL )
	{
		CP( va( "print \"%s\n\"", str) );
...
/* */
void
G_spawnPrintf ( int print_type, int print_time, gentity_t *owner )
{
...
void
G_addStats ( gentity_t *targ, gentity_t *attacker, int dmg_ref, int mod )
{
	int dmg, ref;
...
void
G_addStatsHeadShot ( gentity_t *attacker, int mod )
{
#ifndef DEBUG_STATS
...
static const weap_ws_convert_t	aWeapMOD[MOD_NUM_MODS] =
{
	...
	// where is silencer? // Gordon: up top^
...
unsigned int
G_weapStatIndex_MOD ( unsigned int iWeaponMOD )
{
	unsigned int	i;
...
char *
G_createStats ( gentity_t *refEnt )
{
	...
	if ( dwWeaponMask != 0 )
	{
		Q_strcat( strWeapInfo, sizeof(strWeapInfo), va( " %d %d %d", refEnt->client->sess.damage_given,
	...
	return
		(
			va( "%d %d %d%s %d%s", refEnt - g_entities, refEnt->client->sess.rounds, dwWeaponMask, strWeapInfo,
...
void
G_parseStats ( char *pszStatsInfo )
{
	gclient_t		*cl;
...
void
G_printMatchInfo ( gentity_t *ent )
{
	int			i, j, cnt, eff;
	...
	for ( i = TEAM_AXIS; i <= TEAM_ALLIES; i++ )
	{
		...
		for ( j = 0; j < level.numPlayingClients; j++ )
		{
			...
			CP( va( "sc \"%-10s %s%-15s^3%4d%4d%4d%3d%s%4d^3%4d^2%6d^1%6d^6%5d^3%7d\n\"", aTeams[i], ref, n2,
				cl->sess.kills, cl->sess.deaths, cl->sess.suicides, cl->sess.team_kills, ref, eff,
				cl->sess.game_points - (cl->sess.kills * WOLF_FRAG_BONUS), cl->sess.damage_given,
				cl->sess.damage_received, cl->sess.team_damage, cl->ps.persistant[PERS_SCORE]) );
		}
		...
		aTeams[i], "Totals", tot_kills, tot_deaths, tot_sui, tot_tk, eff, tot_gp - (tot_kills * WOLF_FRAG_BONUS),
			tot_dg, tot_dr, tot_td, tot_gp) );
	}
...
void
G_matchInfoDump ( unsigned int dwDumpType )
{
	int			i, ref;
	gentity_t	*ent;
	gclient_t	*cl;
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
		...
		else if ( dwDumpType == EOM_MATCHINFO )
		{
			...
			if ( g_gametype.integer == GT_WOLF_STOPWATCH )
			{
				...
				{
					float	val = (float)
						( (level.timeCurrent - (level.startTime + level.time - level.intermissiontime)) / 60000.0 );
					if ( val < g_timelimit.value )
					{
						CP( va( "print \">>> ^3Objective reached at %d:%02d (original: %d:%02d)\n\n\n\"", (int) val,
							(int) (60.0 * (val - (int) val)), g_timelimit.integer,
...
int
G_checkServerToggle ( vmCvar_t *cv )
{
	int nFlag;
...
void
G_statsPrint ( gentity_t *ent, int nType )
{
	int		pid;
