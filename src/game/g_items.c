...
// Jaquboss , hmm , maybe they can throw medkits , ammoboxes and syringes , but it shouldnt be...

// We also need to visualise kits , and etc...
int
Pickup_Kit ( gentity_t *ent, gentity_t *other )
{
	switch ( ent->item->giTag )
	{
		case KIT_MEDIC:
			drop->item->giAmmoIndex = ps->ammoclip[BG_FindClipForWeapon( WP_MEDIC_SYRINGE )];
			drop->item->giClipIndex = ps->ammoclip[BG_FindClipForWeapon( WP_MEDKIT )];
			break;
	}
...
case KIT_FIELDOPS:
{
	drop->item->giAmmoIndex = ps->ammoclip[
	BG_FindClipForWeapon( WP_AMMO )
	];
	drop->item->giClipIndex = ps->ammoclip[
	BG_FindClipForWeapon( WP_SMOKE_MARKER )
	];
	break;
}
case KIT_COVERTOPS:
{
	drop->item->giAmmoIndex = ps->ammoclip[
	BG_FindClipForWeapon( WP_SMOKE_BOMB )
	];
...
*/
int
Pickup_Clipboard ( gentity_t *ent, gentity_t *other )
{
	if ( ent->spawnflags & 4 )
	{
...
*/
int
Pickup_Treasure ( gentity_t *ent, gentity_t *other )
{
	return ( -1 );
}
...
*/
void
UseHoldableItem ( gentity_t *ent, int item )
{
	switch ( item )
	{
		case HI_MEDKIT:
			...
			break;
		case HI_HEALTH: // jaquboss , portable medkit
			ent->health += 20;
...
*/
void
Fill_Clip ( playerState_t *ps, int weapon )
{
...
int
Add_Ammo ( gentity_t *ent, int weapon, int count, qboolean fillClip )
{
	int ammoweap = BG_FindAmmoForWeapon( weapon );
	int originalCount;
...
// TODO TCE RELOADS // TODO TCE RELOADS // TODO TCE RELOADS // TODO TCE RELOADS
int
Pickup_Ammo ( gentity_t *ent, gentity_t *other )
{
	...
	if ( ent->count )
	{
		...
		Add_Ammo( other, ent->item->giTag, ent->count2, qfalse );
	}
	else
	{
...
*/
qboolean
AddMagicAmmo ( gentity_t *receiver, int numOfClips )
{
	return
		(
			BG_AddMagicAmmo( &receiver->client->ps, receiver->client->sess.skill, receiver->client->sess.sessionTeam,
							 numOfClips)
		);
}
...
//======================================================================
weapon_t
G_GetSecondaryWeaponForClient ( gclient_t *client )
{
	...
	if ( COM_BitCheck( client->ps.weapons, WP_SILENCED_COLT) )
	{
		return ( WP_SILENCED_COLT );
	}

	if ( COM_BitCheck( client->ps.weapons, WP_COLT) )
	{
		return ( WP_COLT );
	}
	...
	if ( COM_BitCheck( client->ps.weapons, WP_SILENCER) )
	{
		return ( WP_SILENCER );
	}

	if ( COM_BitCheck( client->ps.weapons, WP_LUGER) )
	{
		return ( WP_LUGER );
	}
...
/* */
weapon_t
G_GetPrimaryWeaponForClient ( gclient_t *client )
{
	int					i;
...
// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS
void
G_DropWeapon ( gentity_t *ent, weapon_t weapon )
{
	vec3_t		angles, velocity, org, offset, mins, maxs, org2, offset2;
	...
	if ( BG_IsSecondaryWeapon( weapon) )
	{
		VectorScale( velocity, 128, offset2 );	// something more
	}

	VectorScale( velocity, 512, offset2 );
	offset[2] += client->ps.viewheight / 2.f;
	offset2[2] += client->ps.viewheight / 2.f;
	if ( BG_IsSecondaryWeapon( weapon) )
	{
	...
	VectorAdd( client->ps.origin, offset2, org2 );
	...
	else if ( weapon == WP_30CAL )
	{
		COM_BitClear( client->ps.weapons, WP_30CAL_SET );
	}
	else if ( weapon == WP_SILENCER )
	{
		COM_BitClear( client->ps.weapons, WP_LUGER );
	}
	else if ( weapon == WP_SILENCED_COLT )
	{
		COM_BitClear( client->ps.weapons, WP_COLT );
	}
	...
	else if ( weapon == WP_AKIMBO_LUGER )
	{
		COM_BitClear( client->ps.weapons, WP_LUGER );

		//	ent2->delay = 1;
	}
	else if ( weapon == WP_AKIMBO_SILENCEDLUGER )
	{
		COM_BitClear( client->ps.weapons, WP_SILENCER );

		//	ent2->delay = 1;
	}
	else if ( weapon == WP_AKIMBO_COLT )
	{
		COM_BitClear( client->ps.weapons, WP_COLT );

		//	ent2->delay = 1;
	}
	else if ( weapon == WP_AKIMBO_SILENCEDCOLT )
	{
		COM_BitClear( client->ps.weapons, WP_SILENCED_COLT );
	...
	else
	{
		...
		//	if ( ent2->delay != 1 ) // later for better drops...
		ent2->delay = 0;
	}
...
/* */
qboolean
G_CanPickupWeapon ( weapon_t weapon, gentity_t *ent )
{
	...
	else if ( ent->client->sess.sessionTeam == TEAM_ALLIES )
	{
		...
		else if ( weapon == WP_MP44 )
		{	//well
			weapon = WP_BAR;
		}
	}

	if ( weapon == WP_LUGER || weapon == WP_COLT )
	{
		return ( qtrue );
	}
	...
	if
	(
		(weapon == WP_AKIMBO_LUGER || weapon == WP_AKIMBO_COLT) &&
		(ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 4 || g_akimboPickups.integer)
	)
	{
		return ( qtrue );
	}

	if ( (weapon == WP_SILENCER || weapon == WP_SILENCED_COLT) && ent->client->sess.playerType == PC_COVERTOPS )
	{
		return ( qtrue );
	}
	...
	if
	(
		(weapon == WP_AKIMBO_SILENCEDLUGER || weapon == WP_AKIMBO_SILENCEDCOLT) &&
		(ent->client->sess.skill[SK_LIGHT_WEAPONS] >= 4 || g_akimboPickups.integer) &&
	...
	if
	(
		ent->client->sess.skill[SK_HEAVY_WEAPONS] >= 4 &&
		(weapon == WP_THOMPSON || weapon == WP_MP40 || WP_M1GARAND || WP_G43)
	)
	{
		return ( qfalse );
	}
...
// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS// TODO TCE RELOADS
int
Pickup_Weapon ( gentity_t *ent, gentity_t *other )
{			//1
	int			quantity = 0, quantity2 = 0, quantity3 = 0, quantity4 = 0;
	qboolean	alreadyHave = qfalse;
	qboolean	akimboPick = qfalse;
	...
	{		//2
		...
		if ( ent->parent )
		{
			ent->parent->last_helped_client = other->client->ps.clientNum;
			ent->parent->last_helped_time = level.time;
		}

		if ( other->r.svFlags & SVF_BOT && !(ent->parent->r.svFlags & SVF_BOT) )
		{	// make bots thanks for ammo
			G_AI_Voice( other, NULL, SAY_TEAM, "Thanks", qfalse, 3000 + rand() % 1000, AIVOICE_TALK );	// rather bigger delay so they should thank after latest pack
		}
#endif
	...
	if (ent->item->giTag != WP_AMMO && level.gametype < GT_WOLF && ent->count <= 0)
	{// UQ1: Add ammo to this single player map's weapon...
		ent->count = GetAmmoTableData( ent->item->giTag )->defaultStartingAmmo;
		ent->count2 = GetAmmoTableData( ent->item->giTag )->defaultStartingClip;
	}

	quantity = ent->count;
	quantity2 = ent->count2;

	// check if player already had the weapon
	alreadyHave = COM_BitCheck( other->client->ps.weapons, ent->item->giTag );
	if
	(
		G_GetSecondaryWeaponForClient( other->client) == ent->item->giTag &&
		!(BG_IsAkimboWeapon( ent->item->giTag)) &&
		g_akimboPickups.integer
	)
	{
		akimboPick = qtrue;
		quantity3 = other->client->ps.ammoclip[BG_FindClipForWeapon( G_GetSecondaryWeaponForClient( other->client) )];
		quantity4 = other->client->ps.ammo[BG_FindClipForWeapon( G_GetSecondaryWeaponForClient( other->client) )];
	}

	// JPW NERVE  prevents drop/pickup weapon "quick reload" exploit
	if ( alreadyHave && !(akimboPick) )
	{					//7
		if ( ent->item->giTag != WP_M1GARAND && ent->item->giTag != WP_GARAND && ent->item->giTag != WP_CARBINE )
		{				// Garand weapons used clips cant be finished
			Add_Ammo( other, ent->item->giTag, quantity, qfalse );
		}				//8

		Add_Ammo( other, ent->item->giTag, quantity2, qfalse );
	...
	{					// 7 10
		...
		if
		(
			other->client->ps.weapon == WP_MORTAR_SET ||
			other->client->ps.weapon == WP_MOBILE_MG42_SET ||
			other->client->ps.weapon == WP_30CAL_SET
		)
		{				//12
			return ( 0 );
		}				//12
		...
		if ( G_CanPickupWeapon( ent->item->giTag, other) || akimboPick )
		{				//13
			weapon_t	primaryWeapon = G_GetPrimaryWeaponForClient( other->client );
			weapon_t	secondaryWeapon = G_GetSecondaryWeaponForClient( other->client );
			if ( primaryWeapon && BG_IsPrimaryWeapon( ent->item->giTag) )
			{			//14
			...
			else if ( secondaryWeapon && BG_IsSecondaryWeapon( ent->item->giTag) && !(akimboPick) )
			{
				G_DropWeapon( other, secondaryWeapon );
			}
			...
			else if ( ent->item->giTag == WP_30CAL )
			{
				COM_BitSet( other->client->ps.weapons, WP_30CAL_SET );
			}
			...
			else if ( ent->item->giTag == WP_AKIMBO_SILENCEDLUGER )
			{
				COM_BitSet( other->client->ps.weapons, WP_SILENCER );
			}
			else if ( ent->item->giTag == WP_AKIMBO_LUGER )
			{
				COM_BitSet( other->client->ps.weapons, WP_LUGER );
			}
			else if ( ent->item->giTag == WP_AKIMBO_SILENCEDCOLT )
			{
				COM_BitSet( other->client->ps.weapons, WP_SILENCED_COLT );
			}
			else if ( ent->item->giTag == WP_AKIMBO_COLT )
			{
				COM_BitSet( other->client->ps.weapons, WP_COLT );
			}
			...
			if ( akimboPick )
			{
				...
				{
					case WP_LUGER:
						COM_BitSet( other->client->ps.weapons, WP_AKIMBO_LUGER );

						//COM_BitClear( other->client->ps.weapons, WP_LUGER );
						other->client->ps.ammoclip[BG_FindClipForWeapon( WP_AKIMBO_LUGER )] = quantity;
						other->client->ps.ammoclip[BG_FindClipForWeapon( BG_AkimboSidearm( WP_AKIMBO_LUGER) )] = quantity3;
						other->client->ps.ammo[BG_FindAmmoForWeapon( WP_AKIMBO_LUGER )] = quantity2 + quantity4;
						break;
					case WP_SILENCER:
						COM_BitSet( other->client->ps.weapons, WP_AKIMBO_SILENCEDLUGER );

						//COM_BitClear( other->client->ps.weapons, WP_SILENCER );
						COM_BitClear( other->client->ps.weapons, WP_LUGER );
						other->client->ps.ammoclip[BG_FindClipForWeapon( WP_AKIMBO_SILENCEDLUGER )] = quantity;
						other->client->ps.ammoclip[BG_FindClipForWeapon( BG_AkimboSidearm( WP_AKIMBO_SILENCEDLUGER) )] = quantity3;
						other->client->ps.ammo[BG_FindAmmoForWeapon( WP_AKIMBO_SILENCEDLUGER )] = quantity2 + quantity4;
						break;
					case WP_COLT:
						COM_BitSet( other->client->ps.weapons, WP_AKIMBO_COLT );

						//COM_BitClear( other->client->ps.weapons, WP_COLT );
						other->client->ps.ammoclip[BG_FindClipForWeapon( WP_AKIMBO_COLT )] = quantity;
						other->client->ps.ammoclip[BG_FindClipForWeapon( BG_AkimboSidearm( WP_AKIMBO_COLT) )] = quantity3;
						other->client->ps.ammo[BG_FindAmmoForWeapon( WP_AKIMBO_COLT )] = quantity2 + quantity4;
						break;
					case WP_SILENCED_COLT:
						COM_BitSet( other->client->ps.weapons, WP_AKIMBO_SILENCEDCOLT );

						//COM_BitClear( other->client->ps.weapons, WP_SILENCED_COLT );
						COM_BitClear( other->client->ps.weapons, WP_COLT );
						other->client->ps.ammoclip[BG_FindClipForWeapon( WP_AKIMBO_SILENCEDCOLT )] = quantity;
						other->client->ps.ammoclip[BG_FindClipForWeapon( BG_AkimboSidearm( WP_AKIMBO_SILENCEDCOLT) )] = quantity3;
						other->client->ps.ammo[BG_FindAmmoForWeapon( WP_AKIMBO_SILENCEDCOLT )] = quantity2 + quantity4;
						break;
					default:
						break;
				}
			...
			else if ( !akimboPick )
			{
				...
				other->client->ps.ammo[BG_FindAmmoForWeapon( ent->item->giTag )] = quantity2;
...
Pickup_Health ( gentity_t *ent, gentity_t *other )
{
	...
	if ( other == ent->parent && g_noSelfHeal.integer > 0 )
	{
		return ( 0 );
	}
	...
	// Kamikazee: Players healing medics didn't get any XP.
	//	(Thx to Dreadnought for noticing this)
	if( other != ent->parent )
	{
		if
		(
	...
	{												// can't eat plates damnit!
		return ( 0 );
	}

	if ( other->client->ps.eFlags & EF_ARMSHOT )
	{
		other->client->ps.eFlags &= ~EF_ARMSHOT;	// heal armshot
	}
	...
	if ( other->r.svFlags & SVF_BOT && !(ent->parent->r.svFlags & SVF_BOT) )
	{												// make bots thanks for health
		G_AI_Voice( other, NULL, SAY_TEAM, "Thanks", qfalse, 3000 + rand() % 1000, AIVOICE_TALK );	// rather bigger delay so they should thank after latest pack
	}
#endif
	other->s.dl_intensity = other->health;
	if ( ent->s.density == 2 )
	{	// multi-stage health first stage
		return ( RESPAWN_PARTIAL );
	}
...
*/
void
Touch_Item_Auto ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
...
Touch_Item ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	...
	// Kamikazee: Bots will now allways pick things up without activating them.
	if ( !ent->active && ! (other->r.svFlags & SVF_BOT) )
	{
		return;
	}
	else
	...
	if ( other->client->ps.eFlags & EF_TANK )
	{
		return;							// can't pickup anything from a tank
	}
...
*/
gentity_t *
LaunchItem ( gitem_t *item, vec3_t origin, vec3_t velocity, int ownerNum )
{
	gentity_t	*dropped;
	trace_t		tr;
	vec3_t		vec, temp;
	...
	if
	(
		g_noautopickups.integer > 0 &&
		(item->giType == IT_HEALTH || item->giType == IT_AMMO || item->giType == IT_WEAPON)
	)
	{
	...
	{	// Special case for CTF flags
		...
		*/
		...
		{
			int		n = 0;
			char	*params[2] = { "axis_object_dropped", "allied_object_dropped" };
			if ( flag->item && flag->item->giTag == PW_REDFLAG )
			{
				n = 1;
			}

			G_Script_ScriptEvent( level.gameManager, "trigger", params[n] );
		}
...
*/
gentity_t *
Drop_Item ( gentity_t *ent, gitem_t *item, float angle, qboolean novelocity )
{
	vec3_t	velocity;
	vec3_t	angles;
...
FinishSpawningItem ( gentity_t *ent )
{
	...
	(
		g_noautopickups.integer > 0 &&
		(ent->item->giType == IT_HEALTH || ent->item->giType == IT_AMMO || ent->item->giType == IT_WEAPON)
	)
	{
...
G_BounceItem ( gentity_t *ent, trace_t *trace )
{
	...
	if ( trace->plane.normal[2] > 0 && ent->s.pos.trDelta[2] < 40 )
	{
		...
		if ( ent->item && ent->item->giType == IT_WEAPON && ent->item->giTag != WP_NONE )
		{							// added ent->item, fixing crash
			G_AddEvent( ent, EV_WEAPON_BOUNCE, BG_FootstepForSurface( trace->surfaceFlags) );	// um check bounce :-S
		}
...
*/
void
G_RunItemProp ( gentity_t *ent, vec3_t origin )
{
	gentity_t	*traceEnt;
	trace_t		trace;
	gentity_t	*owner;
