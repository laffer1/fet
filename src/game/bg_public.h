...
#define GAME_VERSION_NUM	"0.5.0"
#define GAME_VERSION		"FalckonET 0.5.0"
#if defined( _DEBUG )
...
//#define MV_SUPPORT		//  dunno how make this work good
//#define	GAME_VERSION_DATED "FalckonET 0.3.9"
//#define SAVEGAME_SUPPORT	// uncomment to enable savegames
// enabling this requires you to run extractfuncs.bat as well before compiling qagame
#define NWEAPS													// jaquboss , ifdefs because missing art ( models...)
#define NAZI_SALUTE												// jaquboss , this is problematic , if we will like to have nazi realism then we can allow it , but now it should be commented , because we want to keep ET neutrality

//#define ALLOW_GSYNC			// jaquboss , can be buggy m but interesting for demo recording..
//#define USEXPSTORAGE
//#define NO_BOT_SUPPORT // Jaquboss , they just suck and it is not needed , with real AI like omni-bot it will be much better , but with omnibots there will be no reason to comment this line...
//#define BONE_HITTESTS
#define SPRINTTIME					35000.0f
#define DEBUG_BOT_RETREATBEHAVIOR	1
#define DEFAULT_GRAVITY				800
#define FORCE_LIMBO_HEALTH			- 50						// JPW NERVE
#define GIB_HEALTH					- 300						// JPW NERVE
...
#define AAGUN_DAMAGE			55
#define AAGUN_SPREAD			200
...
#define MG42_SPREAD_MP			420
#define BROWNING_SPREAD			300
#define MG42_DAMAGE_MP			38
#define BROWNING_DAMAGE			45
#define MG42_RATE_OF_FIRE_MP	40
#define MG42_DAMAGE_SP			40								// this sucks
#define MG42_RATE_OF_FIRE_SP	100
#define AAGUN_RATE_OF_FIRE		125
...
#define MAX_GAME_CHARACTERS		256
...
// New Particle System...
#define CS_PARTICLE_SYSTEMS ( CS_SOUNDS + MAX_SOUNDS )
#define CS_PARTICLE_FILES	( CS_PARTICLE_SYSTEMS + MAX_GAME_PARTICLE_SYSTEMS )
#define CS_SHADERS			( CS_PARTICLE_FILES + MAX_PARTICLE_FILES )
#else //!__PARTICLE_SYSTEM__
...
typedef enum
{
	...
	GT_WOLF_CTF,
#ifdef __SUPREMACY__
	GT_SUPREMACY,			// Kamikazee: keep these at the end, so a change in defines doesn't change other
	GT_SUPREMACY_CAMPAIGN,	//	gametype numbers.
#endif //__SUPREMACY__
	GT_MAX_GAME_TYPE
} gametype_t;
...
typedef enum
{
	...
	WEAPON_LOWERED,												// jaquboss, weapon inactive - ladders and player freezes atm
} weaponstate_t;
typedef enum
{
	WSTATE_IDLE,
...
#define PMF_RADIO			1024								// just for animation
#define PMF_FLAILING		2048
#define PMF_FOLLOW			4096								// spectate following another player
#define PMF_TIME_LOCKMOVE	8192								// hold for this time after a load game, and prevent large thinks
...
#define PMF_FIREHELD		65536
#define PMF_ALL_TIMES		( PMF_TIME_WATERJUMP | PMF_TIME_LAND | PMF_TIME_KNOCKBACK | PMF_TIME_LOCKPLAYER | PMF_TIME_LOCKMOVE )
typedef struct
{
...
typedef struct
{
	...
	// Unique1...
	entityState_t * ( *getentitystate ) (int num);
} pmove_t;
...
void	PM_UpdateViewAnglesVehicle
		(
			playerState_t	*ps,
			pmoveExt_t		*pmext,
			usercmd_t		*cmd,
...
typedef enum
{
	...
	STAT_DMG,													// Jaquboss: for pain animations
	STAT_CROSSHAIR_YAW,
	STAT_CROSSHAIR_PITCH,
#ifdef __SUPREMACY__
	STAT_FLAG_CAPTURE,
	STAT_CAPTURE_ENTITYNUM,
#endif //__SUPREMACY__
} statIndex_t;
...
#define EF_FORCE_END_FRAME	EF_NONSOLID_BMODEL					// force client to end of current animation (after loading a savegame) , jaquboss , this is now clear to use :-)
#define EF_AIMING			0x00000004							// was EF_TELEPORT_BIT , now EF_AIMING , used of ADS
...
#define EF_BROWNING_ACTIVE	0x00000400							// currently using an .30 cal , jaquboss

//#define	EF_ARMSHOT			0x00000800	// not needed for client ;-(
...
//#define EF_LADDER			EF_FIRING		// Player on ladder , not so important flag... , changed , now removes EF_FIRING flag
//#define EF_FLAMER			0x00010000		// Player has ammo for flamethrower , jaquboss
#define EF_PATH_LINK	0x00020000								// Gordon: linking trains together
#define EF_TURRETMOVE	EF_PATH_LINK							// jaquboss , send to entity state only
...
// UQ1: Theres no client side code for this.
// ... So I moved it!
#ifdef __BOT__
#define EF_BOT	0x00200000										// client is a bot
#else //!__BOT__
#define EF_SPARE3	0x00200000									// Free if bots are disabled!
#endif //__BOT__
#define EF_AAGUN_ACTIVE 0x00400000								// Gordon: player is manning an AA gun
#define EF_TANK			0x00800000								// @TODO rename to EF_VEHICLE
...
#define EF_SMOKINGBLACK		0x02000000							// JPW NERVE -- like EF_SMOKING only darker & bigger
#define EF_BOUNCE			0x04000000							// for missiles
#define EF_BOUNCE_HALF		0x08000000							// for missiles
#define EF_ARMSHOT			0x10000000							// not needed for client ;-(
#define EF_SWIMMING			0x20000000							// Player is in water
#define EF_VIEWING_CAMERA	0x40000000							// player is viewing a camera
#define EF_TELEPORT_BIT		0x80000000							// toggled every time the origin abruptly changes

// UQ1: Added - Local EFlags (ent->localEFlags)
#define LEF_MOVER_STOP		0x00000001							// will push otherwise	// (SA) moved down to make space for one more client flag
#define LEF_MOVER_BLOCKED	0x00000002							// mover was blocked dont lerp on the client // xkan, moved down to make space for client flag
#define LEF_MOVER_ANIMATE	0x00000004

#define BG_PlayerMounted( eFlags )			 \
		(									 \
			(eFlags & EF_MG42_ACTIVE) ||	 \
			(eFlags & EF_BROWNING_ACTIVE) || \
...
typedef enum
{
	...
	// Jaquboss new weapons
	WP_G43,														// 47
	WP_M1GARAND,												// 48
	WP_DUMMY_MG42,												// 49 // it is dummy , so it can be at end
	WP_30CAL,													// 50
	WP_30CAL_SET,												// 51
#ifdef NWEAPS
	WP_MP44,
...
#define WEAPS_ONE_HANDED	( (1 << WP_KNIFE) | (1 << WP_LUGER) | (1 << WP_COLT) | (1 << WP_SILENCER) | (1 << WP_SILENCED_COLT) | \
							   (1 << WP_GRENADE_LAUNCHER) | (1 << WP_GRENADE_PINEAPPLE) )
...
typedef enum
{
	...
	EV_FLAK_HIT_WALL,
	EV_PUSH_SOUND,
	EV_GARAND,					// if M1 Garand is beign reloaded , and it is not empty , then make ping sound
	EV_WEAPON_BOUNCE,
	EV_FLAG,
	EV_FLAG_AXIS,
	EV_FLAG_ALLIES,
...
typedef enum
{
	...
	WEAP_ATTACK1,				//2
	WEAP_ATTACK2,				//3
	WEAP_ATTACK_LASTSHOT,		//4	// used when firing the last round before having an empty clip.
	WEAP_DROP,					//5
	WEAP_RAISE,					//6
	WEAP_RELOAD1,				//7
	WEAP_RELOAD2,				//8
	WEAP_RELOAD3,				//9
	WEAP_ALTSWITCHFROM,			//10	// switch from alt fire mode weap (scoped/silencer/etc)
	WEAP_ALTSWITCHTO,			//11	// switch to alt fire mode weap
	WEAP_DROP2,					//12
	WEAP_RELOAD4,				//13
	MAX_WP_ANIMATIONS
} weapAnimNumber_t;
...
qboolean		BG_CheckWeapon ( int weapon );
...
qboolean		BG_IsAltLoadedWeapon ( int weaponNum );
weapon_t		BG_GetPrimaryWeapon ( playerState_t *ps );
...
typedef enum
{
	...
	ANIM_ET_TALK,
	ANIM_ET_NOPOWER,
	ANIM_ET_NAZI,												// this should be made to me more neutral , however it may be used
	ANIM_ET_DROP,
	ANIM_ET_FORCEDEATH,
	ANIM_ET_FORCEDEATH_PRONE,
	ANIM_ET_TAUNT,
	ANIM_ET_DMDEATH,
	ANIM_ET_REVIVING,
	NUM_ANIM_EVENTTYPES
...
int						BG_GetAnimScriptEvent_NPC ( scriptAnimEventTypes_t event );
...
qboolean BG_IsScopedWeapon2 ( int weapon );
qboolean BG_IsPrimaryWeapon ( int weapon );
qboolean BG_IsAdsWeapon ( int weapon );
qboolean BG_IsSelectiveWeapon ( int weapon );
qboolean BG_IsSemiFireWeapon ( int weapon );
qboolean BG_IsSecondaryWeapon ( int weapon );
...
#define MAX_CUSTOM_MODELS	16
#define MAX_CUSTOM_SKINS	32
typedef struct headModelsDef_s
{
	char	modelPath[MAX_CUSTOM_MODELS][MAX_QPATH];
} headModelsDef_t;
typedef struct headSkinsDef_s
{
	char	skinPath[MAX_CUSTOM_SKINS][MAX_QPATH];
} headSkinsDef_t;

//qboolean BG_CheckBag( const char *filename, bg_characterDef_t* characterDef );
qboolean		BG_ParseLoadoutFile ( const char *filename, bg_playerclass_t *characterDef );
qboolean		BG_ParseCharacterFile ( const char *filename, bg_characterDef_t *characterDef );
int				BG_ParseHeadSkinsFile ( const char *filename, headSkinsDef_t *headDef );
