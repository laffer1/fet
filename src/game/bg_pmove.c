...
#include "bg_local.h"
#ifdef CGAMEDLL
#define PM_RealMedic	cg_realmedic.integer
#define PM_NoCapsule	cg_noCapsule.integer
#define PM_GameType		cg_gameType.integer
#elif GAMEDLL
#define PM_RealMedic	g_realmedic.integer
#define PM_GameType		g_gametype.integer
#define PM_NoCapsule	g_noCaps.integer
#endif
#define PM_IsSinglePlayerGame() ( PM_GameType == GT_SINGLE_PLAYER || PM_GameType == GT_COOP )
#ifdef CGAMEDLL
extern vmCvar_t cg_AutoReloads;
#define AUTO_RELOADS	cg_AutoReloads.integer
#elif GAMEDLL
extern vmCvar_t g_AutoReloads;
#define AUTO_RELOADS	g_AutoReloads.integer
#endif


// JPW NERVE -- stuck this here so it can be seen client & server side
float
Com_GetFlamethrowerRange ( void )
{
...
float	pm_proneSpeedScale = 0.28;	// because reduced speed , prone needs to be boosted
...
/* */
void
PM_AddEventExt ( int newEvent, int eventParm )
{
...
/* */
int
PM_IdleAnimForWeapon ( int weapon )
{
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
int
PM_AltSwitchFromForWeapon ( int weapon )
{
	switch ( weapon )
	{
		default:
...
/* */
int
PM_AltSwitchToForWeapon ( int weapon )
{	// IMPORTANT
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
int
PM_AttackAnimForWeapon ( int weapon )
{
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
int
PM_LastAttackAnimForWeapon ( int weapon )
{
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
int
PM_ReloadAnimForWeapon ( int weapon )
{
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
int
PM_RaiseAnimForWeapon ( int weapon )
{
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
int
PM_DropAnimForWeapon ( int weapon )
{
	switch ( weapon )
	{
		case WP_GPG40:
...
/* */
void
PM_ContinueWeaponAnim ( int anim )
{
...
)
{
	...
	if ( forward )
	{
		VectorScale( flatforward, 72, ofs );
	}
	else
	{
		VectorScale( flatforward, -72, ofs );
	...
	tracefunc( trace, org, tankTraceMins, tankTraceMaxs, point, ignoreent, tracemask );
	if ( !bodytrace || trace->fraction < bodytrace->fraction || trace->allsolid )
	{
		...
		VectorAdd( start, ofs, org );
		VectorAdd( end, ofs, point );
		tracefunc( &steptrace, org, tankTraceMins, tankTraceMaxs, point, ignoreent, tracemask );
...
)
{
	...
	vec3_t	flatforward;
	float	angle;
	vec3_t	realmaxs, realmins;
	if ( pm->ps->eFlags & EF_TANK )
	{	// so it dont crash
		return;
	}

	VectorCopy( playerlegsProneMaxs, realmaxs );
	VectorCopy( playerlegsProneMins, realmins );
	...
	VectorAdd( start, ofs, org );
	VectorAdd( end, ofs, point );
	tracefunc( trace, org, realmins, realmaxs, point, ignoreent, tracemask );
	if ( !bodytrace || trace->fraction < bodytrace->fraction || trace->allsolid )
	{
		...
		VectorAdd( start, ofs, org );
		VectorAdd( end, ofs, point );
		tracefunc( &steptrace, org, realmins, realmaxs, point, ignoreent, tracemask );
		if ( !steptrace.allsolid && !steptrace.startsolid && steptrace.fraction > trace->fraction )
		{
...
void
PM_TraceAllLegs ( trace_t *trace, float *legsOffset, vec3_t start, vec3_t end )
{
	vec3_t	maxs;
	vec3_t	mins;
	if ( pm->ps->eFlags & EF_TANK )
	{
		VectorCopy( tankTraceMaxs, maxs );
		VectorCopy( tankTraceMins, mins );
	}
	else
	{
	...
	{
		trace_t tanktrace;
		PM_TraceTank( &tanktrace, qtrue, start, end, trace, pm->ps->viewangles, pm->trace, pm->ps->clientNum,
					  pm->tracemask );
		if ( tanktrace.fraction < trace->fraction || tanktrace.startsolid || tanktrace.allsolid )
		{
			VectorSubtract( end, start, tanktrace.endpos );
			VectorMA( start, tanktrace.fraction, tanktrace.endpos, tanktrace.endpos );
			*trace = tanktrace;
		}

		PM_TraceTank( &tanktrace, qfalse, start, end, trace, pm->ps->viewangles, pm->trace, pm->ps->clientNum,
					  pm->tracemask );
		if ( tanktrace.fraction < trace->fraction || tanktrace.startsolid || tanktrace.allsolid )
		{
			VectorSubtract( end, start, tanktrace.endpos );
	...
	{
		trace_t legtrace;
		PM_TraceLegs( &legtrace, qtrue, legsOffset, start, end, trace, pm->ps->viewangles, pm->trace, pm->ps->clientNum,
					  pm->tracemask );
		...
		PM_TraceLegs( &legtrace, qfalse, legsOffset, start, end, trace, pm->ps->viewangles, pm->trace,
...
/* */
void
PM_TraceAll ( trace_t *trace, vec3_t start, vec3_t end )
{
...
PM_Friction ( void )
{
	...
	if ( pm->waterlevel <= 1 )
	{
		if ( pml.walking && !(pml.groundTrace.surfaceFlags & SURF_SLICK) )
		{
			...
			if ( !(pm->ps->pm_flags & PMF_TIME_KNOCKBACK) && !(pm->ps->eFlags & EF_TANK) )
			{
...
extern vmCvar_t cg_realmedic;
extern vmCvar_t cg_noCapsule;
#endif
#ifdef GAMEDLL
extern vmCvar_t g_gametype;
extern vmCvar_t g_movespeed;
extern vmCvar_t g_realmedic;
extern vmCvar_t g_noCaps;
#endif
...
#define movespeed	100
	...
	(
		...
		pm->waterlevel < 2 &&
		!(pm->ps->eFlags & EF_TANK) &&
		!(pm->ps->pm_flags & PMF_LADDER)
	)		// need to check if sprinting will have effect to avoid stamina draining with no sprinting
	{
	...
	/*	if (pm->ps->teamNum == PC_MEDIC) { // medics are a bit slower to make them easier to shot , when they are running around
		scale *= 0.90f;
	}*/
	...
	if ( !(pm->ps->eFlags & EF_TANK) )
	{
		...
		(
			...
			pm->ps->weapon == WP_30CAL ||
			pm->ps->weapon == WP_MOBILE_MG42_SET ||
			pm->ps->weapon == WP_MORTAR
		)
		{
			if ( pm->skill[SK_HEAVY_WEAPONS] >= 3 && !(pm->ps->eFlags & EF_AIMING) && !(pm->ps->pm_flags & PMF_LADDER) )
			{
				scale *= 0.75;
			}
			else if ( !(pm->ps->eFlags & EF_AIMING) && !(pm->ps->pm_flags & PMF_LADDER) )
			{
				scale *= 0.5;
			}
		}
		...
		{	// trying some different balance for the FT
			if
			(
				(!(pm->skill[SK_HEAVY_WEAPONS] >= 3) || pm->cmd.buttons & BUTTON_ATTACK) &&
				!(pm->ps->eFlags & EF_AIMING) &&
				!(pm->ps->pm_flags & PMF_LADDER)
			)
			{
...
/*
#ifdef GAMEDLL
	extern	vmCvar_t		g_staminaonlyjumps; // we need separate cvars for clientside and serverside
#endif
#ifdef CGAMEDLL
	extern	vmCvar_t		cg_staminaonlyjumps;
#endif
*/
static qboolean
PM_CheckJump ( void )
{
	...
	/*#ifdef GAMEDLL
	// don't allow if player tired , but only if admin decide
	if ( g_staminaonlyjumps.integer > 0) {
	if (pm->pmext->sprintTime < 2500) 
		return qfalse;
	}
#endif
	#ifdef CGAMEDLL
	// don't allow if player tired , but only if admin decide , same effect , but needed too
	if ( cg_staminaonlyjumps.integer > 0) {
	if (pm->pmext->sprintTime < 2500) 
		return qfalse;
	}
#endif
...
PM_CheckProne ( void )
{
	if ( !(pm->ps->eFlags & EF_PRONE) )
	{
		...
		if ( pm->ps->persistant[PERS_HWEAPON_USE] || pm->ps->eFlags & EF_MOUNTEDTANK || pm->ps->eFlags & EF_TANK )
		{
			return ( qfalse );
		}
		...
		(
			(
				(pm->ps->pm_flags & PMF_DUCKED && pm->cmd.doubleTap == DT_FORWARD) ||
	...
	if ( pm->ps->eFlags & EF_PRONE )
	{
		...
		)
		{
			...
			//pm->maxs[2] = pm->ps->maxs[2] - (pm->ps->standViewHeight + PRONE_VIEWHEIGHT)*1.2;
			pm->ps->eFlags &= ~EF_PRONE;
			PM_TraceAll( &trace, pm->ps->origin, pm->ps->origin );
			pm->ps->eFlags |= EF_PRONE;
			if ( !trace.allsolid )
	...
	if ( pm->ps->eFlags & EF_PRONE )
	{
		...
		{
			pm->mins[2] = pm->ps->mins[2] - 2;
		}

		//	pm->maxs[2] = pm->ps->maxs[2] - pm->ps->standViewHeight - PRONE_VIEWHEIGHT;
		pm->maxs[2] = pm->ps->maxs[2] - ( pm->ps->standViewHeight + PRONE_VIEWHEIGHT ) * 1.70;
...
PM_TankMove ( void )
{
	...
	wishspeed = VectorNormalize( wishdir );
	wishspeed *= scale;
	accelerate = 5;						// lower than player , higher than slick
	PM_Accelerate( wishdir, wishspeed * 1.5, accelerate );
	vel = VectorLength( pm->ps->velocity );
...
PM_WalkMove ( void )
{
	...
	wishspeed = VectorNormalize( wishdir );
	wishspeed *= scale;
	if ( pm->cmd.serverTime - pm->pmext->jumpTime < 2500 )
	{											// jaquboss , working anti bunny hopping function , yay!
		wishspeed *= 0.1f + ( ((float) pm->cmd.serverTime - (float) pm->pmext->jumpTime) / 2500 );
	}
	...
	if ( !pm->ps->velocity[0] && !pm->ps->velocity[1] )
	{
		...
		//		if (pm->isNPC)
		//			Com_Printf("PMove_Walk NPC %i is standing still! fmove is %f. smove is %f.\nvelocity is %f (%f). WishVel is %f. WishSpeed is %f.\nScale is %f.\n", pm->ps->clientNum, fmove, smove, VectorLength(pm->ps->velocity), vel, VectorLength(wishvel), wishspeed, scale);
		return;
	}
...
PM_GroundTrace ( void )
{
	...
	point[0] = pm->ps->origin[0];
	point[1] = pm->ps->origin[1];
	if ( pm->ps->eFlags & EF_MG42_ACTIVE || pm->ps->eFlags & EF_BROWNING_ACTIVE || pm->ps->eFlags & EF_AAGUN_ACTIVE )
	{
	...
	if
	(
		(trace.plane.normal[2] < MIN_WALK_NORMAL && !(pm->ps->eFlags & EF_TANK)) ||
		(trace.plane.normal[2] < MIN_WALK_NORMAL + 0.075 && pm->ps->eFlags & EF_TANK)
	)
	{
		if ( pm->debugLevel )
		{
...
PM_CheckDuck ( void )
{
	...
	if ( pm->ps->eFlags & EF_TANK )
	{
		pm->mins[0] = tankTraceMins[0];
		pm->mins[1] = tankTraceMins[1];
		pm->mins[2] = tankTraceMins[2];

		/*		if (PM_NoCapsule)
	pm->mins[2] = 0;*/
		pm->maxs[0] = tankTraceMaxs[0];
		pm->maxs[1] = tankTraceMaxs[1];
		pm->maxs[2] = tankTraceMaxs[2];
		return;
	}
	else
	{
	...
	if ( pm->ps->pm_type == PM_DEAD )
	{
		...
		pm->maxs[2] = pm->ps->maxs[2] - ( pm->ps->standViewHeight + PRONE_VIEWHEIGHT ) * 1.55;	// only a bit higher than prone hitbox
		pm->ps->viewheight = pm->ps->deadViewHeight;
		return;
	}

	if ( pm->ps->weapon == WP_30CAL_SET || pm->ps->weapon == WP_MOBILE_MG42_SET )
	{		// keep it same
		return;
	}
	...
	{		//1
		...
		{	//2
			pm->maxs[2] = pm->ps->crouchMaxZ;
			pm->ps->viewheight = pm->ps->crouchViewHeight + 12;
		}
		else
		{	//2 3
			pm->maxs[2] = pm->ps->crouchMaxZ;
...
PM_Footsteps ( void )
{
	...
	qboolean	iswalking;
	int			animResult = -1;
	if ( pm->ps->eFlags & EF_DEAD && pm->ps->stats[STAT_HEALTH] > FORCE_LIMBO_HEALTH && PM_GameType != GT_WOLF_DM )
	{
	...
	if ( pm->ps->stats[STAT_DMG] )
	{
		int rnd;
		rnd = rand() % 100;
		if ( pm->ps->eFlags & EF_PRONE || pm->waterlevel >= 2  )
		{											// painanims , no anims when proning , or swimming
			pm->ps->stats[STAT_DMG] = 0;
		}
		else
		{
			if ( pm->ps->stats[STAT_DMG] == 1 )
			{
				if ( rnd < 50 )
				{									// we can get which shoulder taked hit, so wee need hack
					BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_KNEE_LEFT, qtrue );
				}
				else
				{
					BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_KNEE_RIGHT, qtrue );
				}
			}
			else if ( pm->ps->stats[STAT_DMG] == 2 )
			{
				if ( rnd < 50 )
				{
					BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_SHOULDER_LEFT, qtrue );
				}
				else
				{
					BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_SHOULDER_RIGHT, qtrue );
				}
			}
			else if ( pm->ps->stats[STAT_DMG] == 3 )
			{
				BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_CHEST, qtrue );
			}
			else if ( pm->ps->stats[STAT_DMG] == 4 )
			{
				BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_HEAD, qtrue );
			}
			else if ( pm->ps->stats[STAT_DMG] == 6 )
			{
				BG_UpdateConditionValue( pm->ps->clientNum, ANIM_COND_IMPACTPOINT, IMPACTPOINT_GROIN, qtrue );
			}

			BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_PAIN, qtrue, qfalse );
			pm->ps->stats[STAT_DMG] = 0;			// null that thing
		}
	}
	...
	{
		if ( !(pm->cmd.buttons & BUTTON_WALKING) && !(pm->ps->eFlags & EF_AIMING) )
		{
			...
			if ( pm->cmd.rightmove && !pm->cmd.forwardmove )
			{
				...
				{
					animResult = BG_AnimScriptAnimation( pm->ps, pm->character->animModelInfo, ANIM_MT_STRAFERIGHT,
														 qtrue );
				}
				else
				{
...
*/
static void
PM_BeginWeaponReload ( int weapon )
{
	gitem_t *item;
	int		reloadTime;
	...
	{								// locked players cant fire or reload
		return;
	}
	...
	if
	(
		(pm->ps->weapon == WP_M1GARAND || pm->ps->weapon == WP_CARBINE || pm->ps->weapon == WP_GARAND) &&
		(pm->ps->ammoclip[item->giAmmoIndex] != 0)
	)
	{
		PM_AddEvent( EV_GARAND );
	}
	...
	if ( weapon != WP_MORTAR && weapon != WP_MORTAR_SET )
	{
		...
		{
			PM_ContinueWeaponAnim( WEAP_RELOAD4 );
		}
		else
		{
...
*/
void
PM_BeginWeaponChange ( int oldweapon, int newweapon, qboolean reload )
{									//----(SA)	modified to play 1st person alt-mode transition animations.
	int			switchtime;
	...
	if ( !(COM_BitCheck( pm->ps->weapons, newweapon)) && newweapon != WP_NONE )
	{
		return;
	}
	...
	if ( newweapon == weapAlts[oldweapon] && newweapon != WP_NONE )
	{
...
PM_FinishWeaponChange ( void )
{
	...
	switch ( newweapon )
	{
		...
		case WP_K43_SCOPE:					// jaquboss , no sniper zooming so lower a lot
		case WP_GARAND_SCOPE:
		case WP_FG42SCOPE:
			pm->ps->aimSpreadScale = 64;	// jaquboss less...
			pm->ps->aimSpreadScaleFloat = 64.0f;
			break;
		case WP_SILENCER:
	...
	if ( doSwitchAnim )
	{
		...
		if ( weapAlts[oldweapon] == newweapon && newweapon != WP_NONE )
		{
...
{
	int switchtime;
	pm->ps->weaponstate = WEAPON_LOWERED;
	if ( pm->ps->pm_flags & PMF_TIME_LOCKPLAYER )
	{
		switchtime = pm->ps->pm_time;
	}
	else
	{
		switchtime = 250;
	...
	PM_ContinueWeaponAnim( PM_DropAnimForWeapon( pm->ps->weapon) );
}
...
PM_ReloadClip ( int weapon )
{
	...
	if ( BG_IsAltLoadedWeapon( pm->ps->weapon) )
	{			// hope this will work..
		pm->ps->ammo[BG_FindAmmoForWeapon( weapon )] -= 1;
		pm->ps->ammoclip[BG_FindClipForWeapon( weapon )] += 1;
	}
	else
	{
		...
		if ( ammomove )
		{
			...
			{	// M1 Garand should be reloaded only with whole clips
				if ( ammoreserve >= GetAmmoTableData( weapon)->maxclip )
				{
					pm->ps->ammo[BG_FindAmmoForWeapon( weapon )] -= GetAmmoTableData( weapon )->maxclip;
				}

				if ( ammoreserve < GetAmmoTableData( weapon)->maxclip )
				{
					pm->ps->ammo[BG_FindAmmoForWeapon( weapon )] = 0;
				}
			}
			else
			{
	...
	// repeat if holding reload button and have per one bullet weapon
	if
	(
		pm->cmd.wbuttons & WBUTTON_RELOAD &&
		BG_IsAltLoadedWeapon( pm->ps->weapon) &&
...
*/
	...
	{
		autoreload = !IS_AUTORELOAD_WEAPON( weapon ) || pm->ps->holding;
	}
...
*/
void
PM_WeaponUseAmmo ( int wp, int amount )
{
	int takeweapon;
	if ( pm->noWeapClips )
	{
	...
	else
	{
		...
		if ( BG_IsAkimboWeapon( wp) )
		{
			if
			(
				!BG_AkimboFireSequence( wp, pm->ps->ammoclip[BG_FindClipForWeapon( wp)],
...
#define AIMSPREAD_DECREASE_RATE		600.0f	// (SA) when I made the increase/decrease floats (so slower weapon recover could happen for scoped weaps) the average rate increased significantly
#define AIMSPREAD_INCREASE_RATE		1200.0f
#define AIMSPREAD_VIEWRATE_MIN		30.0f	// degrees per second
#define AIMSPREAD_VIEWRATE_RANGE	320.0f	// degrees per second


/* */
void
PM_AdjustAimSpreadScale ( void )
{
	...
	aimspread_increase_rate = AIMSPREAD_INCREASE_RATE;
	aimspread_viewrate_range = AIMSPREAD_VIEWRATE_RANGE;
	...
	{
		aimspread_increase_rate = 2000.0f;
		aimspread_viewrate_range = 640.0f;
	}
	...
	switch ( pm->ps->weapon )
	{
		...
		case WP_AKIMBO_SILENCEDCOLT:
			wpnScale = 1.75f;									// doesn't fire as fast, but easier to handle than luger
			break;
	#ifdef NWEAPS
		case WP_BAR:											// todo adjust
		case WP_MP44:
	#endif
		case WP_MP40:
				wpnScale = 1.75f;										// 2 handed, but not as long as mauser, so harder to keep aim
			break;
		case WP_G43:
		case WP_M1GARAND:
			wpnScale = 1.20f;
	...
	if ( wpnScale )
	{
		...
		// Yes but proned ppls even faster!
		if ( pm->ps->eFlags & EF_CROUCHING || pm->ps->eFlags & EF_PRONE || EF_AIMING )
		{
			wpnScale *= 0.33;
		}
		...
		// TOD-O: also check for jump/crouch and adjust accordingly
		...
		else
		{
			...
			for ( i = 0; i < 2; i++ )
			{
				viewchange += ( fabs( pm->ps->velocity[i]) / 15 );
				...
				viewchange += fabs( AngleSubtract( SHORT2ANGLE( AngleNormalizeInt( pm->cmd.angles[i])),
...
PM_Weapon ( void )
{
	...
	currentweapon = pm->ps->weapon;

	/*if (! AUTO_RELOADS ) {
		pm->pmext->bAutoReload = qfalse;
	}*/
	...
	switch ( pm->ps->persistant[PERS_HWEAPON_USE] )
	{
		case 1:
			...
			if ( pm->cmd.buttons & BUTTON_ATTACK )
			{
				...
				{
					rnd = rand() % 8;
					pm->ps->weapHeat[WP_DUMMY_MG42] += 30 + rnd;
				}
				...
				if ( pm->ps->weapHeat[WP_DUMMY_MG42] >= MAX_MG42_HEAT )
				{
					...
					pm->ps->weaponTime = 3000;			// force "heat recovery minimum" to 2 sec right now
				}
			}
		...
		case 3:
			...
			{											// balance jaquboss
				rnd = rand() % 5;
				pm->ps->weapHeat[WP_DUMMY_MG42] += 30 + rnd;
				PM_AddEvent( EV_FIRE_WEAPON_BROWNING );
				rnd = rand() % 12;
				pm->ps->weaponTime += 114 + rnd;
				BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_FIREWEAPON, qfalse, qtrue );
				pm->ps->viewlocked = 2;					// this enable screen jitter when firing
				if ( pm->ps->weapHeat[WP_DUMMY_MG42] >= MAX_MG42_HEAT )
				{
					pm->ps->weaponTime = MAX_MG42_HEAT; // cap heat to max
					PM_AddEvent( EV_WEAP_OVERHEAT );
					pm->ps->weaponTime = 5000;			// force "heat recovery minimum" to 2 sec right now
				}
			}
	...
	if ( pm->ps->eFlags & EF_MOUNTEDTANK )
	{
		...
		{
			rnd = rand() % 8;
			pm->ps->weapHeat[WP_DUMMY_MG42] += 40 + rnd;;
	...
	if ( pm->ps->eFlags & EF_TANK )
	{
		if ( pm->ps->weapHeat[VERYBIGEXPLOSION] > 0 )
		{
			pm->ps->weapHeat[VERYBIGEXPLOSION] -= pml.msec;
			if ( pm->ps->weapHeat[VERYBIGEXPLOSION] <= 0 )
			{
				if ( !(pm->cmd.wbuttons & WBUTTON_ATTACK2) )
				{
					pm->ps->weapHeat[VERYBIGEXPLOSION] = 0;
				}
			}
		}

		if ( pm->cmd.wbuttons & WBUTTON_ATTACK2 && pm->ps->weapHeat[VERYBIGEXPLOSION] <= 0 )
		{												// COAX
			PM_AddEvent( EV_TANK_MG_FIRE );
			pm->ps->weapHeat[VERYBIGEXPLOSION] += 66;
		}
		...
		{												// MAINGUN
			...
			pm->ps->weaponTime += 4500;					// WILL PREVENT MAINGUN FROM SHOOTING, WEIRD, @TODO
		}
	...
	{
		akimboFire = BG_AkimboFireSequence( pm->ps->weapon, pm->ps->ammoclip[BG_FindClipForWeapon( pm->ps->weapon)],
	...
	if ( pm->pmext->weapRecoilTime )
	{
		...
		for ( i = pm->pmext->lastRecoilDeltaTime; i < deltaTime; i += 15 )
		{
			...
			{
				muzzlebounce[PITCH] -= 2 *
					pm->pmext->weapRecoilPitch *
	...
	(
		(
			(BG_IsScopedWeapon2( pm->ps->weapon) && pm->ps->eFlags & EF_AIMING) ||
			(pm->ps->eFlags & EF_ZOOMING) ||
			(pm->ps->eFlags & EF_AIMING)
		) &&
	...
	)
	{
		...
		float	staminafrac;
		float	x = 0, y = 0;
		spreadfrac = (float) pm->ps->aimSpreadScale / 255.0 + ( 100 / pm->ps->stats[STAT_HEALTH] );
		staminafrac = ( 1 / (pm->pmext->sprintTime / (float) SPRINTTIME) ) * 4;
		if ( staminafrac > 5 )
		{
			staminafrac = 5;
		}
		...
		x += ZOOM_PITCH_AMPLITUDE * sin( phase ) * ( staminafrac + ZOOM_PITCH_MIN_AMPLITUDE ) * ( spreadfrac + ZOOM_PITCH_MIN_AMPLITUDE );
		phase = pm->cmd.serverTime / 1000.0 * ZOOM_YAW_FREQUENCY * M_PI;
		y += ZOOM_YAW_AMPLITUDE * sin( phase ) * ( staminafrac + ZOOM_YAW_AMPLITUDE ) * ( spreadfrac + ZOOM_YAW_AMPLITUDE );
		if ( pm->ps->eFlags & EF_ARMSHOT && BG_IsScopedWeapon2( pm->ps->weapon) )
		{
			x *= 1.45;
			y *= 1.45;
		}
		else if ( pm->ps->eFlags & EF_AIMING && !BG_IsScopedWeapon2( pm->ps->weapon) )
		{
			if ( pm->ps->eFlags & EF_ARMSHOT )
			{
				x *= 0.95;
				y *= 0.95;
			}
		...
		if ( pm->skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 3 && BG_IsScopedWeapon2( pm->ps->weapon) )
		{
			x *= 0.75;
			y *= 0.75;
		}

		pm->ps->viewangles[PITCH] += x;					// uch this looks to be much better way i guess
			...
			if ( !(pm->cmd.buttons & BUTTON_ATTACK) || forcethrow || pm->ps->eFlags & EF_PRONE_MOVING )
			{
				if ( pm->ps->weaponDelay == GetAmmoTableData( pm->ps->weapon)->fireDelayTime || forcethrow )
				{
					...
					if ( pm->ps->eFlags & EF_PRONE )
					{
						if ( akimboFire )
						{
							BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_FIREWEAPON2PRONE, qfalse,
												qtrue );
						}
						else
						{
	...
	if ( (BG_IsSemiFireWeapon( pm->ps->weapon) || BG_IsAkimboWeapon( pm->ps->weapon)) && pm->ps->holding != 1 )
	{
		if ( pm->cmd.buttons & BUTTON_ATTACK && !pm->pmext->releasedFire )
		{
		...
		{
			if ( (pm->cmd.buttons & BUTTON_ATTACK) && pm->ps->weaponTime <= 150 && pm->ps->weapon != WP_MORTAR_SET )
			{
				pm->ps->weaponTime = 0;
				pm->ps->pm_flags &= ~PMF_FIREHELD;
			}
	...
	if ( pm->ps->weaponstate == WEAPON_LOWERED && !(pm->ps->pm_flags & PMF_LADDER) && !(pm->ps->pm_flags & PMF_TIME_LOCKPLAYER)  )
	{
		PM_WeaponRaise();
		return;
	}
	...
	if
	(
		(!(pm->cmd.buttons & (BUTTON_ATTACK | WBUTTON_ATTACK2)) && !delayedFire) ||
		(
			pm->ps->leanf != 0 &&
			!(pm->ps->eFlags & EF_AIMING) &&
			pm->ps->weapon != WP_GRENADE_LAUNCHER &&
			pm->ps->weapon != WP_GRENADE_PINEAPPLE &&
			pm->ps->weapon != WP_SMOKE_BOMB &&
			pm->ps->weapon != WP_K43_SCOPE &&
			pm->ps->weapon != WP_FG42SCOPE &&
			pm->ps->weapon != WP_GARAND_SCOPE &&
			pm->ps->weapon != WP_SATCHEL_DET &&
			pm->ps->weapon != WP_BINOCULARS
		) ||
		(pm->ps->pm_flags & PMF_FIREHELD) ||
		(pm->ps->pm_flags & PMF_TIME_LOCKPLAYER) ||
		(pml.ladder && pm->ps->weapon != WP_GRENADE_LAUNCHER && pm->ps->weapon != WP_GRENADE_PINEAPPLE)
	)
	{
		pm->ps->weaponTime = 0;
		pm->ps->weaponDelay = 0;
	...
	// cant do that sir
	if ( PM_RealMedic && pm->ps->weapon == WP_MEDKIT )
	{
		return;
	}
	...
	if ( pm->ps->weapon )
	{
		...
		if ( ammoNeeded > ammoAvailable )
		{
			...
			if
			(
				!pm->pmext->bAutoReload &&
				IS_AUTORELOAD_WEAPON( pm->ps->weapon) &&
				!(pm->cmd.wbuttons & WBUTTON_RELOAD)
			)
			{
				reloading = qfalse;
			...
			if ( !AUTO_RELOADS && IS_AUTORELOAD_WEAPON( pm->ps->weapon) && !(pm->cmd.wbuttons & WBUTTON_RELOAD) )
			{
				reloading = qfalse;
			}
			...
			if ( reloading )
			{
				...
				(
					(
					pm->ps->weapon == WP_M1GARAND ||
					pm->ps->weapon == WP_CARBINE ||
					pm->ps->weapon == WP_GARAND
					) &&
						(pm->ps->ammoclip[pm->ps->weapon] != 0)
				)
				{
					PM_ContinueWeaponAnim( WEAP_RELOAD4 );
				}
	...
	switch ( pm->ps->weapon )
	{
		...
		case WP_MEDIC_SYRINGE:
			if ( PM_RealMedic )
			{
				PM_ContinueWeaponAnim( WEAP_ATTACK2 );
			}
			else
			{
	...
	if
	(
		(pm->ps->weapon == WP_M7 || pm->ps->weapon == WP_GPG40) &&
	...
	switch ( pm->ps->weapon )
	{
		...
		case WP_SILENCER:
			addTime = GetAmmoTableData( pm->ps->weapon )->nextShotTime;
			aimSpreadScaleAdd = 155;
			break;
		case WP_COLT:
		case WP_SILENCED_COLT:
			addTime = GetAmmoTableData( pm->ps->weapon )->nextShotTime;
			aimSpreadScaleAdd = 145;
			break;
		case WP_AKIMBO_COLT:
		case WP_AKIMBO_SILENCEDCOLT:
		...
		case WP_CARBINE:
			...
			{
				aimSpreadScaleAdd = 10 + rand() % 15;									// (SA) new values for DM
			}
			break;
		case WP_GARAND:
		case WP_K43:
		...
		case WP_MP44:
			...
			{
				aimSpreadScaleAdd = 5 + rand() % 15;									// (SA) new values for DM
			}
			break;
		case WP_BAR:
			addTime = 100 + ( rand() % 71 );
			...
			if ( pm->ps->eFlags & EF_AIMING )
			{
				aimSpreadScaleAdd = rand() % 20;
			}
			else
			{
				aimSpreadScaleAdd = 5 + rand() % 25;									// (SA) new values for DM
			}
			break;
	#endif
		case WP_MP40:
			addTime = 120 + ( rand() % 13 );											//GetAmmoTableData(pm->ps->weapon)->nextShotTime;
			if ( pm->ps->eFlags & EF_AIMING )
			{
				aimSpreadScaleAdd = rand() % 12;
			}
			else if ( pm->ps->teamNum == PC_MEDIC )
			{
				aimSpreadScaleAdd = 5 + rand() % 20;									// (SA) new values for DM
			}
			else
			{
				aimSpreadScaleAdd = 5 + rand() % 10;									// (SA) new values for DM
			}
			break;
		case WP_THOMPSON:
			addTime = 83 + ( rand() % 9 );												//GetAmmoTableData(pm->ps->weapon)->nextShotTime;
			if ( pm->ps->eFlags & EF_AIMING )
			{
				aimSpreadScaleAdd = rand() % 15;
			}
		...
		case WP_STEN:
			...
			if ( pm->ps->eFlags & EF_AIMING )
			{
				aimSpreadScaleAdd = rand() % 10;										// (SA) new values for DM // spreads
			}
			else
			{
				aimSpreadScaleAdd = rand() % 20;										// (SA) new values for DM // spreads
			}
			break;
		case WP_30CAL:
		case WP_30CAL_SET:
			...
			{
				addTime = 100 + ( rand() % 20 );										//GetAmmoTableData(pm->ps->weapon)->nextShotTime;
			}
		...
		case WP_MOBILE_MG42_SET:
			...
			{
				addTime = 40 + ( rand() % 8 );											//GetAmmoTableData(pm->ps->weapon)->nextShotTime;
			}
	...
	// set weapon recoil, @FIXME this is crude
	pm->pmext->lastRecoilDeltaTime = 0;
	switch ( pm->ps->weapon )
	{
		case WP_GARAND:
		case WP_K43:
			...
			{															//1
				...
				pm->pmext->weapRecoilYaw = 0.f;							// feels bad with scope
			...
			{
				pm->pmext->weapRecoilTime = pm->cmd.serverTime;
				pm->pmext->weapRecoilDuration = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? 90 : 140;
				pm->pmext->weapRecoilYaw = crandom() * .2f;
				pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? .15f *
					random() *
					.30f : .20f *
					random() *
					.60f;
		...
		case WP_MOBILE_MG42:
			...
			if ( pm->ps->pm_flags & PMF_DUCKED || pm->ps->eFlags & EF_PRONE )
			{
				...
				pm->pmext->weapRecoilPitch = .45f * random() * .75f;
			}
			else
			{
				pm->pmext->weapRecoilYaw = crandom() * .75f;
				pm->pmext->weapRecoilPitch = 0.95f * random() * .75f;
			}
			break;
		case WP_MP44:
		case WP_MP40:
		...
		case WP_THOMPSON:
			...
			{
				pm->pmext->weapRecoilTime = pm->cmd.serverTime;
				pm->pmext->weapRecoilDuration = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? 45 : 55;
				pm->pmext->weapRecoilYaw = crandom() * .3f;
				pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? .25f *
					random() *
					.45f : .25f *
					random() *
					.65f;
			...
			else
			{
				...
				pm->pmext->weapRecoilYaw = crandom() * .2f;
				pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? .32f *
					random() *
					.45 : .32 *
					random() *
					.65f;
		...
		case WP_CARBINE:
			...
			{
				pm->pmext->weapRecoilTime = pm->cmd.serverTime;
				pm->pmext->weapRecoilDuration = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? 22.5 : 35;
				pm->pmext->weapRecoilYaw = crandom() * .3f;
				pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? .5f *
					random() *
					.50f : .5f *
					random() *
					.60f;
			...
			{
				pm->pmext->weapRecoilTime = pm->cmd.serverTime;
				pm->pmext->weapRecoilDuration = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? 45 : 70;
			...
			break;
		case /* WP_FG42SCOPE*/ WP_FG42:
			if ( pm->ps->eFlags & EF_AIMING )
			{
				pm->pmext->weapRecoilTime = pm->cmd.serverTime;
				pm->pmext->weapRecoilDuration = 100;
				pm->pmext->weapRecoilYaw = 0.f;
				pm->pmext->weapRecoilPitch = 1.30f * random() * .70f;	// here is scoped recoil					
				if ( pm->skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 3 )
				{
					pm->pmext->weapRecoilPitch *= .66f;
				}
			}
			else
			{
				...
				pm->pmext->weapRecoilDuration = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? 70 : 100;
				pm->pmext->weapRecoilYaw = crandom() * .3f;
				pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? .45f *
					random() *
					.45f : .68f *
					random() *
					.45f;
		...
		case WP_AKIMBO_SILENCEDCOLT:
			if ( pm->ps->eFlags & EF_AIMING )
			{
				...
				pm->pmext->weapRecoilDuration = pm->skill[SK_LIGHT_WEAPONS] >= 3 ? 70 : 100;
				pm->pmext->weapRecoilYaw = 0.f;
				pm->pmext->weapRecoilPitch = pm->skill[SK_LIGHT_WEAPONS] >= 1.5f ? .25f *
					random() *
					.15f : .45f *
					random() *
					.15f;
	...
	if
	(
		pm->skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 3 &&
...
*/
#define LEAN_MAX			24.0f
#define LEAN_TIME_TO		175.0f	// time to get to/from full lean
#define LEAN_TIME_FR		275.0f	// time to get to/from full lean
#define LEAN_MAX_LEFT		19.5f
#define LEAN_TIME_TO_LEFT	160.0f	// time to get to/from full lean
#define LEAN_TIME_FR_LEFT	260.0f	// time to get to/from full lean
...
*/
void
PM_UpdateLean ( playerState_t *ps, usercmd_t *cmd, pmove_t *tpm )
{
	...
	vec3_t	viewangles;
	trace_t trace;
	float	lean_right_max = LEAN_MAX;
	float	lean_right_to = LEAN_TIME_TO;
	float	lean_right_from = LEAN_TIME_FR;
	float	lean_left_max = LEAN_MAX_LEFT;
	float	lean_left_to = LEAN_TIME_TO_LEFT;
	float	lean_left_from = LEAN_TIME_FR_LEFT;
	...
	{									// go back to center position
		...
		{								// right
			...
			leanofs -= ( ((float) pml.msec / (float) lean_right_from) * lean_right_max );
			if ( leanofs < 0 )
			{
				leanofs = 0;
			}
		...
		{								// left
			//FIXME: play lean anim backwards?
			leanofs += ( ((float) pml.msec / (float) lean_left_from) * lean_left_max );
			if ( leanofs > 0 )
			{
				leanofs = 0;
			}
	...
	if ( leaning )
	{
		...
		{								// right
			if ( leanofs < LEAN_MAX )
			{
				leanofs += ( ((float) pml.msec / (float) lean_right_to) * lean_right_max );
			}
		...
		{								// left
			if ( leanofs > -LEAN_MAX_LEFT )
			{
				leanofs -= ( ((float) pml.msec / (float) lean_left_to) * lean_left_max );
			}

			if ( leanofs < -LEAN_MAX_LEFT )
			{
				leanofs = -LEAN_MAX_LEFT;
			}
		}
	}
...
{									//----(SA)      modified
	...
	if ( ps->pm_type == PM_INTERMISSION || ps->pm_flags & PMF_TIME_LOCKPLAYER || ps->pm_flags & PMF_TIME_LOCKMOVE )
	{
...
#else
		if ( i == YAW )
		ps->stats[STAT_CROSSHAIR_YAW] = SHORT2ANGLE( temp );
		if ( i == PITCH )
		ps->stats[STAT_CROSSHAIR_PITCH] = SHORT2ANGLE( temp );
#endif
	}
#ifdef __FREEHAND__
// clamp hand
	for ( i=0;i<2;i++) {
		...
		// get angles
		if ( i == YAW )
		angle = ps->stats[STAT_CROSSHAIR_YAW];
		if ( i == PITCH )
		angle = ps->stats[STAT_CROSSHAIR_PITCH];
		// done
		
		// limit harc and varc
		// pitch (varc)
		arc = 30;
		arcDiff = AngleNormalize180( angle - ps->viewangles[i] );
	
		if ( arcDiff > arc )
		{
			angle = AngleNormalize180( ps->viewangles[i] + arc );
			// Set delta_angles properly
			ps->delta_angles[i] = ANGLE2SHORT( angle ) - cmd->angles[i];
		}
		else if ( arcDiff < -arc )
		{
			angle = AngleNormalize180( ps->viewangles[i] - arc );
		...
		// set clamped values
		if ( i == YAW )
		ps->stats[STAT_CROSSHAIR_YAW] = angle;
		if ( i == PITCH )
		ps->stats[STAT_CROSSHAIR_PITCH] = angle;
		// done

	}
// done

// move screen, too slow now
	if ( ps->stats[STAT_CROSSHAIR_YAW] != ps->viewangles[YAW] ){
		int	diff;
		diff = AngleNormalize180( ps->stats[STAT_CROSSHAIR_YAW] - ps->viewangles[YAW] );
		ps->viewangles[YAW] = ps->viewangles[YAW] + (10*diff*pml.frametime);
	}

	if ( ps->stats[STAT_CROSSHAIR_PITCH] != ps->viewangles[PITCH]  ){
		int	diff;
		diff = AngleNormalize180( ps->stats[STAT_CROSSHAIR_PITCH] - ps->viewangles[PITCH] );
		ps->viewangles[PITCH] = ps->viewangles[PITCH] + (10*diff*pml.frametime);
	}
// done
#endif
// MOUNTED
...
// PRONE
	else if ( ps->eFlags & EF_PRONE || ps->weapon == WP_MOBILE_MG42_SET || ps->weapon == WP_30CAL_SET )
	{
		float /*yaw, */ oldYaw;
		...
		if ( ps->weapon == WP_MOBILE_MG42_SET || ps->weapon == WP_30CAL_SET )
		{
			pitchMax = 20.f;
		...
		if ( ps->viewangles[YAW] != oldYaw )
		{
			...
			PM_TraceLegs( &traceres, qtrue, &pmext->proneLegsOffset, ps->origin, ps->origin, NULL, ps->viewangles,
						  pm->trace, ps->clientNum, tracemask );
			...
			else
			{
				...
				// jaquboss, back trace
				PM_TraceLegs( &traceback, qfalse, &pmext->proneLegsOffset, ps->origin, ps->origin, NULL, ps->viewangles,
							  pm->trace, ps->clientNum, tracemask );
				if ( traceback.allsolid )
				{
...
{							//----(SA)      modified
	...
	for ( i = 0; i < 3; i++ )
	{
		...
		if ( i == YAW )
		{
			...
			// panzer barrel, limit horizontal move
			if ( ps->persistant[PERS_TANKUSED] == 2 )
			{
	...
	// Check if we rotated into a wall
	if ( ps->viewangles[YAW] != oldYaw )
	{
		PM_TraceTank( &traceres, qtrue, ps->origin, ps->origin, NULL, ps->viewangles, pm->trace, ps->clientNum,
					  tracemask );
		...
		PM_TraceTank( &traceres, qfalse, ps->origin, ps->origin, NULL, ps->viewangles, pm->trace, ps->clientNum,
					  tracemask );
		if ( traceres.allsolid )
		{
...
PmoveSingle ( pmove_t *pmove )
{
	...
	if ( pm->ps->stats[STAT_HEALTH] <= 0 )
	{
		pm->tracemask &= ~CONTENTS_BODY;																// corpses can fly through bodies , jaquboss TEST1!
		pm->ps->eFlags &= ~EF_ZOOMING;
		pm->ps->eFlags &= ~EF_AIMING;
		pm->pmext->adsTime = -pm->cmd.serverTime;
	}
	...
	if ( pm->ps->eFlags & EF_AIMING )
	{																									// this is ADS walk hack :) , it seems that it works
	...
	if ( pm->ps->pm_flags & PMF_LADDER )
	{																									// This slowers players climbing ladders
		if ( pm->cmd.forwardmove > 82 )
		{
			pm->cmd.forwardmove = 82;
		}
		else if ( pm->cmd.forwardmove < -82 )
		{
			pm->cmd.forwardmove = -82;
		}

		if ( pm->cmd.rightmove > 82 )
		{
			pm->cmd.rightmove = 82;
		}
		else if ( pm->cmd.rightmove < -82 )
		{
			pm->cmd.rightmove = -82;
		}
	...
	(
		...
		!(pm->ps->weaponstate == WEAPON_RELOADING) &&
		!(BG_IsScopedWeapon( pm->ps->weapon)) &&
		!(BG_PlayerMounted( pm->ps->eFlags))
	)
	{																									// not everyone can salute...
		int				nazi;
#ifdef CGAMEDLL
		extern vmCvar_t cg_nazi;
		nazi = cg_nazi.integer;
#elif GAMEDLL
		extern vmCvar_t g_nazi;
		nazi = g_nazi.integer;
#endif
#ifdef NAZI_SALUTE
		if ( pm->ps->persistant[PERS_TEAM] == TEAM_AXIS && nazi > 0 )
		{
			BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_NAZI, qtrue, qfalse );
		}
		else
		{
#endif
			BG_AnimScriptEvent( pm->ps, pm->character->animModelInfo, ANIM_ET_TAUNT, qtrue, qfalse );	// this would be nice with salute animation
#ifdef NAZI_SALUTE
		}
#endif
		pm->ps->pm_flags |= PMF_TIME_LOCKPLAYER;														// dont allow anything when saluting
		pm->ps->pm_time = 1900;
	}
	...
	{
		if ( !BG_IsScopedWeapon( pm->ps->weapon) )
		{															// don't allow binocs if using the sniper scope		
			if
			(
				BG_IsAdsWeapon( pm->ps->weapon) ||
	...
	if ( /*pm->ps->eFlags & EF_MG42_ACTIVE || pm->ps->eFlags & EF_BROWNING_ACTIVE ||*/ pm->ps->eFlags & EF_AAGUN_ACTIVE )
	{																// turn it off on mounted guns... , one less bug :)
		pm->ps->eFlags &= ~EF_AIMING;
		pm->pmext->adsTime = -pm->cmd.serverTime;
	}
	...
	{																// these situations turn off ironsights
		pm->ps->eFlags &= ~EF_AIMING;
		pm->pmext->adsTime = -pm->cmd.serverTime;
	}																// fix this hax
	...
	if ( pm->ps->weapon == WP_NONE )
	{
		...
		(
			pm->ps->eFlags & EF_BROWNING_ACTIVE ||
			pm->ps->eFlags & EF_MG42_ACTIVE ||
			pm->ps->eFlags & EF_AAGUN_ACTIVE ||
			pm->ps->eFlags & EF_MOUNTEDTANK
		)
		{
			pm->ps->weapon = cg.weaponSelect_last;
		}
	}
#endif
	...
	if ( !(pm->ps->pm_flags & PMF_RESPAWNED) && (pm->ps->pm_type != PM_INTERMISSION) )
	{
		...
		if ( pm->waterlevel == 3 )
		{
			...
			)
			{
				...
				pm->ps->weaponDelay = 0;							// avoid insta-fire after water exit on delayed weapon attacks
				pm->ps->weaponstate = WEAPON_READY;					// this is best , i guess , jaquboss
				return;
			}
		}
	...
	if ( pm->ps->persistant[PERS_WOLFKICK] > 0 )
	{
		pmove->cmd.upmove = pmove->cmd.rightmove = pmove->cmd.forwardmove = 0;
	}

	if ( pm->ps->eFlags & EF_TANK )
	{

		//		pmove->cmd.rightmove = 0;	// tanks dont jump or strafe :S
		pmove->cmd.upmove = 0;
	}
	...
	if ( pm->ps->pm_type >= PM_DEAD || pm->ps->pm_flags & (PMF_LIMBO | PMF_TIME_LOCKPLAYER | PMF_TIME_LOCKMOVE) )
	{						// DHM - Nerve
	...
	if ( (pm->ps->weapon == WP_30CAL_SET || pm->ps->weapon == WP_MOBILE_MG42_SET) && pm->ps->pm_type == PM_NORMAL )
	{
		pm->cmd.forwardmove = 0;
		pm->cmd.rightmove = 0;
		pm->cmd.upmove = 0;
...
Pmove ( pmove_t *pmove )
{
	...
	if
	(
		(pm->ps->stats[STAT_HEALTH] <= 0 || pm->ps->pm_type == PM_DEAD) &&
		pml.groundTrace.surfaceFlags & SURF_MONSTERSLICK
	)
	{
...
// 	ps->velocity
void
PmovePredict ( pmove_t *pmove, float frametime )
{
	pm = pmove;
	memset( &pml, 0, sizeof(pml) );
