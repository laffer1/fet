...
PM_SlideMove ( qboolean gravity )
{
	...
	for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ )
	{
		...
		if ( pm->debugLevel > 1 )
		{
			Com_Printf( "%i:%d %d (%f %f %f)\n", c_pmove, trace.allsolid, trace.startsolid, trace.endpos[0],
						trace.endpos[1], trace.endpos[2] );
		}
...
PM_StepSlideMove ( qboolean gravity )
{
	...
	if ( pm->debugLevel )
	{
		...
		{
			Com_Printf( "%i:PM_SlideMove solidified! (%f %f %f) -> (%f %f %f)\n", c_pmove, start_o[0], start_o[1],
						start_o[2], pm->ps->origin[0], pm->ps->origin[1], pm->ps->origin[2] );
		}
	...
	{
		PM_TraceLegs( &trace, qtrue, NULL, pm->ps->origin, down, NULL, pm->ps->viewangles, pm->trace, pm->ps->clientNum,
					  pm->tracemask );
		if ( trace.allsolid )
		{
		...
		PM_TraceLegs( &trace, qfalse, NULL, pm->ps->origin, down, NULL, pm->ps->viewangles, pm->trace,
	...
	// check tank parts separately
	if ( pm->ps->eFlags & EF_TANK )
	{
		PM_TraceTank( &trace, qtrue, pm->ps->origin, down, NULL, pm->ps->viewangles, pm->trace, pm->ps->clientNum,
					  pm->tracemask );
		if ( trace.allsolid )
		{
			VectorCopy( down_o, pm->ps->origin );
		...
		PM_TraceTank( &trace, qfalse, pm->ps->origin, down, NULL, pm->ps->viewangles, pm->trace, pm->ps->clientNum,
					  pm->tracemask );
		if ( trace.allsolid )
		{
			VectorCopy( down_o, pm->ps->origin );
