...
{
	if ( g_debugLights.integer > 0 )
	{
		float	scale;
		self->s.eType = ET_CORONA;
		self->dl_color[0] = 255;
		self->dl_color[1] = 255;
		self->dl_color[2] = 0;
		self->s.dl_intensity = (int) self->dl_color[0] | ( (int) self->dl_color[1] << 8 ) | ( (int) self->dl_color[2] << 16 );
		G_SpawnFloat( "scale", "1", &scale );
		self->s.density = (int) ( scale * 255 );
...
*/
static int	attackDurations[] = { ( 11 * 1000 ) / 15, ( 16 * 1000 ) / 15, ( 16 * 1000 ) / 15 };
static int	attackHittimes[] = { ( 7 * 1000 ) / 15, ( 6 * 1000 ) / 15, ( 7 * 1000 ) / 15 };
...
*/
void
grabber_think_idle ( gentity_t *ent )
{
	if ( ent->s.frame > 1 )
	{	// non-idle status
		ent->s.frame = rand() % 2;
...
*/
void
grabber_think_hit ( gentity_t *ent )
{
...
/* */
void
grabber_die ( gentity_t *ent, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
...
*/
void
grabber_close ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
...
*/
void
grabber_pain ( gentity_t *ent, gentity_t *attacker, int damage, vec3_t point )
{
	G_AddEvent( ent, EV_GENERAL_SOUND, ent->sound2to1 );	// sound2to1 is the 'pain' sound
}
...
*/
void
grabber_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
*/
void
grabber_wake_touch ( gentity_t *ent, gentity_t *other, trace_t *trace )
{
	grabber_wake( ent );
}
...
*/
void
SP_misc_grabber_trap ( gentity_t *ent )
{
	int			adist, bdist, range;
	gentity_t	*trig;

	// TOD-O: change from 'trap' to something else.  'trap' is a misnomer.  it's actually used for other stuff too
	ent->s.eType = ET_TRAP;

	// TOD-O: make these user assignable?
...
#ifdef __ET_SPOTLIGHTS__
/* */
void
use_spotlight ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	gentity_t	*tent;
	if ( ent->r.linked )
	{
...
/* */
void
spotlight_finish_spawning ( gentity_t *ent )
{
	if ( ent->spawnflags & 1 )
	{	// START_ON
		ent->active = 0;
...
*/
void
SP_misc_spotlight ( gentity_t *ent )
{
...
#else //!__ET_SPOTLIGHTS__

// UQ1: Use RTCW Single Player Spotlights!!! These actually work lol!!!
...
*/
void
SP_misc_gamemodel ( gentity_t *ent )
{
	vec_t		scale;
	vec3_t		vScale;
...
*/
void
SP_misc_vis_dummy ( gentity_t *ent )
{
...
*/
void
SP_misc_vis_dummy_multiple ( gentity_t *ent )
{
	if ( !ent->targetname )
	{
...
*/
void
SP_misc_light_surface ( gentity_t *ent )
{
	G_FreeEntity( ent );
}
...
Use_Shooter ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	switch ( ent->s.weapon )
	{
		case WP_GRENADE_LAUNCHER:
			...
			fire_grenade( ent, ent->s.origin, dir, WP_GRENADE_LAUNCHER /*, qfalse*/ );
			break;
		case WP_GRENADE_PINEAPPLE:				// same as above but throws pineapples
			VectorScale( dir, 700, dir );
			fire_grenade( ent, ent->s.origin, dir, WP_GRENADE_PINEAPPLE /*, qfalse*/ );
			break;
		case WP_PANZERFAUST:
			if ( g_panzerfaust_gravited.integer >= 2 )
			{
				fire_rocket( ent, ent->s.origin, dir, qtrue );
			}
			else
			{
				fire_rocket( ent, ent->s.origin, dir, qfalse );
			}
			...
			break;
		case WP_MOBILE_MG42:
			fire_lead( ent, ent->s.origin, dir, 35, MOD_MACHINEGUN );
			VectorScale( ent->s.pos.trDelta, 2, ent->s.pos.trDelta );
			SnapVector( ent->s.pos.trDelta );	// save net bandwidth
			break;
		case WP_MAPMORTAR:
...
*/
void
SP_shooter_mortar ( gentity_t *ent )
{

	// (SA) TOD-O: must have a self->target.  Do a check/print if this is not the case.
...
/*QUAKED shooter_mg42 (1 0 0) (-16 -16 -16) (16 16 16)
Fires at either the target or the current direction.
"random" the number of degrees of deviance from the taget. (1.0 default)
*/
void
SP_shooter_mg42 ( gentity_t *ent )
{
	InitShooter( ent, WP_MOBILE_MG42 );
}
...
*/
void
use_corona ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->r.linked )
	{
		trap_UnlinkEntity( ent );
...
*/
void
SP_corona ( gentity_t *ent )
{
	float	scale;
	ent->s.eType = ET_CORONA;
	if
	(
		ent->dl_color[0] <= 0 &&									// if it's black or has no color assigned
		ent->dl_color[1] <= 0 &&
		ent->dl_color[2] <= 0
	)
	{
...
*/
void
dlight_finish_spawning ( gentity_t *ent )
{
...
*/
void
use_dlight ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->r.linked )
	{
		trap_UnlinkEntity( ent );
...
SP_dlight ( gentity_t *ent )
{
	...
	if
	(
		ent->dl_color[0] <= 0 &&								// if it's black or has no color assigned, make it white
		ent->dl_color[1] <= 0 &&
		ent->dl_color[2] <= 0
	)
	{
...
)
{
	...
	if ( g_debugBullets.integer & 1 || g_debugBullets.integer == 10 )
	{
	...
	{
		if ( activator->s.eFlags & EF_AAGUN_ACTIVE )
		{
			tent = G_TempEntity( tr.endpos, EV_AAGUN_HIT_WALL );
		}
		else
		{
...
// interpolation
#ifdef __NPC__
extern void BG_NPCStateToEntityState ( playerState_t *ps, entityState_t *s, qboolean snap );
#endif //__NPC__

/* */
void
clamp_playerbehindgun ( gentity_t *self, gentity_t *other, vec3_t dang )
{
	...
	if ( other->NPC_client )
	{
		VectorCopy( point, other->NPC_client->ps.origin );

		// save results of pmove
		BG_NPCStateToEntityState( &other->NPC_client->ps, &other->s, qfalse );

		// use the precise origin for linking
		VectorCopy( other->NPC_client->ps.origin, other->r.currentOrigin );

		// DHM - Nerve :: Zero out velocity
		other->NPC_client->ps.velocity[0] = other->NPC_client->ps.velocity[1] = 0.f;
	}
	else
	{
...
/* */
void
clamp_hweapontofirearc ( gentity_t *self, vec3_t dang )
{
	float		diff, yawspeed;
	qboolean	clamped;
	clamped = qfalse;
...
//
void
aagun_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	if ( owner && owner->client )
	{
		ent->r.ownerNum = ent->s.number;
		ent->s.otherEntityNum = ent->s.number;
		owner->NPC_client->ps.persistant[PERS_HWEAPON_USE] = 0;
		owner->NPC_client->ps.viewlocked = 0;
		owner->active = qfalse;
	}
#endif //__NPC__
	trap_LinkEntity( ent );
...
/* */
void
aagun_die ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
...
/* */
void
aagun_track ( gentity_t *self, gentity_t *other )
{
	if ( !self->active )
	{
		return;
...
/* */
void
aagun_stopusing ( gentity_t *self )
{
	gentity_t	*owner;
	vec3_t		axis[3];
	vec3_t		pos;
	...
	VectorMA( self->r.currentOrigin, 64, axis[1], pos );
	VectorClear( owner->client->ps.velocity );			// Gordon: dont want them to fly away ;D
	TeleportPlayer( owner, pos, owner->client->ps.viewangles );
	if ( owner && owner->client )
	{
		owner->client->ps.eFlags &= ~EF_AAGUN_ACTIVE;	// DHM - Nerve :: unset flag
		owner->client->ps.persistant[PERS_HWEAPON_USE] = 0;
		self->r.ownerNum = self->s.number;
		owner->client->ps.viewlocked = 0;				// let them look around
		owner->active = qfalse;
...
aagun_fire ( gentity_t *other )
{
	...
	//self->s.frame = self->s.modelindex2;
	self->s.eFlags |= EF_AAGUN_ACTIVE;
	other->s.eFlags |= EF_AAGUN_ACTIVE;

	// snap to integer coordinates for more efficient network bandwidth usage
	SnapVector( muzzle );
	Fire_Lead_Ext( other, other, AAGUN_SPREAD, AAGUN_DAMAGE, muzzle, forward, right, up, MOD_AAGUN );
}


/* */
void
aagun_touch ( gentity_t *self, gentity_t *other, trace_t *trace )
{
...
/* */
void
aagun_spawn ( gentity_t *gun, gentity_t *ent )
{
	gentity_t	*base;
	vec3_t		offset;
	if ( g_knifeonly.integer != 1 )
	{
		...
		{												// alternative base model
			base->s.modelindex = G_ModelIndex( "models/mapobjects/weapons/flak_base2.md3" );
		}
		else
		{
			base->s.modelindex = G_ModelIndex( "models/mapobjects/weapons/flak_base.md3" );
		}
		...
		gun->s.dmgFlags = HINT_AAGUN;
		gun->s.modelindex = G_ModelIndex( "models/mapobjects/weapons/flak_turret.md3" );
		gun->s.origin[2] += 24;
		G_SetOrigin( gun, gun->s.origin );
		G_SetAngle( gun, gun->s.angles );
		VectorSet( gun->r.mins, -16, -8, -24 );
		VectorSet( gun->r.maxs, 16, 16, 32 );
		gun->s.apos.trTime = 0;
...
SP_aagun ( gentity_t *self )
{
	...
	aagun_spawn( self, self );
}


/* */
void
mg42_touch ( gentity_t *self, gentity_t *other, trace_t *trace )
{
	vec3_t	dang;
	int		i;
	if ( !self->active )
	...
	if ( other->NPC_client )
	{
		for ( i = 0; i < 3; i++ )
		{
			dang[i] = SHORT2ANGLE( other->NPC_client->pers.cmd.angles[i] );
		}

		// now tell the client to lock the view in the direction of the gun
		other->NPC_client->ps.viewlocked = 3;
		other->NPC_client->ps.viewlocked_entNum = self->s.number;
...
mg42_fire ( gentity_t *other )
{
	...
	if ( other->NPC_client )
	{
		self = &g_entities[other->NPC_client->ps.viewlocked_entNum];

		//AngleVectors (self->s.apos.trBase, forward, right, up);
		AngleVectors( other->NPC_client->ps.viewangles, forward, right, up );
		VectorCopy( self->s.pos.trBase, muzzle );
	}
	else
	{
	...
	// snap to integer coordinates for more efficient network bandwidth usage
	SnapVector( muzzle );
	Fire_Lead_Ext( self, other, MG42_SPREAD_MP, MG42_DAMAGE_MP, muzzle, forward, right, up, MOD_MACHINEGUN );
}


/* */
void
browning_fire ( gentity_t *other )
{
	gentity_t	*self;
	vec3_t		forward, right, up;
	vec3_t		muzzle;
	...
	self->s.eFlags |= EF_BROWNING_ACTIVE;
	other->s.eFlags |= EF_BROWNING_ACTIVE;

	// snap to integer coordinates for more efficient network bandwidth usage
	SnapVector( muzzle );

	//Fire_Lead_Ext( other, other, BROWNING_SPREAD, BROWNING_DAMAGE, muzzle, forward, right, up, MOD_BROWNING2 );
	Fire_Lead_Ext( self, other, BROWNING_SPREAD, BROWNING_DAMAGE, muzzle, forward, right, up, MOD_BROWNING2 );

	//	Fire_Lead( self, other, BROWNING_SPREAD, BROWNING_DAMAGE, muzzle, forward, right, up , MOD_BROWNING2 );
}


/* */
void
mg42_track ( gentity_t *self, gentity_t *other )
{
	int i;
	if ( !self->active )
	{
	...
	if ( other->NPC_client )
	{
		...
		// move to the position over the next frame
		VectorSubtract( other->NPC_client->ps.viewangles, self->s.apos.trBase, self->s.apos.trDelta );
		for ( i = 0; i < 3; i++ )
		{
			self->s.apos.trDelta[i] = AngleNormalize180( self->s.apos.trDelta[i] );
		}

		VectorScale( self->s.apos.trDelta, 1000 / 50/*BG_GetServerFrameRate()*/, self->s.apos.trDelta );
		self->s.apos.trTime = level.time;
		self->s.apos.trDuration = 50/*BG_GetServerFrameRate()*/;
		SnapVector( self->s.apos.trDelta );
	}
#endif //__NPC__
}
...
mg42_think ( gentity_t *self )
{
	...
	if ( owner && owner->client )
	{
		self->s.dl_intensity = owner->client->ps.clientNum;
	}
	else
	{
		self->s.dl_intensity = -1;
	}
	...
	if ( owner->NPC_client )
	{
		self->mg42weapHeat = owner->NPC_client->ps.weapHeat[WP_DUMMY_MG42];
	}
#endif //__NPC__
...
#ifdef __NPC__
			else if ( owner->NPC_client )
			{
				self->flameQuotaTime = level.time + owner->NPC_client->ps.weaponTime;
			}
#endif //__NPC__
			else
			{
	...
	if ( owner->NPC_client )
	{
		...
		if ( len < usedist && owner->active && owner->health > 0 )
		{
			...
			// ATVI Wolfenstein Misc #433
			//			owner->NPC_client->ps.pm_flags &= ~PMF_DUCKED|PMF_LEANED;
			self->active = qtrue;
			owner->NPC_client->ps.persistant[PERS_HWEAPON_USE] = 1;
			mg42_track( self, owner );
			self->nextthink = level.time + 50/*BG_GetServerFrameRate()*/;
			self->timestamp = level.time + 1000;

			//owner->client->ps.weaponsHeat[WP_DUMMY_MG42] = self->mg42weapHeat;
			clamp_playerbehindgun( self, owner, vec3_origin );
			return;
		}
	}
	...
	{
		owner->NPC_client->ps.persistant[PERS_HWEAPON_USE] = 0;
		owner->NPC_client->ps.viewlocked = 0;					// let them look around
		owner->active = qfalse;

		//owner->NPC_client->ps.gunfx = 0;
		// need this here for players that get killed while on the MG42
		self->backupWeaponTime = owner->NPC_client->ps.weaponTime;
	}
#endif //__NPC__
...
browning_think ( gentity_t *self )
{
	...
	if ( owner->client )
	{
		...
		if ( len < usedist && owner->active && owner->health > 0 )
		{
			...
			owner->client->ps.pm_flags &= ~PMF_DUCKED;
			self->active = qtrue;
			owner->client->ps.persistant[PERS_HWEAPON_USE] = 3;
			mg42_track( self, owner );
			self->nextthink = level.time + 50;
			self->timestamp = level.time + 1000;
...
void
mg42_stopusing ( gentity_t *self )
{
	gentity_t	*owner;
	owner = &g_entities[self->r.ownerNum];
	if ( owner && owner->client )
	{
		owner->client->ps.eFlags &= ~EF_MG42_ACTIVE;
		owner->client->ps.eFlags &= ~EF_BROWNING_ACTIVE;	// DHM - Nerve :: unset flag
		owner->client->ps.persistant[PERS_HWEAPON_USE] = 0;
		self->r.ownerNum = self->s.number;
		owner->client->ps.viewlocked = 0;					// let them look around
		owner->active = qfalse;
...
#ifdef __NPC__
	if ( owner && owner->NPC_client )
	{
		owner->NPC_client->ps.eFlags &= ~EF_MG42_ACTIVE;	// DHM - Nerve :: unset flag
		owner->NPC_client->ps.persistant[PERS_HWEAPON_USE] = 0;
		self->r.ownerNum = self->s.number;
		owner->NPC_client->ps.viewlocked = 0;				// let them look around
		owner->active = qfalse;
		...
		//owner->NPC_client->ps.gunfx = 0;
		//owner->NPC_client->ps.weaponsHeat[WP_DUMMY_MG42] = 0;
		self->mg42weapHeat = owner->NPC_client->ps.weapHeat[WP_DUMMY_MG42];
		self->backupWeaponTime = owner->NPC_client->ps.weaponTime;
		owner->NPC_client->ps.weaponTime = owner->backupWeaponTime;
		self->active = qfalse;
...
/* */
void
mg42_die ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
	gentity_t	*gun;
	gentity_t	*owner;
	trace_t		tr;
	...
	if ( owner && owner->client )
	{
		...
		owner->client->ps.eFlags &= ~EF_BROWNING_ACTIVE;
		owner->client->ps.persistant[PERS_HWEAPON_USE] = 0;
		owner->active = qfalse;
		self->r.ownerNum = self->s.number;
		self->s.otherEntityNum = self->s.number;
	...
	if ( owner && owner->NPC_client )
	{
		...
		if ( tr.startsolid )
		{
			VectorCopy( owner->TargetAngles, owner->NPC_client->ps.origin );
			VectorCopy( owner->TargetAngles, owner->r.currentOrigin );
			owner->r.contents = CONTENTS_CORPSE;			// this will correct itself in ClientEndFrame
		}
...
/* */
void
mg42_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	gentity_t	*owner;
	...
	if ( owner && owner->client )
	{
		...
		ent->s.dl_intensity = owner->client->ps.clientNum;
	}
	else
	{
	...
	if ( owner && owner->NPC_client )
	{
		...
		//owner->client->ps.gunfx = 0;
		other->NPC_client->ps.weapHeat[WP_DUMMY_MG42] = ent->mg42weapHeat;
		ent->backupWeaponTime = owner->NPC_client->ps.weaponTime;
		owner->backupWeaponTime = owner->NPC_client->ps.weaponTime;
	}
#endif //__NPC__
...
mg42_spawn ( gentity_t *ent )
{
	...
	if ( g_knifeonly.integer != 1 )
	{
		...
		{
			gun->s.modelindex = G_ModelIndex( "models/multiplayer/browning/browning.md3" );
		}
		...
		{
			gun->think = browning_think;
			gun->s.dmgFlags = HINT_BROWNING;
		}
...
SP_browning ( gentity_t *self )
{
	...
	self->spawnflags = 4;	// he is hacking mummy !
...
tank_spawn ( gentity_t *ent )
{
	...
	if ( g_knifeonly.integer != 1 )
	{
		...
		{
			trap_SetBrushModel( tank, level.tankclip[0]->model );
		}
...
panzer_spawn ( gentity_t *ent )
{
	...
	if ( g_knifeonly.integer != 1 )
	{
		...
		{
			trap_SetBrushModel( tank, level.tankclip[1]->model );
		}
...
// UQ1: Replaced the following procedures with the ETE versions... They are enhanced a bit...


/*
void tank_spawn_think (gentity_t *self) {

	if (!self->TankMarker ) {  // init
	//	G_Printf("Tank spawner %i activated .. \n", self->s.number ); 
		...
		for (i = 0; i < level.num_entities; i++ ){  
			if ( g_entities[i].TankMarker == self->TankMarker && i != self->TankMarker ) { // k , we found our tank, just sleep
	...
	//			G_Printf("Tank number %i found .. \n", g_entities[i].TankMarker ); 
		...
		if ( !found ) { // time to spawn another tank
...
*/
void
tank_freeteam_spawn_think ( gentity_t *self )
{
#ifdef __SUPREMACY__
	...
	if ( g_gametype.integer == GT_SUPREMACY || g_gametype.integer == GT_SUPREMACY_CAMPAIGN && num_flags >= 2 )
	{
		...
		{
			float	distance = VectorDistance( flag_list[i].flagentity->s.origin, self->s.origin );
		...
		if ( flag_list[best_flag].flagentity->s.modelindex == TEAM_AXIS )
		{	// Switch tank spawner type to new owning team...
			self->think = panzer_spawn_think;
			self->TankMarker = self->s.number;
			self->nextthink = level.time + FRAMETIME;
			return;
		}
		else if ( flag_list[best_flag].flagentity->s.modelindex == TEAM_ALLIES )
		{	// Switch tank spawner type to new owning team...
			self->think = tank_spawn_think;
			self->TankMarker = self->s.number;
			self->nextthink = level.time + FRAMETIME;
...
#endif //__SUPREMACY__
	self->TankMarker = self->s.number;
	self->think = tank_freeteam_spawn_think;
	self->nextthink = level.time + FRAMETIME;
}
...
tank_spawn_think ( gentity_t *self )
{
	...
	if ( g_gametype.integer == GT_SUPREMACY || g_gametype.integer == GT_SUPREMACY_CAMPAIGN && num_flags >= 2 )
	{
		...
		else if ( flag_list[best_flag].flagentity->s.modelindex == TEAM_FREE )
		{		// Switch tank spawner type to new owning team...
			self->think = tank_freeteam_spawn_think;
			self->TankMarker = self->s.number;
			self->nextthink = level.time + FRAMETIME;
...
panzer_spawn_think ( gentity_t *self )
{
	...
	if ( g_gametype.integer == GT_SUPREMACY || g_gametype.integer == GT_SUPREMACY_CAMPAIGN && num_flags >= 2 )
	{
		...
		if ( flag_list[best_flag].flagentity->s.modelindex == TEAM_ALLIES )
		{		// Switch tank spawner type to new owning team...
			self->think = tank_spawn_think;
			self->TankMarker = self->s.number;
			self->nextthink = level.time + FRAMETIME;
...
*/
void
misc_spawner_think ( gentity_t *ent )
{
	gitem_t		*item;
	gentity_t	*drop = NULL;
...
/* */
void
misc_spawner_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
/* */
void
firetrail_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
/* */
void
misc_firetrails_think ( gentity_t *ent )
{
...
/* */
void
SP_misc_firetrails ( gentity_t *ent )
{
...
*/
void
constructiblemarker_setup ( gentity_t *ent )
{
	ent->target_ent = G_FindByTargetname( NULL, ent->target );
	if ( !ent->target_ent )
	{
...
/* */
void
SP_misc_constructiblemarker ( gentity_t *ent )
{
	char	*s;
...
landmine_setup ( gentity_t *ent )
{
	...
	(
		tr.startsolid ||
		tr.fraction == 1.f ||
		!(tr.surfaceFlags & (SURF_GRASS | SURF_SNOW | SURF_GRAVEL | SURF_LANDMINE)) ||
		(
...
/* */
void
SP_misc_landmine ( gentity_t *ent )
{
	if ( ent->spawnflags & 1 )
	{
...
*/
void
SP_misc_commandmap_marker ( gentity_t *ent )
{
...
void
G_InitTempTraceIgnoreEnts ( void )
{
...
/* */
void
G_ResetTempTraceIgnoreEnts ( void )
{
	int i;
...
/* */
void
G_TempTraceIgnoreEntity ( gentity_t *ent )
{
	if ( !ent->r.linked )
	{
		return;
...
/* */
void
G_TempTraceIgnorePlayersAndBodies ( void )
{
	int i;
...
*/
void
SP_use_particle_system ( gentity_t *self, gentity_t *other, gentity_t *activator )
{
...
===============
*/
extern int	G_ParticleSystemIndex ( char *name );


/* */
void
SP_misc_particle_system ( gentity_t *self )
{
	char	*s;
	G_SetOrigin( self, self->s.origin );
	G_SpawnString( "psName", "", &s );

	//G_Printf("GAME: loading particle system %s.\n", s);
...
//
// End new Particle System... Unique1
//
