...
/* */
int
G_GetSysMessageNumber ( const char *sysMsg )
{
	int i;
...
/* */
void
G_SendSystemMessage ( sysMsg_t message, int team )
{
	gentity_t	*other;
	int			*time;
	int			j;
	...
	*time = level.time;
	for ( j = 0; j < level.maxclients; j++ )
	{
		...
		trap_SendServerCommand( other - g_entities,
								va( "vschat 0 %d 3 %s 0 0 0", other - g_entities, systemMessages[message]) );
	}
}
