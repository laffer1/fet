...
static animStringItem_t			animConditionMountedStr[] = { { "** UNUSED **", -1 }, { "MG42", -1 }, { NULL, -1 }, };
...
static animStringItem_t			animHealthLevelStr[] = { { "1", -1 }, { "2", -1 }, { "3", -1 }, };
...
static animStringItem_t			animGenBitFlagStr[] =
{
	...
	// zooming with binoculars
};

// xkan, 1/17/2003 - sorts of duplicate aistateEnum_t.
static animStringItem_t			animAIStateStr[] = { { "RELAXED", -1 }, { "QUERY", -1 }, { "ALERT", -1 }, {
			"COMBAT", -1 } };
typedef enum
{
...
*/
long
BG_StringHashValue ( const char *fname )
{
	int		i;
	long	hash;
	if ( !fname )
...
*/
long
BG_StringHashValue_Lwr ( const char *fname )
{
	int		i;
	long	hash;
	hash = 0;
...
*/
void QDECL
BG_AnimParseError ( const char *msg, ... )
{
	va_list argptr;
	char	text[1024];
	va_start( argptr, msg );
...
*/
static int
BG_AnimationIndexForString ( char *string, animModelInfo_t *animModelInfo )
{
	int			i, hash;
	animation_t *anim;
...
*/
animation_t *
BG_AnimationForString ( char *string, animModelInfo_t *animModelInfo )
{
	int			i, hash;
	animation_t *anim;
	hash = BG_StringHashValue( string );
...
*/
int
BG_IndexForString ( char *token, animStringItem_t *strings, qboolean allowFail )
{
	int					i, hash;
...
*/
char *
BG_CopyStringIntoBuffer ( char *string, char *buffer, int bufSize, int *offset )
{
	char	*pch;
...
*/
void
BG_ParseConditionBits ( char **text_pp, animStringItem_t *stringTable, int condIndex, int result[2] )
{
...
*/
qboolean
BG_ParseConditions ( char **text_pp, animScriptItem_t *scriptItem )
{
...
)
{
	...
	while ( 1 )
	{
		...
		if ( partIndex == 0 )
		{
			...
			{
				BG_AnimParseError( "BG_ParseCommands: exceeded maximum number of animations (%i)",
								   MAX_ANIMSCRIPT_ANIMCOMMANDS );
			}
		...
		switch ( parseMode )
		{
			case PARSEMODE_DEFINES:
				if ( !Q_stricmp( token, "set") )
				{
					...
					defineStr[defineType][numDefines[defineType]].string = BG_CopyStringIntoBuffer( token, defineStrings,
																									sizeof(defineStrings),
																									&defineStringsOffset );
			...
			case PARSEMODE_CANNED_ANIMATIONS:
				...
				else if ( (indentLevel == 2) && (indexes[indentLevel] < 0) )
				{
					...
					{
						BG_AnimParseError( "BG_AnimParseAnimScript: exceeded maximum items per script (%i)",
										   MAX_ANIMSCRIPT_ITEMS );								// RF mod
					}

					// are there enough items left in the global list?
					if ( animModelInfo->numScriptItems >= MAX_ANIMSCRIPT_ITEMS_PER_MODEL )
					{
						BG_AnimParseError( "BG_AnimParseAnimScript: exceeded maximum global items (%i)",
										   MAX_ANIMSCRIPT_ITEMS_PER_MODEL );					// RF mod
					}
...
*/
qboolean
BG_EvaluateConditions ( int client, animScriptItem_t *scriptItem )
{
	int						i;
	...
	for ( i = 0, cond = scriptItem->conditions; i < scriptItem->numConditions; i++, cond++ )
	{
		switch ( animConditionsTable[cond->index].type )
		{
			case ANIM_CONDTYPE_BITFLAGS:
				if
				(
					!(globalScriptData->clientConditions[client][cond->index][0] & cond->value[0]) &&
...
*/
animScriptItem_t *
BG_FirstValidItem ( int client, animScript_t *script )
{
	animScriptItem_t	**ppScriptItem;
	int					i;
	for ( i = 0, ppScriptItem = script->items; i < script->numItems; i++, ppScriptItem++ )
	{
		if ( client == -1 || BG_EvaluateConditions( client, *ppScriptItem) ) // jaquboss may be npc
		{
			return ( *ppScriptItem );
		}
	}
...
*/
void
BG_ClearAnimTimer ( playerState_t *ps, animBodyPart_t bodyPart )
{
	switch ( bodyPart )
	{
		case ANIM_BP_LEGS:
...
)
{
	...
	return
		(
			BG_PlayAnim( ps, animModelInfo, BG_AnimationIndexForString( animName, animModelInfo), bodyPart, 0, setTimer,
						 isContinue, force)
		);
}
...
)
{
	...
	if ( scriptCommand->bodyPart[0] )
	{
		...
		if ( scriptCommand->bodyPart[0] == ANIM_BP_BOTH || scriptCommand->bodyPart[0] == ANIM_BP_LEGS )
		{
			playedLegsAnim =
				(
					BG_PlayAnim( ps, animModelInfo, scriptCommand->animIndex[0], scriptCommand->bodyPart[0], duration,
	...
	if ( scriptCommand->bodyPart[1] )
	{
		...
		if ( scriptCommand->bodyPart[1] == ANIM_BP_BOTH || scriptCommand->bodyPart[1] == ANIM_BP_LEGS )
		{
			playedLegsAnim =
				(
					BG_PlayAnim( ps, animModelInfo, scriptCommand->animIndex[1], scriptCommand->bodyPart[1], duration,
								 setTimer, isContinue, force) > -1
				);
		}
		else
...
)
{
	...
	{
		case 38:	// alert_run_2h
		case 39:	// alert_run_1h
		case 40:	// alert_run_no
		case 41:	// alert_crch_2h
		case 45:	// alert_bk_2h
		case 46:	// alert_bk_1h
		case 48:	// alert_crbk_2h
		case 161:	// prone_crawl
		case 162:	// prone_crawl_bk
			ps->eFlags |= EF_MOTION;
			break;
	}
...
*/
int
BG_AnimScriptCannedAnimation ( playerState_t *ps, animModelInfo_t *animModelInfo )
{
	animScript_t			*script;
	animScriptItem_t		*scriptItem;
	animScriptCommand_t		*scriptCommand;
...
)
{
	...
	animScriptItem_t	*scriptItem;
	animScriptCommand_t *scriptCommand;
	if ( event != ANIM_ET_DEATH && event != ANIM_ET_FORCEDEATH && event != ANIM_ET_DMDEATH && ps->eFlags & EF_DEAD )
	{
		return ( -1 );
	}
...
*/
char *
BG_GetAnimString ( animModelInfo_t *animModelInfo, int anim )
{
...
*/
void
BG_UpdateConditionValue ( int client, int condition, int value, qboolean checkConversion )
{
...
*/
int
BG_GetConditionValue ( int client, int condition, qboolean checkConversion )
{
	int i;
	if ( animConditionsTable[condition].type == ANIM_CONDTYPE_BITFLAGS )
	{
...
====================*/
qboolean
BG_GetConditionBitFlag ( int client, int condition, int bitNumber )
{
	if ( animConditionsTable[condition].type == ANIM_CONDTYPE_BITFLAGS )
	{
...
/* */
void
BG_SetConditionBitFlag ( int client, int condition, int bitNumber )
{
...
/* */
void
BG_ClearConditionBitFlag ( int client, int condition, int bitNumber )
{
...
/*
================
BG_GetAnimScriptEvent_NPC

  returns the animation index for this event
================
*/
int
BG_GetAnimScriptEvent_NPC ( scriptAnimEventTypes_t event )
{
	animModelInfo_t		*animModelInfo;
	animScript_t		*script;
	animScriptItem_t	*scriptItem;
	...
	animModelInfo =  axisClassCharacters[PC_SOLDIER].animModelInfo; // jaquboss, a hack but we dont mind it right now :)
	...
	// find the first script item, that passes all the conditions for this event
	scriptItem = BG_FirstValidItem( -1, script );
	if ( !scriptItem )
	{
		return ( -1 );
	}
...
*/
int
BG_GetAnimScriptEvent ( playerState_t *ps, scriptAnimEventTypes_t event )
{
	animModelInfo_t		*animModelInfo;
	animScript_t		*script;
	animScriptItem_t	*scriptItem;
	...
	if ( ps && event != ANIM_ET_DEATH && event != ANIM_ET_FORCEDEATH && ps->eFlags & EF_DEAD )
	{
		return ( -1 );
	}
...
*/
animation_t *
BG_GetAnimationForIndex ( animModelInfo_t *animModelInfo, int index )
{
...
*/
void
BG_AnimUpdatePlayerStateConditions ( pmove_t *pmove )
{
	...
	if ( ps->eFlags & EF_ZOOMING )
	{
		...
		(
			(
				ps->eFlags & EF_AIMING &&
				ps->weapon != WP_AKIMBO_COLT &&
				ps->weapon != WP_AKIMBO_SILENCEDCOLT &&
				ps->weapon != WP_AKIMBO_LUGER &&
				ps->weapon != WP_AKIMBO_SILENCEDLUGER &&
				ps->weapon != WP_COLT &&
				ps->weapon != WP_SILENCED_COLT &&
				ps->weapon != WP_SILENCER &&
				ps->weapon != WP_PANZERFAUST &&
				ps->weapon != WP_LUGER
			) ||
			((ps->weapon == WP_30CAL_SET || ps->weapon == WP_MOBILE_MG42_SET) && !(ps->eFlags & EF_PRONE))
		)
	{
		BG_UpdateConditionValue( ps->clientNum, ANIM_COND_WEAPON, WP_K43_SCOPE, qtrue );
		BG_ClearConditionBitFlag( ps->clientNum, ANIM_COND_GEN_BITFLAG, ANIM_BITFLAG_ZOOMING );
	}

	/*#ifdef	NWEAPS
	else if ( ps->weapon == WP_BAR || ps->weapon == WP_MP44)  {
		BG_UpdateConditionValue( ps->clientNum, ANIM_COND_WEAPON, WP_MP40, qtrue );
		BG_ClearConditionBitFlag( ps->clientNum, ANIM_COND_GEN_BITFLAG, ANIM_BITFLAG_ZOOMING );
	}
#endif*/
	else
	...
	// MOUNTED
	if ( ps->eFlags & EF_MG42_ACTIVE || ps->eFlags & EF_BROWNING_ACTIVE || ps->eFlags & EF_MOUNTEDTANK )
	{
		BG_UpdateConditionValue( ps->clientNum, ANIM_COND_MOUNTED, MOUNTED_MG42, qtrue );
	}
	else if ( ps->eFlags & EF_AAGUN_ACTIVE )
	{
		BG_UpdateConditionValue( ps->clientNum, ANIM_COND_MOUNTED, MOUNTED_MG42 /*MOUNTED_AAGUN*/, qtrue );
	}
	else
	{
	...
	if ( ps->velocity[0] != 0 || ps->velocity[1] != 0 )
	{
		if ( ps->viewheight <= ps->crouchViewHeight + 12 && ps->viewheight >= ps->crouchViewHeight )
		{
