...
/* */
void
target_speaker_multiple ( gentity_t *ent )
{
	gentity_t	*vis_dummy = NULL;

	G_Printf("Use target speaker multiple!\n");
...
SP_target_speaker ( gentity_t *ent )
{
	...
	//G_Printf("target_speaker with sound %s.\n", s);
...
*/
void
misc_beam_think ( gentity_t *self )
{
...
/* */
void
misc_beam_start ( gentity_t *self )
{
	gentity_t	*ent;
	...
	{				// the misc_beam is it's own ending point
		self->enemy = self;
	}
...
/* */
void
relay_AIScript_AlertEntity ( gentity_t *self )
{
...
*/
void
G_KillEnts ( const char *target, gentity_t *ignore, gentity_t *killer, meansOfDeath_t mod )
{
	gentity_t	*targ = NULL;

	/*if ( killer->client) {
	if ( killer->client->ps.eFlags & EF_AIMING ){
		switch(mod){
		case MOD_FG42:
		mod = MOD_FG42SCOPE;
		break;
...
*/
void
Use_Target_Counter ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->count < 0 )
...
*/
void
Use_Target_Lock ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	gentity_t	*t = 0;
...
*/
void
Use_target_fog ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	//		r,g,b
	//		time to complete
	trap_SetConfigstring( CS_FOGVARS, va( "%f %f %f %f %f %f %i", 1.0f, (float) ent->s.density, 1.0f,
						  (float) ent->dl_color[0], (float) ent->dl_color[1], (float) ent->dl_color[2], ent->s.time) );
}
...
*/
void
SP_target_counter ( gentity_t *ent )
{
...
*/
void
SP_target_autosave ( gentity_t *ent )
{
...
/* */
void
Use_Target_Alarm ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	G_UseTargets( ent, other );
}
...
/* */
void
smoke_toggle ( gentity_t *ent, gentity_t *self, gentity_t *activator )
{
...
*/
void RTCW_target_script_trigger_use (gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	// for all entities/bots with this ainame
	gentity_t	*trent = NULL;

	//G_Printf("RTCW_target_script_trigger_use: aiName: %s targetname: %s.\n", ent->aiName, ent->targetname);

	if (ent->aiName) {
		// Find the first entity with this name
		while (trent = G_Find( trent, FOFS( scriptName), ent->aiName ))
		{
			// Was there one?
			if ( trent )
			{
				...
				G_UseTargets ( trent, ent);
				//G_Printf("Use target %i (%s).\n", trent->s.number, trent->classname);
			}
		}
	}
	else if (ent->target) {
		// Find the first entity with this name
		trent = G_Find( trent, FOFS( scriptName/*target*/), ent->targetname );

		// Was there one?
		if ( trent )
		{
			// Play the script
			G_Script_ScriptEvent( trent, "trigger", ent->targetname );
			G_UseTargets ( trent, ent);
//			G_Printf("Use target %i (%s). Target: %s. TargetName: %s. AIName: %s.\n", trent->s.number, trent->classname, trent->target, trent->targetname, trent->aiName);
		}
	}
...
void SP_RTCW_target_script_trigger (gentity_t *ent)
{
	G_SetOrigin (ent, ent->s.origin); 
	//ent->r.svFlags = SVF_USE_CURRENT_ORIGIN;
	ent->s.eType = ET_GENERAL;
	ent->use = RTCW_target_script_trigger_use;
}
...
*/
void
target_script_trigger_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
/* */
void
SP_target_script_trigger ( gentity_t *ent )
{
	if (level.gametype < GT_WOLF)
	{
		SP_RTCW_target_script_trigger(ent);
		return;
	}
...
/* */
void
target_rumble_think ( gentity_t *ent )
{
	gentity_t	*tent;
	float		ratio;
	float		time, time2;
...
/* */
void
target_rumble_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->spawnflags & 1 )
	{
		ent->spawnflags &= ~1;
...
/* */
void
SP_target_rumble ( gentity_t *self )
{
	char	*pitch;
	char	*yaw;
	char	*rampup;
