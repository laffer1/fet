...
static const cmd_reference_t	aCommandInfo[] =
{
	...
	{ "callvote", qtrue, qfalse,
			( void(*) (gentity_t *, unsigned int, qboolean) ) Cmd_CallVote_f, " <params>:^7 Calls a vote" },
	...
	// Kamikazee: Added to FalckonET for ETAdmin compatability.
	{ "cp", qtrue, qfalse, NULL, " [message]:^7 Displays text in the bottom of all clients' screens." },
	{ "cpm", qtrue, qfalse, NULL, " [client] [message]:^7 Displays text in the bottom of all clients' screen, plus the console." },
	{ "cpmsay", qtrue, qfalse, NULL, " [client] [message]:^7 Displays text in the text area of the client's screen." },
	{ "qsay", qtrue, qfalse, NULL, " [client] [message]:^7 Displays text in the text area of all clients' screens, p." },
	// Uh-oh... Not implemented yet
	// { "bp", qtrue, qfalse, NULL, " [message]:^7 Displays text in the bottom of all clients' screen." },

	{ "head", qtrue, qtrue, Cmd_Head_f, "" },
...
qboolean
G_commandCheck ( gentity_t *ent, char *cmd, qboolean fDoAnytime )
{
...
qboolean
G_commandHelp ( gentity_t *ent, char *pszCommand, unsigned int dwCommand )
{
...
qboolean
G_cmdDebounce ( gentity_t *ent, const char *pszCommandName )
{
	if ( ent->client->pers.cmd_debounce > level.time )
	{
		CP( va( "print \"Wait another %.1fs to issue ^3%s\n\"",
			1.0 * (float) (ent->client->pers.cmd_debounce - level.time) / 1000.0, pszCommandName) );
		return ( qfalse );
	}
...
/* */
void
G_noTeamControls ( gentity_t *ent )
{
...
void
G_commands_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
void
G_lock_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fLock )
{
	int tteam;
...
void
G_pause_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fPause )
{
	char	*status[2] = { "^5UN", "^1" };
	if ( team_nocontrols.integer )
	{
...
// Show client info
void
G_players_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
void
G_ready_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean state )
{
	char	*status[2] = { " NOT", "" };
...
void
G_say_teamnl_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
void
G_scores_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fValue )
{
...
void
G_specinvite_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fLock )
{
	int			tteam, pid;
	gentity_t	*player;
	char		arg[MAX_TOKEN_CHARS];
...
void
G_speclock_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fLock )
{
	int tteam;
	if ( team_nocontrols.integer )
	{
...
void
G_weaponStats_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fDump )
{
	G_statsPrint( ent, 0 );
}
...
void
G_statsall_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean fDump )
{
	int			i;
	gentity_t	*player;
	for ( i = 0; i < level.numConnectedClients; i++ )
...
void
G_teamready_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean state )
{
...
int QDECL
SortStats ( const void *a, const void *b )
{
	gclient_t	*ca, *cb;
	float		accA, accB;
...
void
G_weaponStatsLeaders_cmd ( gentity_t *ent, qboolean doTop, qboolean doWindow )
{
		...
		for ( i = 0; i < cClients; i++ )
		{
			...
			if ( ((doTop) ? acc : (float) wBestAcc + 0.999) >= ((doTop) ? wBestAcc : acc) )
			{
				Q_strcat( z, sizeof(z), va( " %d %d %d %d %d %d", iWeap + 1, aClients[i],
						  cl->sess.aWeaponStats[iWeap].hits, cl->sess.aWeaponStats[iWeap].atts,
...
void
G_weaponRankings_cmd ( gentity_t *ent, unsigned int dwCommand, qboolean state )
{
	gclient_t	*cl;
	...
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
		...
		if ( shots >= cQualifyingShots[iWeap] )
		{
			...
			Q_strcat( z, sizeof(z), va( " %d %d %d %d %d", level.sortedStats[i], cl->sess.aWeaponStats[iWeap].hits,
