...
qboolean				G_ScriptAction_Camo ( gentity_t *ent, char *params );
qboolean				G_ScriptAction_Camo_Axis ( gentity_t *ent, char *params );
qboolean				G_ScriptAction_Camo_Allies ( gentity_t *ent, char *params );
...
g_script_stack_action_t gScriptActions[] =
{
	...
	{ "wm_camo", G_ScriptAction_Camo },
	{ "wm_camo_axis", G_ScriptAction_Camo_Axis },
	{ "wm_camo_allies", G_ScriptAction_Camo_Allies },
	...
	// Jaquboss : Oh Bani , how many people thought that Shrub is dead because of this :D
...
/*	{"mu_start",				G_SP_ScriptAction_MusicStart},	// (char *new_music, int time)		// time to fadeup
	...
	{"mu_queue",				G_SP_ScriptAction_MusicQueue},	// (char *new_music)*/
	// ETPub
	{ NULL, NULL }
};
...
g_script_event_define_t gScriptEvents[] =
{
	...
	{ "trigger", G_Script_EventMatch_StringEqual },

	// something has triggered us (always followed by an identifier)
	{ "pain", G_Script_EventMatch_IntInRange },

	// we've been hurt
	{ "death", NULL },

	// RIP
	{ "activate", G_Script_EventMatch_StringEqual },

	// something has triggered us [activator team]
	{ "stopcam", NULL },
	{ "playerstart", NULL },
	...
	{ "message", G_Script_EventMatch_StringEqual },
...
*/
qboolean
G_Script_EventMatch_StringEqual ( g_script_event_t *event, char *eventParm )
{
...
*/
qboolean
G_Script_EventMatch_IntInRange ( g_script_event_t *event, char *eventParm )
{
	char	*pString, *token;
	int		int1, int2, eInt;
	...
	if (level.gametype < GT_WOLF)
	{
		return G_SP_RTCW_script_EventMatch_IntInRange();
	}
#endif //__RTCW_SP_SCRIPTS__
...
*/
int
G_Script_EventForString ( const char *string )
{
	int i, hash;
	...
	if (level.gametype < GT_WOLF)
	{
		return G_SP_RTCW_script_EventForString();
	}
#endif //__RTCW_SP_SCRIPTS__
...
G_Script_ActionForString ( char *string )
{
	int i, hash;
	...
	if (level.gametype < GT_WOLF)
	{
		return G_SP_RTCW_script_ActionForString();
	}
#endif //__RTCW_SP_SCRIPTS__
...
G_Script_ScriptLoad ( void )
{
	...
	/*Q_strncpyz( scriptdir, g_mapScriptsDir.string, sizeof(scriptdir) );
	Q_strcat( scriptdir, sizeof(scriptdir), "/" );

	Q_strncpyz( filename, scriptdir, sizeof(filename) );
	...
	if ( g_gametype.integer == GT_WOLF_TDM ) {
		Q_strcat( filename, sizeof(filename), "_tdm" );
	}

	Q_strcat( filename, sizeof(filename), ".script" );// .script .dat
	...
	G_Printf( "^1Was not loaded, using default script!\n" );
	}
	...
	if (len < 0 ) {
	Q_strncpyz( filename, "maps/", sizeof(filename) ); // bad mapscripts dir , using default
	Q_strcat( filename, sizeof(filename), mapname.string );
	...
	if(len >= 0) {
	G_Printf( "Default script was loaded!\n" );
	} else {
	G_Printf( "^1Was not loaded, no script avaiable!\n" );
	}

	}
*/
	if ( g_mapScriptsDir.string[0] )
	{
		Q_strncpyz( filename, g_mapScriptsDir.string, sizeof(filename) );
...
#ifdef __SUPREMACY__
		if ( g_gametype.integer == GT_SUPREMACY_CAMPAIGN || g_gametype.integer == GT_SUPREMACY )
		{
			Q_strcat( filename, sizeof(filename), "_supremacy" );
		}
#endif
		if ( g_gametype.integer == GT_WOLF_TDM )
		{
		...
		{
			Q_strcat( filename, sizeof(filename), "_dm" );
		}

		if ( g_gametype.integer == GT_WOLF_CTF )
		{
			Q_strcat( filename, sizeof(filename), "_ctf" );
		}
...
*/
void
G_Script_ParseSpawnbot ( char **ppScript, char params[], int paramsize )
{
...
*/
void
G_Script_ScriptParse ( gentity_t *ent )
{
	char					*pScript;
	char					*token;
	qboolean				wantName;
	...
	if (level.gametype < GT_WOLF)
	{
		G_SP_RTCW_script_ScriptParse( ent );
		return;
	}
#endif //__RTCW_SP_SCRIPTS__
	...
	while ( 1 )
	{
		...
		else if ( inScript )
		{
			...
			while ( (token = COM_Parse( &pScript)) != NULL && (token[0] != '}') )
			{
				...
				else				// hackly precaching of custom characters
						...
						if ( i == 0 )
						{
							...
							else if ( !Q_stricmp( action->actionString, "changemodel") )
							{
								...
								(
									buildScript &&
									(
										!Q_stricmp( action->actionString, "mu_start") || !Q_stricmp( action->actionString,
																									 "mu_play") || !Q_stricmp( action->actionString, "mu_queue") || !Q_stricmp( action->actionString,
"startcam")
									)
								)
							{
...
/* */
void
G_Script_ScriptChange ( gentity_t *ent, int newScriptNum )
{
...
*/
int
G_Script_GetEventIndex ( gentity_t *ent, char *eventStr, char *params )
{
	int i, eventNum = -1;
	...
	for ( i = 0; i < ent->numScriptEvents; i++ )
	{
		if ( ent->scriptEvents[i].eventNum == eventNum )
		{
			...
			(
				...
				(
					!gScriptEvents[eventNum].eventMatch || gScriptEvents[eventNum].eventMatch( &ent->scriptEvents[i],
																							   params)
				)
			)
			{
...
================
*/
qboolean G_SP_script_ScriptRun( gentity_t *ent );
void G_SP_script_ScriptChange( gentity_t *ent, int newScriptNum )
{
	g_script_status_t scriptStatusBackup;
...
/*
================
G_SP_script_ScriptEvent

  An event has occured, for which a script may exist
================
*/
void G_SP_script_ScriptEvent( gentity_t *ent, char *eventStr, char *params )
{
	int i, eventNum;
	...
	if (eventNum < 0)
	{
		if (g_cheats.integer) {	// dev mode
			G_Printf( "devmode-> G_SP_script_ScriptEvent(), unknown event: %s\n", eventStr );
		}
		return;
	}
	...
	for (i=0; i<ent->numScriptEvents; i++)
	{
		if (ent->scriptEvents[i].eventNum == eventNum)
		{
			if (	(!ent->scriptEvents[i].params)
				...
				G_SP_script_ScriptChange( ent, i );
				break;
			}
		}
	}
...
*/
void
G_Script_ScriptEvent ( gentity_t *ent, char *eventStr, char *params )
{
#ifdef __RTCW_SP_SCRIPTS__
	if (level.gametype < GT_WOLF)
	{
		G_SP_RTCW_script_ScriptEvent( ent );
		return;
	}
	else
#endif //__RTCW_SP_SCRIPTS__
...
*/
qboolean
G_Script_ScriptRun ( gentity_t *ent )
{
		...
		if ( ent->scriptStatus.scriptStackHead < stack->numItems )
		{
			G_Printf( "%i : (%s) GScript command: %s %s\n", level.time, ent->scriptName,
					  stack->items[ent->scriptStatus.scriptStackHead].action->actionString, (
						  stack->items[ent->scriptStatus.scriptStackHead].params ? stack->items[ent->scriptStatus.
						  scriptStackHead].params : "") );
		}
	}
	...
	while ( ent->scriptStatus.scriptStackHead < stack->numItems )
	{
		...
		if
		(
			!stack->items[ent->scriptStatus.scriptStackHead].action->actionFunc( ent,
																				 stack->items[ent->scriptStatus.scriptStackHead].params)
		)
		{
			ent->scriptStatus.scriptFlags &= ~SCFL_FIRST_CALL;
			return ( qfalse );
...
// Script Entities
void
mountedmg42_fire ( gentity_t *other )
{
	...
	if ( self->s.density & 8 )
	{
		Fire_Lead_Ext( other, other, BROWNING_SPREAD, BROWNING_DAMAGE, muzzle, forward, right, up, MOD_BROWNING );
	}
	else
	{
...
/* */
float
tankAngle ( gentity_t *ent )
{
	trace_t trf, trb;
	vec3_t	forward, front, back, front2, back2, yawangles;
	float	slopeangle;
	yawangles[0] = yawangles[2] = 0;
	yawangles[1] = ent->s.apos.trBase[1];
	AngleVectors( yawangles, forward, NULL, NULL );

	// front
	VectorCopy( ent->s.pos.trBase, front2 );
	VectorMA( front2, 32, forward, front2 );
	front2[2] -= 128;
	VectorCopy( ent->s.pos.trBase, front );
	VectorMA( front, 32, forward, front );
	front[2] += 16;

	// back
	VectorCopy( ent->s.pos.trBase, back2 );
	VectorMA( back2, -32, forward, back2 );
	back2[2] -= 128;
	VectorCopy( ent->s.pos.trBase, back );
	VectorMA( back, -32, forward, back );
	back[2] += 16;

	// back
	trap_Trace( &trb, back, NULL, NULL, back2, ent->client->ps.clientNum, CONTENTS_SOLID );

	// front
	trap_Trace( &trf, front, NULL, NULL, front2, ent->client->ps.clientNum, CONTENTS_SOLID );

	// disable all just now
	if ( trf.endpos[2] - trb.endpos[2] > 0 )
	{
		slopeangle = ( trb.endpos[2] - trf.endpos[2] ) / 2;
	}
	else
	{
		slopeangle = ( trb.endpos[2] - trf.endpos[2] ) / 2;
...
tank_fire ( gentity_t *ent )
{
	...
	angles[0] = tankAngle( ent ) + ent->client->ps.stats[STAT_CROSSHAIR_PITCH];

	//	if ( ent->client->ps.persistant[PERS_TANKUSED] == 1 )
	angles[1] = ent->client->ps.stats[STAT_DEAD_YAW];
	AngleVectors( angles, forward, NULL, NULL );
	VectorCopy( ent->s.pos.trBase, muzzle );
	muzzle[2] += 64;
	VectorMA( muzzle, 120, forward, muzzle );
	SnapVector( muzzle );
	m = fire_tankshell( ent, muzzle, forward );
}


/* */
void
tank_mgfire ( gentity_t *ent )
{
	vec3_t	forward, right, up;
	vec3_t	muzzle, angles;
	VectorCopy( ent->client->ps.viewangles, angles );
	angles[0] = tankAngle( ent ) + ent->client->ps.stats[STAT_CROSSHAIR_PITCH];
	angles[1] += ent->turretYaw;
	AngleVectors( angles, forward, right, up );
	VectorCopy( ent->s.pos.trBase, muzzle );
	muzzle[2] += 32;
	VectorMA( muzzle, 120, forward, muzzle );
	SnapVector( muzzle );
	Fire_Lead_Ext( ent, ent, MG42_SPREAD_MP, MG42_DAMAGE_MP, muzzle, forward, right, up, MOD_MG42 );
}


/* */
void
script_linkentity ( gentity_t *ent )
{
...
/* */
void
script_mover_die ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod )
{
...
/* */
void
script_mover_set_blocking ( gentity_t *ent )
{
...
/* */
void
script_mover_aas_blocking ( gentity_t *ent )
{
...
/* */
void
script_mover_spawn ( gentity_t *ent )
{
	if ( ent->spawnflags & 128 )
	{
		if ( !ent->tagBuffer )
...
/* */
void
script_mover_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	if ( ent->spawnflags & 8 )
	{
		if ( ent->count )
...
/* */
void
script_mover_blocked ( gentity_t *ent, gentity_t *other )
{
	...
	// kill them
	if ( !ent->client || (other->client && !(other->client->ps.eFlags & EF_TANK)) )
	{	// movers crush everything, players cant crush tanks
...
//..............................................................................
void
script_model_med_spawn ( gentity_t *ent )
{
	if ( ent->spawnflags & 2 )
	{
		ent->clipmask = CONTENTS_SOLID;
...
/* */
void
script_model_med_use ( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
...
*/
void
SP_script_model_med ( gentity_t *ent )
{
	if ( !ent->model )
	{
...
*/
void
SP_script_camera ( gentity_t *ent )
{
	if ( !ent->scriptName )
	{
...
*/
void
SP_script_multiplayer ( gentity_t *ent )
{
//	if (level.gametype < GT_WOLF)
//		ent->scriptName = "checker1"; // UQ1: Load SP map scripts from RTCW...
//	else
