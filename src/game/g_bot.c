...
//vmCvar_t				bot_dynamic;	// Kamikazee: moved to g_local.h
...
int
Bot_GetWeaponForClassAndTeam ( int classNum, int teamNum, const char *weaponName )
{
...
/* */
void
G_BotParseCharacterParms ( char *characterFile, int *characterInt )
{
...
G_SpawnBot ( const char *text )
{
	...
	spawnBotCommand_t	params[] =
	{
		...
		{ "/class", pClass, qfalse, "[SOLDIER/MEDIC/LIEUTENANT/ENGINEER/COVERTOPS/FIELDOPS]" },

		// FIXME: remove LIEUTENANT from missionpack
		...
		{ "/skills", botSkills, qfalse, "[botskills]" },

		// not really to be exposed to script
		// END Mad Doc - TDF
	...
	// END Mad Doc - TDF
	G_AddBot( name, atoi( skill), team, spawnPoint, pClassInt, weaponSpawnNumber, characterInt, respawn, scriptName,
			  rankNum, skills, prisonerOfWar );
...
*/
void
G_BotDelayedInit ( gentity_t *ent )
{
	G_FreeEntity( ent );
...
void
ServerEntityDropToFloor ( g_serverEntity_t *ent )
{
	vec3_t	dest;
	trace_t tr;
	vec3_t	checkMins, checkMaxs;
...
void
SP_AIMarker_Setup ( g_serverEntity_t *ent )
{
...
*/
void
SP_bot_sniper_spot ( gentity_t *ent )
{
	ent->s.origin[2] += 16;
	if ( !(ent->spawnflags & 1) )
	{
...
*/
void
bot_landminespot_setup ( gentity_t *self )
{
...
/* */
void
SP_bot_landminespot_spot ( gentity_t *ent )
{
...
*/
void
SP_bot_attractor ( gentity_t *ent )
{
	if ( !(ent->spawnflags & 1) )
	{
		BotDropToFloor( ent );
...
*/
void
SP_bot_jump_source ( gentity_t *ent )
{
...
*/
void
SP_bot_jump_dest ( gentity_t *ent )
{
...
*/
void
bot_seek_cover_spot_think ( g_serverEntity_t *ent )
{
...
void
SP_SeekCover_Setup ( g_serverEntity_t *ent )
{
...
/* */
void
SP_Seek_Cover_Spawn ( gentity_t *ent, int team )
{
...
/* */
void
SP_bot_seek_cover_spot ( gentity_t *ent )
{
...
*/
void
SP_bot_axis_seek_cover_spot ( gentity_t *ent )
{
...
*/
void
bot_seek_cover_sequence_touch ( gentity_t *self, gentity_t *other, trace_t *trace )
{
...
/* */
void
bot_seek_cover_sequence_init ( gentity_t *ent )
{
	g_serverEntity_t	*trav, *lastTrav;
...
/* */
void
SP_bot_seek_cover_sequence ( gentity_t *ent )
{
...
*/
void
SP_bot_landmine_area ( gentity_t *ent )
{
	char	*spawnString;
