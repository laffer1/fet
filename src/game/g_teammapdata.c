...
*/
void
G_PushMapEntityToBuffer ( char *buffer, int size, mapEntityData_t *mEnt )
{
	char	buf[32];
	if ( level.ccLayers )
	{
...
*/
void
G_InitMapEntityData ( mapEntityData_Team_t *teamList )
{
	int				i;
	...
	for
	(
		i = 0, trav = teamList->mapEntityData_Team + 1, lasttrav = teamList->mapEntityData_Team;
		i < MAX_GENTITIES - 1;
		i++, trav++
	)
	{
...
*/
mapEntityData_t *
G_FreeMapEntityData ( mapEntityData_Team_t *teamList, mapEntityData_t *mEnt )
{
...
*/
mapEntityData_t *
G_AllocMapEntityData ( mapEntityData_Team_t *teamList )
{
...
*/
mapEntityData_t *
G_FindMapEntityData ( mapEntityData_Team_t *teamList, int entNum )
{
	mapEntityData_t *mEnt;
...
*/
mapEntityData_t *
G_FindMapEntityDataSingleClient ( mapEntityData_Team_t *teamList, mapEntityData_t *start, int entNum, int clientNum )
{
	mapEntityData_t *mEnt;
	if ( start )
	{
...
/* */
void
G_SetupFrustum_ForBinoculars ( gentity_t *ent )
{
...
*/
static qboolean
G_CullPointAndRadius ( vec3_t pt, float radius )
{
	int		i;
	float	dist;
	plane_t *frust;
...
/* */
qboolean
G_VisibleFromBinoculars ( gentity_t *viewer, gentity_t *ent, vec3_t origin )
{
	vec3_t	vieworg;
	trace_t trace;
...
/* */
void
G_UpdateTeamMapData_Construct ( gentity_t *ent )
{
...
/* */
void
G_UpdateTeamMapData_Tank ( gentity_t *ent )
{
	int						num = ent - g_entities;
	mapEntityData_Team_t	*teamList;
	mapEntityData_t			*mEnt;
...
/* */
void
G_UpdateTeamMapData_Destruct ( gentity_t *ent )
{
	int						num = ent - g_entities;
	mapEntityData_Team_t	*teamList;
	mapEntityData_t			*mEnt;
	...
	else
	{
		...
		(
			...
			(
				ent->parent->target_ent->s.eType == ET_CONSTRUCTIBLE ||
				ent->parent->target_ent->s.eType == ET_EXPLOSIVE
			)
		)
		{
...
/* */
void
G_UpdateTeamMapData_Player ( gentity_t *ent, qboolean forceAllied, qboolean forceAxis )
{
	int						num = ent - g_entities;
	mapEntityData_Team_t	*teamList;
	mapEntityData_t			*mEnt;
	...
	if ( forceAxis && ent->client && !(ent->client->ps.pm_flags & PMF_LIMBO) /*ent->health > 0*/ )
	{
		...
		mEnt->data = num;
		mEnt->startTime = level.time;
		if ( g_gametype.integer == GT_WOLF_DM && g_allonmap.integer == 0 )
		{
			mEnt->type = ME_PLAYER_DM;
		}
		else
		{
			if ( ent->health <= 0 )
...
/* */
static void
G_UpdateTeamMapData_DisguisedPlayer ( gentity_t *spotter, gentity_t *ent, qboolean forceAllied, qboolean forceAxis )
{
	int						num = ent - g_entities;
	mapEntityData_Team_t	*teamList;
	mapEntityData_t			*mEnt;
...
/* */
void
G_UpdateTeamMapData_LandMine ( gentity_t *ent, qboolean forceAllied, qboolean forceAxis )
{
...
/* */
void
G_UpdateTeamMapData_CommandmapMarker ( gentity_t *ent )
{
	int						num = ent - g_entities;
	mapEntityData_Team_t	*teamList;
	mapEntityData_t			*mEnt;
...
/* */
void
G_SendSpectatorMapEntityInfo ( gentity_t *e )
{
...
/* */
void
G_SendMapEntityInfo ( gentity_t *e )
{
	mapEntityData_t			*mEnt;
	mapEntityData_Team_t	*teamList;
	char					buffer[2048];
...
G_UpdateTeamMapData ( void )
{
	...
	for ( i = 0, ent = g_entities; i < level.num_entities; i++, ent++ )
	{
		...
		{
			case ET_PLAYER:
				if ( g_allonmap.integer > 0 || g_gametype.integer == GT_WOLF_DM )
				{
					G_UpdateTeamMapData_Player( ent, qtrue, qtrue );
				}
				else
				{
		...
		if ( ent->client->sess.playerType == PC_FIELDOPS )
		{
			if ( ent->client->sess.skill[SK_SIGNALS] >= 4 && ent->health > 0 )
			{
				...
				for ( j = 0, ent2 = g_entities; j < level.maxclients; j++, ent2++ )
				{
					...
					if
					(
						G_VisibleFromBinoculars( ent, ent2, pos[0]) ||
												...
												if ( mEnt && level.time - mEnt->startTime > 5000 )
												{
													for ( k = 0; k < MAX_CLIENTS; k++ )
													{
														...
														(
															g_entities[k].inuse &&
															g_entities[k].client &&
															g_entities[k].client->sess.sessionTeam == ent->client->sess.
																sessionTeam
														)
														{
						...
						switch ( ent2->s.eType )
						{
							case ET_MISSILE:
								if ( ent2->methodOfDeath == MOD_LANDMINE )
								{
									if
									(
										(ent2->s.teamNum < 4 || ent2->s.teamNum >= 8) &&
												...
												case TEAM_AXIS:
													if ( !ent2->s.modelindex2 )
													{
														...
														if ( ent->client->landmineSpotted->count2 >= 250 )
														{
															...
															// for marker // CHRUKER: b036 - Landmine flags shouldn't block our view
															ent->client->landmineSpotted->s.frame = rand() % 20;
															ent->client->landmineSpotted->r.contents =	/*CONTENTS_CORPSE*/CONTENTS_TRANSLUCENT;
