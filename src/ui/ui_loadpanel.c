...
panel_button_text_t missiondescriptionTxt = { 0.2f, 0.2f, { 0.0f, 0.0f, 0.0f, 1.f }, 0, 0, &bg_loadscreenfont2, };
...
panel_button_text_t campaignpTxt = { 0.35f, 0.35f, { 1.0f, 1.0f, 1.0f, 0.6f }, 0, 0, &bg_loadscreenfont2, };
...
/* */
void
UI_DrawLoadPanel ( qboolean forcerefresh, qboolean ownerdraw, qboolean uihack )
{
...
/* */
void
UI_LoadPanel_RenderPercentageMeter ( panel_button_t *button )
{
	float		hunkfrac;
	float		w, h;
	vec2_t		org;
...
void
MiniAngleToAxis ( vec_t angle, vec2_t axes[2] )
{
...
/* */
void
SetupRotatedThing ( polyVert_t *verts, vec2_t org, float w, float h, vec_t angle )
{
	vec2_t	axes[2];
...
/* */
void
UI_LoadPanel_RenderHeaderText ( panel_button_t *button )
{
	uiClientState_t cstate;
...
/* */
const char *
UI_DownloadInfo ( const char *downloadName )
{
	...
	{
		s = va( "%s\n %s\n%s\n\n%s\n estimating...\n\n%s\n\n%s copied", dlText, ds, totalSizeBuf, etaText, xferText,
				dlSizeBuf );
		return ( s );
	}
	else
	{
		...
		if ( xferRate )
		{
			s = va( "%s\n %s\n%s\n\n%s\n %s\n\n%s\n %s/sec\n\n%s copied", dlText, ds, totalSizeBuf, etaText, dlTimeBuf,
					xferText, xferRateBuf, dlSizeBuf );
		}
		else
		{
			if ( downloadSize )
			{
				s = va( "%s\n %s\n%s\n\n%s\n estimating...\n\n%s\n\n%s copied", dlText, ds, totalSizeBuf, etaText,
						xferText, dlSizeBuf );
			}
			else
			{
...
/* */
void
UI_LoadPanel_RenderLoadingText ( panel_button_t *button )
{
	uiClientState_t		cstate;
	char				downloadName[MAX_INFO_VALUE];
	char				buff[2560];
	...
	while ( *p )
	{
		...
		{
			*p++ = '\0';
			Text_Paint_Ext( button->rect.x + 4, y, button->font->scalex, button->font->scaley, button->font->colour, s,
