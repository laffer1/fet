...
*/
static void
UI_PositionEntityOnTag ( refEntity_t *entity, const refEntity_t *parent, clipHandle_t parentModel, char *tagName )
{
	int				i;
...
*/
static int
UI_GetAnimation ( playerInfo_t *pi, const char *name )
{
	int i;
...
UI_DrawPlayer ( float x, float y, float w, float h, playerInfo_t *pi, int time )
{
	...
	if ( dp_realtime <= pi->muzzleFlashTime )
	{
		...
		if ( pi->flashDlightColor[0] || pi->flashDlightColor[1] || pi->flashDlightColor[2] )
		{
			...
			trap_R_AddLightToScene( flash.origin, 256, 0.75 + 8.0 / (rand() & 31), pi->flashDlightColor[0],
									pi->flashDlightColor[1], pi->flashDlightColor[2], 0, 0 );
		}
	}
...
static long
UI_StringHashValue ( const char *fname )
{
	int		i;
	long	hash;
	char	letter;
...
*/
static qboolean
AnimParseAnimConfig ( playerInfo_t *animModelInfo, const char *filename, const char *input )
{
	char			*text_p, *token;
	...
	for ( i = 0; (animModelInfo->version > 1) || (i < MAX_ANIMATIONS); i++ )
	{
		...
		if ( animModelInfo->version > 1 )
		{											// includes animation names at start of each line
			if ( !Q_stricmp( token, "ENDANIMS") )
			{										// end of animations
				break;
			}
		...
		if ( !animModelInfo->isSkeletal )
		{											// skeletal models dont require adjustment
	...
	if ( token && token[0] )
	{
		if ( animModelInfo->version < 2 || !Q_stricmp( token, "HEADFRAMES") )
		{
			...
			for ( i = 0; i < MAX_HEAD_ANIMS; i++ )
			{
				...
				headAnims[i].firstFrame -= animations[MAX_ANIMATIONS - 1].firstFrame +
					animations[MAX_ANIMATIONS - 1].numFrames +
					skip;
				token = COM_ParseExt( &text_p, qfalse );
				if ( !token || !token[0] )
				{
...
*/
static qboolean
UI_ParseAnimationFile ( const char *filename, playerInfo_t *pi )
{
	char			*text_p, *prev;
	int				len;
	int				i;
