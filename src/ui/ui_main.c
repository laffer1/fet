...
static char					*netnames[] = { "???", "UDP", "IPX", NULL };
...
#pragma GCC visibility pop
	...
	{							// UQ1: To load supremacyCampaign files...
		UI_LoadCampaigns();
		last_gametype = ui_netGameType.integer;
	}
#endif // __SUPREMACY__

	// UQ1: Update musicvolumePercent integer..
	CG_GetMusicVolume();
#ifdef __MUSIC_ENGINE__
	if ( command != UI_INIT && command != UI_SHUTDOWN )
	{
...
/* */
int
Text_Width_Ext ( const char *text, float scale, int limit, fontInfo_t *font )
{
	int			count, len;
	float		out = 0;
	glyphInfo_t *glyph;
...
/* */
int
Multiline_Text_Width ( const char *text, float scale, int limit )
{
	int			count, len;
	float		out = 0;
...
/* */
int
Text_Height_Ext ( const char *text, float scale, int limit, fontInfo_t *font )
{
	int			len, count;
	float		max;
	glyphInfo_t *glyph;
...
/* */
int
Multiline_Text_Height ( const char *text, float scale, int limit )
{
	int			len, count;
	float		max;
	float		totalheight = 0;
		...
		while ( s && *s && count < len )
		{
			...
			else
			{
				...
				if ( style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE )
				{
					...
					Text_PaintCharExt( x + (glyph->pitch * scalex) + ofs, y - yadj + ofs, glyph->imageWidth,
				...
				Text_PaintCharExt( x + (glyph->pitch * scalex), y - yadj, glyph->imageWidth, glyph->imageHeight, scalex,
			...
			else
			{
				...
				if ( !dummy )
				{
					if ( style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE )
					{
						...
						colorBlack[3] = newColor[3];
						trap_R_SetColor( colorBlack );
						Text_PaintChar( x + ofs, y - yadj + ofs, glyph->imageWidth, glyph->imageHeight, useScale,
										glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph );
						trap_R_SetColor( newColor );
						colorBlack[3] = 1.0;
					}

					Text_PaintChar( x, y - yadj, glyph->imageWidth, glyph->imageHeight, useScale, glyph->s, glyph->t,
...
)
{
	...
	if ( text )
	{
		...
		while ( s && *s && count < len )
		{
			...
			} else*/
			{
				...
				if ( style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE )
				{
					...
					colorBlack[3] = newColor[3];
					trap_R_SetColor( colorBlack );
					Text_PaintChar( x + (glyph->pitch * useScale) + ofs, y - yadj + ofs, glyph->imageWidth,
									glyph->imageHeight, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph );
					colorBlack[3] = 1.0;
					trap_R_SetColor( newColor );
				}

				Text_PaintChar( x + (glyph->pitch * useScale), y - yadj, glyph->imageWidth, glyph->imageHeight,
				...
				{
					Text_PaintChar( x + (glyph->pitch * useScale), y - yadj, glyph2->imageWidth, glyph2->imageHeight,
		...
		{
			yadj = useScale * glyph2->top;
			Text_PaintChar( x + (glyph2->pitch * useScale), y - yadj, glyph2->imageWidth, glyph2->imageHeight, useScale,
...
*/
static void
UI_DrawSaveGameShot ( rectDef_t *rect, float scale, vec4_t color )
{
	trap_R_SetColor( color );
...
UI_DrawClanCinematic ( rectDef_t *rect, float scale, vec4_t color )
{
	...
	if ( i >= 0 && i < uiInfo.teamCount )
	{
		if ( uiInfo.teamList[i].cinematic >= -2 )
		{
			...
			{
				uiInfo.teamList[i].cinematic = trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.teamList[i].imageName), 0,
...
UI_DrawPreviewCinematic ( rectDef_t *rect, float scale, vec4_t color )
{
	...
	{
		uiInfo.previewMovie = trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.movieList[uiInfo.movieIndex]), 0, 0, 0, 0,
													  (CIN_loop | CIN_silent) );
...
UI_DrawMapPreview ( rectDef_t *rect, float scale, vec4_t color, qboolean net )
{
...
#ifdef __SUPREMACY__
	if ( game == GT_WOLF_CAMPAIGN || game == GT_SUPREMACY_CAMPAIGN )
#else //__SUPREMACY__
...
#endif //__SUPREMACY__
		{
			...
			if ( uiInfo.campaignList[map].mapTC[0][0] && uiInfo.campaignList[map].mapTC[1][0] )
			{
				...
				h = rect->h;
				UI_AdjustFrom640( &x, &y, &w, &h );
				trap_R_DrawStretchPic( x, y, w, h, uiInfo.campaignList[map].mapTC[0][0] / 1024.f,
				...
				for ( i = 0; i < uiInfo.campaignList[map].mapCount; i++ )
				{
					...
					x = rect->x +
						(
							(uiInfo.campaignList[map].mapInfos[i]->mappos[0] - uiInfo.campaignList[map].mapTC[0][0]) /
							650.f *
							rect->w
						);
					y = rect->y +
						(
							(uiInfo.campaignList[map].mapInfos[i]->mappos[1] - uiInfo.campaignList[map].mapTC[0][1]) /
							650.f *
							rect->h
						);
					...
					if ( x + 10 + w > rect->x + rect->w )
					{
						Text_Paint( x - w - 10, y + 3, scale, colorWhite, uiInfo.campaignList[map].mapInfos[i]->mapName,
									0, 0, 0 );
					}
					else
					{
						Text_Paint( x + 10, y + 3, scale, colorWhite, uiInfo.campaignList[map].mapInfos[i]->mapName, 0,
									0, 0 );
					}
				}
			}
	...
	if ( uiInfo.mapList[map].mappos[0] && uiInfo.mapList[map].mappos[1] )
	{
		...
		h = rect->h;
		UI_AdjustFrom640( &x, &y, &w, &h );
		trap_R_DrawStretchPic( x, y, w, h, tl[0] / 1024.f, tl[1] / 1024.f, br[0] / 1024.f, br[1] / 1024.f,
							   uiInfo.campaignMap );
...
/* */
void
UI_DrawNetMapPreview ( rectDef_t *rect, float scale, vec4_t color, qboolean net )
{
...
UI_DrawMapCinematic ( rectDef_t *rect, float scale, vec4_t color, qboolean net )
{
	...
	if ( uiInfo.mapList[map].cinematic >= -1 )
	{
		if ( uiInfo.mapList[map].cinematic == -1 )
		{
			uiInfo.mapList[map].cinematic = trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[map].mapLoadName), 0,
																	0, 0, 0, (CIN_loop | CIN_silent) );
		}
...
/* */
static void
UI_DrawCampaignPreview ( rectDef_t *rect, float scale, vec4_t color, qboolean net )
{
...
/* */
static void
UI_DrawCampaignCinematic ( rectDef_t *rect, float scale, vec4_t color, qboolean net )
{
	int campaign = ( net ) ? ui_currentNetCampaign.integer : ui_currentCampaign.integer;
	if ( campaign < 0 || campaign > uiInfo.campaignCount )
	{
	...
	if ( uiInfo.campaignList[campaign].campaignCinematic >= -1 )
	{
		if ( uiInfo.campaignList[campaign].campaignCinematic == -1 )
		{
			uiInfo.campaignList[campaign].campaignCinematic = trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.campaignList[campaign].campaignShortName),
																						  0, 0, 0, 0,
																						  (CIN_loop | CIN_silent) );
		}
...
/* */
static void
UI_DrawCampaignName ( rectDef_t *rect, float scale, vec4_t color, int textStyle, qboolean net )
{
	int campaign = ( net ) ? ui_currentNetCampaign.integer : ui_currentCampaign.integer;
	if ( campaign < 0 || campaign > uiInfo.campaignCount )
	{
...
)
{
	...
	if
	(
		!uiInfo.campaignList[campaign].unlocked ||
...
/* */
static void
UI_DrawCampaignMapPreview ( rectDef_t *rect, float scale, vec4_t color, qboolean net, int map )
{
	int campaign = ( net ) ? ui_currentNetCampaign.integer : ui_currentCampaign.integer;
	if ( campaign < 0 || campaign > uiInfo.campaignCount )
	{
...
/* */
static void
UI_DrawMissionBriefingMap ( rectDef_t *rect )
{
	char				buffer[64];
...
/* */
static void
UI_DrawMissionBriefingTitle ( rectDef_t *rect, float scale, vec4_t color, int textStyle )
{
	char	buffer[64];
	mapInfo *mi;
	trap_Cvar_VariableStringBuffer( "mapname", buffer, 64 );
...
/* */
static void
UI_DrawMissionBriefingObjectives
(
	rectDef_t	*rect,
	float		scale,
	vec4_t		color,
...
UI_DrawNetFilter ( rectDef_t *rect, float scale, vec4_t color, int textStyle )
{
	...
	Text_Paint( rect->x, rect->y, scale, color, va( "Filter: %s", serverFilters[ui_serverFilterType.integer].description),
				0, 0, textStyle );
}
	...
	if ( uiInfo.serverStatus.refreshActive )
	{
		...
		if ( serverCount >= 0 )
		{
			Text_Paint( rect->x, rect->y, scale, newColor,
						va( trap_TranslateString( "Getting info for %d servers (ESC to cancel)"), serverCount), 0, 0,
						textStyle );
		}
		else
		{
			Text_Paint( rect->x, rect->y, scale, newColor,
						trap_TranslateString( "Waiting for response from Master Server"), 0, 0, textStyle );
		}
	}
	else
	{
		...
		Text_Paint( rect->x, rect->y, scale, color, va( trap_TranslateString( "Refresh Time: %s"), buff), 0, 0,
					textStyle );
	}
}
	...
	if ( uiInfo.serverStatus.motdLen )
	{
		...
		if ( uiInfo.uiDC.realTime > uiInfo.serverStatus.motdTime )
		{
			...
			if ( uiInfo.serverStatus.motdPaintX <= rect->x + 2 )
			{
				if ( uiInfo.serverStatus.motdOffset < uiInfo.serverStatus.motdLen )
				{
					uiInfo.serverStatus.motdPaintX += Text_Width( &uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset],
																  scale, 1 ) - 1;
		...
		if ( uiInfo.serverStatus.motdPaintX2 >= 0 )
		{
			...
			Text_Paint_Limit( &maxX2, uiInfo.serverStatus.motdPaintX2, rect->y, scale, color, uiInfo.serverStatus.motd,
							  0, uiInfo.serverStatus.motdOffset );
		}
...
/* */
static void
UI_DrawKeyBindStatus ( rectDef_t *rect, float scale, vec4_t color, int textStyle, float text_x, float text_y )
{
...
/* */
static float
UI_GetValue ( int ownerDraw, int type )
{
	return ( 0 );
}
...
UI_ServersQsortCompare ( const void *arg1, const void *arg2 )
{
	return
		(
			trap_LAN_CompareServers( ui_netSource.integer, uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir,
									 *(int *) arg1, *(int *) arg2)
		);
}
...
/* */
void
WM_setItemPic ( char *name, const char *shader )
{
	menuDef_t	*menu = Menu_GetFocused();
	itemDef_t	*item;
...
/* */
void
WM_setVisibility ( char *name, qboolean show )
{
	menuDef_t	*menu = Menu_GetFocused();
	itemDef_t	*item;
	item = Menu_FindItemByName( menu, name );
...
#ifdef __MUSIC_ENGINE__
extern void CG_SoundEngineStartMusicTrack( char* sound, qboolean loop);
#endif
...
UI_RunMenuScript ( char **args )
{
	...
	if ( String_Parse( args, &name) )
	{
		if ( Q_stricmp( name, "StartServer") == 0 )
		{
			...
			trap_Cvar_SetValue( "g_gametype", Com_Clamp( 0, (GT_MAX_GAME_TYPE - 1), ui_netGameType.integer) );
#ifdef __SUPREMACY__
			if ( ui_netGameType.integer == GT_WOLF_CAMPAIGN || ui_netGameType.integer == GT_SUPREMACY_CAMPAIGN )
#else //__SUPREMACY__
		...
		else if ( Q_stricmp( name, "RunSPDemo") == 0 )
		{
			if ( uiInfo.demoAvailable )
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va( "demo %s_%i", uiInfo.mapList[ui_currentMap.integer].mapLoadName,
									  uiInfo.gameTypes[ui_gameType.integer].gtEnum) );
			}
		}
		...
		else if ( Q_stricmp( name, "deleteDemo") == 0 )
		{
			if ( uiInfo.demoIndex >= 0 && uiInfo.demoIndex < uiInfo.demoCount )
			{
				trap_FS_Delete( va( "demos/%s.dm_%d", uiInfo.demoList[uiInfo.demoIndex],
...
/* */
static int
UI_MapCountByCampaign ( qboolean singlePlayer )
{
	int campaign;
	int i, count = 0;
...
*/
static int
UI_CampaignCount ( qboolean singlePlayer )
{
	int i, c /*, game*/ ;
	c = 0;
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( ping > /*=*/ 0 || ui_netSource.integer == AS_FAVORITES )
		{
			...
			if ( ui_browserShowPasswordProtected.integer )
			{
				...
				if
				(
					(password && ui_browserShowPasswordProtected.integer == 2) ||
			...
			if ( ui_browserShowFriendlyFire.integer )
			{
				...
				if
				(
					(friendlyFire && ui_browserShowFriendlyFire.integer == 2) ||
			...
			if ( ui_browserShowMaxlives.integer )
			{
				...
				if
				(
					(maxlives && ui_browserShowMaxlives.integer == 2) ||
			...
			if ( ui_browserShowPunkBuster.integer )
			{
				...
				if
				(
					(punkbuster && ui_browserShowPunkBuster.integer == 2) ||
			...
			if ( ui_browserShowAntilag.integer )
			{
				...
				if
				(
					(antilag && ui_browserShowAntilag.integer == 2) ||
			...
			if ( ui_browserShowWeaponsRestricted.integer )
			{
				...
				if
				(
					(weaponrestricted != 100 && ui_browserShowWeaponsRestricted.integer == 2) ||
			...
			if ( ui_browserShowTeamBalanced.integer )
			{
				...
				if
				(
					(balancedteams && ui_browserShowTeamBalanced.integer == 2) ||
			...
			}
			{
				...
				if ( Q_stricmp( gamename, "FalckonET") )
				{
					trap_LAN_MarkServerVisible( ui_netSource.integer, i, qfalse );
					continue;
				}
	...
	for ( i = 0; i < MAX_SERVERSTATUSREQUESTS; i++ )
	{
		...
		)
		{
			...
			if ( uiInfo.pendingServerStatus.num < uiInfo.serverStatus.numDisplayServers )
			{
				...
				trap_LAN_GetServerInfo( ui_netSource.integer,
										uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num], infoString,
										sizeof(infoString) );
...
/* */
static const char *
UI_SelectedMap ( qboolean singlePlayer, int index, int *actual )
{
	int i, c, game;
	c = 0;
	game = singlePlayer ? uiInfo.gameTypes[ui_gameType.integer].gtEnum : ui_netGameType.integer;
...
/* */
static const char *
UI_SelectedCampaign ( int index, int *actual )
{
	int i, c;
	c = 0;
	*actual = 0;
...
// NERVE - SMF
static void
UI_FeederAddItem ( float feederID, const char *name, int index )
{
...
//----(SA)	end
const char *
UI_FeederItemText ( float feederID, int index, int column, qhandle_t *handles, int *numhandles )
{
	...
	*numhandles = 0;
	...
	else if ( feederID == FEEDER_GLINFO )
	{
		...
		else if ( index == 2 )
		{
			return
				(
					va( "Pixelformat: color(%d-bits) Z(%d-bits) stencil(%d-bits)", uiInfo.uiDC.glconfig.colorBits,
...
#ifdef __SUPREMACY__
		if ( feederID == FEEDER_ALLMAPS && game != GT_WOLF_CAMPAIGN && game != GT_SUPREMACY_CAMPAIGN )
#else //__SUPREMACY__
	...
	else if ( feederID == FEEDER_CAMPAIGNS || feederID == FEEDER_ALLCAMPAIGNS )
	{
		...
		if ( feederID == FEEDER_CAMPAIGNS )
		{
			...
			ui_currentCampaignCompleted.integer =
				(
					uiInfo.campaignList[ui_currentCampaign.integer].progress == uiInfo.campaignList[campaignCount - 1].
						mapCount
				);
			trap_Cvar_Set( "ui_currentCampaignCompleted", va( "%i", (
							   uiInfo.campaignList[ui_currentCampaign.integer].progress == uiInfo.campaignList[
							   campaignCount - 1].mapCount)) );
		}
		else
		{
			ui_currentNetCampaign.integer = actual;
			trap_Cvar_Set( "ui_currentNetCampaign", va( "%d", actual) );
			uiInfo.campaignList[ui_currentNetCampaign.integer].campaignCinematic = trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.campaignList[ui_currentNetCampaign.integer].campaignShortName),
																											   0, 0, 0,
																											   0,
																											   (CIN_loop | CIN_silent) );
		}
...
/* */
qboolean
UI_FeederSelectionClick ( itemDef_t *item )
{
	...
	if ( item->special == FEEDER_SERVERS && !Menus_CaptureFuncActive() )
	{
		...
		if ( BG_CursorInRect( &rect) )
		{
			...
			if
			(
				trap_LAN_ServerIsInFavoriteList( ui_netSource.integer,
	...
	while ( 1 )
	{
		...
		if ( token[0] == '{' )
		{
			if ( join )
			{
				if
				(
					!String_Parse( p, &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes].gameType) ||
...
#ifdef _DEBUG	// UQ1: Added for debugging gametype loading...
		Com_Printf( "Parse gametype: [%i] %s (%s) - %s.\n", uiInfo.gameTypes[uiInfo.numGameTypes - 1].gtEnum,
					uiInfo.gameTypes[uiInfo.numGameTypes - 1].gameType,
					uiInfo.gameTypes[uiInfo.numGameTypes - 1].gameTypeShort,
					uiInfo.gameTypes[uiInfo.numGameTypes - 1].gameTypeDescription );
#endif
	}
...
MapList_Parse ( char **p )
{
	...
	while ( 1 )
	{
		...
		if ( token[0] == '{' )
		{
			if
			(
				!String_Parse( p, &uiInfo.mapList[uiInfo.mapCount].mapName) ||
				...
				if ( (*buf) && (Q_stricmp( buf, ";")) )
				{
					...
					else
					{
						...
						if ( UI_Cvar_VariableString( "com_errorDiagnoseIP")[0] )
						{
							...
							if ( missing_files[0] )
							{
								trap_Cvar_Set( "com_errorMessage", va( "%s\n\n%s\n%s",
...
UI_DisplayDownloadInfo ( const char *downloadName, float centerPoint, float yStart, float scale )
{
	...
	else
	{
		...
		if ( downloadSize && xferRate )
		{
			...
			Text_PaintCenter( centerPoint, yStart + 340, scale, colorYellow,
		...
		if ( xferRate )
		{
			Text_Paint( 260, yStart + 260, scale, colorYellow, va( "%s/Sec", xferRateBuf), 0, 0,
...
cvarTable_t cvarTable[] =
{
	...
	// so sound stuff latches, strange as that seems
	// jpw
	...
	{ &ui_netGameType, "ui_netGametype", "4", CVAR_ARCHIVE },
	...
	// -NERVE - SMF
	{ &gamename, "gamename", "", CVAR_SERVERINFO | CVAR_LATCH },
	...
	{ NULL, "s_musicvolume", "0.5", CVAR_ARCHIVE },
	{ NULL, "s_inetRadio", "0", CVAR_ARCHIVE },
	...
	for ( i = 0, cv = cvarTable; i < cvarTableSize; i++, cv++ )
	{
		if ( cv->vmCvar )
		{
			...
			if ( cv->modificationCount != cv->vmCvar->modificationCount )
			{
				...
				{
					BG_setCrosshair( cg_crosshairColor.string, uiInfo.xhairColor, cg_crosshairAlpha.value,
									 "cg_crosshairColor" );
				}
