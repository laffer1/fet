...
*/
int
UI_ParseInfos ( char *buf, int max, char *infos[], int totalmax )
{
	char	*token;
	int		count;
	...
	while ( trap_PC_ReadToken( handle, &token) )
	{
		...
		else if ( !Q_stricmp( token.string, "type") )
		{
			...
			else
			{
				...
				if ( strstr( token.string, "wolftdm") )
				{
					uiInfo.mapList[uiInfo.mapCount].typeBits |= ( 1 << GT_WOLF_TDM );
					uiInfo.mapList[uiInfo.mapCount].typeBits |= ( 1 << GT_WOLF_DM );
				}

				if ( strstr( token.string, "wolfctf") )
				{
					uiInfo.mapList[uiInfo.mapCount].typeBits |= ( 1 << GT_WOLF_CTF );
				}

#ifdef __SUPREMACY__
				if ( strstr( token.string, "supremacy") )
				{
					uiInfo.mapList[uiInfo.mapCount].typeBits |= ( 1 << GT_SUPREMACY );
				}
#endif //__SUPREMACY__
			}
		}
...
/* */
mapInfo *
UI_FindMapInfoByMapname ( const char *name )
{
	int i;
...
static void
UI_LoadCampaignsFromFile ( const char *filename )
{
	int			handle, i;
	pc_token_t	token;
	handle = trap_PC_LoadSource( filename );
	...
	while ( trap_PC_ReadToken( handle, &token) )
	{
		...
		else if ( !Q_stricmp( token.string, "type") )
		{
			...
			if ( strstr( token.string, "wolfcoop") )
			{
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= ( 1 << GT_COOP );
			}
			...
			if ( strstr( token.string, "wolftdm") )
			{
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= ( 1 << GT_WOLF_TDM );
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= ( 1 << GT_WOLF_DM );
			}

			if ( strstr( token.string, "wolfctf") )
			{
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= ( 1 << GT_WOLF_CTF );
			}

#ifdef __SUPREMACY__
			if ( strstr( token.string, "supremacy") )
			{
				uiInfo.campaignList[uiInfo.campaignCount].typeBits |= ( 1 << GT_SUPREMACY );
			}
#endif //__SUPREMACY__
		}
		else if ( !Q_stricmp( token.string, "maps") )
...
/* */
const char *
UI_DescriptionForCampaign ( void )
{
	int		i = 0, j = 0;
	char	*mapname;
	char	info[MAX_INFO_STRING];
...
*/
int
UI_FindCampaignInCampaignList ( const char *shortName )
{
	int i;
	if ( !shortName )
	{
...
#ifdef __SUPREMACY__
	if ( ui_netGameType.integer == GT_SUPREMACY_CAMPAIGN )
	{
		numdirs = trap_FS_GetFileList( "scripts", ".supremacyCampaign", dirlist, 1024 );
		Com_Printf( "Loading supremacy campaigns.\n" );
	}
	else
#endif //__SUPREMACY__
