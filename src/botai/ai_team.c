...
*/
int
BotNumTeamMates ( bot_state_t *bs, int *list, int maxList )
{
	int i, j, numplayers;
	numplayers = 0;
...
*/
int
BotNumTeamMatesWithTarget ( bot_state_t *bs, int targetEntity, int *list, int maxList )
{
	int i, j, numplayers;
	numplayers = 0;
	for ( i = 0; i < level.numConnectedClients; i++ )
...
*/
int
BotNumTeamMatesWithTargetByClass ( bot_state_t *bs, int targetEntity, int *list, int maxList, int playerType )
{
	int i, j, numplayers;
	numplayers = 0;
	for ( i = 0; i < level.numConnectedClients; i++ )
...
*/
float *
BotSortPlayersByDistance ( vec3_t target, int *list, int numList )
{
...
*/
float *
BotSortPlayersByTraveltime ( int areanum, int *list, int numList )
{
	static float	outDistances[MAX_CLIENTS];
	float			distances[MAX_CLIENTS], bestDist;
	int				i, j, outList[MAX_CLIENTS], best = 0;
	...
	for ( i = 0; i < numList; i++ )
	{
		...
		if ( lbs && lbs->inuse )
		{
			distances[i] = (float) trap_AAS_AreaTravelTimeToGoalArea( BotGetArea( list[i]), BotGetOrigin( list[i]),
																	  areanum, lbs->tfl );
		}
		else
		{
...
*/
gentity_t *
BotGetEnemyFlagCarrier ( bot_state_t *bs )
{
	int			i;
...
*/
int
BotGetTeamFlagCarrier ( bot_state_t *bs )
{
	int			i;
	char		buf[MAX_INFO_STRING];
	static int	maxclients;
...
*/
int
BotFlagAtBase ( int team, gentity_t **returnEnt )
{
	gentity_t				*ent;
...
*/
qboolean
BotFindDroppedFlag ( gentity_t **returnEnt )
{
	gentity_t	*ent;
...
*/
int
BotFindSparseDefendArea ( bot_state_t *bs, bot_goal_t *goal, qboolean force )
{
	...
	for ( i = 0; i < numTeam; i++ )
	{
		if
		(
			!g_entities[teammates[i]].inuse ||
			!(g_entities[teammates[i]].r.svFlags & SVF_BOT) ||
			g_entities[teammates[i]].health <= 0
		)
		{
			continue;
		}
	...
	(
		...
		!BotCoop() &&
		goal->entitynum >= 0 &&
		goal->entitynum < MAX_CLIENTS &&
		BotCarryingFlag( goal->entitynum)
	)
	{
		...
		{											// use the bot's current goal
			...
			(
				obs->alt_goal.number &&
				obs->alt_goal.entitynum == obs->target_goal.entitynum &&
				obs->alt_goal.number == obs->target_goal.number
			)
			{
...
*/
	...
	//
	numareas = trap_AAS_ListAreasInRange( targetPos, goal->areanum, maxrange,
										  bs->tfl &~(TFL_BARRIERJUMP | TFL_LADDER | TFL_JUMP), (float **) waypoints, 64 );
...
/* */
gentity_t *
G_FindMissile ( gentity_t *start, weapon_t weap )
{
...
/* */
gentity_t *
G_FindDynamite ( gentity_t *start )
{
...
/* */
gentity_t *
G_FindSmokeBomb ( gentity_t *start )
{
...
/* */
gentity_t *
G_FindLandmine ( gentity_t *start )
{
...
/* */
gentity_t *
G_FindSatchels ( gentity_t *start )
{
...
qboolean
G_ConstructionBegun ( gentity_t *ent )
{
...
qboolean
G_ConstructionIsFullyBuilt ( gentity_t *ent )
{
...
qboolean
G_ConstructionIsPartlyBuilt ( gentity_t *ent )
{
...
/* */
qboolean
G_ConstructionIsDestroyable ( gentity_t *ent )
{
	if ( !G_ConstructionIsPartlyBuilt( ent) )
	{
		return ( qfalse );
...
gentity_t *
G_ConstructionForTeam ( gentity_t *toi, team_t team )
{
...
/* */
gentity_t *
G_IsConstructible ( team_t team, gentity_t *toi )
{
	gentity_t	*ent;
...
/* */
gentity_t *
G_FindDynamiteTargetForTeam ( gentity_t *trav, team_t team )
{
	gentity_t	*targ;
...
/* */
gentity_t *
G_FindSatchelChargeTargetForTeam ( gentity_t *trav, team_t team )
{
	gentity_t	*targ;
	while ( (trav = BotFindNextStaticEntity( trav, BOTSTATICENTITY_OBJECTIVE_INFO)) )
	{
...
*/
int
BotGetTargetsForSatchelCharge ( team_t team, int *list, int listSize, qboolean ignoreSatchelCharge )
{
	int					count;
	gentity_t			*trav, *dyn;
	vec3_t				pos, vec;
	...
	for
	(
		trav = G_FindSatchelChargeTargetForTeam( NULL, team);
		trav;
		trav = G_FindSatchelChargeTargetForTeam( trav->parent, team)
	)
	{
...
*/
int
BotGetTargetExplosives ( team_t team, int *list, int listSize, qboolean ignoreDynamite )
{
	int					count;
	gentity_t			*trav, *dyn;
	vec3_t				pos, vec;
	...
	for
	(
		trav = G_FindDynamiteTargetForTeam( NULL, team);
		trav;
		trav = G_FindDynamiteTargetForTeam( trav->parent, team)
	)
	{
		if ( !ignoreDynamite )
		{
...
/* */
int
GetTargetExplosives ( team_t team, qboolean ignoreDynamite )
{
...
*/
int
BotGetTargetDynamite ( int *list, int listSize, gentity_t *target )
{
	gentity_t	*dyn, *trav;
	vec3_t		vec;
	int			count = 0;
		...
		for ( team = TEAM_AXIS; team <= TEAM_ALLIES; team++ )
		{
			...
			if ( target )
			{
				if ( target->s.eType == ET_EXPLOSIVE )
				{
					...
					if
					(
						BG_BBoxCollision( dyn->r.absmin, dyn->r.absmax, target->parent->r.absmin,
										  target->parent->r.absmax)
					)
					{
						if ( list )
						{
				...
				else
				{
					...
					if
					(
						(VectorLengthSquared( vec) <= SQR( dyn->splashRadius)) &&
						CanDamage( target, dyn->r.currentOrigin)
					)
					{
						if ( list )
						{
			...
			else
			{
				...
				)
				{
					if ( trav->s.eType == ET_EXPLOSIVE )
					{
						...
						if
						(
							BG_BBoxCollision( dyn->r.absmin, dyn->r.absmax, trav->parent->r.absmin,
											  trav->parent->r.absmax)
						)
						{
							if ( list )
							{
					...
					else
					{
						...
						(
							(VectorLengthSquared( vec) <= SQR( dyn->splashRadius)) &&
							CanDamage( trav, dyn->r.currentOrigin)
						)
						{
							if ( list )
							{
...
*/
int
BotGetConstructibles ( team_t team, int *list, int listSize, qboolean ignoreBuilt )
{
	gentity_t	*trav, *targ;
	int			count = 0;
	trav = NULL;
...
*/
int
BotNumTeamClasses ( team_t team, int mpClass, int ignore )
{
	gclient_t	*cl;
	int			i, cnt, j;
	cnt = 0;
...
*/
qboolean
BotCheckNeedEngineer ( bot_state_t *bs, team_t team )
{
	...
	*lastTime = level.time;
	...
	// flag checks
	if ( BotCarryingFlag( bs->client) || (bs->leader > -1 && BotCarryingFlag( bs->leader)) )
	{						// dont abandon a leader that needs us
		return ( qfalse );
	}
...
*/
int
BotSuggestClass ( bot_state_t *bs, team_t team )
{
	gclient_t	*cl;
	int			i;
	...
	if ( lastMg42Death && ((level.time - lastMg42Death) < (30 * 1000)) )
	{
		...
		numRequired[PC_SOLDIER] =
			(
				numTeamMembers -
				3 > numRequired[PC_SOLDIER] ? numTeamMembers -
				3 : numRequired[PC_SOLDIER]
			);
...
*/
int
BotSuggestWeapon ( bot_state_t *bs, team_t team )
{
	int			i, r;
	gentity_t	*trav;
	...
	if ( level.captureFlagMode )
	{
		...
		if
		(
			(bs->mpClass == PC_COVERTOPS) &&
			(rand() % 3) &&
			(level.time < level.startTime + 120000) &&
			(bs->mpTeam != level.attackingTeam)
		)
		{
			r = rand() % 2;
			switch ( r )
		...
		if
		(
			(bs->mpClass == PC_SOLDIER) &&
			(rand() % 3) &&
			(level.time < level.startTime + 30000) &&
			(bs->mpTeam == level.attackingTeam)
		)
		{
...
qboolean
BotClass_CovertOpsCheckDisguises ( bot_state_t *bs, int maxTravel, bot_goal_t *goal )
{
	gentity_t	*trav;
	...
	while ( (trav = G_Find( trav, FOFS( classname), "corpse")) )
	{
		...
		if ( numList )
		{
			numList = BotReduceListByTravelTime( list, numList, BotGetOrigin( trav->s.number),
												 BotGetArea( trav->s.number), BotTravelTimeToEntity( bs, trav->s.number) );
		}
...
*/
qboolean
BotClass_MedicCheckRevives ( bot_state_t *bs, int maxtravel, bot_goal_t *goal, qboolean lookForBots )
{
	gentity_t	*trav;
	...
	for ( i = 0; i < numTeammates; i++ )
	{
		...
		if ( (numList = BotNumTeamMatesWithTargetByClass( bs, teammates[i], list, 32, PC_MEDIC)) )
		{
			if
			(
				BotReduceListByTravelTime( list, numList, BotGetOrigin( teammates[i]), BotGetArea( teammates[i]),
										   BotTravelTimeToEntity( bs, teammates[i]))
			)
			{
				continue;
			}
...
*/
qboolean
G_RequestedHealth ( bot_state_t *bs, int client, qboolean clearRequest )
{
	bot_chat_t	*trav;
	int			i;
	trav = bs->vchats;
...
*/
qboolean
BotClass_MedicCheckGiveHealth ( bot_state_t *bs, int maxTravelTime, bot_goal_t *goal )
{
	gentity_t	*trav;
...
*/
qboolean
G_RequestedAmmo ( bot_state_t *bs, int client, qboolean clear )
{
	bot_chat_t	*trav;
	int			i;
...
*/
qboolean
BotClass_LtCheckGiveAmmo ( bot_state_t *bs, int maxTravelTime, bot_goal_t *goal )
{
	gentity_t	*trav;
...
*/
void
BotRecordVoiceChat ( int client, int destclient, const char *id, int mode, qboolean noResponse )
{
	int			i, j, vchat_id;
...
*/
void
BotCheckVoiceChatResponse ( bot_state_t *bs )
{
	int			i;
	bot_chat_t	*trav;
	gentity_t	*ent, *other;
...
*/
void
BotDelayedVoiceChat ( gentity_t *ent )
{
	bot_state_t *bs;
...
*/
void
BotSendVoiceChat ( bot_state_t *bs, const char *id, int mode, int delay, qboolean voiceonly, qboolean forceIfDead )
{
	gentity_t	*thinker;
...
//
void
AI_Team_Init ( AI_Team_t *thisOne )
{
...
)
{
	...
	if
	(
		g_aiTeam[bs->sess.sessionTeam].last_voice_chat &&
		(g_aiTeam[bs->sess.sessionTeam].last_voice_chat > level.time - idleTime)
	)
	{
...
*/
qboolean
G_RequestedFollow ( bot_state_t *bs, int client )
{
	bot_chat_t	*trav;
	int			i;
	qboolean	clearRequest = qfalse;
...
*/
int
BotGetLeader ( bot_state_t *bs, qboolean onlyRequested )
{
	...
	for ( i = 0; i < level.numConnectedClients; i++, trav++ )
	{
		...
		if
		(
			(ourDist = VectorDistanceSquared( trav->r.currentOrigin, bs->origin)) >
				(MAX_BOTLEADER_DIST * MAX_BOTLEADER_DIST)
		)
		{
			continue;
		}
		...
		if ( requested )
		{
			...
			if ( j == 0 )
			{
				BotVoiceChatAfterIdleTime( bs->client, "WhereTo", SAY_TEAM, 1000 + rand() % 3000, BOT_SHOWTEXT,
										   (requested ? 4000 : 20000), qfalse );
			}
			else if ( j == 1 )
			{
				BotVoiceChatAfterIdleTime( bs->client, "LetsGo", SAY_TEAM, 1000 + rand() % 3000, BOT_SHOWTEXT,
										   (requested ? 4000 : 20000), qfalse );
			}
...
/* */
int
EntGetNumBotFollowers ( int entNum )
{
	return ( 0 );
}
...
*/
void
BotSetLeaderTagEnt ( bot_state_t *bs )
{
...
/* */
int
BotEngagementFunc ( bot_state_t *bs )
{
	return ( 0 );
}
...
/* */
int
BotBehaviourFunc ( bot_state_t *bs )
{
	return ( 0 );
}
...
/* */
int
BotObjectiveFunc ( bot_state_t *bs )
{
	return ( 0 );
}
