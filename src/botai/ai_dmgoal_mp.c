...
*/
qboolean
BotMP_CheckClassActions ( bot_state_t *bs )
{
	qboolean	hasLeader;
...
*/
qboolean
BotMP_CheckEmergencyGoals ( bot_state_t *bs )
{
	gentity_t	*trav, *flag;
	int			i, t;
	int			list[32], numList;
		...
		if ( BotFindDroppedFlag( &trav) )
		{
			...
			if ( !(trav->r.ownerNum == bs->client && trav->botIgnoreTime > level.time) )
			{
				...
				if ( BotGoalForEntity( NULL, trav->s.number, &target, BGU_HIGH) )
				{
					...
					if
					(
						VectorDistanceSquared( target.origin, bs->origin) < (1600 * 1600) ||
						trap_InPVS( target.origin, bs->origin)
					)
					{
	...
	if ( level.captureFlagMode )
	{
		...
		if ( bs->sess.sessionTeam != level.attackingTeam )
		{
			...
			if ( !BotFlagAtBase( bs->sess.sessionTeam, &flag) )
			{
				...
				(
					flag &&
					(
						VectorDistanceSquared( flag->r.currentOrigin, bs->eye) < (1024 * 1024) ||
						trap_InPVS( flag->r.currentOrigin, bs->eye) ||
						BotNumTeamMatesWithTarget( bs, flag->s.number, NULL, 0) < 1
					)
				)
				{
					...
					(
						...
						trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, areanum, bs->tfl)
					)
					{
		...
		else							// if we are close to the flag, get it!
		{
			...
			(
				(
					(bs->leader == -1) ||
					!trap_InPVS( bs->origin, g_entities[bs->leader].r.currentOrigin) ||
					(
			...
			)
			{
				...
				if ( bs->ainode == AINode_MP_MedicRevive || bs->ainode == AINode_MP_MedicGiveHealth )
				{
					...
					if
					(
						trap_InPVS( bs->origin, g_entities[bs->target_goal.entitynum].r.currentOrigin) &&
						...
						if ( t )
						{
							...
							if ( t < 700 )
							{
								if ( BotGoalWithinMovementAutonomy( bs, &target, BGU_MEDIUM) )
								{
									if
									(
										bs->target_goal.entitynum == target.entitynum &&
	...
	if
	(
		(g_entities[bs->client].client->respawnTime < level.time - 12000) &&
		g_entities[bs->client].awaitingHelpTime > level.time + 200
	)
	{
			...
			for ( i = 0; i < numList; i++ )
			{
				...
				if ( bs->sess.playerType != PC_ENGINEER )
				{
					if ( BotCanSnipe( bs, qtrue) )
					{
						if
						(
							level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] != BotGetTargetExplosives( bs->sess.sessionTeam, NULL, 0, qfalse) ||
							BotNumTeamMatesWithTarget( bs, list[i], NULL, 0) >= (int)
								(floor( 0.3 * numTeammates / numList))
						)
						{
							continue;
						}
					...
					if ( trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, areanum, bs->tfl) )
					{
						...
						if ( bs->sess.playerType == PC_ENGINEER && ((trav->s.teamNum % 4) != bs->sess.sessionTeam) )
						{
							if ( BotGoalWithinMovementAutonomy( bs, &target, BGU_HIGH) )
							{
								...
								if
								(
									bs->target_goal.entitynum &&
									bs->target_goal.entitynum == trav->s.number &&
							...
							(
								(
									BotNumTeamMatesWithTargetAndCloser( bs, target.entitynum, target.areanum, NULL, 0,
																		-1) < (int) (ceil( 0.3 * numTeammates / numList))
								)
							)
						{
...
#define BFG_CHECKMAX					if ( goalNum >= maxGoals ) \
	{															   \
		return ( maxGoals );									   \
	}
...
/* */
int
BotMP_FindGoal_BuildGoalList ( bot_state_t *bs, botgoalFind_t *pGoals, int maxGoals )
{
	int			goalNum = 0;
...
/* */
int QDECL
BotMP_FindGoals_Sort_Standard ( const void *a, const void *b )
{
...
/* */
int QDECL
BotMP_FindGoals_Sort_Engineer ( const void *a, const void *b )
{
	botgoalFind_t	*bg1 = (botgoalFind_t *) a;
	botgoalFind_t	*bg2 = (botgoalFind_t *) b;
...
/* */
int QDECL
BotMP_FindGoals_Sort_CovertOps ( const void *a, const void *b )
{
	botgoalFind_t	*bg1 = (botgoalFind_t *) a;
	botgoalFind_t	*bg2 = (botgoalFind_t *) b;
...
/* */
int
BotMP_FindGoal_ClassForGoalType ( botgoalFindType_t type )
{
	switch ( type )
	{
		case BFG_CONSTRUCT:
...
#define BFG_RETURN_FAILURE_AND_IGNORE	bg->ignore = qtrue; \
	BFG_RETURN_FAILURE
...
/* */
botMPpg_t
BotMP_FindGoal_ProcessGoal ( bot_state_t *bs, botgoalFind_t *bg, bot_goal_t *target_goal )
{
	int			t, i;
	vec3_t		mins, maxs;
	...
	switch ( bg->type )
	{
		...
		case BFG_TRIGGER:
			...
			if
			(
				((bg->ent->spawnflags & 1) && (bs->sess.sessionTeam == TEAM_AXIS)) ||
				((bg->ent->spawnflags & 2) && (bs->sess.sessionTeam == TEAM_ALLIES))
			)
			{
		...
		case BFG_MINE:
			...
			(
				(++i < 5) &&
				((target_goal->areanum < 0) || !PointInBounds( vec, bg->ent->r.absmin, bg->ent->r.absmax))
			);
			if ( i < 5 )
		...
		case BFG_MG42:
			if
			(
				(bs->target_goal.entitynum == BFG_GOAL_ENTITYNUM) &&
				(bs->ainode == AINode_MP_MG42Mount || bs->ainode == AINode_MP_MG42Scan)
			)
			{
				BFG_RETURN_ALREADY;
			}
...
/* */
void
BotMP_FindGoal_PostProcessGoal ( bot_state_t *bs, botgoalFind_t *bg, bot_goal_t *goal )
{
	bs->target_goal = *goal;
	switch ( bg->type )
	{
...
/* */
qboolean
BotMP_AlreadyDoing_FastOut ( bot_state_t *bs, botgoalFind_t *bg )
{
...
/* */
qboolean
BotMP_FindGoal_New ( bot_state_t *bs )
{
	...
	for ( i = 0; i < numGoals; i++ )
	{
		...
		switch ( res )
		{
			...
			case BPG_NEWGOAL:
				...
				findGoals[i].numPlayers = numCloser = BotNumTeamMatesWithTargetAndCloser( bs, goal.entitynum, goal.areanum,
																						  NULL, 0,
																						  BotMP_FindGoal_ClassForGoalType( findGoals[i].type) );
				if ( bestGoal_numCloser >= 0 )
				{
					...
					if ( numCloser == bestGoal_numCloser )
					{
						...
						if
						(
							(findGoals[i].defaultPriority == findGoals[bestGoal_InList].defaultPriority) &&
...
BotMP_FindGoal ( bot_state_t *bs )
{
	...
	// IMPORTANT TRIGGER MULTIPLE
	{
		...
		while ( (trav = BotFindNextStaticEntity( trav, BOTSTATICENTITY_TRIGGER_MULTIPLE)) )
		{
			...
			(
				(
					numList = BotNumTeamMatesWithTargetAndCloser( bs, trav->s.number, target.areanum, list, MAX_CLIENTS,
																  -1)
				) > (int) floor( 0.3 * numTeammates)
			)
			{
		...
		if ( closestTime )
		{
			...
			if
			(
				((trav->spawnflags & 1) && (bs->sess.sessionTeam == TEAM_AXIS)) ||
				...
				if ( ent->s.eType == ET_OID_TRIGGER )
				{
					...
					if ( numList )
					{
						...
						if ( oldestTime > 0 )
						{
							...
							trap_Trace( &tr, center, bs->cur_ps.mins, bs->cur_ps.maxs, end, -1,
				...
				else
				{
					...
					if ( numList )
					{
						...
						for ( i = 0; i < numList; i++ )
						{
							...
							if
							(
								(VectorLengthSquared( vec) <= SQR( G_GetWeaponDamage( WP_DYNAMITE))) &&
								CanDamage( ent, center)
							)
							{
								t = trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, list[i], bs->tfl );
								if ( t > 0 && (!gotTarget || t < 400) )
		...
		while ( (trav = BotFindNextStaticEntity( trav, BOTSTATICENTITY_MG42)) )
		{
			...
			if ( trav->health <= 0 )
			{
				if ( trav->melee )
				{
					...
					if
					(
						(ent->aiTeam == bs->sess.sessionTeam) &&
						(ent->botIgnoreTime < level.time) &&
						(BotNumTeamMatesWithTargetByClass( bs, trav->s.number, NULL, -1, PC_ENGINEER) == 0)
					)
					{
	...
	// TRIGGER MULTIPLE
	{
		...
		if ( closestTime )
		{
			...
			)
			{
				...
				{								// enough people going for it, so only go for it if there's nothing else to do
					targetTime = closestTime;
					gotTarget = -1;
					defendTarget = qfalse;
				}
	...
	if ( !gotTarget )
	{
		...
		while ( (trav = BotFindNextStaticEntity( trav, BOTSTATICENTITY_CHECKPOINT)) )
		{
					...
					if ( t && (!gotTarget || t < targetTime) )
					{
						...
						if ( t < 500 && BotNumTeamMatesWithTarget( bs, trav->s.number, NULL, 0) < 1 )
						{
							if ( BotGoalWithinMovementAutonomy( bs, &bestTarget, BGU_LOW) )
							{
								if
								(
									bs->target_goal.entitynum == bestTarget.entitynum &&
									bs->ainode == AINode_MP_TouchTarget
								)
								{
									return ( qfalse );
			...
			else
			{
				...
				if
				(
					!level.captureFlagMode &&
					BotNumTeamMatesWithTarget( bs, trav->s.number, NULL, 0) < (c > 1 ? c / 2 : c)
				)
				{
	...
	if ( level.captureFlagMode )
	{
		if ( bs->sess.sessionTeam == level.attackingTeam )
		{
			...
			{									// we have it!!
				...
				if ( ent )
				{
					...
					if ( bestDist > 0 )
					{
						if
						(
							bs->target_goal.entitynum == ent->s.number &&
							bs->ainode == AINode_MP_TouchTarget &&
						...
						if ( BotGoalWithinMovementAutonomy( bs, &target, BGU_LOW) )
						{
							...
							if ( ent && VectorDistanceSquared( bs->origin, ent->r.currentOrigin) < (384 * 384) )
							{
								...
								(
									(t > 1000) &&
									(
										numList = trap_AAS_AlternativeRouteGoals( bs->origin, bestAreaCenter, bs->tfl,
																				  altroutegoals, 40, 0)
									)
								)
								{
			...
			else
			{
				...
				if
				(
					(target.entitynum > -1 && target.entitynum != bs->client) &&
					(
						VectorDistanceSquared( g_entities[target.entitynum].r.currentOrigin, bs->origin) <
							(1600 * 1600)
					)							/*&& 
				...
				)
				{
					...
					if ( (numList = BotNumTeamMatesWithTarget( bs, target.entitynum, list, MAX_CLIENTS)) >= BOT_FLAG_CARRIER_DEFENDERS )
					{
						...
						distances = BotSortPlayersByDistance( g_entities[target.entitynum].r.currentOrigin, list,
															  numList );
				...
				if
				(
					target.entitynum != bs->client &&
					ent &&
					(BotNumTeamMatesWithTarget( bs, ent->s.number, NULL, 0) < (int) ceil( 0.3 * numTeammates))
				)
				{
					VectorAdd( ent->r.absmin, ent->r.absmax, brushPos );
					VectorScale( brushPos, 0.5, brushPos );
		...
		{										// ================================================================
			...
			(
				!gotTarget ||
				!level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] ||
				(
					level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] - BotGetTargetExplosives( bs->sess.sessionTeam,
																												NULL, 0,
																												qtrue) > 0
				)
			)
			{
				...
				if ( BotFlagAtBase( bs->sess.sessionTeam, &ent) == qtrue )
				{
					...
					(
						!gotTarget ||
						BotNumTeamMatesWithTarget( bs, ent->s.number, NULL, 0) < (int) ceil( 0.4 * numTeammates)
					)
					{
						...
						if ( t && (!gotTarget || defendTarget || t < targetTime) )
						{
							if ( BotGoalWithinMovementAutonomy( bs, &target, BGU_MEDIUM) )
							{
								...
								if ( BotGoalWithinMovementAutonomy( bs, &target, BGU_MEDIUM) )
								{
									...
									// we are on defense
									BotVoiceChatAfterIdleTime( bs->client, "OnDefense", SAY_TEAM, 1000, qfalse,
															   10000 + rand() % 5000, qfalse );
									bs->target_goal = target;
									AIEnter_MP_DefendTarget( bs );
									return ( qtrue );
								}
