...
*/
qboolean
Bot_ScriptAction_SetSpeed ( bot_state_t *bs, char *params )
{
	vec3_t	speed;
	char	*pString;
	int		i;
...
*/
void
Bot_ScriptError ( bot_state_t *bs, char *fmt, ... )
{
	va_list ap;
	char	text[512];
...
*/
qboolean
Bot_ScriptAction_Print ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_SetAccumToPlayerCount ( bot_state_t *bs, char *params )
{
	...
	{
		Bot_ScriptError( bs, "accum buffer index out of range, %s found (range is 0 - %i): SetAccumToPlayerCount %s",
						 token, MAX_SCRIPT_ACCUM_BUFFERS - 1, params );
	}
	...
	while ( qtrue )
	{
		...
		if ( !Q_stricmp( token, "/weapon") )
		{
			...
			for ( i = 0; i < level.maxclients; i++ )
			{
				...
				if
				(
					!(g_entities[i].client->ps.weapons[0] & weapons[0]) &&
					!(g_entities[i].client->ps.weapons[1] & weapons[1])
				)
				{
					validPlayers[i] = 0;
					count--;
...
*/
qboolean
Bot_ScriptAction_SpawnBot ( bot_state_t *bs, char *params )
{
...
/* */
qboolean
Bot_ScriptAction_Accum ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_Wait ( bot_state_t *bs, char *params )
{
	if ( !params || !params[0] )
	{
...
*/
qboolean
Bot_ScriptAction_MoveToMarker ( bot_state_t *bs, char *params )
{
	char				*pString, *token;
	...
	*/
	...
	if ( VectorDistanceSquared( bs->origin, target->origin) < SQR( radius) )
	{
		...
		(
			(bs->script.status.stackChangeTime < level.time - 500) &&
			VectorDistanceSquared( bs->origin, target->origin) < SQR( 48)
		)
	{
...
*/
qboolean
Bot_ScriptAction_Trigger ( bot_state_t *bs, char *params )
{
	gentity_t	*trent, *ent;
...
*/
qboolean
Bot_ScriptAction_Logging ( bot_state_t *bs, char *params )
{
	struct tm	*localTime;
	time_t		long_time;
	...
	if ( !Q_stricmp( params, "ON") )
	{
		...
		Q_strncpyz( filename, va( "BotLog_%s_[%i]_[%4i_%2i_%2i]_[%2i_%2i_%2i].txt", g_entities[bs->client].aiName,
					bs->client, 1900 + localTime->tm_year, 1 + localTime->tm_mon, 1 + localTime->tm_mday,
					localTime->tm_hour, localTime->tm_min, localTime->tm_sec), sizeof(filename) );
...
*/
qboolean
Bot_ScriptAction_AbortIfWarmup ( bot_state_t *bs, char *params )
{
	if ( level.warmupTime )
	{
...
char	*botAttributeStrings[] = { "BOT_REACTION_TIME", "BOT_AIM_ACCURACY", "BOT_WIMP_FACTOR", NULL };
...
*/
qboolean
Bot_ScriptAction_SetAttribute ( bot_state_t *bs, char *params )
{
	int		i;
	char	*pString, *token;
...
*/
qboolean
Bot_ScriptAction_MountMG42 ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_PlaySoundAtPlayer ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_SetWeapon ( bot_state_t *bs, char *params )
{
	...
	if ( weapon == -1 )
	{
		...
		Bot_ScriptError( bs, "Bot %s on team %s cannot use weapon %s\n", g_entities[bs->client].aiName,
						 (g_entities[bs->client].client->sess.sessionTeam == TEAM_AXIS) ? "Axis" : "Allies", params );
	}
...
*/
qboolean
Bot_ScriptAction_SetClass ( bot_state_t *bs, char *params )
{
	int		val = -1;
	char	userinfo[MAX_INFO_STRING];
	...
	else if ( !Q_stricmp( params, "lieutenant") )
	{	// FIXME: remove this from missionpack? once all scripts have been updated
		val = PC_FIELDOPS;
	}
...
*/
qboolean
Bot_ScriptAction_GlobalAccum ( bot_state_t *bs, char *params )
{
	char	*pString, *token, lastToken[MAX_QPATH];
	int		bufferIndex;
	pString = params;
...
*/
qboolean
Bot_ScriptAction_FollowLeader ( bot_state_t *bs, char *params )
{
	char		*pString, *token;
	gentity_t	*target;
...
*/
qboolean
Bot_ScriptAction_Cvar ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_SetMovementAutonomy ( bot_state_t *bs, char *params )
{
	int mlevel;
	if ( !params || !params[0] )
	{
...
*/
qboolean
Bot_ScriptAction_MovementAutonomy ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_NoTarget ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_ResetScript ( bot_state_t *bs, char *params )
{
	return ( qtrue );
}
...
*/
qboolean
Bot_ScriptAction_SetFieldOfView ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_SetHearingRange ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_SetCrouch ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
	if ( !params || !params[0] )
	{
...
/* */
qboolean
Bot_ScriptAction_SetProne ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
	if ( !params || !params[0] )
	{
...
*/
qboolean
Bot_ScriptAction_PrintAccum ( bot_state_t *bs, char *params )
{
	char		*token, *pString;
	gentity_t	*ent;
	int			bufferIndex;
...
*/
qboolean
Bot_ScriptAction_PrintGlobalAccum ( gentity_t *ent, char *params )
{
	char	*token, *pString;
	int		bufferIndex;
	if ( !params || !params[0] )
...
*/
qboolean
Bot_ScriptAction_BotDebugging ( gentity_t *ent, char *params )
{
	char	*token, *pString;
	if ( !params || !params[0] )
	{
...
*/
qboolean
Bot_ScriptAction_SetFireRate ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
	float	fireRate;
	if ( !params || !params[0] )
...
*/
qboolean
Bot_ScriptAction_SetFireCycleTime ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
	if ( !params || !params[0] )
	{
...
*/
qboolean
Bot_ScriptAction_SetVisionRange ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
	float	visionRange = 0;
	if ( !params || !params[0] )
...
*/
qboolean
Bot_ScriptAction_SetFarSeeingRange ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
...
*/
qboolean
Bot_ScriptAction_SetCloseHearingRange ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
...
*/
qboolean
Bot_ScriptAction_SetSpeedCoefficient ( bot_state_t *bs, char *params )
{
	return ( qtrue );
}
...
qboolean
Bot_ScriptAction_SetActiveWeapon ( bot_state_t *bs, char *params )
{
...
*/
qboolean
Bot_ScriptAction_Announce ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
...
*/
qboolean
Bot_ScriptAction_FireAtTarget ( bot_state_t *bs, char *params )
{
	gentity_t	*ent;
	vec3_t		vec, org, src;
	char		*pString, *token;
	...
	else								//if (bs->weaponnum != WP_MORTAR_SET)
	{
		for ( i = 0; i < 2; i++ )
		{
			...
			{
				if ( diff > 25 )
				{						// allow some slack when target is moving
					return ( qfalse );
				}
			}
		}
...
*/
qboolean
Bot_ScriptAction_SetScriptAutonomy ( bot_state_t *bs, char *params )
{
...
qboolean
Bot_ScriptAction_SetAmmoAmount ( bot_state_t *bs, char *params )
{
	char		*pString, *token;
	int			weapon, amount;
	...
	weapon = Bot_GetWeaponForClassAndTeam( g_entities[bs->client].client->sess.playerType,
										   g_entities[bs->client].client->sess.sessionTeam, token );
	if ( weapon == -1 )
	{

		// can't use this weapon
		Bot_ScriptError( bs, "Bot %s on team %s cannot use weapon %s\n", g_entities[bs->client].aiName,
						 (g_entities[bs->client].client->sess.sessionTeam == TEAM_AXIS) ? "Axis" : "Allies", token );
	}

	// Do we have this weapon?
	if ( !COM_BitCheck( bs->cur_ps.weapons, weapon) )
	{
		Bot_ScriptError( bs, "Bot_ScriptAction_SetAmmoAmount: Bot %s does not have weapon %s",
						 g_entities[bs->client].aiName, token );
	}
...
/* */
qboolean
Bot_ScriptAction_SetCivilian ( bot_state_t *bs, char *params )
{
	char	*pString, *token;
	if ( !params || !params[0] )
	{
