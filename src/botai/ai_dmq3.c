...
float			movementAutonomyRange[NUM_BMA] = { 256, // LOW
	1024, // MEDIUM
	99999, // HIGH
};

// BOT MOVEMENT AUTONOMY for Single Player
float			movementAutonomyRangeSP[NUM_BMA] = { 200, // LOW
	350, // MEDIUM
	700, // HIGH
};
...
*/
int
BotFirstReachabilityArea ( int entnum, vec3_t origin, int *areas, int numareas, qboolean distCheck )
{
	int		i, best = 0;
	trace_t tr;
	vec3_t	center;
		...
		if ( trap_AAS_AreaReachability( areas[i]) )
		{
			...
			if ( distCheck )
			{
				...
				if ( dist < bestDist )
				{
					...
					if ( tr.fraction > .99f || tr.startsolid )
					{	// if we start in solid, ignore trace test
						best = areas[i];
						bestDist = dist;
...
*/
int
BotFirstLadderArea ( int entnum, int *areas, int numareas )
{
	int i;
...
*/
int
BotPointAreaNum ( int entnum, vec3_t origin )
{
...
*/
int
BotReachableBBoxAreaNum ( bot_state_t *bs, vec3_t absmin, vec3_t absmax )
{
...
*/
static qboolean
sDoNearbyGoalCheck ( bot_state_t *bs, vec3_t loc, gentity_t *target )
{
	int			t, areanum;
	bot_goal_t	goal;
...
qboolean
BotFindNearbyTriggerGoal ( bot_state_t *bs )
{
	gentity_t	*trav;
	vec3_t		loc;
	int			i;
...
/* */
qboolean
BotFindNearbyGoal ( bot_state_t *bs )
{
	vec3_t		org;
	int			i;
	qboolean	needAmmo;
...
*/
qboolean
BotGotEnoughAmmoForWeapon ( bot_state_t *bs, int weapon )
{
	int ammo, clip;
...
/* */
qboolean
G_WeaponCharged ( playerState_t *ps, team_t team, int weapon, int *skill )
{
	switch ( weapon )
	{
		case WP_PANZERFAUST:
...
/* */
qboolean
BotWeaponCharged ( bot_state_t *bs, int weapon )
{
...
*/
qboolean
BotWeaponOnlyUseIfInInRange ( int weaponnum )
{
	switch ( weaponnum )
	{
		case WP_BINOCULARS:
...
*/
float
BotWeaponClosestDist ( int weaponnum )
{
	switch ( weaponnum )
	{
		case WP_PANZERFAUST:
...
/* */
int
BotTeamMatesNearEnemy ( bot_state_t *bs )
{
	int		i, j;
	vec_t	*vec;
...
*/
float
BotWeaponWantScale ( bot_state_t *bs, weapon_t weapon )
{
	qboolean	moving;
...
*/
int
BotBestFightWeapon ( bot_state_t *bs )
{
	weapon_t	bestWeapon;
	int			i, *ammo;
	...
	for ( i = 0; i < WP_NUM_WEAPONS; i++ )
	{
		if ( COM_BitCheck( bs->cur_ps.weapons, i) )
		{
			...
			(
				(inRange && !bestInRange) ||
				(inRange && wantScale >= bestWantScale) ||
				(!inRange && thisRange > bestRange)
			)
			{
				bestWeapon = i;
...
BotUpdateInventory ( bot_state_t *bs )
{
	...
	{
		bs->inventory[GOAL_TRAVELTIME] = trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin,
																			bs->target_goal.areanum, bs->tfl );
	}
	else
	{
...
*/
qboolean
EntityInLimbo ( aas_entityinfo_t *entinfo )
{
...
*/
		...
		if
		(
			trap_AAS_PredictClientMovement( &move, bs->client, bs->origin, -1, qtrue, sideward, vec3_origin, 10, 0, 0.1,
											SE_GAP | SE_LEAVEGROUND | SE_STUCK, -1, qfalse)
		)
		{
...
/* */
qboolean
BotEntInvisibleBySmokeBomb ( vec3_t start, vec3_t end )
{
	gentity_t	*ent = NULL;
	vec3_t		smokeCenter;
	float		smokeRadius;
...
*/
float
BotEntityVisible ( int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent, vec3_t entorigin )
{
	...
	if
	(
		(botstates[viewer].inuse && BotCanSnipe( &botstates[viewer], qtrue)) &&
		(VectorLengthSquared( dir) > SQR( 1024))
	)
	{
		checkCount = 1;
	}
	...
	for ( i = 0; i < checkCount; i++ )
	{
		...
		(
			trace.fraction >= .99f ||
			trace.ent == hitent ||
			((entinfo.flags & EF_TAGCONNECT) && (g_entities[trace.ent].nextTrain == g_entities[ent].tagParent))
		)
		{
...
*/
qboolean
BotVisibleFromPos ( vec3_t srcorigin, int srcnum, vec3_t destorigin, int destent, qboolean dummy )
{
	vec3_t	eye;
...
*/
qboolean
BotCheckAttackAtPos ( int entnum, int enemy, vec3_t pos, qboolean ducking, qboolean allowHitWorld )
{
	vec3_t	eye;
...
*/
int
BotGetMovementAutonomyLevel ( bot_state_t *bs )
{
	return ( BMA_HIGH );
}
...
*/
qboolean
BotGetMovementAutonomyPos ( bot_state_t *bs, vec3_t pos )
{
...
*/
float
BotGetRawMovementAutonomyRange ( bot_state_t *bs )
{
	float	range;
	int		level;
...
*/
float
BotGetMovementAutonomyRange ( bot_state_t *bs, bot_goal_t *goal )
{
	float	range;
...
qboolean
BotWithinLeaderFollowDist ( bot_state_t *bs )
{
	float		dist;
	gentity_t	*leader;
	if ( !BotSinglePlayer() && !BotCoop() )
...
*/
qboolean
BotPointWithinMovementAutonomy ( bot_state_t *bs, bot_goal_t *goal, vec3_t point )
{
	float	dist;
	vec3_t	pos;
...
/* */
qboolean
BotPointWithinRawMovementAutonomy ( bot_state_t *bs, vec3_t point )
{
	float	dist;
	vec3_t	pos;
	if ( !BotGetMovementAutonomyPos( bs, pos) )
...
*/
qboolean
BotGoalWithinMovementAutonomy ( bot_state_t *bs, bot_goal_t *goal, int urgency )
{
	int				i;
...
*/
qboolean
BotGoalForEntity ( bot_state_t *bs, int entityNum, bot_goal_t *goal, int urgency )
{
	vec3_t		p;
...
/* */
void
botindicator_think ( gentity_t *ent )
{
...
float
sAngleBetweenVectors ( vec3_t a, vec3_t b )
{
...
void
sAdjustPointTowardsPlayer
(
	vec3_t		playerLoc,
	vec3_t		endPos,
	qboolean	shouldLoop,
	vec3_t		outPos	/* Gordon: vec3_t is an array, no need for the pointer (was vec3_t*) */
)
{
...
*/
qboolean
BotCheckMovementAutonomy ( bot_state_t *bs, bot_goal_t *goal )
{
...
*/
void
BotIgnoreGoal ( bot_state_t *bs, bot_goal_t *goal, int duration )
{
	int				i;
	...
	for ( i = 0, ignoreTrav = bs->ignoreGoals; i < MAX_IGNORE_GOALS; i++, ignoreTrav++ )
	{
		if ( ignoreTrav->expireTime >= level.time )
		{
			...
			(
				(ignoreTrav->expireTime < level.time + duration) &&
				(!ignoreOldest || (ignoreOldest->expireTime < ignoreTrav->expireTime))
			)
			{
...
/* */
qboolean
BotDangerousGoal ( bot_state_t *bs, bot_goal_t *goal )
{
	int			i, j;
	vec3_t		bangPos;
	float		radius = 0.0;
	...
	for ( i = 0; i < dangerEntsCount; i++ )
	{
			...
			case ET_MISSILE:
				switch ( trav->methodOfDeath )
				{
					...
					case MOD_GRENADE_LAUNCHER:
						...
						if
						(
							!G_PredictMissile( trav, trav->nextthink - level.time, bangPos,
											   trav->methodOfDeath == MOD_PANZERFAUST ? qfalse : qtrue)
						)
						{
		...
		if
		(
			trav->s.eType == ET_CONSTRUCTIBLE ||
			(VectorDistanceSquared( bangPos, goal->origin) < SQR( radius + 90)) ||
			(VectorDistanceSquared( bangPos, bs->origin) < SQR( radius + 90))
		)
		{
...
*/
float
BotNoLeaderPenalty ( bot_state_t *bs )
{
	...
	if ( (distanceToPlayer <= kBOT_NEAR_LEADER_DISTANCE) || (bs->mpTeam == TEAM_AXIS) )
	{
	...
	// What's our penalty?
	noLeaderPenalty = ( distanceToPlayer - kBOT_NEAR_LEADER_DISTANCE ) / ( kBOT_FAR_FROM_LEADER_DISTANCE - kBOT_NEAR_LEADER_DISTANCE );
...
=====================*/
qboolean
ChangeBotAlertState ( bot_state_t *bs, aistateEnum_t newAlertState, qboolean force )
{
	...
	if
	(
		newAlertState != bs->alertState &&
		level.time > bs->
			alertStateAllowChangeTime	// if we are relaxing, make sure we don't relax too quickly, this would prevent model from
...
*/
float
BotGetEntitySurfaceSoundCoefficient ( int clientNum )
{
...
//
qboolean
BotIsValidTarget ( bot_state_t *bs, int target, int curenemy )
{
...
//
void
BotFindEnemies ( bot_state_t *bs, int *dangerSpots, int *dangerSpotCount )
{
	int					i;
	float				dist;
	aas_entityinfo_t	entinfo;
...
//
float
BotWeaponRange ( bot_state_t *bs, int weaponnum )
{
	switch ( weaponnum )
	{
...
/* */
qboolean
BotHasWeaponWithRange ( bot_state_t *bs, float dist )
{
	int i, *ammo;
	ammo = bs->cur_ps.ammo;
...
*/
void
BotSortClientsByDistance ( vec3_t srcpos, int *sorted, qboolean hasPanzer )
{
	int		i, j, best = 0;
...
*/
int
BotFindEnemyMP ( bot_state_t *bs, int curenemy, qboolean ignoreViewRestrictions )
{
	...
	for ( i = 0; i < level.numConnectedClients; i++ )
	{
		...
		(
			(
				!BotWantsToChase( bs) || !trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, BotGetArea( j),
																			 bs->tfl)
			) &&
...
float
BotGetReactionTime ( bot_state_t *bs )
{
...
BotAimAtEnemy ( bot_state_t *bs )
{
	...
	if ( enemyvisible )
	{
		...
		if ( wi.speed )
		{
			...
			if ( !(dist > 100 && VectorLengthSquared( dir) < SQR( 32)) )
			{
				...
				if ( aim_skill > 0.8 && //if the weapon is ready to fire
				bs->cur_ps.weaponstate == WEAPON_READY )
				{
					...
					trap_AAS_PredictClientMovement( &move, bs->enemy, origin, PRESENCE_CROUCH, qfalse, dir, cmdmove, 0,
	...
	else
	{
		...
		if ( aim_skill > 0.5 )
		{
			...
			if ( wi.number == WP_PANZERFAUST || wi.number == WP_GRENADE_LAUNCHER || wi.number == WP_GRENADE_PINEAPPLE )
			{
...
*/
void
BotAimAtEnemySP ( bot_state_t *bs )
{
...
BotCheckAttack ( bot_state_t *bs )
{
	...
	(
		...
		(bs->enemy == bs->last_botcheckattack_enemy) &&
		(bs->last_botcheckattack > level.time - 250)
	)
	{
	...
	if
	(
		(bs->cur_ps.weapon == WP_GRENADE_LAUNCHER || bs->cur_ps.weapon == WP_GRENADE_PINEAPPLE) &&
		bs->cur_ps.grenadeTimeLeft
	)
	{
		goto passed;
	}
	...
	if ( wi.proj.damagetype & DAMAGETYPE_RADIAL )
	{
		...
		for ( i = 0; i < level.maxclients; i++ )
		{
			...
			if
			(
				g_entities[i].client->sess.sessionTeam != TEAM_AXIS &&
...
int
BotEntityToActivate ( int entitynum )
{
...
/* */
void
BotModelMinsMaxs ( int modelindex, vec3_t mins, vec3_t maxs )
{
	gentity_t	*ent;
	int			i;
	ent = BotGetEntity( 0 );
...
BotAIBlocked ( bot_state_t *bs, bot_moveresult_t *moveresult, int activate )
{
	...
	if ( (entinfo.number < level.maxclients) && (bs->blockentTime < level.time) )
	{
		...
		(
			(
				...
				(bs->target_goal.flags & GFL_LEADER) &&
				(bs->leader == entinfo.number)
			)
		)
		{
		...
		if ( bs->target_goal.flags & GFL_LEADER )
		{
			if
			(
				(BotPointWithinMovementAutonomy( bs, &bs->target_goal, bs->origin)) &&
	...
	if ( entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex && activate )
	{
		...
		(
			(
				(Q_stricmp( g_entities[entinfo.number].classname, "func_door") == 0) ||
				(Q_stricmp( g_entities[entinfo.number].classname, "func_door_rotating") == 0)
			)
		)
		{
			if
			(
				(g_entities[entinfo.number].moverState == MOVER_POS1) ||
				(g_entities[entinfo.number].moverState == MOVER_POS1ROTATE)
			)
			{
				...
				(
					(g_entities[entinfo.number].moverState == MOVER_POS2) ||
					(g_entities[entinfo.number].moverState == MOVER_POS2ROTATE)
				)
			{
...
void
BotUpdateReconInfo ( bot_state_t *bs )
{
...
*/
void
BotDeathmatchAIFirstCalled ( bot_state_t *bs )
{
...
BotDeathmatchAI ( bot_state_t *bs, float thinktime )
{
	...
	if ( (level.warmupTime > level.time) && (bs->lastClassCheck < level.time - 1000) )
	{								// check for a better class
...
*/
void
BotSetBlockEnt ( int client, int blocker )
{
...
*/
void
BotMoveToIntermission ( int client )
{
...
*/
void
BotRecordTeamChange ( int client )
{
	int			team;
	int			i;
	bot_state_t *bs;
...
*/
void
BotRecordKill ( int client, int enemy )
{
	...
	if ( BotSameTeam( bs, enemy) )
	{
		...
		if ( !BotSinglePlayer() && !BotCoop() )
		{
			BotVoiceChatAfterIdleTime( bs->client, "Sorry", SAY_TEAM, 1000 + rand() % 4000, qfalse, 3000 + rand() % 2000,
									   qfalse );
		}
...
*/
void
BotRecordPain ( int client, int enemy, int mod )
{
	bot_state_t *bs = &botstates[client];
	gentity_t	*targ;
	if ( client == enemy )
	...
	if ( enemy < level.maxclients && BotSameTeam( bs, enemy) )
	{
		...
		// friendly fire!
		// Don't use excess voices in Single Player
		BotVoiceChatAfterIdleTime( bs->client, "HoldYourFire", SAY_TEAM, 1000 + rand() % 1000, qfalse, 3000 + rand() % 2000,
								   qfalse );
		return;
	}
	...
	if ( bs->defendgoal.entitynum >= level.maxclients )
	{
		...
		if
		(
			Q_stricmp( targ->classname, "team_CTF_redflag") &&
		...
		if ( !BotSinglePlayer() && !BotCoop() )
		{
			BotVoiceChatAfterIdleTime( bs->client, "TakingFire", SAY_TEAM, 1000 + rand() % 1000, qfalse,
									   5000 + rand() % 4000, qfalse );
		}
	}
}
...
*/
void
BotRecordDeath ( int client, int enemy )
{
	bot_state_t *bs = &botstates[client];
	gentity_t	*targ;
	if ( client == enemy )
...
*/
void
BotGetAimAccuracySkill ( bot_state_t *bs, float *outAimAccuracy, float *outAimSkill )
{
	...
	aim_accuracy = aim_accuracy * ( 1.0f - (NO_LEADER_MAX_AIM_PENALTY * noLeaderPenalty) );
...
*/
int
BotBestSniperSpot ( bot_state_t *bs )
{
...
*/
int
BotBestLandmineSpotingSpot ( bot_state_t *bs )
{
	gentity_t	*trav, *bestSpot;
	int			areanum, t, bestTime;
	trav = NULL;
...
*/
int
BotBestMG42Spot ( bot_state_t *bs, qboolean force )
{
	...
	while ( (mg42 = BotFindNextStaticEntity( mg42, BOTSTATICENTITY_MG42)) )
	{
		...
		if ( trav->melee->active )
		{
			...
			if
			(
				mg42->r.ownerNum < level.maxclients &&
				(g_entities[mg42->r.ownerNum].client->sess.sessionTeam != bs->sess.sessionTeam)
			)
			{
...
*/
int
BotGetNumVisibleSniperSpots ( bot_state_t *bs )
{
	gentity_t	*trav;
	int			cnt;
	vec3_t		dest;
...
*/
int
BotGetRandomVisibleSniperSpot ( bot_state_t *bs )
{
...
*/
int
BotMovementAutonomyForString ( char *string )
{
...
*/
char *
BotStringForMovementAutonomy ( int value )
{
	switch ( value )
	{
		case BMA_LOW:
...
*/
int
BotWeaponAutonomyForString ( char *string )
{
	if ( !Q_stricmp( string, "high") )
	{
		return ( BWA_HIGH );
...
*/
char *
BotStringForWeaponAutonomy ( int value )
{
	switch ( value )
	{
		case BMA_LOW:
...
*/
int
BotScriptAutonomyForString ( char *string )
{
...
*/
qboolean
BotCheckEmergencyTargets ( bot_state_t *bs )
{
	qboolean	retval;
	int			startTime = 0;
	if ( bot_profile.integer == 1 )
...
*/
qboolean
BotFindSpecialGoals ( bot_state_t *bs )
{
	qboolean	retval;
	int			startTime = 0;
	if ( bot_profile.integer == 1 )
...
*/
void
BotCheckVoiceChats ( bot_state_t *bs )
{
	if ( VectorLengthSquared( bs->cur_ps.velocity) < SQR( 10) )
	{
		...
		{
			BotVoiceChatAfterIdleTime( bs->client, "NeedAmmo", SAY_TEAM, 2000 + rand() % 10000, qfalse,
									   40000 + rand() % 15000, qfalse );
		}
		...
		if
		(
			(bs->sess.playerType == PC_MEDIC && BotHealthScale( bs->client) <= 0.0) ||
			(bs->sess.playerType != PC_MEDIC && BotHealthScale( bs->client) <= 0.2)
		)
		{
			BotVoiceChatAfterIdleTime( bs->client, "Medic", SAY_TEAM, 2000 + rand() % 10000, qfalse, 30000 + rand() % 10000,
									   qfalse );
		}
		...
		if
		(
			bs->sess.playerType != PC_MEDIC &&
			bs->last_checkvoice_health > 0 &&
			bs->cur_ps.stats[STAT_HEALTH] > bs->last_checkvoice_health
		)
		{
			BotVoiceChatAfterIdleTime( bs->client, "Thanks", SAY_TEAM, 500 + rand() % 1000, qfalse, 5000 + rand() % 5000,
									   qfalse );
		}
	}
...
*/
void
BotShareLastAttacked ( bot_state_t *bs )
{
	int			i;
...
*/
qboolean
BotEnemyCarryingFlag ( int entnum )
{
	bot_state_t *pbs;
	int			i;
...
*/
int
BotLastAttacked ( bot_state_t *bs )
{
...
BotSeekCover ( bot_state_t *bs )
{
	...
	//
	area = trap_AAS_NearestHideArea( bs->client, bs->origin, bs->areanum, bs->enemy,
									 g_entities[bs->enemy].r.currentOrigin, enemyarea, bs->tfl,
									 BotGetMovementAutonomyRange( bs, NULL), autonomyPos );
...
*/
int
BotReduceListByTravelTime ( int *list, int numList, vec3_t destpos, int destarea, int traveltime )
{
...
*/
int
BotTravelTimeToEntity ( bot_state_t *bs, int entnum )
{
	int area = 0;
...
*/
qboolean
BotBattleNewNode ( bot_state_t *bs )
{
	return ( qfalse );
}
...
*/
qboolean
BotDirectMoveToGoal ( bot_state_t *bs, bot_goal_t *goal, bot_moveresult_t *moveresult )
{
	vec3_t				dir;
	aas_clientmove_t	move;
	trace_t				tr;
	...
	if
	(
		trap_AAS_PredictClientMovement( &move, bs->client, bs->origin, goal->entitynum, qfalse, dir, goal->origin, -1,
										40, 0.05,
										SE_ENTERAREA | SE_HITGROUNDDAMAGE | SE_HITENT | SE_HITGROUNDAREA | SE_STUCK | SE_GAP,
										goal->areanum,
#ifdef _DEBUG
				qtrue)
	)
...
#endif
	{
		...
		switch ( move.stopevent )
		{
			...
			case SE_HITGROUNDAREA:
				...
				trap_Trace( &tr, bs->origin, bs->cur_ps.mins, bs->cur_ps.maxs, goal->origin, bs->client,
							MASK_SHOT	/*CONTENTS_BODY*/
						   );
				if
				(
					tr.fraction < .99f &&
					VectorDistanceSquared( bs->origin, tr.endpos) < SQR( 30) &&
					tr.entityNum != ENTITYNUM_WORLD
				)						// best not be worldspawn
				{
...
*/
qboolean
BotEntityTargetClassnameMatch ( int entityNum, const char *classname )
{
	gentity_t	*ent;
...
*/
qboolean
BotGetReachableEntityArea ( bot_state_t *bs, int entityNum, bot_goal_t *goal )
{
...
*/
qboolean
BotIsConstructible ( team_t team, int toiNum )
{
	gentity_t	*ent;
...
*/
int
BotCanSnipe ( bot_state_t *bs, qboolean checkAmmo )
{
...
*/
qboolean
EnemyIsCloseEnoughToFight ( bot_state_t *bs )
{
...
void
BotFindAndAttackEnemy ( bot_state_t *bs )
{
...
/* */
void
BotUpdateViewAngles ( bot_state_t *bs, bot_goal_t *goal, bot_moveresult_t moveresult )
{
	vec3_t	target;
	vec3_t	dir;
...
void
BotSetPOW ( int entityNum, qboolean isPOW )
{
...
*/
int
BotBestTargetWeapon ( bot_state_t *bs, int targetNum )
{
...
*/
gentity_t *
BotGetVisibleDamagableScriptMover ( bot_state_t *bs )
{
	gentity_t	*trav;
	int			i, wpn;
...
void
BotPickupWeapon ( int client, int weaponnum, qboolean alreadyHave )
{
...
*/
qboolean
BotEntityWithinView ( bot_state_t *bs, int viewEnt )
{
	vec3_t		dir, ang;
	pmoveExt_t	*pmExt;
