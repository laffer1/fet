...
static bot_script_event_define_t	botScriptEvents[] =
{
	...
	{ "trigger", Bot_EventMatch_StringEqual },

	// something has triggered us (always followed by an identifier)
	{ "pain", Bot_EventMatch_IntInRange },

	// we've been hurt
	{ "death", NULL },

	// RIP
	{ "activate", Bot_EventMatch_StringEqual },

	// "param" has just activated us
	{ "objective", Bot_EventMatch_StringEqual },

	// something has occured involving the objective
	{ "respawn", Bot_EventMatch_StringEqual },

	// respawned at "param" spawnpoint
...
*/
int
Bot_FindSriptGlobalData ( bot_script_data_t *data )
{
	int i;
...
*/
qboolean
Bot_EventMatch_StringEqual ( bot_script_event_t *event, char *eventParm )
{
...
*/
qboolean
Bot_EventMatch_IntInRange ( bot_script_event_t *event, char *eventParm )
{
	char	*pString, *token;
	int		int1, int2, eInt;
...
*/
int
Bot_EventForString ( char *string )
{
	int i;
...
Bot_ActionForString ( char *string )
{
	int i;
...
*/
void
Bot_ScriptParseAllCharacters ( void )
{
	char						*pScript;
	char						*token;
...
*/
void
Bot_ScriptParse ( bot_script_data_t *bsd, char **text )
{
	char						*token;
	qboolean					inScript;
	int							eventNum;
...
*/
void
Bot_ScriptChange ( bot_state_t *bs, int newScriptNum )
{
...
*/
void
Bot_ScriptEvent ( int entityNum, char *eventStr, char *params )
{
	int			i, eventNum;
	bot_state_t *bs;
	...
	if ( g_scriptDebug.integer )
	{
		...
		{
			G_Printf( "%i : (%s) GScript event: %s %s\n", level.time, g_entities[entityNum].scriptName, eventStr,
					  params ? params : "" );
		}
		else
		{
	...
	for ( i = 0; i < bs->script.data->numEvents; i++ )
	{
		if ( bs->script.data->events[i].eventNum == eventNum )
		{
			if
			(
				(!bs->script.data->events[i].params) ||
				(
					!botScriptEvents[eventNum].eventMatch || botScriptEvents[eventNum].eventMatch( &bs->script.data->events[i],
																								   params)
				)
			)
			{
...
*/
void
Bot_ForceScriptEvent ( int entityNum, char *eventStr, char *params )
{
	int			oldPauseTime;
	bot_state_t *bs;
...
*/
void
Bot_TeamScriptEvent ( int team, char *eventStr, char *params )
{
	int			i;
	bot_state_t *bs;
...
*/
qboolean
Bot_ScriptRun ( bot_state_t *bs, qboolean force )
{
	...
	while ( bs->script.status.stackHead < stack->numItems )
	{
		...
		if ( G_IsSinglePlayerGame() )
		{
			...
			{
				trap_SendServerCommand( 0, va( "botdebugprint %i \"Line: %i %s %s\"", bs->client,
										Bot_Script_GetCurrentLine( bs), item->action->actionString, item->params) );
			}
		}
...
*/
int
Bot_Script_GetCurrentLine ( bot_state_t *bs )
{
	if ( bs->script.status.eventIndex < 0 )
	{
		return ( -1 );
...
*/
void
Bot_ScriptLog_Entry ( bot_state_t *bs, qboolean showDetails, char *preText, char *fmt, ... )
{
	va_list			ap;
