...
*/
void
BotInputToUserCommand ( bot_state_t *bs, bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time )
{
	...
	if
	(
		(bi->actionflags & ACTION_PRONE || bs->script.flags & BSFL_PRONE) &&
	...
	*/
	...
	if
	(
		(bi->actionflags & ACTION_CROUCH) // START	xkan, 8/23/2002
		// if the script says crouch, then crouch.
		||
		(bs->script.flags & BSFL_CROUCH)	// END		xkan, 8/23/2002
	)
	{
		ucmd->upmove -= 127;
	}
...
*/
void
BotUpdateInput ( bot_state_t *bs, int time )
{
	bot_input_t bi;
	int			j;
...
*/
int
BotTravelFlagsForClient ( int client )
{
	int			tfl;
...
*/
void
BotGetInitialAttributes ( bot_state_t *bs )
{
...
//
void
ParseBotDefaultAttributes ( char *fileName )
{
			...
			if ( foundBot )
			{
				...
				if ( fieldCount < MAX_BOT_DEFAULT_KEYS )
				{
					Com_Printf( "File %s has bot %s without enough fields\n", fileName,
								g_botDefaultValues[g_botDefaultValueCount].m_botName );
				}				// if (fieldCount < MAX_BOT_DEFAULT_KEYS)...
			...
			if ( !Q_stricmp( token, g_botDefaultKeys[j]) )
			{
				...
				switch ( j )
				{
					...
					case e_SETFIRECYCLETIME:
						...
						// Read min fire rate
						token = COM_ParseExt( p, qtrue );
						g_botDefaultValues[g_botDefaultValueCount].m_minFireRateCycleTime = atoi( token );

						// Read max fire rate
						token = COM_ParseExt( p, qtrue );
						g_botDefaultValues[g_botDefaultValueCount].m_maxFireRateCycleTime = atoi( token );
...
//
void
BotSetCharacterAttributes ( bot_state_t *bs, // What default attributes should we use?
...
//
void
BotSetUpCharacter ( bot_state_t *bs )
{
	...
	for ( i = 0; i < g_botDefaultValueCount; i++ )
	{
		...
		if ( !Q_stricmp( g_entities[bs->client].scriptName, g_botDefaultValues[i].m_botName) )
		{
...
*/
int
BotAISetupClient ( int client, struct bot_settings_s *settings )
{
...
*/
int
BotAIShutdownClient ( int client )
{
	bot_state_t *bs;
	bs = &botstates[client];
	if ( !bs->inuse )
	...
	for ( i = lastbot + 1, botcount = 0; botcount < MAX_CLIENTS; i++, botcount++ )
	{
		...
		(
			botstates[i].botthink_residual >= thinktime *
				((VectorLengthSquared( botstates[i].cur_ps.velocity) < SQR( 10)) ? 2 : 1)
		)
		{
...
*/
gentity_t *
BotFindEntity ( gentity_t *from, int fieldofs, char *match )
{
...
*/
bot_state_t *
FindBotByName ( // Name of the bot to look up
char *botName )
{
...
*/
void
BotBuildStaticEntityCache ( void )
{
	int			i;
	gentity_t	*trav, *p;
	...
	for ( i = 0; i < NUM_BOTSTATICENTITY; i++ )
	{
		...
		while ( (trav = G_Find( trav, FOFS( classname), botStaticEntityStrings[i])) )
		{
			...
			{	// add trav to the end of the list
				while ( p->botNextStaticEntity )
				{
					p = p->botNextStaticEntity;
				}
...
*/
gentity_t *
BotFindNextStaticEntity ( gentity_t *start, botStaticEntityEnum_t entityEnum )
{
	gentity_t	*trav;
...
*/
gentity_t *
BotFindEntityForName ( char *name )
{
	gentity_t	*trav;
	int			i;
...
*/
void
G_SetAASBlockingEntity ( gentity_t *ent, int blocking )
{
	...
	(
		...
		ent->AASblocking & 1 &&
		(
			!VectorCompare( ent->r.absmin, ent->AASblocking_mins) || !VectorCompare( ent->r.absmax,
																					 ent->AASblocking_maxs)
		)
	)
	{
	...
	if
	(
		(blocking == ent->AASblocking) &&
		(
			VectorCompare( ent->r.absmin, ent->AASblocking_mins) &&
			VectorCompare( ent->r.absmax, ent->AASblocking_maxs)
		)
	)
	{
		return;
...
*/
void
BotRecordAttack ( int src, int dest )
{
...
BotDebug ( int clientNum )
{
	...
	if ( bs->inuse )
	{
		...
		if ( bs->leader > -1 )
		{
			trap_Cvar_Set( "bot_debug_curAINode",
						   va( "%s: leader = %i tagent = %i", bs->ainodeText, bs->leader, bs->leader_tagent) );
		}
		else
		{
		...
		// TAT 12/9/2002 - Throwing some extra info into the cover spot display
		{
			...
			Com_sprintf( buf, sizeof(buf), "%i(%s)  Enemy = %i", bs->seekCoverSpot, coverSpot ? coverSpot->name : "",
						 bs->enemy );
...
*/
void
GetBotAutonomies ( int clientNum, int *weapAutonomy, int *moveAutonomy )
{
...
*/
void
GetBotAmmo ( int clientNum, int *weapon, int *ammo, int *ammoclip )
{
	gentity_t	*ent;
...
void
BotSetIdealViewAngles ( int clientNum, vec3_t angle )
{
...
void
BotInitMovementAutonomyPos ( gentity_t *bot )
{
