...
*/
void
BotMP_MoveToGoal ( bot_state_t *bs, bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags )
{
#ifdef _DEBUG
...
*/
void
AIEnter_MP_Intermission ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Intermission ( bot_state_t *bs )
{
...
*/
void
AIEnter_MP_Observer ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Observer ( bot_state_t *bs )
{
...
*/
void
AIEnter_MP_Stand ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Stand ( bot_state_t *bs )
{
	bot_goal_t	goal;
	vec3_t		pos;
...
*/
void
AIEnter_MP_Respawn ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Respawn ( bot_state_t *bs )
{
	...
	//	}
	//
	if ( bs->lastClassCheck < level.time - 4000 )
		...
		if ( !BotIsDead( bs) )
		{
			...
			if ( bs->flags & BFL_MISCFLAG )
			{
				...
				if ( lastCall > level.time || lastCall < level.time - 2000 )
				{
					...
					{
						case PC_SOLDIER:
							BotVoiceChatAfterIdleTime( bs->client, "IamSoldier", SAY_TEAM, 1000 + rand() % 5000,
													   BOT_SHOWTEXT, 20000, qfalse );
							break;
						case PC_MEDIC:
							BotVoiceChatAfterIdleTime( bs->client, "IamMedic", SAY_TEAM, 1000 + rand() % 5000, BOT_SHOWTEXT,
													   20000, qfalse );
							break;
						case PC_FIELDOPS:
							BotVoiceChatAfterIdleTime( bs->client, "IamLieutenant", SAY_TEAM, 1000 + rand() % 5000,
													   BOT_SHOWTEXT, 20000, qfalse );
							break;
						case PC_ENGINEER:
							BotVoiceChatAfterIdleTime( bs->client, "IamEngineer", SAY_TEAM, 1000 + rand() % 5000,
													   BOT_SHOWTEXT, 20000, qfalse );
							break;
					}
				}
			...
			else if ( bs->sess.sessionTeam == level.attackingTeam )
			{
				if ( rand() % 2 )
				{
					BotVoiceChatAfterIdleTime( bs->client, "LetsGo", SAY_TEAM, 1000 + rand() % 2000, qfalse, 20000,
											   qfalse );
				}
			}
...
*/
void
AIEnter_MP_Seek_ActivateEntity ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Seek_ActivateEntity ( bot_state_t *bs )
{
	bot_goal_t			*goal;
	vec3_t				target, dir;
...
*/
void
AIEnter_MP_Seek_NBG ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Seek_NBG ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_AvoidDanger ( bot_state_t *bs )
{
	int bestarea;
	...
	if
	(
		g_entities[bs->avoid_goal.entitynum].s.eType == ET_MISSILE &&
		g_entities[bs->avoid_goal.entitynum].methodOfDeath == MOD_DYNAMITE
	)
	{
		if ( !(rand() % 3) )
		{
	...
	(
		!(
			bestarea = trap_AAS_AvoidDangerArea( bs->origin, bs->areanum, bs->avoid_goal.origin,
												 BotPointAreaNum( -1, bs->avoid_goal.origin),
												 bs->avoid_goal.number + 100, bs->tfl)
		)
	)
	{	// no hiding spot, ignore it
		bs->flags |= BFL_MISCFLAG;
	}
	else
	{
...
*/
int
AINode_MP_AvoidDanger ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	else if ( trav->s.eType == ET_CONSTRUCTIBLE )
	{
		if
		(
			(g_entities[bs->client].client->lastConstructibleBlockingWarnEnt != goal.entitynum) ||
	...
	if
	(
		(trav->methodOfDeath == MOD_LANDMINE) &&
		VectorDistanceSquared( bs->origin, trav->r.currentOrigin) > SQR( 256)
	)
	{
	...
	else
	{
		...
		else
		{
			if
			(
				!G_PredictMissile( trav, trav->nextthink - level.time, target,
								   (trav->s.eFlags & (EF_BOUNCE | EF_BOUNCE_HALF)))
			)
			{
		...
		if
		(
			(bs->last_avoiddangerarea < level.time - 200) &&
			(VectorDistanceSquared( target, goal.origin) < (SQR( trav->splashRadius + 100)))
		)
		{
			...
			(
				!(
					bestarea = trap_AAS_AvoidDangerArea( bs->origin, bs->areanum, target,
														 BotPointAreaNum( -1, target), trav->splashRadius + 100,
														 bs->tfl)
				)
			)
			{
...
*/
void
AIEnter_MP_GiveAmmo ( bot_state_t *bs )
{
...
*/
int
AINode_MP_GiveAmmo ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	if ( !trav->inuse || !trav->client || (trav->client->ps.pm_type != PM_NORMAL) )
	{
	...
	if ( !ClientNeedsAmmo( trav->s.number) )
	{
		if ( VectorDistanceSquared( bs->origin, goal.origin) < SQR( 72) )
		{
			...
			{
				BotVoiceChatAfterIdleTime( trav->s.number, "Thanks", SAY_TEAM, 1000 + rand() % 2000, qfalse,
										   3000 + rand() % 2000, qfalse );
			}
		}
	...
	if
	(
		VectorDistanceSquared( bs->origin, goal.origin) < SQR( 100) &&
		BotEntityVisible( bs->client, bs->eye, bs->viewangles, 360, trav->s.number, NULL)
	)
	{
	...
	(
		...
		(
			bs->cur_ps.ammoclip[BG_FindClipForWeapon( bs->cur_ps.weapon)] < (int)
				(0.8 * (GetAmmoTableData( bs->cur_ps.weapon))->maxclip)
		) &&
...
*/
void
AIEnter_MP_MedicGiveHealth ( bot_state_t *bs )
{
	bs->give_health_time = 0;
	bs->flags &= ~BFL_MISCFLAG;
...
*/
int
AINode_MP_MedicGiveHealth ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_MedicRevive ( bot_state_t *bs )
{
	bs->flags &= ~BFL_MISCFLAG;
...
*/
int
AINode_MP_MedicRevive ( bot_state_t *bs )
{
...
*/
void
AIEnter_MP_PanzerTarget ( bot_state_t *bs )
{
	bs->flags &= ~BFL_MISCFLAG;
...
*/
int
AINode_MP_PanzerTarget ( bot_state_t *bs )
{
	vec3_t	vec;
...
*/
void
AIEnter_MP_AttackTarget ( bot_state_t *bs )
{
	bs->flags &= ~BFL_MISCFLAG;
...
*/
int
AINode_MP_AttackTarget ( bot_state_t *bs )
{
	vec3_t		vec;
	bot_goal_t	goal;
	gentity_t	*check;
...
*/
void
AIEnter_MP_FixMG42 ( bot_state_t *bs )
{
...
*/
int
AINode_MP_FixMG42 ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_Battle_MobileMG42 ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Battle_MobileMG42 ( bot_state_t *bs )
{
	bot_goal_t	goal;
	vec3_t		dir, ang;
	int			tookDamage = 0;
	...
	if
	(
		!(bs->cur_ps.eFlags & EF_PRONE) &&
		(fabs( AngleDifference( bs->ideal_viewangles[YAW], bs->viewangles[YAW])) > 5.0f)
	)
	{
		return ( qtrue );
	}
...
*/
void
AIEnter_MP_MG42Scan ( bot_state_t *bs )
{
...
*/
int
AINode_MP_MG42Scan ( bot_state_t *bs )
{
	bot_goal_t	goal;
	vec3_t		dir, ang;
	gentity_t	*ent, *mg42;
	...
	if ( bs->enemy >= 0 )
	{
		...
		if ( bs->enemy >= 0 )
		{
			...
			if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 120, bs->enemy, NULL) )
			{
				if
				(
					(fabs( AngleDifference( ang[PITCH], mg42->s.angles[PITCH])) >= mg42->varc) ||
...
*/
void
AIEnter_MP_MG42Mount ( bot_state_t *bs )
{
...
*/
int
AINode_MP_MG42Mount ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
				...
				if ( bs->enemy >= 0 )
				{
					...
					if ( bs->enemy >= 0 )
					{
						...
						if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy, NULL) )
						{
							if
							(
								VectorDistanceSquared( bs->origin, goal.origin) > SQR( 128) ||
...
*/
void
AIEnter_MP_ScanForLandmines ( bot_state_t *bs )
{
	bs->arrive_time = level.time;
	bs->altenemy = 0;
...
*/
int
AINode_MP_ScanForLandmines ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_SniperSpot ( bot_state_t *bs )
{
	bs->arrive_time = level.time;
	bs->enemyposition_time = 0;
...
*/
int
AINode_MP_SniperSpot ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	if ( VectorLengthSquared( dir) < SQR( 32) )
	{
			...
			if ( bs->enemy >= 0 )
			{
				if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 50, bs->enemy, NULL) )
				{
					...
					(
						...
						BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 5, bs->enemy, NULL)
					)
					{
						BotCheckAttack( bs );
					}
		...
		else
		{
			...
			else if ( bs->viewchangetime > level.time )
			{
				...
				(
					!(rand() % (1 + BotGetNumVisibleSniperSpots( bs))) &&
					(spotNum = BotGetRandomVisibleSniperSpot( bs)) > -1
				)
			{
...
*/
void
AIEnter_MP_DefendTarget ( bot_state_t *bs )
{
...
*/
int
AINode_MP_DefendTarget ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
	...
	if ( goal.flags & GFL_LEADER )
	{
		...
		if ( bs->leader > -1 )
		{
			...
			else if ( bs->sess.playerType == PC_MEDIC )
			{
				...
				(
					(VectorLengthSquared( g_entities[bs->leader].client->ps.velocity) < SQR( 10)) &&
					(VectorLengthSquared( bs->cur_ps.velocity) < SQR( 10))
				)
			{
				if ( !(g_entities[bs->leader].r.svFlags & SVF_BOT) )
				{
					BotVoiceChatAfterIdleTime( bs->client, "WhereTo", SAY_BUDDY, 1000 + rand() % 3000, BOT_SHOWTEXT,
											   12000, qfalse );
				}
			}
	...
	switch ( ent->s.eType )
	{
		...
		case ET_ITEM:
			if ( !Q_stricmp( ent->classname, "team_CTF_redflag") && !(ent->flags & FL_DROPPED_ITEM) )
			{
				if ( bs->sess.sessionTeam != TEAM_AXIS )
				{
					...
					else if ( BotCarryingFlag( bs->client) )
					{					// we have it!
						BotDefaultNode( bs );
						return ( qfalse );
					}
		...
		case ET_TRIGGER_MULTIPLE:
			...
			if ( bs->arrive_time < level.time - 5000 )
			{
				if
				(
					(VectorDistanceSquared( bs->origin, BotGetOrigin( ent->s.number)) < SQR( 600)) &&
					trap_InPVS( bs->origin, BotGetOrigin( ent->s.number))
				)
				{

					// check the trace
					trap_Trace( &tr, bs->eye, vec3_origin, vec3_origin, BotGetOrigin( ent->s.number), -1,
								(MASK_SHOT) &~(CONTENTS_BODY | CONTENTS_CORPSE) );
	...
	(
		ent->client &&
		(bs->target_goal.number < level.time - 300) &&
		!VectorCompare( ent->r.currentOrigin, bs->defendgoal_origin)
	)
	{
		int		list[MAX_CLIENTS], numList;
		float	ourDist, *distances;
	...
	else
	{
		...
		(
			!move &&
			VectorDistanceSquared( bs->origin, goal.origin) > ((goal.flags & GFL_LEADER) ? SQR( 80) : SQR( 32))
		)
		{
			move = qtrue;
		}
	...
	if ( move )
	{
		...
		if ( bs->blockentTime > level.time - 500 && bs->blockent < level.maxclients )
		{
			...
			if ( goal.flags & GFL_LEADER )
			{
				if
				(
					(BotPointWithinMovementAutonomy( bs, &goal, bs->origin)) &&
			...
			if ( bs->enemy >= 0 )
			{
				...
				if ( bs->enemy >= 0 )
				{
					...
					if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy, NULL) )
					{
						...
						if
						(
							!BotCarryingFlag( bs->target_goal.entitynum) &&
							(
								(bs->weaponnum == WP_LUGER) ||
								(bs->weaponnum == WP_COLT) ||
								(bs->weaponnum == WP_MOBILE_MG42)
							)
						)
						{
		...
		if ( (ent->s.eType == ET_OID_TRIGGER) && (objtarg = ent->target_ent) )
		{
			if ( objtarg->spawnflags & 64 )
			{
				if ( ent->lastHintCheckTime < level.time && rand() % 2 )
				{
					...
					{
						BotVoiceChatAfterIdleTime( bs->client, "NeedEngineer", SAY_TEAM, 500 + rand() % 4000, qfalse,
												   5000 + rand() % 5000, qfalse );
					}
				}
				...
				if
				(
					bs->sess.playerType == PC_FIELDOPS &&
					(
						level.time - bs->cur_ps.classWeaponTime >
							(level.lieutenantChargeTime[bs->sess.sessionTeam - 1] * 0.5f)
					)
				)
				{
		...
		else
		{
			...
			if ( goal.entitynum > -1 && goal.entitynum < level.maxclients && BotCarryingFlag( goal.entitynum) )
			{
				...
				if ( !setdir )
				{
					...
					VectorAdd( g_entities[bs->target_goal.entitynum].r.absmax,
							   g_entities[bs->target_goal.entitynum].r.absmin, target );
...
*/
void
AIEnter_MP_TouchTarget ( bot_state_t *bs )
{
...
*/
int
AINode_MP_TouchTarget ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	if ( bs->alt_goal.number && bs->alt_goal.entitynum == goal.entitynum && bs->alt_goal.number == goal.number )
	{
		if
		(
			(bs->alt_goal.number < level.time - 12000) ||
			(trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl) < 700)
		)
		{
	...
	)
	{
		if ( bs->enemy > -1 )
		{
			...
			if ( DotProduct( edir, bs->cur_ps.velocity) > 0 )
			{
				...
				(
					(
						numList = trap_AAS_AlternativeRouteGoals( bs->origin, bs->target_goal.origin, bs->tfl,
																  altroutegoals, 40, 0)
					)
				)
				{
		...
		else if ( bs->enemy < 0 )
		{
			...
			if
			(
				(bs->stand_time < level.time - 3000) &&
				(trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl) > 600) &&
			...
			)
			{
				...
				for ( i = 0; i < numList; i++ )
				{
					...
					if
					(
						(VectorDistanceSquared( tbs->origin, tbs->defendgoal.origin) < SQR( 1500)) &&
						(VectorDistanceSquared( tbs->origin, bs->origin) < SQR( 1024)) &&
						(trap_InPVS( tbs->origin, bs->origin)) &&
						(
							trap_AAS_AreaTravelTimeToGoalArea( tbs->defendgoal.areanum, tbs->defendgoal.origin,
															   goal.areanum, bs->tfl) < trap_AAS_AreaTravelTimeToGoalArea( bs->areanum,
																														   bs->origin,
																														   bs->target_goal.areanum,
																														   bs->tfl)
						) &&
						(
							trap_AAS_AreaTravelTimeToGoalArea( tbs->areanum, tbs->origin, goal.areanum, bs->tfl) -
							trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, goal.areanum, bs->tfl)
						)
					)
					{
						t = trap_AAS_AreaTravelTimeToGoalArea( tbs->areanum, tbs->origin, tbs->defendgoal.areanum,
															   bs->tfl );
						if ( t > 300 )
						{
							t = 300;
...
*/
		...
		if ( bs->enemy < 0 && !(bs->flags & BFL_IDEALVIEWSET) )
		{
			...
			(
				...
				trap_InPVS( bs->origin, goal.origin) &&
				Distance( bs->origin, goal.origin) < 512
			)
			{
...
*/
void
AIEnter_MP_SatchelChargeTarget ( bot_state_t *bs )
{
...
*/
int
AINode_MP_SatchelChargeTarget ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	if ( goalDist < SQR( 32) )
	{
		...
		if ( !trav )
		{
			...
			if ( BotWeaponCharged( bs, WP_SATCHEL) )
			{
				...
				if
				(
					!VectorLengthSquared( bs->velocity) &&
					fabs( bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 &&
					bs->cur_ps.weapon == WP_SATCHEL &&
...
*/
void
AIEnter_MP_DynamiteTarget ( bot_state_t *bs )
{
...
*/
int
AINode_MP_DynamiteTarget ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	if ( goalDist < SQR( 32) )
	{
		...
		else if ( trav->s.teamNum >= 4 )
		{										// the dynamite is not armed
...
*/
void
AIEnter_MP_ConstructibleTarget ( bot_state_t *bs )
{
	bs->target_goal.flags &= ~GFL_NOSLOWAPPROACH;
...
*/
int
AINode_MP_ConstructibleTarget ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	//if the movement failed
	goalDist = VectorDistanceSquared( bs->origin, goal.origin );
	if ( moveresult.failure || (VectorLengthSquared( bs->cur_ps.velocity) < SQR( 3) && goalDist < (128 * 128)) )
	{									// allow for boxes at construction site that may get in the way
		//reset the avoid reach, otherwise bot is stuck in current area
		trap_BotResetAvoidReach( bs->ms );
...
*/
void
AIEnter_MP_PlantMine ( bot_state_t *bs )
{
	bs->target_goal.flags &= ~GFL_NOSLOWAPPROACH;
...
*/
int
AINode_MP_PlantMine ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_DisarmDynamite ( bot_state_t *bs )
{
...
*/
int
AINode_MP_DisarmDynamite ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_Battle_Fight ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Battle_Fight ( bot_state_t *bs )
{
	int					areanum;
	aas_entityinfo_t	entinfo;
	bot_moveresult_t	moveresult;
	...
	(
		...
		(enemydist > SQR( BotWeaponRange( bs, bs->weaponnum))) ||
...
*/
void
AIEnter_MP_Battle_Chase ( bot_state_t *bs )
{
	bs->altenemy = -1;
	BotClearGoal( &bs->target_goal );
...
*/
int
AINode_MP_Battle_Chase ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
	...
	if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy, NULL) )
	{
		...
		if ( bs->cur_ps.groundEntityNum != ENTITYNUM_NONE || bs->cur_ps.velocity[2] < 0 )
		{
			...
			(
				(bs->weaponnum != WP_KNIFE) &&
				(
					(!BotCarryingFlag( bs->enemy) && (enemydist < SQR( BotWeaponRange( bs, bs->weaponnum)))) ||
					(enemydist < SQR( 64))
				)
			)
			{
			...
			{							// attack and keep moving towards them
				//choose the best weapon to fight with
				BotChooseWeapon( bs );
...
*/
void
AIEnter_MP_Battle_Retreat ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Battle_Retreat ( bot_state_t *bs )
{
	aas_entityinfo_t	entinfo;
	bot_moveresult_t	moveresult;
	int					areanum;
...
*/
void
AIEnter_MP_Script_MoveToMarker ( bot_state_t *bs )
{
...
*/
int
AINode_MP_Script_MoveToMarker ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_MoveToAutonomyRange ( bot_state_t *bs )
{
	vec3_t	pos;
...
*/
int
AINode_MP_MoveToAutonomyRange ( bot_state_t *bs )
{
	bot_goal_t			goal;
	vec3_t				target, dir;
	bot_moveresult_t	moveresult;
...
*/
void
AIEnter_MP_NavigateFromVoid ( bot_state_t *bs )
{
...
*/
int
AINode_MP_NavigateFromVoid ( bot_state_t *bs )
{
	...
	if ( bs->flags & BFL_MISCFLAG )
	{
		...
		if ( bs->last_attackShout < level.time - 250 )
		{
			...
			if
			(
				trap_AAS_PredictClientMovement( &move, bs->client, bs->origin, -1, qfalse, dir, vec3_origin,
												checkFrames, 0, checkFrametime,
												SE_HITGROUNDDAMAGE | SE_HITGROUNDAREA | SE_STUCK | SE_GAP, -1, qfalse)
			)
			{
